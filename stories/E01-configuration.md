...
### E01US016 — Définir l'identité visuelle du tournoi — ✅ **absorbée par E16US006 (25/08/2026)**

> ⚠️ **Cette US n'a pas été livrée sous son numéro : elle a été absorbée par `E16US006`.**
> `E16US006` demandait « un second logo, en plus du logo du tournoi » (questionnaire A05) — or le
> **premier n'existait pas**, cette US étant ⬜. Livrer un second logo sans le premier n'avait pas de
> sens ; le commanditaire a donc fusionné les deux au cadrage du 25/08/2026. Le CA ci-dessous est
> **livré en entier**, à une réserve nommée près (l'usage de l'accent secondaire, cf.
> [ADR-0097](../docs/adr/0097-un-logo-de-tournoi-vit-en-base-avec-lui.md) § Conséquences).
> Le CA à jour vit désormais dans [`stories/E16`](E16-retours-maquettes.md) § E16US006, qui **fait
> foi** ; ce qui suit est conservé pour l'historique.
...
- **Notes** : `D-27`, `D-28` · [CDC design §3.6](../cahier-des-charges-design.md) (`DV-06`). **Absent des 117 US** : le CDC design v0.1 le portait en question ouverte (`Q-D8`), fermée le 14/07/2026. **La dérivation est du code, pas une décision de designer** : teinte et saturation conservées, clarté ajustée jusqu'au seuil AA — le calcul est reproductible. Cas d'école **vérifié sur la charte réelle** : le rouge club `#B71918` donne **2,55:1** sur le fond anthracite `#1D1D1B` de sa propre charte (échec texte **et** UI) → aplat + variantes `#CC1C1B` (bordure, 3,01:1) et `#E84E4D` (texte, 4,52:1). **Pourquoi l'admin est exclu** : le jour J, un bénévole n'a pas le temps de réapprendre des repères visuels. ~~**Ouvertes** : `Q-UX10` (qui produit le logo — un SVG de graphiste ou un JPEG de téléphone à recadrer ?), `Q-UX11` (une archive fige-t-elle son identité ?).~~ — **les deux sont fermées**, cf. puce suivante.
  ⚠️ **Le cas d'école ci-dessus a servi d'ORACLE à l'implémentation, et il s'est vérifié** : les trois valeurs conservent teinte et saturation à moins d'un demi-degré près, ce qui prouve que la charte avait déjà appliqué la règle que cette US automatise. Le contour est reproduit **exactement** ; le texte tombe un cran plus bas (`#E84D4D`, 4,50:1), la charte s'étant arrêtée un pas après le franchissement là où `DV-05` demande d'ajuster **jusqu'au** seuil. Écart instruit et testé dans les deux sens, cf. [ADR-0097](../docs/adr/0097-un-logo-de-tournoi-vit-en-base-avec-lui.md) §3.
- **Questions fermées par la livraison** : ~~`Q-UX10`~~ (qui produit le logo ?) → **un fichier déjà calibré** ; l'appli ne recadre ni ne détoure, elle **refuse en le disant**. ~~`Q-UX11`~~ (une archive fige-t-elle son identité ?) → **oui, par construction** : les octets vivent dans la ligne du tournoi, et `archivé` refuse toute écriture (ADR-0097 §1).
...
- **CA — un tournoi vide se supprime sans rien demander.** La confirmation doit rester rare pour
  rester lue. **« Vide » = aucun archer, aucune inscription, aucune flèche tirée, aucune série,
  aucun duel, aucun forfait, aucun barrage, aucun poste enrôlé, aucun scoreur, aucun acte au
  journal d'audit, aucune inscription payée, aucun remboursement** *(arbitrage du commanditaire,
  19/09/2026)*. Ne comptent **pas** : créneaux, catégories, blasons, gabarits, déroulé composé et
  identité visuelle — de la **configuration** qui se ressaisit. Un tournoi monté puis abandonné
  avant la première inscription part donc d'un clic.
  ⚠️ **Le critère est « est-ce que ça se ressaisit ? », pas « est-ce de la configuration ? »**
  *(corrigé en revue, 19/09/2026)* : postes et scoreurs portent des codes tirés au hasard, donc les
  QR déjà imprimés ne se retrouvent pas ; le journal d'audit est une trace que rien ne reconstitue.
  Une première version les laissait dehors, et un tournoi entièrement **préparé** la veille partait
  sur un seul clic.
  ⚠️ Le § Contexte d'ADR-0077 dit « depuis E02US010 plus aucun tournoi `prêt` n'est vide » : c'est
  vrai du **500**, pas du **signalement** — les deux phrases ne parlaient pas du même « vide ».
- **CA — un tournoi peuplé est signalé (409) avec un décompte chiffré** de ce qui partira : archers,
  inscriptions, **flèches tirées**, séries, duels, forfaits, barrages, postes, scoreurs, actes au
  journal d'audit, inscriptions payées et remboursements. ⚠️ **« flèches tirées » et non
  « scores »** *(arbitrage tranché le 19/09/2026)* : l'agrégat `Score` du walking skeleton n'a plus
  aucun écrivain (`DETTE-011`), le décompte porte sur les **volées** — validées ou non, sans quoi
  une fin de journée non validée annonçait « 0 flèche » pour 7 200 qui partaient. Le message **nomme les
...
- **CA — l'argent part avec le reste, mais il est chiffré en euros, en deux sommes distinctes** :
  l'**encaissé** (inscriptions payées au tarif du créneau) et le **reste à rendre** (registre des
  remboursements, postes déjà traités exclus). ⚠️ Les confondre — défaut de la 1ʳᵉ version, relevé
  en revue — faisait annoncer **zéro euro** à un tournoi de 400 payants sans remboursement.
- **CA — les remboursements partent avec le reste**
