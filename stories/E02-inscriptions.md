...
- **CA** : édition du nom, du prénom, de la **catégorie** et du **club** — l'édition rejoue les contrôles de l'inscription (nom/prénom non vides, catégorie du tournoi, club existant) ; un archer **devenu** homonyme d'un inscrit est **signalé** (409 `homonyme_archer`) mais **acceptable après confirmation**, comme à l'inscription ([ADR-0015](../docs/adr/0015-signaler-un-doublon-plutot-que-l-interdire.md)) ; changer la **catégorie** d'un archer qui porte déjà des **scores** est de même **signalé** (409 `changement_categorie_archer_engage`) et **acceptable après confirmation** ; **suppression refusée** (409 `archer_engage`) tant que l'archer est **placé** (cible renseignée) ou **engagé** (au moins un score) — refus **définitif**, qu'aucune confirmation ne lève : on retire d'abord le placement et les scores. Persisté via la file ; **écran d'administration des archers** (liste, édition, suppression), où le **club inconnu** devient corrigeable ([ADR-0014](../docs/adr/0014-club-inconnu-plutot-que-club-sentinelle.md)).
- **Notes** : ajoute `Archer.modifier` (patron `Club.modifier`), `ArcherRepository.supprimer` et `ScoreRepository.par_archer`. `ArcherRepositorySQL.enregistrer` recopie **déjà** tous les champs mutables — posé par E02US002 en prévision de cette US. **Aucune migration** : aucun champ nouveau.
  > **CA v0.1 tranché le 15/07/2026.** Il portait deux règles opposées sans choisir — « suppression bloquée … **ou** confirmation + recalcul ». (1) **Refus définitif**, sur le patron constant du projet (`ClubReference`, `BlasonReference`, `TournoiEnCoursNonSupprimable` : on refuse plutôt que de cascader en silence). Le « recalcul » n'a rien à recalculer tant qu'E03 n'existe pas, et `score.archer_id` est une FK **sans `ON DELETE`** ([DETTE-001](../docs/dette.md)) : une purge serait à écrire à la main, avant même que le moteur qu'elle est censée servir existe. La confirmation + recalcul se rouvrira en US dédiée quand le placement sera réel. (2) « placé/engagé » ne désignait rien d'**exerçable** : les départs (E02US004) et le vrai placement (E03) n'existent pas — on s'en tient donc à ce qui existe, hérité du walking skeleton E00US011 : **placé** = `cible` renseignée, **engagé** = au moins un score. La règle s'élargira avec les notions qu'elle vise, sans changer de forme. (3) L'édition de la catégorie d'un archer engagé est **confirmable**, non bloquée : figer la catégorie à la première flèche rendrait inrattrapable une erreur de saisie découverte tard, alors que corriger une catégorie mal saisie est le cas d'usage même de l'US.
