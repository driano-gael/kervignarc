...
- **CA — l'effectif est celui du CRÉNEAU, et il a déjà une définition** : des **archers distincts** inscrits sur ce départ — celle de `CompteurEngages`, qui dimensionne déjà le déroulé et l'exigence d'effectif. Le recompter localement (`len` des inscriptions) donnerait le même chiffre partout **sauf** sur une double inscription, et l'accueil annoncerait alors un effectif que le reste de l'application dément. Se compare au **quota du créneau**, jamais au total du tournoi (ADR-0075).
...
- **✅ CA — ce que l'encart ENLÈVE, arbitré le 12/09/2026.** Trois retraits, pour que l'accueil garde trois surfaces et non quatre :
  ...
  3. **Le chiffre « Inscrits »** du bandeau, que la somme des effectifs par créneau redit. ⚠️ **« Réglés » et « Postes en ligne » restent** : ce sont des agrégats **tournoi** que rien ne sait ventiler par départ — les retirer les perdrait au lieu de les déplacer (arbitrage du commanditaire, après signalement).
- **✅ Tranché — pas de route d'agrégation neuve.** Les quatre données de cycle (horaire, quota, état, effectif) tiennent dans `GET /tournois/{id}/departs`, **un appel pour tous les créneaux** ; les pauses restent lues créneau par créneau sur la route que le pilotage polle déjà, **cache partagé**. Bilan : zéro aller-retour HTTP de plus qu'avant l'US.
  - ✅ **Point vérifié au cadrage** : il **n'existait** aucune lecture agrégée « tous les créneaux, tout leur état ». Mais `ServiceDeparts.lister_avec_etat` calculait déjà un `AvancementDepart` complet par créneau et **jetait tout sauf `.etat`** : l'US n'a donc eu qu'à cesser de jeter, et à ajouter l'effectif par le port existant. `suivi-deroule` reste hors jeu (cf. le CA d'énumération).
