...
- **CA — ~~questions restées sans réponse, à reposer~~ → tranchées le 07/08/2026.** Les deux questions ont été reposées au commanditaire en tête d'US, et **les deux confirment le CA tel qu'il était écrit** :
  ...
  - *« "Terminer" fige le sportif et laisse les paiements ouverts : est-ce le bon découpage ? »* → **oui** ; le bouton reste au pilotage et ne se garde que sur `sportif_complet`. Les deux autres options (exiger l'administratif ; n'exiger rien et se contenter d'avertir) ont été explicitement **écartées**.
...
- **⚠️ Arbitrage de mise en œuvre (07/08/2026) — pas de destination neuve.** Le CA dit « **deux destinations** » ; `hors_sportif` ne porte aujourd'hui qu'**une** ligne, `paiements` (`domain/completude.py`), et l'axe gestion a **déjà** une destination `paiements`. La section hors-sportif est donc rendue **en tête de l'écran Paiements** (`CompletudeAdministrative`, hors des onglets), et non sur une destination « Complétude administrative » qui aurait posé un écran d'une ligne au-dessus de l'écran traitant exactement ce sujet. Une destination dédiée se rouvre le jour où le hors-sportif porte **plusieurs** sujets — pas avant (« pas de remède structurel sur une évolution supposée »). Conséquence : `AXE_PAR_DESTINATION` / `BESOIN_TOURNOI` / `AIDE_ECRANS` sont **inchangés** (31/30 destinations), seul le **libellé** de `completude` devient « Complétude du déroulé ».
- **⚠️ La planche A14 redessinée du 05/08/2026 est écartée pour cette US.** Elle propose autre chose que la séparation demandée — trois listes « Bloquant / À voir / Vérifié » autour du bouton « Donner le feu vert », **paiements compris**, donc toujours mélangés — et elle n'a **jamais été validée** (le tour 2 n'a pas eu lieu). Ne pas rouvrir le débat en la relisant : c'est le questionnaire du **tour 1** qui fait foi ici. Ce qu'elle propose reste une piste valable pour `E16US008` (feu vert), pas pour la complétude.
