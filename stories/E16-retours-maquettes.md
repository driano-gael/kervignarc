...
- ⚠️ **Question de cadrage — l'AGRÉGATION, et elle décide si l'US a une valeur.** *(Ce n'est plus un CA : la 1ʳᵉ correction en avait fait un — « le panneau ne se referme que si **toutes** ses lignes ont une suite » — et la 2ᵉ passe a montré que cette règle **annule la capacité de l'US** sur l'un de ses deux écrans.)* En élimination directe, `SaisieDuels` monte le panneau sur **les deux duellistes du duel qu'on vient de valider** : il y a donc **une ligne terminale à chaque duel, à tous les tours**. Sous la règle « il reste dès qu'une ligne est terminale », le panneau de duels **ne se referme jamais tout seul** — le minuteur de 3 min, c'est-à-dire l'US entière, serait mort-né sur cet écran, et les tests seraient verts.
  **Deux options, à trancher avant de prendre l'US** : **(a) fermeture par ligne** — chaque ligne ayant une suite s'efface après 3 min, les lignes terminales restent (demande de redessiner le panneau) ; **(b) minuteur global borné** — le panneau se referme dans tous les cas, mais seulement après acquittement ou après un délai plus long quand une ligne est terminale. **Ne pas retenir « il reste dès qu'une ligne est terminale » sans mesurer ce qu'elle coûte.**
...
- ⚠️ **Question de cadrage — le cas terminal.** *(Ce n'est pas un CA non plus : la rédaction précédente écrivait « il ne se referme pas quand c'est fini » et se démentait dans sa propre puce.)* L'intention est qu'une issue terminale laisse à l'archer le temps de lire sa place finale. ⚠️ **Déduction, pas citation** : le questionnaire est **muet** sur ce cas (il ne dit que *« 3 mn si un autre tour suit »*). ⚠️ Un « jamais » laisse chaque tablette de la salle bloquée sur son annonce au **dernier tour** — l'inverse de l'intention de l'US. **Poser une borne testable** (« il reste jusqu'à acquittement », ou « au moins N minutes ») plutôt qu'un absolu, et la faire confirmer.
...
- **Question de cadrage — la granularité de l'annulation** *(ce N'EST PAS un CA : la fiche ne sait pas encore trancher, et le ranger parmi les CA le rendrait normatif — défaut relevé en 2ᵉ passe)*. « Annuler **une** validation » n'est pas dérivable en l'état : `Serie.valider` valide un **lot** dont la taille dépend d'une politique injectée (`GrainValidation` : `FIN_DE_SERIE`, ou `TOUTES_LES_N_VOLEES` avec reliquat de fin de barème), et `validee_par` est un **nom**, pas un identifiant d'acte — deux lots validés par le même scoreur sont indistinguables. Recommandation : **le lot**, sinon on rouvre un trou au milieu d'un bloc validé. ⚠️ **Tracer l'acte de validation pour le rendre annulable PEUT exiger une migration**, contrairement à ce qu'annonce la note « aucune migration » plus bas — laquelle ne vaut que pour le **verrou** lui-même.
...
- **Notes** : ⚠️ **Le constat d'origine était à moitié faux, et cela réduit l'US.**
  ...
  - ⚠️ **LE POINT LE PLUS COÛTEUX DE L'US : la volée dévalidée sort du total, ET DÉSARME TROIS GARDES.** `Serie.cumul`, `compter` et `nb_fleches_validees` ne somment que les volées **verrouillées**. Le versant visible est connu — classement, palmarès, écran de salle, et l'exposition publique qui rebascule en `en_attente` (ADR-0039). ⚠️ **Le versant invisible l'est moins, et il a été manqué en 1ʳᵉ correction** : `nb_fleches_validees` n'alimente aucun total, c'est la mesure de « **l'archer a déjà tiré** », et elle garde **trois décisions** —
