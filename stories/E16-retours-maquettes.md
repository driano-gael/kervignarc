...
- **✅ QUATRE ARBITRAGES RENDUS le 10/09/2026** *(reversés ici, règle 9)* :
  ...
  3. **S09 : la hiérarchie est retenue**, au pied de la lettre du questionnaire — mais **sur les
     ...
     **Décision rectifiée : `poste de cible < scoreur < admin`.** ⚠️ **Le revers reste exposé et
     ...
     le **même** rang *(lire « rattachées à la **même** cible » : rédaction rectifiée le 12/09/2026,
     cf. le CA ci-dessous et ADR-0107 §3)*, donc la hiérarchie n'arbitre **rien** entre elles — le dernier écrit gagne,
...
- **CA — à rôles ÉGAUX, la règle n'arbitre rien** *(conséquence dite au commanditaire avant qu'il confirme)*. Deux tablettes rattachées à la **même** cible portent le même rôle *(rédaction rectifiée le 12/09/2026 : « cibles **différentes** » décrivait un cas impossible — hors de sa cible, un poste est refusé en amont, `SaisieHorsCible`)* : entre elles, le dernier écrit gagne, exactement comme aujourd'hui. ⚠️ **C'est le cas le plus fréquent en salle**, et c'est celui que le refus symétrique aurait couvert. Le CA se prouve par un **test négatif** — « deux postes écrivent successivement : le second gagne, `200`, aucun `409` » — sans lequel ce renoncement serait invisible dans le code. **Critère de réouverture écrit** : on reprend l'alternative si un tournoi réel remonte ≥ 1 saisie perdue entre deux postes.
...
- **CA — CORRIGER est une écriture comme une autre** *(ajouté en revue le 12/09/2026 — quatre axes ont trouvé le même trou)*. `POST /saisie/corrections` (garde `exiger_scoreur`) **pose** `Role.SCOREUR` sur la volée. ⚠️ **Sans ce CA, la règle se contournait par le CHOIX DE L'ENDPOINT** : deux chemins d'écriture sur le même agrégat, un seul qui revendique — un rang 1 écrasait donc un rang 2 **en silence**. La correction **pose** un rang et en **respecte** un, comme la saisie. ⚠️ Ce n'est pas une affaire d'écran : `POST /saisie/corrections` n'a aucun appelant front (`DETTE-100`). ⚠️ **Et la sortie de l'état ainsi créé tient en DEUX gestes** : une volée corrigée par le scoreur refuse la tablette, et `annuler_validation` refuse à son tour (« déjà en correction ») — il faut **refermer** la correction *puis* annuler. Les deux sont sur l'écran du scoreur ; sans cette phrase, la décision 4 d'ADR-0107 serait fausse précisément là où elle compte. ⚠️ **C'est aussi le SEUL site où `Role.SCOREUR` s'inscrit en base** : la saisie n'admet que l'admin et le poste. Sans lui, le rang du milieu n'existait que dans l'énumération.
- **Notes** : ✅ **ADR écrit** — [ADR-0107](../docs/adr/0107-une-ecriture-concurrente-est-arbitree-par-le-role-de-qui-ecrit.md). Un ordre entre rangs est une politique d'autorisation, là où le dépôt n'a aujourd'hui que des prédicats indépendants : c'est le **premier** du dépôt. *(Écrit **avant** l'implémentation, sur le patron d'[ADR-0102](../docs/adr/0102-la-documentation-porte-des-pointeurs-pas-des-copies.md) : § « Porté dans le code par » = « rien à ce jour, et c'est écrit exprès » — ce qui évite le défaut d'ADR-0017 sans pour autant laisser la décision sans trace.)*
  ...
  - ⚠️ **Inscrit en `DETTE-100` : le refus reste injoignable depuis les écrans** — mais pas pour la raison écrite en 1ʳᵉ passe. **Aucun** des deux croisements n'est joignable à l'écran, pour deux causes distinctes : l'appelant front de `POST /saisie/volees` est la tablette en portée `'poste'` **en dur** (et deux tablettes sont à rang égal), et `POST /saisie/corrections` — le seul endpoint qui fasse écrire un **scoreur** — n'a **aucun appelant front**, `PanneauValidationQualif.tsx` le disant lui-même (« la correction ne se fait pas ici »). ⚠️ **La 2ᵉ passe de revue avait remplacé cet énoncé JUSTE par un faux** (« le scoreur écrit depuis l'écran d'E16US019 ») : ce que la revue avait corrigé, c'est la **règle serveur**, pas la surface. Réflexe manquant : chercher l'**appelant**, jamais raisonner sur l'endpoint. La règle est donc **tenue et prouvée côté serveur**, et le rendu ambre livré est **prêt mais injoignable**. ⚠️ **Ne pas lire « le front est inutile »** : le laisser retomber sur le message générique afficherait, le jour où un admin écrit par l'API, une phrase de conflit **sans recours**. Le scénario de recette le dit franchement plutôt que de faire semblant.
...
- **CA — un bloc par départ** : chaque créneau porte ce qui sert au déroulé, tous visibles **côte à côte** et non un à la fois. ✅ **Énumération ARRÊTÉE au cadrage du 12/09/2026** (elle n'était qu'une piste jusque-là, et la borner était l'objet même du cadrage) : **horaire**, **quota**, **état du créneau** (ouvert / lancé / clos), **effectif engagé**, **pause en cours**. ⚠️ **L'avancement des phases est EXCLU**, et c'est un arbitrage de coût autant que de périmètre : il ne se lit que par `suivi-deroule`, une route scopée à **un** créneau (ADR-0075) dont chaque appel reconstruit tous les tableaux (`DETTE-031`) — sur l'écran d'atterrissage, qui polle, cela aurait été N lectures chères à chaque tick. Les quatre autres données se servent en **un seul appel** déjà existant.
- **CA — l'effectif est celui du CRÉNEAU, et il a déjà une définition** : des **archers distincts** inscrits sur ce départ — celle de `CompteurEngages`, qui dimensionne déjà le déroulé et l'exigence d'effectif. Se compare au **quota du créneau**, jamais au total du tournoi (ADR-0075). ⚠️ **Ce n'est PAS une divergence de chiffres qu'on évite** — corrigé en revue, quatre axes l'ayant relevé : `UNIQUE(archer_id, depart_id)` et la garde `DejaInscrit` rendent la double inscription **inatteignable**, donc un `len()` local rendrait aujourd'hui le **même** chiffre (le garde-fou de quota, lui, compte bien des lignes : `application/inscriptions.py`). Ce qu'on évite est une **seconde définition** que rien ne confronte : juste aujourd'hui, fausse le jour où l'une des deux bouge.
...
- **✅ CA — ce que l'encart ENLÈVE, arbitré le 12/09/2026.** Trois retraits, pour que l'accueil garde trois surfaces et non quatre :
  1. **Les boutons « Démarrer » / « Terminer » de la frise** deviennent un **renvoi** vers leur écran « prêt à… » (la 2ᵉ option de la *Résorption* de `DETTE-082`). ⚠️ Les autres transitions (`annuler`, `archiver`, `revenir-brouillon`) **gardent leur bouton** : aucune n'a de jalon, et celui d'`archiver` répond `404 jalon_non_instruit` — l'y renvoyer mènerait à un écran vide. `DETTE-082` est donc **resserrée, pas close** : la destination « Tournoi » monte la même frise sans le renvoi.
  2. **La pastille de relance** est **absorbée** : une pause s'affiche dans le bloc du créneau qu'elle arrête, ce que la pastille globale ne savait pas faire (elle disait « une phase attend » sans dire où). ⚠️ **Coût réseau inchangé** — la pastille faisait déjà une lecture d'arrêts par créneau.
  3. **Le chiffre « Inscrits »** du bandeau. ⚠️ **Corrigé en revue — il n'était PAS « la somme des effectifs par créneau »**, et l'écrire ainsi était faux dans les deux sens : `lister_par_archer` rend une ligne par **archer saisi du tournoi**, inscrit ou non (donc la somme le sous-compte), pendant qu'un archer inscrit sur deux créneaux compte **deux fois** dans les blocs (donc elle le sur-compte). Il part parce que, affiché seul à côté des blocs, il se lisait comme un total qu'aucune addition ne retrouve. Son dénominateur reste lisible dans « Réglés », dont le libellé dit désormais **ce qu'il compte** (« Réglés (archers du tournoi) »). ⚠️ **« Réglés » et « Postes en ligne » restent** : agrégats **tournoi** que rien ne sait ventiler par départ — les retirer les perdrait au lieu de les déplacer (arbitrage du commanditaire, après signalement).
- **✅ Tranché — pas de route d'agrégation neuve.** Les quatre données de cycle (horaire, quota, état, effectif) tiennent dans `GET /tournois/{id}/departs`, **un appel pour tous les créneaux** ; les pauses restent lues créneau par créneau sur la route que le pilotage polle déjà, **cache partagé**. ⚠️ **Cette route doit être POLLÉE par l'accueil, et elle ne l'était pas** (relevé en revue, axes C1 et D) : `useDeparts` ne tenait sa fraîcheur que de `useRealtime`, qui n'invalide que **sur événement**, ignore le message de reconnexion — et les inscriptions n'émettent aucun `LiveEvent`. L'effectif du matin serait resté figé, et un `etat` changé pendant une coupure wifi ne serait jamais revenu. Le poll est **optionnel sur le hook** : seul l'accueil le demande, la liste étant montée par ~25 écrans.
  - ✅ **Point vérifié au cadrage** : il **n'existait** aucune lecture agrégée « tous les créneaux, tout leur état ». `ServiceDeparts.lister_avec_etat` lisait déjà l'avancement de chaque créneau pour en dériver l'état ; l'US y **ajoute une seconde lecture par créneau** (les inscriptions, via le port existant) et **zéro aller-retour HTTP**. ⚠️ **Corrigé en revue — la 1ʳᵉ rédaction disait « l'US n'a eu qu'à cesser de jeter »**, ce qui est faux : `nb_places`, `nb_ayant_tire` et `nb_series_closes` sont toujours calculés puis abandonnés, et l'effectif ne vient pas d'eux. L'écrire ainsi aurait fait croire à la prochaine US que le chemin est gratuit. `suivi-deroule` reste hors jeu (cf. le CA d'énumération).
  - ✅ **« toutes les informations utiles au déroulé » a été bornée le 12/09/2026**, avec le commanditaire, avant la première ligne de code — cf. les deux CA ✅ ci-dessus. ⚠️ **Deux réponses se contredisaient** et l'ont été signalées avant d'implémenter : retirer la pastille de relance *sans* retenir les pauses aurait fait disparaître de l'accueil la capacité livrée par `E05US034` ; retirer les trois chiffres aurait perdu « Réglés » et « Postes en ligne », que rien ne ventile par départ. Les deux ont été rouvertes et tranchées.
