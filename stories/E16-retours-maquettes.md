...
- **✅ QUATRE ARBITRAGES RENDUS le 10/09/2026** *(reversés ici, règle 9)* :
  1. **S08 n'était pas une contradiction, mais une procédure en deux temps.** *« Plus de modification
     ...
     par lequel un scoreur peut écrire un score** : `saisir_volee` est gardée par `autoriser_saisie`,
     qui ne rend qu'un `Poste` ou l'admin — **le scoreur n'y a aucune identité**. Le retirer sans
     rien mettre à la place lui aurait fait redescendre demander à la cible. **Décision rectifiée du
     commanditaire : `autoriser_saisie` est ÉLARGIE au scoreur** — même patron qu'`autoriser_forfait`
     en `E16US008` (une route, plusieurs identités), et c'est le « les deux » de la réponse. Le poste
     de cible garde son accès : `corriger_volee` disparaît **sans rien retirer à personne**.
...
- **CA — agrégation** : le panneau ne se referme que si **toutes** ses lignes ont une suite ; **dès qu'une ligne est terminale, il reste**. C'est la lecture sûre — on ne prive personne de sa place finale pour rendre une tablette trois minutes plus tôt.
- **CA — le panneau se referme au bout de 3 minutes lorsqu'un tour suit** : passé ce délai, la tablette revient d'elle-même à la saisie, sans geste. ⚠️ **Le compte à rebours visible est un AJOUT, pas une demande** : le questionnaire S06 a retenu la variante **A** et **écarté la variante C — « Retour automatique compté »**, dont un compteur est la signature. La justification (un écran qui disparaît sans prévenir se lit comme un plantage) vient de l'assistant. **À faire confirmer au cadrage, et à écrire dans la planche S06** — sans quoi `E17US008`, qui confrontera les neuf planches `S**` aux écrans livrés, le relèvera comme écart de structure et le **retirera**, cassant ce CA.
- **CA — il ne se referme pas quand c'est fini** : sur une issue terminale, le panneau reste, parce que c'est le seul endroit où l'archer lit sa place finale. ⚠️ **Déduction, pas citation** : le questionnaire est **muet** sur le cas terminal (il ne dit que *« 3 mn si un autre tour suit »*). ⚠️ Et cela laisse chaque tablette de la salle bloquée sur son annonce au **dernier tour** — soit l'inverse de l'intention de l'US. À trancher au cadrage : borner (« il reste au moins N minutes », « jusqu'à acquittement ») plutôt que « jamais ».
...
- **Notes** : ⚠️ **Vérifié dans le code au découpage — la moitié du travail est déjà là, l'autre pas du tout.**
  ...
  - ⚠️ **Deux points distincts à trancher au cadrage, à ne pas confondre** : **(1) quelles issues** comptent comme « un tour suit » — lecture proposée : *suite* pour `prochain_duel` / `prochaine_manche` / `repeche`, *terminal* pour `termine`, **à décider** pour `en_attente` et `indisponible` (l'archer y a bien une suite, mais elle n'est pas encore connue) ; **(2) comment on agrège** plusieurs lignes, cf. le CA d'agrégation. La 1ʳᵉ rédaction ne posait que (1) et raisonnait encore par issue unique.
  - ⚠️ **Deux cas adverses à couvrir, rendus par la revue** : un **repêché sans destination** (`REPECHAGE_SANS_DESTINATION`, `destination = None`) a « une suite », donc le minuteur emporterait le seul écran qui lui dit qu'il est repêché **sans lui dire où aller** ; et un **`TERMINE` sans aucune place** (`rang_final`, `rang_min` et `rang_max` tous `None`, motif `RANG_A_VENIR`) fait tomber la prémisse du CA « il reste, c'est là qu'il lit sa place » — il n'y a aucune place à lire.
...
- **CA — QUI peut réécrire après l'annulation** *(CA neuf, ajouté en revue le 10/09/2026 — son absence était un bloquant)*. La réponse S08 complète dit *« **les deux, archer et scoreur** »* : la volée rouverte doit être écrivable par **le poste de cible ET le scoreur**. Or `saisir_volee` est gardée par `autoriser_saisie`, qui ne rend qu'un `Poste` ou l'admin — **le scoreur n'y a aucune identité**, et `corriger_volee` (`exiger_scoreur`) est aujourd'hui son **seul** chemin d'écriture. **Décision du commanditaire : `autoriser_saisie` est ÉLARGIE au scoreur**, même patron qu'`autoriser_forfait` en `E16US008` — une route, plusieurs identités, jamais une route parallèle. ⚠️ **Sans ce CA, l'US livrait DEUX défauts en même temps** : une **régression** (le scoreur ne peut plus corriger, il doit redescendre demander à la cible) et un **élargissement non voulu** (la réécriture d'un score validé, aujourd'hui réservée au scoreur, tombait au niveau du jeton de tablette).
...
- **CA — la granularité de l'annulation** *(à trancher au cadrage, relevé en revue)*. « Annuler **une** validation » n'est pas dérivable en l'état : `Serie.valider` valide un **lot** dont la taille dépend d'une politique injectée (`GrainValidation` : `FIN_DE_SERIE`, ou `TOUTES_LES_N_VOLEES` avec reliquat de fin de barème), et `validee_par` est un **nom**, pas un identifiant d'acte — deux lots validés par le même scoreur sont indistinguables. Recommandation : **le lot**, sinon on rouvre un trou au milieu d'un bloc validé. ⚠️ **Contrairement à ce qu'annonce la note « aucune migration » ci-dessous, tracer l'acte de validation pour le rendre annulable PEUT en exiger une.**
...
- **Notes** : ⚠️ **Le constat d'origine était à moitié faux, et cela réduit l'US.**
  - ⚠️ **L'INVENTAIRE DES SURFACES — refait en revue, la 1ʳᵉ rédaction en oubliait deux.** Il y a **quatre** surfaces où un résultat s'acte, pas deux :
    ...
    |---|---|---|
    | **Qualification** | `corriger_volee` (`POST /saisie/corrections`, `exiger_scoreur`, tracé `CORRECTION_SCORE`) réécrit une série verrouillée ; aucune annulation | **oui** — c'est le périmètre |
    | **Duels** | `DuelVerrouille` refuse `saisir_manche`, `saisir_barrage` et une seconde `valider` ; aucune route de correction sous `/api/v1/duels/` | **non** — S08 y est **déjà tenu**, rien à faire |
    | **Barrage de places** | ⚠️ `PUT …/barrages/{id}/manche` *« Saisit la manche suivante, ou **réécrit** celle indiquée »*, **aucune garde sur `clos`** ; et `DELETE …/barrages/{id}` annule un barrage **même clos** | **à trancher** — S08 n'y est **pas** tenu |
    | **Big Shoot Off** | l'agrégat `Serie` se verrouille ; `POST /validations` mais **aucune** route de correction **ni d'annulation** | **à trancher** — immuable, donc S08 tenu à moitié, l'autre moitié impossible |

    Sans cet inventaire, le CA « il n'existe plus qu'un seul chemin d'écriture » aurait été **faux à l'instant de sa livraison**. Si les deux dernières surfaces restent dehors, **l'écrire** — sinon la carte laisse croire que S08 sera tenu partout.
  - **Le verrou est dérivé, pas stocké** : `Volee.verrouillee` est une property qui vaut `validee_par is not None` (`backend/domain/serie.py`). **Annuler = reposer `validee_par` à `None`** — donc probablement **aucune migration** pour le verrou lui-même (sous réserve du CA de granularité ci-dessus).
  - ⚠️ **CONSÉQUENCE MÉTIER NON ÉCRITE, trouvée en revue : la volée dévalidée SORT DU CLASSEMENT.** `Serie.cumul` somme `v.points for v in self.volees if v.verrouillee` — idem `compter` et `nb_fleches_validees`. Le temps de la correction, la volée quitte donc le total : classement, palmarès, écran de salle, et l'exposition publique la rebascule en `en_attente` (ADR-0039). **Le chemin actuel n'a pas cette fenêtre** — `corriger_volee` réécrit en place sans déverrouiller. C'est le **coût réel** de l'arbitrage, constatable dans le code d'aujourd'hui. À trancher : soit annulation + ressaisie sont **un seul acte** côté service (la volée ne quitte jamais le total), soit la fenêtre est assumée **et les écrans la disent**.
  - ⚠️ **Tension à lever avec `E16US020`** : un **admin** qui annule devient le dernier écrivain ; sous la hiérarchie de `E16US020`, la volée se verrouille alors contre le scoreur **et** contre la tablette — c'est-à-dire contre ceux qui doivent la corriger. La règle S09 arbitre une **concurrence**, pas une **préséance permanente** : les deux fiches doivent se borner mutuellement.
  ...
  - ⚠️ **Cette US ROUVRE [ADR-0035](../docs/adr/0035-atomicite-acte-trace-session-partagee.md)**, dont les § Décision 2 et 3 nomment explicitement la **réécriture d'un score corrigé** et `CORRECTION_SCORE` : section « Porté dans le code par » **exigée** (règle « décision structurante ⇒ ADR », ADR-0075 § Portée). L'audit gagne donc un type de trace (`E10US005`) — `CORRECTION_SCORE` disparaît avec son endpoint, une annulation doit laisser la sienne, sinon l'US **retire** de la traçabilité au lieu d'en ajouter.
...
### E16US020 — Deux écritures, une même volée : le rang tranche
...
- **CA — le rang supérieur écrase, l'inférieur est refusé** : **`poste de cible < scoreur < admin`**, les trois identités que le dépôt porte vraiment.
- **CA — à rangs ÉGAUX, la hiérarchie n'arbitre rien** *(conséquence dite au commanditaire avant qu'il confirme)*. Deux tablettes de cibles différentes portent le **même** rang : entre elles, le dernier écrit gagne, exactement comme aujourd'hui. Cette US ne règle que *poste vs scoreur* et *poste vs admin*. ⚠️ **Le cas le plus fréquent en salle est justement poste vs poste** — c'est ce que le `409` aurait couvert, et c'est la raison pour laquelle il est écarté **et non oublié**.
- **CA — ce qui est écarté est le refus SYMÉTRIQUE, pas le refus** *(précision de revue : la 1ʳᵉ rédaction se contredisait)*. Le rang **supérieur** écrase sans notifier celui qu'il écrase — c'est le revers assumé. Le rang **inférieur** est refusé (`409`), et l'écran lui dit pourquoi : un refus muet serait pire que l'écrasement qu'il remplace. Ce qui a été écarté, c'est le refus **indépendant du rang**, qui aurait montré le conflit aux **deux** parties.
- **CA — le rang est celui du JETON, jamais celui du message** : l'arbitrage se fait sur l'identité authentifiée, pas sur ce que la requête déclare. ⚠️ **Mais le jeton du rang le plus bas identifie un LIEU, pas une personne** (`autoriser_saisie` rend un `Poste`) — c'est une conséquence assumée d'`E10US007`, pas un oubli, et c'est précisément ce qui rend l'ordre nécessaire.
- **CA — la préséance ne survit pas au conflit** *(borne ajoutée en revue)* : le rang de la dernière écriture ne verrouille pas la volée **pour toujours** contre les rangs inférieurs. Sans cette borne, un admin qui annule une validation (`E16US019`) rendrait la volée inaccessible au scoreur **et** à la tablette — c'est-à-dire à ceux qui doivent la corriger. La règle arbitre une **concurrence**, pas une préséance permanente : la fenêtre est à définir au cadrage.
- **Notes** : ✅ **ADR écrit** — [ADR-0107](../docs/adr/0107-une-ecriture-concurrente-est-arbitree-par-le-rang-de-qui-ecrit.md). Un ordre entre rangs est une politique d'autorisation, là où le dépôt n'a aujourd'hui que des prédicats indépendants : c'est le **premier** du dépôt. *(Écrit **avant** l'implémentation, sur le patron d'[ADR-0102](../docs/adr/0102-la-documentation-porte-des-pointeurs-pas-des-copies.md) : § « Porté dans le code par » = « rien à ce jour, et c'est écrit exprès » — ce qui évite le défaut d'ADR-0017 sans pour autant laisser la décision sans trace.)*
  - ⚠️ **Le piège central, trouvé au découpage et absent de toutes les rédactions** : `Volee.saisie_par` **existe** mais sa propre docstring le qualifie de **« déclaratif »** — c'est un **nom libre**, pas un rang, et il vient du corps de la requête. **L'utiliser comme source d'autorité livrerait une hiérarchie qu'un poste contourne en se déclarant admin.**
  - ⚠️ **« Migration attendue » était trop affirmatif** *(corrigé en revue)* : **migration probable, à trancher**. Les trois rangs sont **déjà** discriminables à la frontière API sans aucune colonne (`autoriser_saisie` → `Poste | None`, `exiger_scoreur` → `Scoreur`, `exiger_admin`) ; le rang peut donc se **dériver de la garde** au lieu de se persister sur la volée. Le choix dépend de la fenêtre retenue au CA de préséance.
  ...
  - ⚠️ **Un cas adverse rendu par la revue** : un **admin** saisit par erreur sur la cible 7, puis le **poste de cible 7** — rang inférieur — veut corriger. La hiérarchie **refuse** : la cible ne peut plus se corriger elle-même, et le seul recours est un admin. Le questionnaire ne couvre pas ce sens ; c'est le CA de préséance qui doit le borner.
  - ⚠️ **Question de périmètre à trancher au cadrage** : la hiérarchie vaut-elle pour les **duels** et les autres formats, ou seulement pour la volée de qualification que le questionnaire visait ?
