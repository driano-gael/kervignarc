...
- **CA — recherche d'archer en pilotage** : scopée au tournoi, fiche **en consultation** puis action. ✅ **Les actions sont tranchées au cadrage** : *corriger sa fiche* et *modifier son placement*. ⚠️ **Le forfait est demandé mais NON livré comme geste**, et c'est un constat de code, pas un oubli : la route de forfait **en qualification** est réservée au scoreur (`exiger_scoreur`), `E16US008` n'ayant élargi que celle des **duels**. L'écran dit où le geste se fait plutôt que d'armer un bouton qui partirait en 401 — même arbitrage qu'`E16US008` sur « cible non attribuée ». **Élargir cette route à l'organisateur est un choix de rôles, laissé au commanditaire.**
