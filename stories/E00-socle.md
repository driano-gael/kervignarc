...
*En tant qu'*utilisateur qui ouvre l'appli sur un appareil neuf, *je veux* un **écran de choix** au 1ᵉʳ lancement (Tablette de cible / Téléphone public / Admin), *afin de* déclarer explicitement mon rôle plutôt que de le déduire d'une session ouverte.
...
- **CA — écran de choix au 1ᵉʳ lancement** : tant qu'**aucun rôle n'est posé**, l'appli présente trois entrées : **Tablette (cible)**, **Téléphone (public)**, **Admin (PC)**. Le choix **persiste** (`localStorage`) : aux ouvertures suivantes l'appli va **droit** au rôle choisi, **sans réafficher l'écran** — c'est le geste initial explicite, pas un menu récurrent (`D-09` : l'archer, le plus nombreux, ne subit pas de friction à chaque ouverture).
- **CA — chaque choix mène où il doit** : **Tablette** → **rattachement** (QR de la cible, **ou** URL + code de rattachement + n° de cible, E04US001) puis reste un poste et **ne revoit jamais l'écran de choix ni l'admin** (`D-13` : contrôle d'accès physique — quiconque tape sur une tablette de cible est légitime) ; **Public** → l'appli publique (l'archer **et sa famille** suivent l'archer via l'écran de suivi, E07US006) ; **Admin** → le **login** (E10US002). Le public **ne peut pas escalader** : il n'a ni code scoreur ni mot de passe admin.
- **Notes** : **révise le modèle d'entrée de l'appli** (`D-09`/`D-10`/`D-13`, [CDC UX §3–§6](../cahier-des-charges-ux.md)) — **décision structurante ⇒ ADR** à écrire **à l'implémentation** (le dépôt livre ses ADR `Accepté` avec le code, cf. E07US009). Tension à assumer dans l'ADR : le CDC veut que l'archer tombe sur **sa journée**, pas sur un menu (`D-09`, « GPS, pas tableau de résultats ») — acceptable car l'écran n'apparaît qu'au **1ᵉʳ lancement** puis se souvient. Une **sortie de rôle** (« ce n'est pas ma tablette », changer d'appareil) reste à cadrer, cohérente avec le détachement de poste existant (`detacher`). ⚠️ Front sans tests de rendu : vérifier **à l'écran** (3 profils/appareils isolés). Réemploi : jeton de poste (`sessionPosteStore`), login admin (`sessionAdminStore`), aiguillage `App.tsx`.
