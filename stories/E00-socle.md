...
- **CA** : `MessageErreur` vit dans `frontend/src/shared/ui/` ; les 10 features le consomment ; **zéro copie locale restante** ; les **3 blocs de confirmation** `role="alert"` **hors** `MessageErreur` (`competition/TrancheVerticale.tsx` — « Inscrire quand même » ; `archers/Archers.tsx` ×3 — « Enregistrer quand même », « Changer quand même de catégorie », « Supprimer définitivement ») reçoivent le **même** traitement : ils portent un ton **neutre** et une action, un `grep MessageErreur` ne les trouve pas, et les oublier appliquerait le token ambre à dix endroits sur treize ; le rendu est **inchangé** à l'écran (mêmes classes, même `role="alert"`) ; marqueurs `DETTE-004` retirés et ligne déplacée en « Dette résorbée ».
