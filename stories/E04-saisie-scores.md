...
- **CA — file hors-ligne (ex-010)** : hors-ligne, les saisies sont **mises en file côté front** et **rejouées à la reconnexion** ; **pas de doublon** (idempotence, [ADR-0036](../docs/adr/0036-idempotence-de-la-saisie-par-identifiant-en-memoire.md)).
- **CA — indicateur (ex-011)** : **état de connexion visible** en permanence — connecté / hors-ligne / synchronisation en cours.
- **Notes** : la mention « ADR-0005 » du CA « file hors-ligne » était **fausse** (même coquille que celle corrigée en E04US002 pour l'idempotence de saisie) : ADR-0005 ne couvre que la **sérialisation** single-writer ; le zéro-doublon au rejeu vient de l'**identifiant de saisie** ([ADR-0036](../docs/adr/0036-idempotence-de-la-saisie-par-identifiant-en-memoire.md)), garanti **serveur**. Corrigé ci-dessus.
- **Arbitrages tranchés le 20/07/2026** (reversés ici — règle 9 ; [ADR-0037](../docs/adr/0037-file-de-saisie-hors-ligne-et-rejeu.md)) :
  - **Diffusion live (ex-009) : aucun code neuf.** Une validation passe par la file d'écriture ; le listener post-commit d'E00US008 diffuse déjà un `LiveEvent` à tous les abonnés `/ws` (→ invalidation React Query, refetch < 1–2 s). Le CA est satisfait par l'infrastructure existante ; l'affinage en événement **typé par sujet/tournoi** reste ouvert (non requis ici).
  - **Détection du hors-ligne par la nature de l'échec, pas `navigator.onLine`** : le `fetch` **rejette** (panne réseau) → mise en file ; une **`ErreurApi`** (le serveur a répondu un refus) → vraie erreur, jamais mise en file. Sur un LAN sans internet, `navigator.onLine` vaut souvent `true` alors que le serveur est injoignable — il mentirait.
  - **Série optimiste pour ne pas bloquer le marqueur** : une saisie mise en file injecte la volée localement (`en_attente`) → la grille avance. Le **cumul officiel reste inchangé** (il ne compte que le **validé**). Faute quoi le marqueur resterait coincé sur la volée en échec et re-taperait — chaque re-tape étant un **geste distinct** (identifiant neuf), le dédoublonnage serveur ne le rattraperait pas.
  - **Identifiant figé à la mise en file, jamais régénéré au rejeu** : c'est ce qui rend le rejeu idempotent (ADR-0036 ; et pour une volée, l'écriture est un **upsert** par `(série, numéro)`, idempotent même si le serveur a redémarré et vidé son registre). Le rejeu se déclenche **par transition** sur trois signaux — reconnexion WebSocket, **retour du jeton de poste** (re-rattachement après un 401), et **succès d'une saisie en ligne** — jamais sur la longueur de file (boucle chaude).
  - **Un refus au rejeu n'est pas l'autre — sinon perte silencieuse.** `fetchJson` lève `ErreurApi` pour **tout** non-2xx. On discrimine par le statut : seul un **refus définitif** (4xx métier : 400, 403 hors-cible, 404 blason introuvable, 422) est **retiré + journalisé** ; un **transitoire** — **401** (serveur redémarré, jeton de poste volatil perdu), 409 (départ courant perdu), 429, et **tout 5xx** (writer unique saturé à la reconnexion des ~30 tablettes) — est **gardé en file** et rejoué plus tard. « Ne rien perdre » prime (cf. ADR-0037). *(Bloquant remonté par la revue adversariale du 20/07 : le traitement indifférencié de toute `ErreurApi` comme définitive perdait un lot de saisies au redémarrage serveur / à la reconnexion de masse.)*
...
### E04US015 — Gérer abandon / disqualification ✅ *(livrée 27/07/2026)*
*En tant que* scoreur, *je veux* enregistrer un abandon/DSQ, *afin de* refléter la réalité.
- **CA** : statut spécial sur un archer, en **qualification comme en duels** ; impact correct sur la progression et le classement ; **les flèches déjà tirées sont préservées** — un archer qui abandonne reste dans les résultats avec son statut, il n'en disparaît pas. **En qualification** : un **abandon** est **relégué en fin** de classement (rangé, derrière tous les en-lice, score affiché) ; une **disqualification** est **sortie** du classement (pas de rang), son score restant conservé. **En duels** : le forfaitaire **cède** son match, l'adversaire passe d'office (walkover), le tableau reste cohérent. L'acte est **daté, attribué, motivé (optionnel), réversible** (annulation tant que le tournoi n'est pas terminé, `D-15`) et **audité** (`FORFAIT`). Acteur : le **scoreur**.
- **Notes** : c'est l'**alternative désignée à la suppression d'archer** (E02US003), qui **préserve** les flèches là où la suppression les détruit ([ADR-0016](../docs/adr/0016-supprimer-un-archer-engage-plutot-que-le-refuser.md), arbitrage métier du 16/07/2026). Un abandon qui effacerait les résultats rendrait ADR-0016 faux.
  > **Incohérence latente arbitrée le 27/07/2026** (cf. [ADR-0050](../docs/adr/0050-forfait-abandon-et-disqualification.md)) : la note « abandon **en qualification** seul » contredisait le jalon (J2) et la dépendance (duels). Tranché : **une seule US couvre les deux contextes**, ce qui **absorbe E12US004** (« tracer un forfait en duels », désormais caduque). Le concept `Forfait` est **scopé à la phase** (`phase_id`) — un abandon en duels ne relègue pas le rang de qualif d'un archer qui avait pourtant qualifié. `Q-UX5` d'E12US004 (acteur admin/scoreur) est **fermée** : c'est le **scoreur** (celui qui constate l'abandon sur le terrain).
- **Dépend de** : E04US002 (saisie qualif, livrée), E04US013 (saisie duels, livrée), E05US005 (arbre) · **Jalon** : J2
...
- **CA** : dès la validation, la tablette **bascule en panneau de routage** : pour **chaque** archer concerné, sa **prochaine affectation** (cible, position, tour, adversaire), ou son **rang final** s'il est éliminé ; l'affichage est **instantané** — rien n'est calculé à cet instant (`D-08`) ; retour à la grille pour la suite. Ce qui n'est **pas encore connu** est **nommé** (`P-3`), jamais laissé en blanc.
...
- **Arbitrages tranchés au cadrage du 30/07/2026** (reversés ici — règle 9) :
  - **Les deux surfaces, pas une.** Le CA disait « dès la validation (E04US002) », donc la **qualification**, alors que toute la note parle de matchs et de duels. Périmètre retenu : **les deux** — après validation des séries d'une cible (routage vers le 1ᵉʳ tour, le cas le mieux outillé puisque le plan de duels pose le tour 1) **et** après validation d'un duel (routage des deux duellistes). Un seul service, un seul endpoint, un seul composant : c'est la même question.
  - **Trois informations du CA étaient insatisfiables en l'état** — constat fait au cadrage, avant d'implémenter. Elles ne sont **pas** livrées, et leur **absence est affichée en clair** plutôt que masquée : (a) l'**heure** du prochain tour n'existe nulle part — les horaires vivent sur les `Depart` (qualif), pas sur les tours de tableau ; c'est le **lancement** (E12US002) qui fait partir un tour, pas une heure. Le mot « heure » est **retiré du CA** ; (b) le **repêchage** est **E05US016**, non livrée — le routing du jour est l'élimination sèche ; la mention est **retirée du CA**, elle y reviendra avec l'US qui l'implémente ; (c) le **rang final** n'est calculable que pour le **podium** (rangs 1-4, `Tableau.podium`) — les rangs intermédiaires (9ᵉ-16ᵉ…) supposent l'agrégation d'**E06US004**, non livrée. Le panneau affiche alors le **tour de sortie** (« Éliminé — Quart de finale ») et « rang publié en fin de phase », **jamais un rang inventé**.
  - **Pas de cible au-delà du tour 1.** Le plan de duels ne pose que le 1ᵉʳ tour ([ADR-0048](../docs/adr/0048-cote-a-cote-des-duellistes-par-reordonnancement.md)) ; le placement intégral 1→N est **E05US010**. Un archer de tour ≥ 2 garde bien une ligne dans `placement_tableau`, mais c'est **celle de son tour 1** : la réutiliser enverrait un finaliste sur son ancienne butte. Le panneau n'attribue donc **aucune** cible au-delà du tour 1 et affiche « cible attribuée au lancement du tour » — **même garde** que le feu vert d'E12US002, pour la même raison.
  - **Ce qui n'est pas connu est nommé côté serveur.** Les phrases d'attente (« cible attribuée au lancement du tour », « en attente du duel n°2 », « rang publié en fin de phase », « phase finale non configurée ») viennent de l'**application**, pas du front : les **quatre canaux** de routage doivent dire la même chose, et c'est le serveur qui sait *pourquoi* la donnée manque. Même parti pris que le `blocage` du feu vert.
  - **Lecture publique, sans authentification**, comme le déroulé ([ADR-0039](../docs/adr/0039-exposition-publique-du-deroule-scores-provisoires.md)) : c'est la projection que liront E07US008 et E07US004. Conforme au contrat de lecture d'E10US001.
  - **Hors-ligne, pas de bascule.** Une validation de duel mise en file (E04US009) n'a pas fait avancer le tableau côté serveur : router à cet instant annoncerait le duel qu'on vient de scorer comme « prochain ». Le panneau ne s'ouvre donc **que** sur une validation réellement partie.
  - **Corrections issues de la revue (`/revue-us`, 30/07/2026)** — reversées ici parce qu'elles changent le **comportement attendu**, pas seulement le code : (a) le panneau reste **ouvrable à la main** en toutes circonstances — un archer **forfait** (E04US015) reste dans la grille avec une série qui ne se complétera jamais, et sans garde la cible entière aurait perdu la fonctionnalité à vie ; la ligne de grille porte donc un signal `forfait` **serveur**, et une série forfaite compte comme close (même notion que la complétude, DETTE-014) ; (b) **toutes** les lignes du panneau sont **nominatives**, y compris le panneau dégradé (pas de phase de tableau — l'état le plus fréquent de la journée) et un archer forfait : les noms viennent des **archers du tournoi**, jamais du classement, dont un abandon est relégué et une disqualification **sortie** (ADR-0050) — les lire là aurait rendu anonymes précisément les archers qu'on route encore ; (c) une pose douteuse est **annoncée avec une alerte**, jamais effacée : un simple **reclassement** (correction de score) suffit à désaccorder l'appariement *recalculé* du plan *persisté*, et le signal vient du domaine (`duels_separes`, E03US009) plutôt que d'un contrôle refait ici. On n'efface pas la cible parce que rien ne distingue « plan périmé » de « cibles trop petites pour rapprocher les duellistes » — cas que le placement **accepte** — et que dans les deux cas la pose reste la place physique de l'archer ; (d) un `phase_id` **imposé** par le client est **validé** (404 s'il est inconnu ou d'un autre tournoi) et la résolution implicite vise le premier tableau **non terminé** ; (e) **deux** causes de « pas de cible » sont distinguées (tour à venir / non attribuée), et une **alerte** distincte accompagne une pose annoncée mais non côte à côte — qualifiée selon que l'adversaire est sur la **même** butte (se décaler) ou sur une **autre** (ne pas s'installer, voir l'organisateur) : un message unique pour deux situations opposées se trompait forcément sur l'une des deux.
  - ⚠️ **Deux de ces arbitrages ont été révisés le 02/08/2026 par E07US008** ([ADR-0065](../docs/adr/0065-rang-acquis-lu-sur-la-plage-et-issue-repechee.md)), et ce panneau-ci en **bénéficie**, la projection étant la même pour les quatre canaux : (a) le **rang de l'éliminé** n'est plus renvoyé à E06US004 — il se lit sur la **plage du match perdu** (*Règle R*), donc sous forme de **fourchette** (« 5ᵉ-8ᵉ ») ; « rang publié en fin de phase » ne subsiste que là où **rien** n'est acquis. Le point (c) ci-dessus est donc **périmé pour la partie « rangs intermédiaires »** ; (b) le **repêchage** est traité et porte une **issue distincte** de « éliminé » (`repeche`) : un repêché ne consomme aucun rang, il peut encore remonter — la mention retirée du CA au point (b) y **revient**, comme annoncé. Seule l'**heure** (point a) reste hors périmètre, pour la raison inchangée.
  - **Aucun ADR** : la décision structurante de fond (le lancement comme **événement**, et les 4 canaux récepteurs) est déjà posée par [ADR-0056](../docs/adr/0056-lancement-d-un-tour-acte-audite-et-diffuse.md) ; cette US en **construit le premier récepteur**, elle ne tranche rien de neuf. Le service `ServiceRoutage` est le **jumeau** de `ServicePilotageTour` (2ᵉ occurrence de « archer → cible du plan » et de la garde tour-1) : duplication **assumée**, extraction à la 3ᵉ.
- **Dépend de** : E04US002, E04US013, E03US009, E05US005, E12US002 · **Jalon** : J2
