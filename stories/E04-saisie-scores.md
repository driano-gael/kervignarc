...
- **Arbitrages tranchés le 20/07/2026** (reversés ici — règle 9 ; [ADR-0037](../docs/adr/0037-file-de-saisie-hors-ligne-et-rejeu.md)) :
  ...
  - **Identifiant figé à la mise en file, jamais régénéré au rejeu** : c'est ce qui rend le rejeu idempotent (ADR-0036). Déclencheur du rejeu = **reconnexion du WebSocket** (sur un LAN, coïncide avec le retour réseau) ; pas de boucle de rejeu chaude.
  - **Refus serveur au rejeu = perte visible assumée** : une saisie en file n'a jamais été validée par le serveur ; s'il la refuse définitivement au rejeu (`ErreurApi`), elle est **retirée** (sinon elle bloque la file) et **journalisée**, la relecture de série la faisant disparaître de la grille. Cas rare, pas de flux de reprise dédié (cf. ADR-0037, limites).
