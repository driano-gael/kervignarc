...
- **Arbitrages tranchés au cadrage du 30/07/2026** (reversés ici — règle 9) :
  ...
  - **Corrections issues de la revue (`/revue-us`, 30/07/2026)** — reversées ici parce qu'elles changent le **comportement attendu**, pas seulement le code : (a) le panneau reste **ouvrable à la main** en toutes circonstances — un archer **forfait** (E04US015) reste dans la grille avec une série qui ne se complétera jamais, et sans garde la cible entière aurait perdu la fonctionnalité à vie ; la ligne de grille porte donc un signal `forfait` **serveur**, et une série forfaite compte comme close (même notion que la complétude, DETTE-014) ; (b) les lignes du panneau **dégradé** (pas de phase de tableau — l'état le plus fréquent de la journée) sont **nominatives**, le classement étant lisible sans tableau ; (c) une pose douteuse est **annoncée avec une alerte**, jamais effacée : un simple **reclassement** (correction de score) suffit à désaccorder l'appariement *recalculé* du plan *persisté*, et le signal vient du domaine (`duels_separes`, E03US009) plutôt que d'un contrôle refait ici. On n'efface pas la cible parce que rien ne distingue « plan périmé » de « cibles trop petites pour rapprocher les duellistes » — cas que le placement **accepte** — et que dans les deux cas la pose reste la place physique de l'archer ; (d) un `phase_id` **imposé** par le client est **validé** (404 s'il est inconnu ou d'un autre tournoi) et la résolution implicite vise le premier tableau **non terminé** ; (e) trois causes de « pas de cible » sont **distinguées** (tour à venir / non attribuée / placement périmé).
