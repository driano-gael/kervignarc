...
(`tournoi_id`), comme `scoreur`/`poste` : FK **sans `ON DELETE`** — purge **applicative**, dans
`TournoiRepositorySQL.supprimer` (ADR-0077). ⚠️ Le journal **compte** au décompte de suppression :
c'est une trace, pas de la configuration, et rien ne le reconstitue (E01US026).
