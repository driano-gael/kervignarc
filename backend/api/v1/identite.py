...
class LogoPresentReponse(BaseModel):
    ...
    du logo. Une URL stable ne provoque aucune requête sur une image déjà montée — `Cache-Control:
    no-cache` ne s'y applique pas — et un organisateur qui corrigeait son fichier ne le voyait
    jamais ; versionner par l'horloge, à l'inverse, retéléchargeait 512 Ko à chaque événement.
    """
...
class IdentiteReponse(BaseModel):
    ...
    ⚠️ `reglee` à `false` **n'est pas un vide** : les accents rendus sont ceux du club, et l'écran
    doit dire « hérité de l'identité du club » plutôt qu'afficher un formulaire vierge.
    `seuil_contour`/`seuil_texte` voyagent avec la réponse plutôt que d'être recopiés côté front —
    un front qui porterait sa copie annoncerait « conforme » sur un seuil désormais inappliqué.
    """
...
async def apercu_d_une_identite(
    ...
    Permet à l'écran de montrer le rendu et le chiffre de contraste pendant que l'organisateur
    choisit, sans que le navigateur recalcule la dérivation. Route de calcul pur : aucun accès à la
    base, donc ni file d'écriture ni threadpool. ⚠️ `reglee` vaut `true` dans la réponse — c'est un
    aperçu de ce que donnerait l'enregistrement, pas un état persisté (aucun tournoi n'est cité).
    """
...
async def deposer_un_logo(
    ...
    Le format est lu dans `Content-Type` (`image/png` ou `image/svg+xml`). `422` sur un format non
...
async def logo_du_tournoi(
    ...
    ⚠️ **Les trois en-têtes de sûreté ne sont pas décoratifs** : un SVG servi depuis l'origine de
    la SPA d'administration partagerait sa session. `default-src 'none'`, `nosniff`,
    `Content-Disposition: inline` sans nom de fichier — seconde barrière si un fichier est entré
    sous des règles antérieures. ⚠️ **L'empreinte est lue AVANT les octets** : charger le blob pour
    l'`ETag` annulait, sur la route la plus chaude, la projection sans blob de la table séparée.
    """
    service: ServiceIdentite = request.app.state.service_identite
