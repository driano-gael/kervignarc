"""Placement d'une phase de **poules** sur les cibles de la salle (E05US023, [ADR-0083]).

Règle **fournie par le commanditaire le 09/08/2026**, elle n'est écrite nulle part ailleurs :

> Une poule occupe un **bloc de couloirs contigus**. Quand elle déborde d'une cible, elle prend la
> suite sur la cible d'après, et **la poule suivante démarre au couloir libre juste après** — on
> n'ouvre pas une cible neuve pour elle. La salle se remplit en continu, poule après poule.

**L'unité placée est la poule, pas l'archer**, et c'est la différence de fond avec le placement de
qualification (`domain/placement.py`, un archer par couloir) comme avec le plan de duels
(`ServicePlacementDuels`, deux adversaires côte à côte). La raison est dans `couloirs_occupes` : une
poule de 5 tient sur 4 couloirs parce qu'un membre se repose à chaque tour — mais **ce n'est jamais
le même**. Aucun des cinq n'a de couloir attitré ; ils tournent sur le bloc. Persister « archer →
couloir » serait donc écrire une information fausse.

Ce que le bloc porte est **matérialisé** ; les couloirs de chaque rencontre, tour par tour, sont
**dérivés** à la lecture — même parti que l'appariement d'un tableau, recalculé et jamais persisté
(ADR-0023/0048).

Domaine **pur** : aucun framework, aucune autre couche (règle 1).

[ADR-0083]: ../../docs/adr/0083-le-contrat-de-phase-jouable.md
"""

from __future__ import annotations

from collections.abc import Sequence
from dataclasses import dataclass
from enum import Enum

from domain.gabarit_salle import GabaritSalle
from domain.poule import Poule, couloirs_occupes


@dataclass(frozen=True)
class BlocDePoule:
    """La plage de couloirs contigus attribuée à une poule.

    `places` porte la liste **ordonnée** des couloirs occupés, chacun `(index de cible, lettre)`.
    C'est le seul champ d'état : `cible_index`, `position_depart` et `nb_couloirs` s'en déduisent,
    plutôt que d'être stockés en double et de pouvoir diverger. Un bloc traverse librement plusieurs
    cibles — c'est même son intérêt.
    """

    poule: int
    places: tuple[tuple[int, str], ...]

    def __post_init__(self) -> None:
        if not self.places:
            raise ValueError(f"La poule {self.poule} n'occuperait aucun couloir.")

    @property
    def cible_index(self) -> int:
        """L'index (1-based) de la cible où la poule **commence**."""
        return self.places[0][0]

    @property
    def position_depart(self) -> str:
        """La lettre du premier couloir occupé (A..D)."""
        return self.places[0][1]

    @property
    def nb_couloirs(self) -> int:
        """Le nombre de couloirs du bloc — l'empreinte de la poule."""
        return len(self.places)

    def couloirs(self) -> tuple[tuple[int, str], ...]:
        """Les couloirs occupés, cible par cible, dans l'ordre de remplissage.

        Ce que lisent le plan de salle et la feuille de poule : un bloc annoncé « cible 1, couloir
        A, 6 couloirs » reste illisible pour qui doit poser les archers, alors que la liste dit
        exactement où ils vont.
        """
        return self.places


class RaisonConflitPoule(str, Enum):
    """Pourquoi une poule n'a pas pu être posée."""

    SALLE_PLEINE = "salle_pleine"
    """Il ne reste pas assez de couloirs contigus pour l'empreinte de cette poule."""

    SANS_RENCONTRE = "sans_rencontre"
    """La poule compte moins de deux membres : elle n'apparie personne, donc n'occupe rien."""

    NON_POSEE = "non_posee"
    """Aucun bloc ne porte cette poule — le plan n'a pas encore été posé, ou l'a été sur une autre
    composition (E05US023).

    ⚠️ **Cette raison ne naît jamais de `placer_les_poules`**, qui pose ou rapporte `SALLE_PLEINE` :
    elle naît à la **lecture**, quand la composition du jour ne retrouve pas son bloc en base. Le
    cas est normal avant la première pose, et il est un **signal** après : l'effectif a bougé depuis
    que le plan a été écrit, donc il faut le reposer. Le distinguer de `SALLE_PLEINE` évite
    d'annoncer une salle trop petite à un organisateur qui a simplement oublié de générer son
    plan."""


@dataclass(frozen=True)
class ConflitDePoule:
    """Une poule que le placement n'a pas pu poser, et pourquoi.

    Même parti que `PlanDeCibles.conflits` en qualification (ADR-0024) : le placement **rapporte**
    ce qu'il n'a pas pu faire au lieu de tronquer en silence. L'organisateur doit voir à l'atelier
    qu'une poule n'a pas de cible, pas le découvrir le jour J.
    """

    poule: int
    raison: RaisonConflitPoule


@dataclass(frozen=True)
class PlanDePoules:
    """Résultat du placement : les blocs posés + les poules restées sans cible."""

    blocs: tuple[BlocDePoule, ...]
    conflits: tuple[ConflitDePoule, ...] = ()


def _couloirs_du_gabarit(gabarit: GabaritSalle) -> tuple[tuple[int, str], ...]:
    """Tous les couloirs de la salle, à plat et dans l'ordre de remplissage.

    Mettre la salle à plat est ce qui rend la contiguïté triviale : « la poule suivante démarre au
    couloir libre juste après » devient un simple curseur qui avance, et le débordement d'une cible
    sur la suivante n'est plus un cas particulier à traiter. La **capacité réelle de chaque cible**
    est respectée au passage (`GabaritSalle` l'autorise de 1 à 4, et variable d'une cible à l'autre
    depuis `ajuster`) : c'est elle qui décide où l'on déborde, jamais un 4 supposé.
    """
    return tuple(
        (cible.index, position) for cible in gabarit.cibles for position in cible.positions
    )


def placer_les_poules(poules: Sequence[Poule], gabarit: GabaritSalle) -> PlanDePoules:
    """Pose les poules **dans l'ordre**, chacune sur un bloc de couloirs contigus.

    L'empreinte de chaque poule vient de `couloirs_occupes` — son parallélisme, pas son effectif
    (cf. la docstring de cette fonction, c'est l'arbitrage qui surprend).

    ⚠️ **À la première poule qui ne tient pas, on s'arrête** : les suivantes sont rapportées en
    conflit même si l'une d'elles, plus petite, serait entrée dans la place restante. Poser une
    poule 5 dans le trou laissé par la poule 4 casserait l'ordre du plan — l'organisateur lit « la
    poule *n* est à tel endroit », et une salle où l'ordre saute est plus coûteuse à exploiter
    qu'une salle où deux poules manquent visiblement à la fin. On préfère un plan tronqué et lisible
    à un plan complet et surprenant.

    Fonction **pure et déterministe** (règle 9) : mêmes poules, même gabarit, même plan.
    """
    disponibles = _couloirs_du_gabarit(gabarit)
    curseur = 0
    blocs: list[BlocDePoule] = []
    conflits: list[ConflitDePoule] = []
    salle_pleine = False

    for poule in poules:
        besoin = couloirs_occupes(len(poule.membres))
        if besoin == 0:
            conflits.append(ConflitDePoule(poule.numero, RaisonConflitPoule.SANS_RENCONTRE))
            continue
        if salle_pleine or curseur + besoin > len(disponibles):
            salle_pleine = True
            conflits.append(ConflitDePoule(poule.numero, RaisonConflitPoule.SALLE_PLEINE))
            continue
        blocs.append(BlocDePoule(poule.numero, disponibles[curseur : curseur + besoin]))
        curseur += besoin

    return PlanDePoules(blocs=tuple(blocs), conflits=tuple(conflits))
