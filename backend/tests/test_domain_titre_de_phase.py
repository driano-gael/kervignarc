...
⚠️ **Écrits avant l'implémentation** (règle 9) : ce fichier a été committé avant que `titre`
n'existe sur `EtapeDeroule`. Ce que le CA ne dit pas a été tranché ici et **reversé à la fiche**
dans le même commit — un CA muet qu'on complète en silence est un CA périmé pour l'US suivante.
"""
