...
def test_une_profondeur_au_dela_du_plafond_est_refusee() -> None:
    """Un podium a un plafond : au-delà ce n'est plus un podium, c'est le classement — qui suit.

    Borné **au domaine** et non à la frontière (parti d'`ReglagePages`, E16US009) : sans plafond,
    une valeur absurde traversait tout et cassait à l'insertion SQLite en 500 non typé.
    """
    assert ReglagePodiums(profondeur=PROFONDEUR_PODIUM_MAX).profondeur == 64, "la borne est légale"
    with pytest.raises(ProfondeurPodiumInvalide):
        ReglagePodiums(profondeur=PROFONDEUR_PODIUM_MAX + 1)
...
def test_le_bloc_porte_le_libelle_de_ce_qu_il_recompense() -> None:
    ...
    ⚠️ Le bloc sans regroupement se dit « **Toutes catégories** » et **pas** « Scratch » : le
    glossaire réserve ce mot à un libellé de catégorie (arc nu). Un club qui nomme ainsi sa
    catégorie arc nu imprimait deux blocs de même titre sur la même page (relevé en revue).
    """
    palmares = _palmares_de_huit({i: 1 if i <= 4 else 2 for i in range(1, 9)})
    reglage = ReglagePodiums(
    ...
    assert libelles == [
        (PorteePodium.SCRATCH, None, "Toutes catégories"),
...
def test_le_podium_par_club_renumerote_les_archers_du_club_depuis_un() -> None:
    ...
    catégorie : le 2ᵉ du tournoi (archer 1) est **1ᵉʳ de son club**. Le vainqueur du tableau
    (archer 6) est au club 2 : personne ne monte deux fois sur le même podium.

    ⚠️ **Ce test ne prouve QUE la renumérotation depuis 1** : aucune agrégation n'étant injectée,
    `AggregationParQualification` éclate déjà le paquet 5ᵉ-8ᵉ en singletons, donc tous les rangs
    scratch sont exacts. La borne « un rang en fourchette au scratch devient exact dans l'espace du
    club » est éprouvée par le test suivant — la justification portée ici était fausse (revue).
    """
    palmares = _palmares_de_huit({i: 1 if i <= 4 else 2 for i in range(1, 9)})
...
def test_un_ex_aequo_au_scratch_monte_au_podium_de_son_club_s_il_y_est_seul() -> None:
    """CA « chacun sur **son** rang » — la borne positive, celle qui distingue les trois espaces.

    Sous `AggregationExAequo`, les archers 4, 5, 7 et 8 restent *ex æquo* 5ᵉ-8ᵉ : aucun n'a de place
    au podium scratch. Mais l'archer 4 est **seul de son club** dans ce paquet, donc son rang de
    club, lui, est **exact** — et il monte sur le podium du club 3. C'est exactement ce que
    « chacun sur son rang » veut dire, et rien ne le prouvait.
    """
    clubs: dict[int, int | None] = {1: 1, 2: 1, 3: 1, 4: 3, 5: 1, 6: 1, 7: 2, 8: 2}
    ...
    assert 4 not in _archers(palmares, PorteePodium.SCRATCH, None)
    assert _archers(palmares, PorteePodium.CLUB, 3) == [4]
...
def test_l_effectif_ne_compte_que_les_archers_qui_peuvent_occuper_une_place() -> None:
    ...
    hors_classement = [ligne for ligne in palmares.lignes if ligne.rang_min is None]

    assert len(palmares.lignes) == 4, "les quatre archers restent au palmarès"
    assert [ligne.archer_id for ligne in hors_classement] == [4], "le DSQ n'a aucun rang (ADR-0050)"
...
def test_un_palmares_vide_ne_decerne_aucun_bloc() -> None:
    """Le scratch ne regroupe rien : sans cette sortie, il existerait **toujours**.

    Un bloc « Toutes catégories » à effectif zéro flottait alors au-dessus de personne, et l'écran
    y écrivait une phrase d'état. C'est aussi ce qui rend vraie la garde de vacuité des surfaces.
    """
    vide = Palmares(lignes=())

    assert vide.podiums(ReglagePodiums(portees=frozenset({PorteePodium.SCRATCH}))) == ()
    assert vide.podiums(ReglagePodiums()) == ()


def test_pendant_la_qualification_les_blocs_disent_en_cours_et_non_jamais() -> None:
    """CA « l'écran distingue *pas encore* de *plus jamais* » — le cas le plus long de la journée.
    ...
    assert matinee.podiums(reglage)[0].en_attente is True, "des duels restent à tirer"
    assert apres.podiums(reglage)[0].en_attente is False, "plus aucun duel : c'est définitif"
