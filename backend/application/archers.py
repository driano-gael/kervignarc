...
class ServiceArchers:
    ...
    def ajouter(self, tournoi_id: TournoiId, nom: str) -> Archer:
        """Inscrit un archer à un tournoi. Lève `TournoiIntrouvable` si le tournoi n'existe pas."""
        if self._tournois.par_id(tournoi_id) is None:
            raise TournoiIntrouvable(f"Aucun tournoi d'identifiant {tournoi_id}.")
        return self._archers.ajouter(Archer.creer(nom, tournoi_id))
