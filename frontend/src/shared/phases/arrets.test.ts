...
function etat(...arrets: [string, 'phase' | 'depart'][]): EtatArrets {
  return {
...
describe('le défaut, qui est l’enchaînement automatique', () => {
  ...
  it('se reconstruit à l’identique depuis une étape qui n’a rien', () => {
    expect(depuisEtape(null)).toEqual({ lignes: [] })
    expect(depuisEtape(undefined)).toEqual({ lignes: [] })
  })
})
...
describe('la conversion vers le serveur', () => {
  ...
  it('distingue « aucun arrêt » de « ne soumets pas »', () => {
    // ⚠️ La garde qui compte. Une liste **vide** veut dire « aucun arrêt » et s'envoie telle quelle
    // (l'édition est totale côté API) ; `undefined` veut dire « illisible, ne soumets pas ». Les
    // confondre effacerait le planning d'un organisateur en train de retaper un champ.
    expect(versArrets(etat())).toEqual([])
    expect(versArrets(etat(['2', 'phase']))).toEqual([{ apres_tour: 2, portee: 'phase' }])
    expect(versArrets(etat(['deux', 'phase']))).toBeUndefined()
    expect(versArrets(etat(['0', 'phase']))).toBeUndefined()
    expect(versArrets(etat(['2.5', 'phase']))).toBeUndefined()
    expect(versArrets(etat([String(TOURS_MAX_REGLABLES + 1), 'phase']))).toBeUndefined()
  })
