...
| `openpyxl` | 3.1.5 | Écriture de classeurs `.xlsx` (exports tableur) | Format Excel réclamé par le CA A18 et écarté d'`E16US007` faute de dépendance — arbitrage du commanditaire du 18/09/2026 ([ADR-0101 §6](adr/0101-le-catalogue-d-exports-porte-les-formats-pas-les-url.md)). Le CSV ne couvre pas ce que l'US demande : une feuille nommée, l'en-tête figé, et des montants **réellement numériques** plutôt que du texte. Pur Python (aucune bibliothèque native), embarquable PyInstaller ; `pip-audit` vert. `xlsxwriter` (écriture seule, plus légère) a été présenté et écarté. ⚠️ **N'expose pas `py.typed`** : même cas et même remède que ReportLab — override `ignore_missing_imports` ciblé, **sans** paquet de stubs (règle 11), bien que `types-openpyxl` existe et soit officiel. Tire `et-xmlfile` (transitif, MIT) | MIT |
| `xlrd` | 2.0.2 | **Lecture** du classeur Résult'Arc (`.xls` OLE2) à l'import des inscrits | Arbitrage du commanditaire du 26/09/2026 au cadrage d'`E02US007` ([ADR-0115](adr/0115-l-import-des-inscrits-est-un-plan-pur-ecrit-en-une-transaction.md)) : Résult'Arc exporte un `.xls` binaire hérité, qu'`openpyxl` (`.xlsx` seul) ne lit pas. Pur Python, **aucune** transitive, lecture seule et sans XML (donc sans surface XXE). ⚠️ Le fichier réel est jugé **corrompu** par xlrd (table d'allocation OLE2 mal formée) : il s'ouvre avec `ignore_workbook_corruption=True`, que l'adapter pose et commente. ⚠️ **Entrée non fiable lue sans contrôle de corruption** : risque accepté, borné par la garde `exiger_admin`, le plafond de corps de 2 Mo, `on_demand` + `ragged_rows` et un plafond de 5 000 rangées (revue d'E02US007, axe adversarial). **Maintenance minimale** (bibliothèque en fin de vie fonctionnelle : lecture `.xls` seule, rares versions) mais **très adoptée** (dépendance optionnelle de pandas pour le `.xls`) — non vérifié en ligne ce jour. Alternative écartée : convertir par LibreOffice (outil absent le jour J). **N'expose pas `py.typed`** : override ciblé, `DETTE-102` élargie (`types-xlrd` existe) | BSD |
