...
| Conséquence — point unique de réponse binaire | `backend/api/documents.py` (`reponse_document`, `reponses_document`, `MEDIA_TYPES`) — module d'API **sans routeur**, appelé par `listes_impression.py`, `feuille_de_marque.py` et, depuis E16US016, `palmares.py` et `audit.py` — **quatre** routeurs. Il porte aussi la politique d'en-têtes de tout document servi : `X-Content-Type-Options: nosniff` systématique, et `inline` réservé au **PDF** du palmarès (les tableurs partent en `attachment`) ; exhaustivité gardée par `test_chaque_format_porte_un_media_type_distinct` | oui — ⚠️ **corrigé en revue (axes A, C2)** : il vivait dans le routeur du catalogue, et le dictionnaire OpenAPI réénumérait les types MIME **à la main**, dans le module même qui importe le registre |
