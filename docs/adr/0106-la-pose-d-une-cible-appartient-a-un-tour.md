...
`ServicePlacementDuels` charge le décor du **tour à poser** : le plus petit tour encore à jouer, et
seulement s'il est **entièrement déterminé** (`domain.tableau.tour_a_poser`). Un tableau terminé
garde son dernier tour affiché, pour que le plan de la finale reste consultable.
...
L'acte automatique **complète les trous**, il ne régénère pas. C'est ce qui garantit trois
propriétés d'un coup :

- une validation de duel ne peut jamais **écraser** un ajustement manuel de l'organisateur ;
- il est **idempotent** — un tour déjà posé n'a pas de trou, rien n'est écrit ;
- il **repose** un tour qu'une correction de score a périmé : les poses devenues orphelines sont
  purgées par `_poses_a_jour`, donc redeviennent des trous, donc sont recomplétées.
...
⚠️ **Deux chemins d'écriture, et pas un** : un tour s'achève par la validation d'un duel **ou** par
un forfait en duel (walkover), qui tranche sans qu'aucun score soit saisi. N'en brancher qu'un
rendait la pose automatique muette dès qu'un tour se finissait sur un forfait — le mode de panne de
`DETTE-028`, qu'aucun test ne fait rougir.
...
- **La garde tour-1 tombe aux quatre sites** de `DETTE-019` (feu vert, routage, et leurs deux
  jumeaux **front**). Elle n'est pas supprimée mais **généralisée** : la comparaison n'est plus
  `match.tour == 1` mais `match.tour == <tour posé>`. Le principe de sûreté est intact — une pose
  n'est annoncée que pour le tour auquel elle appartient.
...
- `backend/domain/tableau.py` — `paires_du_tour` (généralise `paires_du_premier_tour`, décision 2)
  et `tour_a_poser`, fonction **pure** qui porte à elle seule « quel tour peut être posé » —
  décision 2, y compris le saut d'un tour sans duel disputé et le `None` d'un tableau terminé.
...
- `backend/domain/ports.py` — `PlacementTableauRepository` : les quatre gestes portent le tour
  ...
  au port `application.formats.LecteurDonneesDePhase`, question distincte.
...
- `backend/application/placement_duels.py` — `_Contexte.tour` et `_charger` (décisions 2 et 3),
  `PlanDeDuels.tour`, `poser_le_tour_courant` et `_completer` (décision 4, avec les deux exclusions).
- `backend/application/pose_du_tour.py` — `PoseurDeTour` et `DeclencheurPoseDeTour` (décision 5).
- `backend/application/saisie_duels.py` (`brancher_poseur_de_tour`, `_signaler_validation_de`) et
  `backend/application/forfaits.py` (`declarer_en_duel`, `annuler_en_duel`) — les **deux** chemins
  d'écriture de la décision 5.
...
- `frontend/src/features/feu-vert/etat.ts` (`actionDuel`) et
  `frontend/src/features/routage/presentation.ts` (`partitionner`) — la garde généralisée, côté
  front : les deux sites que `DETTE-019` désignait comme les plus dangereux.
