...
| CA pastille de rappel | `frontend/src/features/accueil/BlocsParDepart.tsx` (`BlocDepart`, `useQueries` sur la route déjà pollée) ; `frontend/src/shared/ui/useMaintenant.ts` (`useMaintenant`) | oui — `BlocsParDepart.test.tsx` couvre la **phrase**, le **résumé** et le fait que l'annonce tombe sur le **bon créneau** |
