...
| CA pastille de rappel | `frontend/src/features/accueil/BlocsParDepart.tsx` (`BlocDepart`, `useQueries` sur la route déjà pollée) ; `frontend/src/shared/ui/useMaintenant.ts` (`useMaintenant`) | oui — `frontend/src/features/accueil/BlocsParDepart.test.tsx` couvre le **montage** : la phrase rendue, et sur le **bon** créneau (un seul `role="status"` quand deux blocs sont affichés). ⚠️ La phrase et le résumé eux-mêmes gardent leurs oracles dans `frontend/src/shared/phases/relance.test.ts` — le test de montage pose `arrete_depuis: null`, donc il ne calcule aucune durée |
