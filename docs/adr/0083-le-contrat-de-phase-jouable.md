...
| `backend/domain/placement_par_bloc.py` *(ex-`placement_poules.py`, renommé en E05US026)* | §3 — le bloc contigu, le débordement, l'accolement de la poule suivante, le rapport de conflits |
...
| `backend/domain/contrat_phase.py` | §1 — le **registre** : une ligne par type, huit capacités, et les tables dérivées. ⚠️ **Deux dérivées vivent hors de ce module** depuis E05US026 — `saisie_duels.TYPES_DELEGUES` et `palmares._TYPES_CLASSANTS_AU_PALMARES` : elles dérivent bien du registre, mais leur domicile est le consommateur, parce qu'elles retranchent ce que *ce* consommateur traite lui-même. Le §1 disait « les dix tables dérivées » comme si elles vivaient toutes ici (`TYPES_EN_TABLEAU`, `TYPES_DEROULES`, `TYPES_CLASSANTS_LUS`, `TYPES_EN_TABLEAU_JOUE`, `TYPES_JOUES`, `TYPES_SIGNALES_EN_ECART`, …) |
...
| `backend/infrastructure/db/models.py` · `PlacementParBlocORM` + `migrations/versions/{0045_placement_des_poules,0046_placement_par_bloc}.py` | §3 — « groupe → couloirs », clé primaire sur le **couloir** (un couloir, un occupant). ⚠️ Table et colonne **renommées en E05US026** (`placement_poule` → `placement_par_bloc`, `poule_numero` → `groupe_numero`) : le mécanisme sert désormais deux formats, et le nom désignait le mauvais concept |
...
| `backend/application/prelevement.py` · `LecteurClassementDePhase` | §6 — le **port étroit** qui casse le cycle `ServicePoules` ↔ `ServiceSaisieDuels`, et qui fait traverser le résolveur (donc le cache de reconstruction **et** la chaîne anti-boucle). ⚠️ **Fondu en E05US026** ([ADR-0084](0084-un-seul-port-de-lecture-de-classement-resolu-par-type.md)) : il s'appelait `LecteurClassementPoules`, avait été recopié en `LecteurClassementBigShootOff`, et le système suisse a fourni la 3ᵉ preuve qui justifiait de les réunir |
...
| `frontend/src/shared/stores/fileDuelsHorsLigneStore.ts` · `FamilleDuel` | §7 — un acte de rencontre de poule entre dans la **même** file hors-ligne et s'y rejoue (E04US009). ⚠️ **Trois valeurs depuis E05US030** (`tableau` \| `poule` \| `suisse`) |
| `backend/application/suisse.py` · `ServiceSuisse` · `backend/domain/{suisse,classement_de_suisse}.py` | §1, §6 — le **3ᵉ format jouable** (E05US026) : appariement ronde par ronde au classement du moment, borne `rondes_maximales`, classement prélevable. `conflits` est renseigné **à la lecture** depuis E05US030 (§3 : on rapporte le manque, on ne le comble pas) |
| `frontend/src/shared/phases/{suisse.ts,ReglageSuisse.tsx}` | §4 — la fiche **sans état** partagée par les deux écrans de composition (E05US030), et la borne d'effectif en clair : miroir assumé de `domain/suisse.py::rondes_maximales`, 19 tests |
| `frontend/src/features/suisse/{SaisieSuisse.tsx,presentation.ts,ClassementSuisse.tsx}` | §1, §7 — la **navigation par ronde** (décor `RONDES_APPARIEES`), l'attente nommée tant que la ronde en cours n'est pas close, et le pavé de duel remonté tel quel (`DuelCharge`, famille `suisse`). **§7 tenu à son 3ᵉ format sans céder** — contrairement à E05US028, qui l'avait fait céder sur un nom |
| `frontend/src/features/saisie-duels/{api.ts,hooks.ts}` · `ROUTES` · `CLE_DECOR` · `PHOTO` | §7 — les aiguillages par famille, passés de conditions binaires à des **`Record` exhaustifs** en E05US030 : un 4ᵉ format ne peut plus tomber en silence du côté tableau, la compilation le refuse |
| `backend/tests/test_domain_placement_par_bloc.py` *(renommé en E05US026)* · `test_domain_reglage_poules.py` · `test_domain_classement_de_poules.py` | §3, §4, §5, §6 — écrits **depuis le CA** avant l'implémentation (règle 9) |
| `backend/tests/test_domain_contrat_phase.py` · `test_service_poules.py` · `test_placement_par_bloc_repository.py` *(renommé en E05US026)* · `test_poules_api.py` | §1, §3, §5, §6 — dont le seul test qui éprouve le branchement tardif de bout en bout |

### Ce que la tranche n'a **pas** fait, et qui n'est pas un oubli

1. ✅ **`route_l_archer` restait `False`** — *levé par `E05US026` le 16/08/2026*. Le routage sait
   désormais dire à un membre de poule où il tire ensuite, par le même chemin que le système suisse
   (`ServiceRoutage._routage_par_rencontres`) : une rencontre de groupe **est** un duel, donc
   `ProchainDuel` a suffi, sans issue neuve. Et la cible est donnée, le plan étant posé.
2. ✅ **Les poules n'entraient pas au palmarès** — *levé par `E05US026`*. Elles y entrent par
   `_resultat_classant`, qui lit leur **classement de phase** plutôt qu'un arbre — c'est bien le
   « `_resultat` propre au format » que ce paragraphe annonçait. La règle qui décide *quand* une
   phase décerne est tranchée par [ADR-0085](0085-une-phase-decerne-ses-rangs-si-rien-ne-preleve-dedans.md).
