...
| `frontend/src/features/suisse/{SaisieSuisse.tsx,presentation.ts,ClassementSuisse.tsx}` · `backend/domain/contrat_phase.py` | §1, §7 — la **navigation par ronde** (décor `RONDES_APPARIEES`, défini côté serveur), l'attente nommée tant que la ronde en cours n'est pas close, et le pavé de duel remonté tel quel (`DuelCharge`, famille `suisse`). **§7 tenu à son 3ᵉ format sans céder** — contrairement à E05US028, qui l'avait fait céder sur un nom |
