...
- **Amende** : [`cahier-des-charges.md`](../../cahier-des-charges.md) (M8 —
  ...
  E08US002 — découplée du calcul du dû) ; nouvelles US d'évolution **E01US020** et **E01US021**.
...
seulement le payeur groupé de ses archers — rejoint l'**abstraction participant** ([ADR-0028](0028-epreuves-par-equipes-participant.md),
E13US001) : archer et club sont deux **sujets** possibles d'un même dû.
...
**5. E08US002 se construit découplée du calcul du dû.** Le suivi des paiements enregistre un
**encaissement** (montant réglé, partiel possible, reste = dû − réglé) **contre un dû qu'il lit** via
le service existant (`montant_du_par_archer`). Il ne recalcule pas le dû et n'a pas connaissance de la
stratégie. Ce découplage n'est **pas** un pattern spéculatif : c'est une **séparation de
responsabilités** (encaisser ≠ calculer le dû) justifiée par le code d'aujourd'hui.

## Conséquences

- **+** L'**ouverture demandée est acquise et documentée** (règle 2 satisfaite en *direction*) sans
  écrire une ligne de moteur spéculatif (règle 9 respectée). Le jour où le 2ᵉ modèle arrive, l'US est
  déjà pensée et se branche.
...
- **−** Le sujet `club` **dépend d'E13** (abstraction participant) : il ne peut pas précéder E13US001.
