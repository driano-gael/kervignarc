...
| §3 — **`question_posee`** dit si la question a encore un objet | `backend/domain/jalon.py` (`PreparationJalon.question_posee`, renseigné par les deux membres) · `backend/api/v1/jalons.py` (champ du DTO) · `frontend/src/features/jalons/PretA.tsx` (prop `questionPosee`, **obligatoire** : `tsc` force chaque membre à trancher) — gardé par `backend/tests/test_domain_jalon.py` et `Completude.test.tsx` (mutation) | oui — ⚠️ **la ligne citait auparavant la convention « liste vide »**, et nommait `evaluer_terminer` comme la portant alors qu'il avait cessé de vider sa liste dans le commit même. Le signal machine avait disparu sans être remplacé — un écran neuf recopiant le patron aurait rétabli le bloquant de la 3ᵉ passe (6ᵉ passe, axes C1 et D) |
