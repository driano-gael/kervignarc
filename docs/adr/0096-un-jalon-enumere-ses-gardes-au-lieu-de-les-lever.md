...
| §3 — la **liste** et le **verdict** se gardent séparément | `frontend/src/features/jalons/PretA.tsx` (`questionPosee` garde le verdict, `lignes.length > 0` garde la section) — gardé par `Completude.test.tsx` (`hors du tournoi en cours…` assère liste **présente** et verdict **absent**) | oui — ⚠️ les piloter par le même booléen a produit **deux défauts opposés** : un verdict qui accusait une liste verte (3ᵉ passe), puis la disparition d'un affichage livré — la liste sportive n'était plus visible pendant la pause déjeuner (4ᵉ passe, axe C1) |
...
| §3 — l'écran *terminer* porte lui aussi la garde de statut | `frontend/src/features/completude/Completude.tsx` (`questionPosee={enCours}` — plus `pret` et `detail`, conditionnés au même booléen) · `frontend/src/features/jalons/PretA.tsx` (la prop `questionPosee`, qui garde le **verdict** sans toucher à la liste) — gardé par `Completude.test.tsx` (`hors du tournoi en cours…`, `en pause avec le sportif complet…`, `un tournoi terminé…`), dont le filet a été **vérifié par mutation** dans les deux sens | oui — ⚠️ **la ligne nommait `bloquant={!enCours}`, une prop inerte** : le seul cas où elle valait `true` était celui où le verdict n'est pas rendu, donc `bloquant={false}` aurait passé les mêmes tests. Un module nommé qui ne porte rien se lit comme une preuve (4ᵉ passe, axe C2) |
| §3 — une **liste vide** ne dit pas « rien ne manque » mais « plus rien à préparer » | `backend/domain/jalon.py` (`evaluer_demarrer` et `evaluer_terminer`, branche `transition_offerte`) · `backend/api/v1/jalons.py` (`PreparationJalonReponse`, docstring de `lignes`) · `frontend/src/features/jalons/PretA.tsx` (`lignes.length > 0`) — gardé par `backend/tests/test_domain_jalon.py` (`test_un_tournoi_qui_n_est_plus_a_lancer_dit_pourquoi_sans_se_tromper`, qui assère `lignes == ()`) et `test_jalons_api.py` | oui — ⚠️ c'est la convention qui **dispense le front de connaître le statut** ; elle n'a longtemps vécu qu'en commentaires, donc invisible au contrat que `E16US007` et `E16US008` liront (3ᵉ passe, axe C2) |
