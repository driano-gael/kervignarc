...
**lui** qui doit redéduire la garde de statut (`bloquant={!enCours}`), là où `PretADemarrer` n'en
...
| §3 — l'écran *terminer* reste du bon côté de l'asymétrie | `frontend/src/features/completude/Completude.tsx` (`bloquant={!enCours}` : aucune garde de **contenu**, mais la garde de **statut**) — gardé par `Completude.test.tsx` (`le verdict avertit sans annoncer de refus`) ⚠️ un booléen inversé ne se verrait **que** là — le typage n'en dit rien | oui — ⚠️ **la ligne disait `bloquant={false}`** jusqu'à la 3ᵉ passe de revue, alors que le commit précédent avait déjà écrit l'inverse quatre lignes plus bas : un tableau de portage qui se contredit lui-même ne prouve plus rien |
...
| §3 — l'écran *terminer* porte lui aussi la garde de statut | `frontend/src/features/completude/Completude.tsx` (`bloquant={!enCours}`) — gardé par `Completude.test.tsx` (`hors du tournoi en cours, le verdict n'affirme pas que rien n'empêche`) | oui — ⚠️ **c'était le bloquant de la 2ᵉ passe** : `bloquant={false}` figé faisait dire à l'écran « l'application ne vous en empêchera pas » sur un tournoi en pause, la phrase même que cette US supprimait ailleurs |
