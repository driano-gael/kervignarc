...
| role_de_saisie | TEXT | le **rang** de qui a écrit en dernier — `Role.name`, jamais le numéro ; `NULL` = **aucune préséance revendiquée**, nullable (E16US020) |
...
> tablette qui doit la corriger. ⚠️ **`role_de_saisie` n'est PAS `saisie_par`** : le premier vient de
> la garde et fait autorité, le second est un nom libre venu du corps de requête. Aucune reprise à la
