...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-09-26 | `E17US006` *(1ʳᵉ passe — mode multi-agent, worktree isolé)* | 20 | +441/−130 | ~45 min (13:24→14:09) : **2 portes complètes** — `.porte/10068` rouge sur `pip-audit` seul (*No module named pip_audit* : le venv neuf du worktree n'a pas l'outil, que la CI installe à la volée ; défaut d'environnement, pas de code), `.porte/11548` verte 14/14 après installation. **Après correctifs et merge d'`origin/main`** : `.porte/9536` verte 14/14 (~18 min), lancée à 22:51 seulement — le verrou partagé (`refs/locks/porte-complete`) est resté ~8 h sur la porte **morte** d'une autre session (E02US007, arrêtée à 15:19), libéré après échange entre sessions. ⚠️ Attendre un verrou sans vérifier qu'un processus tourne derrière coûte des heures : un verrou sans processus se signale à son détenteur | ~6 min (14:10→14:16) | D (14:10→14:16) | OK (mineur:1 suggestion:1) | majeur:1 mineur:2 suggestion:1 | OK (mineur:1 suggestion:1) | majeur:1 | mineur:2 suggestion:1 | — (aucun bloquant) ; majeurs : **B** (clause « seul bouton à contour épais » du CA non testée), **C2** (dette de nommage `bouton--danger` hors registre). **Enseignement** : le garde-fou neuf protégeait la **couleur** sur trois sélecteurs exacts, pas la **forme** — alors que l'ADR fait de la forme le **seul** signal restant. Quatre axes sur cinq (A, B, C1, D) ont convergé sur le même test, chacun par un angle (`--sur-*`, `find` sur la 1ʳᵉ règle, enfants BEM, « seul » non vérifié) : la décision déplace le signal, le test doit se déplacer avec lui. | 1 |
| 2026-09-26 | `E17US012` *(1ʳᵉ passe — cinq axes, D requis : migration `0057`)* | 59 | +1827/−479 | ~15 min avant revue (14/14, `.porte/6568`, **0 rouge** — **réutilisée**, lancée sur ce même `HEAD` `60e00a8`, arbre propre, `origin/main` inchangé) ; ~18 min après correctifs (14/14, `.porte/11424`, **0 rouge**) — **aucune porte rouge sur la passe** | ~6 min (12:48→12:54) | **D** (12:48→12:53) | mineur:1 | mineur:3 suggestion:3 | mineur:2 suggestion:1 | mineur:3 suggestion:2 | mineur:3 suggestion:2 | **aucun bloquant**. **Quatre axes sur cinq (A, C1, C2, D)** ont relevé la même phrase fausse — « le seul créateur de production » d'une inscription — que l'auteur avait **vérifiée puis mal rédigée** (il avait lu `simulation_format`, jugé « en mémoire », écrit « seul »). **B et C1** : « Gratuit » dans TARIF pour un archer inscrit nulle part — le défaut même que l'US corrigeait sur la colonne voisine. **B, C1, C2** : la liste des tournois est pollée à 5 s par la coquille, ce qu'ignorait le « tenable » de la docstring. **D seul** : la fusion d'archers écrase `cree_le` (rajeunit une dette) — un **trou déplacé** hors des lignes du diff ; la porte publique hérite du tableau d'admin. **C2 seul** : `DETTE-104` avait son déclencheur (« l'US qui touche ces décors ») atteint trois fois sans résorption. | 1 |
| 2026-09-26 | `E17US009` + `E17US011` + `E17US007` *(2ᵉ passe, sur les correctifs `de48877` — trois axes : B imposé par le code de production, C1, D ; A et C2 écartés, leurs remarques étant closes)* | 27 | +481/−119 | ~14 min avant (14/14, `.porte/2932`, 0 rouge) ; ~14 min après correctifs (14/14, `.porte/3664`, 0 rouge) | ~6 min | **D** (10:12→10:18) | — | mineur:6 suggestion:1 | mineur:3 | — | majeur:1 mineur:4 suggestion:1 | **aucun bloquant**. **Le seul majeur est une RÉGRESSION de mon correctif**, trouvée par D seul : pour régler le pavé collé sur téléphone, le correctif a retiré l'ancrage sous 60 rem — donc aussi sur la **tablette en portrait**, l'appareil visé. Le trou a été **déplacé**, pas fermé ; seuil ramené à 45 rem. Les trois axes ont trouvé la même absence : le hook `usePlansDuTournoi` sans test, contourné par les deux tests voisins (l'un le mocke, l'autre part de sa sortie). Contradiction arbitrée sur preuve : C1 craignait un `onSuccess` tardif écrasant la visée d'un autre archer ; D a vérifié que TanStack v5 ne rappelle pas après démontage — retenu D, et l'invariant « l'ouverture n'appartient qu'à l'archer actif » posé par construction (S1). ⚠️ La passe n'était **pas exigée** par la procédure (aucun bloquant restant) : elle a été lancée parce que l'historique montre que les correctifs introduisent des défauts — et elle en a trouvé un | 2 |
| 2026-09-26 | `E17US009` + `E17US011` + `E17US007` *(1ʳᵉ passe — trois US dans une PR, demande du commanditaire)* | 63 | +2163/−434 | ~15 min avant revue (14/14, `.porte/788`, **0 rouge**) ; ~14 min après correctifs (14/14, `.porte/2932`, **0 rouge**) — **aucune porte rouge sur la passe** | ~7 min | **D** (09:34→09:41) | mineur:1 suggestion:1 | majeur:3 mineur:7 | majeur:2 mineur:4 suggestion:1 | majeur:2 mineur:1 | majeur:3 mineur:5 | **aucun bloquant**. Le défaut central — la ligne d'archer ne montre pas la volée ouverte par le pavé (navigateur, volée rendue) — trouvé par **B, C1 et D** chacun sous un angle ; le faux zéro du cumul sans barème par **les quatre axes B, C1, C2, D**. **D seul** : « Non placés » aveugle aux archers inscrits à aucun départ, garde-fou `chargee` contourné par les cases de ligne, pavé collé plus haut que la fenêtre sur téléphone. **C2 seul** : `DETTE-114` doublait `DETTE-103` avec un constat faux (« copies identiques » — elles divergent), faute d'avoir cherché au registre avant d'ouvrir. ⚠️ **Enseignement** : les trois majeurs de saisie viennent d'un **seul** raccourci — recalculer dans la ligne ce que le pavé calcule déjà. Un commentaire affirmait « ils ne peuvent pas montrer deux volées différentes » ; trois axes l'ont réfuté par trois chemins. Et le seul test de la ligne montait le cas où les deux calculs coïncident (volée neuve, un archer) | 1 |
| 2026-09-24 | `E17US008` *(2ᵉ passe, sur les correctifs — quatre axes, A écarté faute de surface)* | 34 | +531/−111 | ~14 min (14/14, `.porte/10200` ; **2 rouges avant**, tous deux de l'auteur : `vitest` règle 13 puis `prettier`) | ~22 min | **D** (22:12→22:29) | — | majeur:3 mineur:7 | majeur:4 mineur:2 suggestion:1 | majeur:3 mineur:7 suggestion:3 | majeur:9 mineur:6 | **aucun bloquant** — les deux de la 1ʳᵉ passe sont fermés, confirmé séparément par les quatre axes. ⚠️ **Mais 4 des 13 majeurs sont des défauts que le commit de correctifs avait INTRODUITS** (commentaire dupliqué par une ancre, panneau effondré à 123 px par un `position:absolute`, bandeau débordant de 8 px sous 600 px, classe CSS inexistante **affirmée par un test** de la 1ʳᵉ passe). ⚠️ **Et 5 affirmations de fait vérifiable étaient fausses** — appelants de `mettreEnFile`, causes de refus de `S01` (2 fois), compte de planches (2 fois). Enseignement : **corriger n'est pas plus sûr que créer**, et rien de mécanique ne lit une affirmation documentaire. D a fourni 9 des 13 majeurs ; c'est le seul axe qui soit allé mesurer ce que les autres ont lu | 2 |
| 2026-09-24 | `E17US008` *(1ʳᵉ passe)* | 25 | +1184/−112 | ~13 min avant revue (14/14, `.porte/8584`). ⚠️ **5 portes complètes pour 1 passe, dont 3 rouges, toutes évitables et toutes de l'auteur** : 2 × `npm ci` en EPERM (serveur Vite laissé en fond, 8/14 puis reprise), `vitest` (règle 13 — un commentaire à 9 lignes écrit *pour corriger une remarque de revue*, 12/14), `prettier` (fichier édité après le dernier `format`, 10/14). ⚠️ **Un rouge arrête tout son groupe** : 10/14 et 12/14 ne prouvent rien. Verte finale : `.porte/5264` | ~28 min | **D** (20:46→21:14) | majeur:1 mineur:3 suggestion:1 | majeur:6 mineur:3 suggestion:1 | majeur:5 mineur:4 suggestion:1 | majeur:5 mineur:4 suggestion:2 | **bloquant:2** majeur:4 mineur:6 suggestion:1 | **D seul**, sur les deux. ⚠️ Le 2ᵉ est le résultat le plus lourd du registre à ce jour : l'US différait son unique écart 🔴 au nom d'un invariant de revue du 05/08 (« une zone tapable de plus détruit le tampon de frappe du pavé ») que le code a **supprimé** depuis — les brouillons ont été remontés dans `Saisie`, et le commentaire qui le dit (`Saisie.tsx:51-58`) était **dans le diff relu par les quatre autres axes**. D est allé lire le code que la justification invoque ; les axes de grille ont lu la justification. Le 1ᵉʳ bloquant (bandeau hors-ligne promettant une file qui n'existe pas hors tablette) était vu par **quatre axes en majeur** — c'est D qui l'a qualifié bloquant, en constatant que la surface touchée est le PC d'organisation | **2** — les deux bloquants sont résolus, mais les correctifs touchent **une dizaine de fichiers de `frontend/src`** : l'étape 3 impose alors l'axe **B** quoi qu'il arrive (la règle 9 détecte une absence, un correctif non testé n'éveille pas son relecteur). 2ᵉ passe doublement cadrée — fichiers corrigés, axes B, C1, C2 et D. **A écarté** : aucune frontière ni couche déplacée, le seul Python reste une docstring |
...
| 2026-09-23 | `E17US010` *(1ʳᵉ passe)* | 10 | +264/−36 | ~21 min (14/14 vertes) | ~11 min | D (19:36→19:46) | majeur:4 mineur:1 suggestion:3 | majeur:1 mineur:7 | majeur:3 mineur:5 suggestion:3 | majeur:4 mineur:3 suggestion:2 | majeur:4 mineur:2 suggestion:1 | **aucun axe — promu par l'auteur au sabotage** (A-F3 = C1-3 = D-M3, rendus *majeur* par les trois) | 1 |
| 2026-09-21 | `E05US022` *(5ᵉ passe, sur les correctifs)* | 25 | +542/−224 | ~13 min (14/14 vertes) | ~38 min | A (23:34→00:03) | mineur:2 suggestion:2 | majeur:2 mineur:6 suggestion:1 | majeur:1 mineur:7 | majeur:6 mineur:3 suggestion:3 | majeur:3 mineur:4 suggestion:3 | **Le motif s'arrête, et c'est le fait de cette passe.** Pour la première fois en cinq tours, **aucun axe ne trouve de défaut de production** : les cinq rapports le disent de leur côté (« plus aucun défaut de production », « le code livré est sain sur tout ce que j'ai su attaquer »). Les majeurs restants sont **tous** dans les artefacts qui *décrivent* le code — et deux d'entre eux sont prouvés **par le diff lui-même** : `DETTE-078` déclarée résorbée sans **changer de table**, et l'audit d'ADR-0075 inscrit dans la colonne *Résorption* de `DETTE-091` — l'atlas régénéré par ce même commit publiait donc `078` comme *ouverte et introduite par E05US022*, et `091` comme *résorbée par elle*. Exactement l'inverse des faits, vérifiable sans discussion. ⚠️ **L'enseignement d'outillage de la branche** : le contrôle `portage-symbole-absent` de l'atlas cherche ses symboles **par sous-chaîne dans le texte entier du module, commentaires compris** — un symbole *supprimé* reste publié « présent » dès que la prose du module le mentionne. La 4ᵉ passe a dû sortir un nom d'ADR *à la main* pour que le contrôle cesse de mentir ; `DETTE-068` est élargie de ce mode de panne. ⚠️ **Ce que les cinq passes auront coûté et rapporté** : ~3 h de revue et de porte pour **3 défauts de production** (`DETTE-078` résorbée, une borne d'entrée qui aurait fait tomber `GET /formats` en 500, un recouvrement `PhaseId`/`EtapeDerouleId` dans le contrat de ports) et une quarantaine d'artefacts rendus exacts. | 0 |
| 2026-09-20 | `E05US022` *(4ᵉ passe, sur les correctifs)* | 27 | +742/−274 | ~13 min (14/14 vertes) | ~21 min | D (22:58→23:19) | majeur:2 mineur:4 | majeur:4 mineur:4 suggestion:2 | **bloquant:1** majeur:2 mineur:3 suggestion:2 | majeur:4 mineur:5 suggestion:3 | majeur:7 mineur:6 suggestion:4 | **Le motif tient une QUATRIÈME fois, et la 4ᵉ passe est celle qui en donne la forme la plus pure : « le `finally` ci-dessous » — il n'y en avait aucun, relevé par **quatre axes sur cinq** ; « c'est l'invariant qui est testé, plus la liste du jour » — la liste était toujours écrite à la main ; « deux décors traités **de même** » — l'un brûlait 5 000 identifiants, l'autre trois, donc `PhaseId 4 == EtapeId 4` ; « ADR-0075 corrigé » — le symbole supprimé restait backtické dans l'entrée de porteur, et l'atlas le publiait comme **présent**, la prose explicative nourrissant le contrôle au lieu de le déclencher. ⚠️ **Le bloquant vient d'une règle, pas d'un code neuf** : C1 (et B hors grille) ont vu que la ligne de registre écrite au commit précédent qualifiait elle-même `ServicePhases.modifier` de « casse un cas utilisateur réel » — définition littérale d'un **bloquant** au § Dette de `CLAUDE.md`, donc à corriger, pas à inscrire. Le remède tenait en une ligne par site, la machinerie ayant été introduite deux passes plus tôt : `DETTE-078` est **résorbée**. ⚠️ **Contradiction arbitrée** : C2 déclarait la borne `ge=1` saine (« pas de route parallèle ») ; A, B et C1 ont montré que le DTO sert **aussi** la réponse, donc qu'un format déjà stocké à `ordre_source: 0` ferait tomber tout `GET /formats` en 500. Borne retirée — elle était de surcroît redondante depuis le changement de critère de la 3ᵉ passe. ⚠️ **Première ligne écrite AVANT les correctifs**, selon l'étape 2 que la 3ᵉ passe venait d'ajouter à la procédure. | 1 |
| 2026-09-20 | `E05US022` *(3ᵉ passe, sur les correctifs)* | 24 | +655/−114 | ~13 min (1 rouge réel, puis 14/14) | ~22 min | D (21:56→22:18) | majeur:2 mineur:3 suggestion:1 | majeur:5 mineur:6 suggestion:2 | majeur:5 mineur:5 suggestion:2 | majeur:7 mineur:4 suggestion:4 | **bloquant:1** majeur:3 mineur:6 | **Le motif tient une TROISIÈME fois, et c'est désormais la mesure la plus stable de cette branche : sur les 22 majeurs des cinq axes, la grande majorité se loge dans une phrase où le commit précédent déclare le problème résolu.** « purge les **TROIS** tables filles » (il y en a **huit** — et l'argument qui justifie les trois, la cascade inerte sous Alembic, condamne les cinq autres : **bloquant**, des feuilles de marque survivaient en pointant une phase supprimée) ; « décors décorrélés aux **DEUX** étages » (brûler **un** identifiant ne décale que de 1, donc `id == ordre` revient dès qu'un décor commence au rang 2 — reproduit sur `test_duel_repository`) ; « `RANG_INTROUVABLE` ne fabrique **plus** de fausse anomalie » (il y a **deux** sentinelles, la seconde côté front vaut `taille + 1` et arrive par l'atelier) ; « la dette ne tient plus que sur `ServicePhases.ajouter` » (`modifier` est un 3ᵉ site, **pire** — il persiste puis rend le tournoi illisible) ; « les quatre gardes » (la 4ᵉ n'était pas `decoupage`, déjà fermé, mais `profondeur`) ; « ADR-0078 inscrit » (l'inscription faite a révélé qu'**ADR-0076 manquait aussi**, et que l'intervalle `0071`-`0079` n'avait jamais été audité). ⚠️ **Enseignement de méthode, à garder** : l'axe adversarial a relu une extraction propre (`git archive`) et non l'arbre — qui bougeait sous lui — et a **reproduit quatre findings à l'exécution** plutôt que de les déduire ; c'est ce qui a produit le bloquant. ⚠️ **Ce qu'une revue paie en plus** : la décorrélation des décors a fait rougir 26 tests à la 2ᵉ passe, puis 14 de plus à la 3ᵉ — zéro défaut de production, mais autant de faux verts retirés. | 1 |
| 2026-09-20 | `E05US022` *(2ᵉ passe, sur les correctifs)* | 38 | +779/−119 | ~13 min (14/14 vertes) | ~43 min | D (20:27→21:10) | *comptes perdus* | *comptes perdus* | *comptes perdus* | *comptes perdus* | **bloquant:1** majeur:7 mineur:2 | **Le 2ᵉ passage confirme la leçon d'`E05US033` : un correctif de bloquant déplace le trou plus souvent qu'il ne le ferme.** Sur les 10 findings de D, **six** se logent exactement dans une phrase où le commit de correctifs déclare le problème résolu — « les *trois* appelants » (il y en avait quatre, dont un sur le chemin de saisie), « décors décorrélés » (l'étage SQL ne l'était pas, et l'étage mémoire l'était *faux* : l'identité ne dépendait pas du tournoi), « purge les `franchissement_arret` » (deux tables filles sur trois oubliées), « instancier avant de détruire tient » (quatre gardes sur neuf vivent sur `Phase`, donc après la destruction), « reprise sans perte » (elle retire désormais des prélèvements), « ADR-0078 inscrit au § Portée » (un chapeau avait été ajouté, **pas** la liste nominative — 5ᵉ occurrence de ce mode de panne). ⚠️ **Enseignement de méthode** : D a relu une extraction propre de `HEAD` (`git archive`) et non l'arbre de travail, qui portait 13 fichiers non commités — sans quoi il concluait faux deux fois. ⚠️ **Décorréler l'étage SQL a fait rougir 26 tests**, dont 25 dans un seul décor qui passait un `PhaseId` à une route attendant une `EtapeDerouleId` : aucun défaut de production, mais la coïncidence les aurait tous rendus aveugles au suivant. ⚠️ **Comptes par axe A/B/C1/C2 perdus** : le contexte de la session a été compacté entre la synthèse et la tenue de ce tableau. Le correctif de procédure est **dans `.claude/commands/revue-us.md`**, pas ici : la ligne s'ouvre **dès la synthèse rendue** (étape 2), et seuls `durée porte`, `passes` et les portes rouges se complètent au point 5. Une leçon laissée dans une cellule de données n'est lue par personne. ⚠️ **La taille du diff inscrite était elle aussi fausse** (22 · +635/−109, soit le diff des *correctifs* produits) : la colonne mesure le diff **soumis** à la passe, sinon elle cesse d'être comparable d'une ligne à l'autre — relevé en 3ᵉ passe. | 1 |
| 2026-09-20 | `E05US022` *(1ʳᵉ passe)* | 68 | +2267/−947 | ~13 min (14/14 vertes) | ~39 min | D (18:32→19:11) | majeur:1 mineur:6 suggestion:3 | **bloquant:1** majeur:5 mineur:5 suggestion:2 | **bloquant:3** majeur:2 mineur:5 suggestion:1 | **bloquant:1** majeur:7 mineur:7 suggestion:4 | **bloquant:2** majeur:2 mineur:6 suggestion:1 | **C1 et D, sur la même conjonction** — `EtapeDerouleId` est un alias d'`int` (`DETTE-044`), donc mypy ne distingue pas rang et identité ; et les décors adossés à un repository allouaient les identités à partir de 1, donc `etape.id == etape.ordre`. Trois appelants du résolveur de classement étaient restés sur le rang (`palmares` ×2, `completude`) et la clé du cache aussi : **4376 tests verts sur du code cassé**. D les a **reproduits à l'exécution**, C1 par lecture. L'axe B a trouvé le 4ᵉ (migration de reprise sans aucun test, quand le dépôt en compte 14), et **C2 le 3ᵉ** (`verifier_applicable` plus faible que la pose : appliquer un format brouillon détruisait le déroulé **puis** échouait). ⚠️ Le garde-fou `identite_d_etape = 100 + ordre` avait bien été posé — mais seulement sur les décors du domaine, pas sur ceux du palmarès, de la complétude, ni sur le contrat de conformité de port. **Décorréler l'allocation (`_sequence = 200`) a fait rougir 41 tests d'un coup** : la mesure de ce que la coïncidence couvrait. | 1 (+3 portes rouges avant verte) |
| 2026-09-20 | `E06US009` *(3ᵉ passe, sur les correctifs)* | 24 | +388/−112 | ~12 min (14/14 vertes) | ~23 min | D (11:05→11:28) | majeur:1 mineur:4 | majeur:2 mineur:8 suggestion:2 | majeur:2 mineur:7 suggestion:0 | majeur:4 mineur:5 suggestion:2 | majeur:2 mineur:8 suggestion:1 | **aucun bloquant, mais un arbitrage rendu au commanditaire** — l'axe B a vu que l'auteur avait **rétréci le CA** « la simulation suit » à « côté affichage » pour que le code le satisfasse, et ajouté un test qui entérinait le défaut. C'est un arbitrage de **périmètre** (cas 2), pris seul dans un commit correctif ; le commanditaire a tranché pour la **correction du moteur**, faite dans l'US. ⚠️ **D a de nouveau payé par l'exécution** : son sabotage a montré que le test de la garde `every` restait vert sous un `creneaux[0]`, faute d'avoir monté l'arbre sur le SECOND créneau — la coïncidence de décor du backend, déplacée vers le front. ⚠️ **C2 s'est trompé sur son majeur n°1** (il a lu un `creneaux[0]` là où le code portait `every`) : arbitré sur preuve, C1 disait vrai. ⚠️ **Le motif de la branche est stable** : chaque commit correctif remplaçait un artefact faux par un autre — trois passes pour que le scénario de recette décrive l'écran réel. ⚠️ **Défaut de procédure de l'auteur** : le préambule commun a été transmis **résumé** à trois axes au lieu de verbatim — A l'a signalé de lui-même. Dettes : `DETTE-107` ouverte puis **refermée dans la branche** (moteur corrigé, jamais parvenue à `main`), `DETTE-109` ouverte. | 3 |
| 2026-09-19 | `E06US009` *(2ᵉ passe, sur les correctifs)* | 30 | +884/−197 | ~12 min (14/14 vertes) | ~30 min | D (23:29→23:48) | majeur:2 mineur:1 suggestion:1 | **bloquant:1** majeur:3 mineur:4 suggestion:2 | majeur:2 mineur:5 suggestion:2 | majeur:3 mineur:8 suggestion:2 | majeur:4 mineur:5 suggestion:2 | **D, et de loin** — seul axe à avoir **exécuté** ses sondes et son propre sabotage, et seul à trouver que l'auteur avait **réintroduit le défaut d'E01US025** : `depart_id` du classement rempli avec `tournoi_id`, une ligne sous `depart_id=creneau.depart_id`, dans un DTO dont la docstring met en garde contre exactement cela (`DETTE-044` : mypy est aveugle, les deux sont des `int`). ⚠️ **D a aussi prouvé par exécution que les tests neufs restaient verts sous le sabotage le plus probable** — `set().isdisjoint(set())` : l'auteur avait saboté le trou *plein*, jamais le trou *vide*. ⚠️ **Le bloquant vient de B** (règle 9-doc) : le correctif avait livré **toute** la surface front du cockpit sans toucher la fiche. ⚠️ **Trois axes convergent** sur un décor de test `as unknown as` infidèle au contrat — dans le fichier dont l'en-tête dénonce les miroirs non vérifiés. ⚠️ **Le motif du dépôt se confirme** : l'auteur a reproduit dans son commit correctif le défaut même que la 1ʳᵉ passe lui reprochait (deux CA contradictoires). Deux dettes ouvertes : `DETTE-107` (refermée en 3ᵉ passe), `DETTE-108`. | 3 |
| 2026-09-19 | `E06US009` | 43 | +1557/−456 | ~13 min (14/14 vertes) | ~32 min | D (21:55→22:27) | **bloquant:1** mineur:1 suggestion:1 | **bloquant:1** majeur:3 mineur:3 | **bloquant:1** majeur:3 mineur:4 | **bloquant:1** majeur:1 mineur:4 suggestion:2 | **bloquant:1** majeur:3 mineur:4 suggestion:1 | **les CINQ axes, sur le même** — unanimité inédite dans ce journal, et c'est le signe de sa nature : l'US changeait **deux** DTO et n'a migré le front que du premier, laissant `features/simulation/` déréférencer des champs disparus. ⚠️ **Faux vert intégral** : miroir TS écrit à la main au-dessus d'un `fetchJson<T>` (transtypage, pas validation), **zéro test** dans le dossier, et **aucun `ErrorBoundary` dans tout `frontend/src`** — donc l'appli admin entière tombait en page blanche pendant que la porte rendait 14/14. ⚠️ **D seul** a démoli l'affirmation « le coût est irréductible », écrite dans **trois** artefacts : `pour_phase` refait quatre lectures à la maille tournoi par créneau, et le correctif de l'auteur avait été calibré sur la portée de son détecteur (`_ClubsComptes` ne compte que les clubs). ⚠️ **C2 seul** a vu qu'aucun ADR ne portait « une route, N sections », **contre** le remède que le dépôt avait inscrit — d'où ADR-0111. ⚠️ **B et D convergents** sur les deux CA homonymes contradictoires laissés dans `stories/`, et sur `DETTE-097` aggravée (le défaut corrigé côté serveur réapparaissait au vidéoprojecteur). **Aucun axe n'était muet.** | 2 |
| 2026-09-19 | `E00US031` | 28 | +1503/−405 | ~12 min (14/14 vertes) | ~28 min | D (15:13→15:26) | bloquant:0 majeur:3 mineur:5 suggestion:1 | bloquant:0 majeur:4 mineur:7 suggestion:4 | bloquant:0 majeur:1 mineur:10 suggestion:5 | **bloquant:1** majeur:5 mineur:6 suggestion:3 | **bloquant:1** majeur:8 mineur:7 suggestion:4 | **C2 et D** *(le même : `revue-us.md` décrivait l'ancienne porte — et il n'était pas dans le diff. D seul a trouvé les trois faux verts : `6/6` au lieu de `6/14`, l'étiquette `ligne_ci` jamais confrontée à la commande, et l'aveuglement aux cliquets documentaires ; D seul aussi a relevé le savoir supprimé « ne pipe jamais une commande de test », absent de tout le dépôt après coup)* | 1 |
| 2026-09-19 | `E01US026` | 76 | +1674/−280 | ~34 min (3 rouges, dont l'atlas hors périmètre) | ~25 min | D (11:27→11:52) | bloquant:0 majeur:2 mineur:5 | bloquant:0 majeur:3 mineur:6 suggestion:3 | bloquant:0 majeur:4 mineur:7 suggestion:3 | bloquant:0 majeur:7 mineur:5 suggestion:2 | **bloquant:1** majeur:3 mineur:4 suggestion:2 | **le seul bloquant vient de D**, et aucune grille ne l'aurait trouvé : en remplaçant la confirmation *inline* à deux clics par un dialogue ouvert par le 409, l'US rendait destructible **en un tap** un tournoi entièrement préparé (postes et scoreurs hors décompte, codes tirés par `secrets`, QR déjà collés). Deux majeurs convergents sur le décompte (`entree_audit` absent — A, C1, C2) et un sur l'argent (`montant_encaisse` était la somme du **registre**, pas de l'encaissé — D). ⚠️ **Le seul défaut d'exécution de l'US est venu d'un script** : un `sed` de masse sur 45 marqueurs de dette a coupé trois phrases voisines, dont l'avertissement qui **justifiait** une cascade — trouvé par B, C1 et D séparément. La leçon n'est pas « relire », c'est que le découpage en phrases coupait aussi sur « : ». ⚠️ C2 a relevé **trois manquements de registre** (ADR-0075 non tenue, DETTE-007 et DETTE-016 non élargies) que l'US réussissait pourtant sur ses **propres** artefacts |
| 2026-09-18 | `E16US016` | 55 | +2466/−258 | ~15 min | ~28 min | D (20:03→20:31) | majeur:2 mineur:3 | majeur:3 mineur:4 suggestion:2 | majeur:3 mineur:6 suggestion:2 | majeur:3 mineur:5 suggestion:1 | majeur:3 mineur:7 suggestion:3 | **aucun bloquant** — majeurs : `DETTE-031` par **4 axes** (A, B, C1, C2), collision de vocabulaire `Tableau` par A+C2, neutralisation des en-têtes par **D seul** (prouvée sur le XML produit), divergence `stories/`↔`docs/fonctionnel/` par C1+D | 1 |
| 2026-09-12 | `E16US020` *(3ᵉ passe, diff intégral)* | 41 | +1810/−115 | ~22 min | ~24 min | D (16:00→16:11) | majeur:1 mineur:3 suggestion:1 | majeur:2 mineur:2 suggestion:2 | majeur:1 mineur:3 suggestion:1 | majeur:1 mineur:4 suggestion:2 | majeur:4 mineur:2 suggestion:2 | **aucun bloquant** — mais **quatre axes sur cinq convergent** sur le même majeur (le rang posé à la frontière, prouvé par rien), ce qui est le signal le plus net de la passe. ⚠️ **D seul** a vu que la garde ajoutée par la 2ᵉ passe est **inerte sur le chemin nominal** (`corriger_volee` sort tôt sur `verrouillee`) : 6ᵉ fois d'affilée qu'un correctif déplace le trou. ⚠️ **C2 seul** a vu qu'un 409 définitif **amende ADR-0037** sans que rien ne l'écrive, sur un garde-fou de perte de score. ⚠️ **Passe lancée sur le diff INTÉGRAL** et non cadrée (étape 3) : les rapports des passes 1 et 2 ayant été perdus, le motif « le reste a déjà été relu » n'était pas garanti. | 3ᵉ passe |
| 2026-09-12 | `E16US020` *(passes 1 et 2)* | 41 | — | — | — | — | — | — | — | — | — | **MESURES PERDUES** — le terminal de la session a planté entre l'écriture des correctifs de la 2ᵉ passe et leur commit. Les cinq rapports de chaque passe sont irrécupérables : durées et décomptes par sévérité ne sont **pas** reconstituables. Inscrit vide plutôt que deviné — une métrique inventée dans ce registre est pire qu'absente, puisqu'il sert à arbitrer la procédure. Ce qui reste vérifiable : les deux passes ont bien eu lieu (commits `54a5260`, `45a154c`, `bc5ac4a`) et leurs correctifs sont dans le diff. | 1ʳᵉ et 2ᵉ |
| 2026-09-11 | `E16US019` *(2ᵉ passe, sur les correctifs)* | 38 | +781/−99 | ~13 min | ~50 min | C1 (21:52→22:44) | majeur:2 mineur:3 | majeur:3 mineur:5 suggestion:1 | majeur:3 mineur:5 suggestion:2 | majeur:2 mineur:6 suggestion:3 | **bloquant:1** majeur:5 mineur:3 suggestion:2 | **D seul** — et c'est le constat de la passe : **cinq des huit correctifs déplaçaient le trou**. Le bloquant est une **régression du correctif lui-même** (revalider refermait *toutes* les corrections de la feuille, re-signant des volées jamais ressaisies), sondée à l'exécution. ⚠️ **Quatre axes sur cinq** ont convergé sur la trace d'audit rendue **falsifiable** par le correctif censé la rétablir ; **C1 seul** a vu que le recours annoncé pour la pause (`corriger_volee`) **n'existait pas**, et que le pavé de la tablette restait collé sur la volée rendue. Leçon : un correctif de revue mérite la même passe que le code d'origine. | 2ᵉ passe |
| 2026-09-11 | `E16US019` | 34 | +1888/−89 | ~15 min | ~25 min | D (20:05→20:29) | majeur:1 mineur:2 | **bloquant:1** majeur:4 mineur:5 suggestion:2 | majeur:3 mineur:6 suggestion:2 | majeur:2 mineur:5 suggestion:4 | **bloquant:1** majeur:4 mineur:7 suggestion:2 | **B et D, convergents** — le bloquant est le **5ᵉ lecteur** de `verrouillee` (`deroule.py`), que l'US n'avait pas relu : le public affichait « en attente » sur des volées dont les points étaient dans le même `cumul`, et l'**ADR affirmait le contraire**. ⚠️ **C1 seul** a trouvé les deux défauts de conjonction les plus coûteux — le parcours en **impasse pendant une pause** (annuler permis, refermer gelé) et la **portée affichée puis ignorée** (DETTE-052). ⚠️ **D seul** a vu que la tablette ouvrait un pavé **verrouillé** après annulation, et que la ressaisie **perdait la trace d'audit** que `corriger_volee` garantissait. A a trouvé le seul producteur du dépôt qui violait déjà l'invariant. **Aucun axe n'était muet.** | 1 (1ʳᵉ passe) |
| 2026-09-10 | `E16US011` (découpage, 0 ligne de code) | 4 | +282/−60 | ~18 min | ~24 min | D (19:22→19:32) | majeur:2 mineur:2 | majeur:6 mineur:5 suggestion:1 | **bloquant:2** majeur:3 mineur:6 suggestion:2 | majeur:5 mineur:3 suggestion:3 | **bloquant:3** majeur:7 mineur:6 | **C1 et D, convergents** — les 3 bloquants sont des CA bâtis sur du faux ; **D seul** a trouvé les 2 pires en relisant les **questionnaires bruts**, que personne d'autre n'avait rouverts | 2 |
| 2026-09-10 | `E16US011` (2ᵉ passe, sur les correctifs) | 9 | +316/−70 | ~6 min | ~35 min | D (19:55→20:07) | majeur:3 mineur:3 suggestion:1 | **bloquant:2** majeur:5 mineur:4 suggestion:2 | **bloquant:3** majeur:3 mineur:4 suggestion:1 | majeur:5 mineur:4 | **bloquant:2** majeur:6 mineur:3 suggestion:2 | **B, C1 et D, convergents** — le commit de correctifs avait **reproduit le défaut d'E16US007** (jumeaux non corrigés) et **créé 3 défauts neufs**, dont un CA qui rendait la capacité de l'US morte-née | 2 |
...
| 2026-09-12 | `E16US021` | 31 | +892/−223 | ~20 min (1 rouge réel) | ~27 min | D (21:06→21:33) | majeur:0 mineur:5 suggestion:3 | majeur:1 mineur:6 suggestion:2 | majeur:3 mineur:3 suggestion:1 | majeur:3 mineur:4 suggestion:2 | majeur:5 mineur:5 | **aucun bloquant de revue — mais la PORTE en a trouvé un** que quatre vérifications manuelles avaient manqué (`tsc` refusait `portee: 'creneau'` ; `vitest` ne typecheck pas, et le `typecheck` avait été lancé **avant** l'écriture du test). **C1 et D, convergents et seuls** : la liste des départs n'était **pollée par personne** — fraîcheur héritée de `useRealtime`, qui n'invalide que sur événement, ignore la reconnexion, et les inscriptions n'émettent aucun `LiveEvent`. Aucun axe mono-couche ne pouvait le voir. **Quatre axes sur le même `as`** (A, B, C1, D). **B seul** : le chiffre « Inscrits » n'était pas la somme des effectifs — faux **dans les deux sens**. **C2 seul** : `ADR-0096` affirmait le contraire du code livré, 2ᵉ occurrence sur ce même fichier. **D seul** : « il n'y avait qu'à cesser de jeter » faux, répété dans 4 artefacts durables ; et un contraste à ≈ 2,95:1 sous AA, trouvé **par recalcul** (`DETTE-086`) | 0 |
| 2026-09-10 | `E16US018` | 17 | +746/−93 | ~9 min | ~23 min | D (21:31→21:52) | **OK** | majeur:1 mineur:5 suggestion:3 | majeur:1 mineur:3 suggestion:3 | majeur:1 mineur:2 suggestion:1 | majeur:6 mineur:2 | **aucun bloquant** — mais **2 tests placebo** trouvés, un par axe (C1+D sur le test backend, B+D sur le test front) : les deux épinglaient un CA sans pouvoir échouer. D seul a trouvé l'ancre de minuteur perdue au remontage, la phrase fausse sur le palmarès et les avis permanents emportés | 0 |
...
⚠️ **La colonne `passes` a été renseignée deux fois de suite comme un PRONOSTIC** — à `1`, puis à
`2` —, chaque fois à l'étape 2 de la passe qu'elle décrit, par la partie relue, et chaque fois
démentie par la passe suivante. C2 l'a relevé les deux fois, sur ce fichier même. **Correctif de
procédure, plus utile que la valeur** : `passes` ne se renseigne **que dans le commit qui clôt la
revue** — celui qui ne reçoit plus de bloquant ni de majeur. Avant cela, la case vaut `—`.
...
d'avant.* **Six** rédactions ont dû être refaites parce qu'elles restaient vertes sur le défaut
qu'elles gardaient :

1. assertion sur une **image chargée en asynchrone** — nulle au moment où on l'interroge, donc vraie
   quoi qu'il arrive ;
2. `rerender` dans un **provider supplémentaire**, qui déplace le composant dans l'arbre et le
   **remonte** — détruisant l'état même qu'on observe ;
3. `QueryClient` de test à **`staleTime: 0`** : il refetche à chaque montage, donc **teste un cache
   qui n'existe pas** ;
4. assertion vraie **par vacuité** (`slice(1).every(...)` sur un tableau vide) ;
5. **doublure de store constante**, qui rend la transition à observer structurellement impossible ;
6. oracle porté sur un **compteur d'appels** au lieu de la **donnée servie** — vert sur toute
   régression qui refetche *et* sert du périmé.

Les six ont un trait commun : **l'oracle ne peut pas prendre la valeur qui ferait échouer le test.**
C'est le contrôle à faire avant de commiter un test de non-régression — plus rapide que de relire
l'assertion : *quelle valeur, ici, ferait rougir ?* Si la réponse n'existe pas, le test est décoratif.
...
## Enseignement d'E17US010 (23/09/2026) — le trou se déplace, et il faut trois passes pour le voir

⚠️ **La colonne « bloquants par » peut dire « personne » — et ce n'est pas un axe qui a
manqué.** En 1ʳᵉ passe, trois axes ont décrit le même défaut (le parseur du garde-fou jetait une
ligne qu'il ne savait pas lire) et l'ont classé **majeur**. Il était bloquant : un commentaire de fin
de ligne suffisait à rendre invisible le fantôme que l'US venait de retirer. Ce qui a tranché n'est
pas une relecture de plus mais **l'exécution du cas**. Un axe lit et qualifie ; l'auteur, lui, peut
**essayer**.

⚠️ **Mais les deux passes suivantes ont démenti la moitié de cet énoncé** : leurs bloquants
viennent d'un **axe**, et d'un seul — l'axe D. En 2ᵉ, il a vu ce qu'aucun autre ne pouvait voir,
parce que c'était **hors du dépôt local** : la PR d'`E05US022` avait été mergée entre temps, et les
...
du littéral*. À chaque fois le correctif durcissait **l'endroit montré** et laissait l'endroit
voisin où le même raisonnement s'applique. Ce qui a fini par fermer la série n'est pas un cas de
plus mais un **changement de règle** : passer d'une liste de formes refusées à une **liste blanche**
des formes admises. Le rejeu par sabotage se fait donc **après** correction autant qu'avant — c'est
ainsi que le 2ᵉ et le 3ᵉ trou ont été trouvés.
