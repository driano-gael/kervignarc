...
| 2026-09-26 | `E02US007` *(1ʳᵉ passe)* | 74 | +3401/−168 | *à compléter — porte non passée avant la revue (verrou machine tenu par une autre session), revue lancée en parallèle de l'attente* | ~6 min | D (14:29→14:35) | **bloquant:1** mineur:5 | majeur:2 mineur:9 suggestion:1 | **bloquant:1** mineur:4 | mineur:6 suggestion:1 | mineur:4 suggestion:1 | **A et C1, sur deux défauts distincts, B confirmant C1 en majeur** — A : la denylist du garde-fou d'isolation n'avait pas reçu `xlrd` (le même trou qu'`openpyxl`/`reportlab` avant E16US016). C1 **et** B, indépendamment : l'arme lue dans Ianseo est un **code** (`CL`, `TA`) comparé à un **libellé** du catalogue FFTA (« Arc Classique ») — tout import dans un tournoi aux catégories du référentiel aurait été rejeté ligne à ligne. **Masqué par deux fixtures irréalistes** (`"CLASSIQUE"`/`"Classique"` au domaine, catégorie « Scratch » sans arme à l'API) : le test épousait la comparaison écrite, pas le fichier que le CA vise — le piège exact de la règle 9, commis **alors même que le test précédait le code**. Quatre axes sur cinq ont vu `isdigit()`+`int("²")` (500 au lieu d'un rejet). D seul a vu l'amplification xlrd sans `ragged_rows` et le rapport qui ne nomme pas la fiche désignée par la licence ; B seul, le quota jamais testé sur la branche INSCRIRE (donc sur tout Résult'Arc). Deux arbitrages rendus au commanditaire en cours de revue (`TA` = tous arcs ; licence réservée à l'admin). | *à compléter* |
