...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-09-26 | `E17US006` *(1ʳᵉ passe — mode multi-agent, worktree isolé)* | 20 | +441/−130 | ~45 min (13:24→14:09) : **2 portes complètes** — `.porte/10068` rouge sur `pip-audit` seul (*No module named pip_audit* : le venv neuf du worktree n'a pas l'outil, que la CI installe à la volée ; défaut d'environnement, pas de code), `.porte/11548` verte 14/14 après installation | ~6 min (14:10→14:16) | D (14:10→14:16) | OK (mineur:1 suggestion:1) | majeur:1 mineur:2 suggestion:1 | OK (mineur:1 suggestion:1) | majeur:1 | mineur:2 suggestion:1 | — (aucun bloquant) ; majeurs : **B** (clause « seul bouton à contour épais » du CA non testée), **C2** (dette de nommage `bouton--danger` hors registre). **Enseignement** : le garde-fou neuf protégeait la **couleur** sur trois sélecteurs exacts, pas la **forme** — alors que l'ADR fait de la forme le **seul** signal restant. Quatre axes sur cinq (A, B, C1, D) ont convergé sur le même test, chacun par un angle (`--sur-*`, `find` sur la 1ʳᵉ règle, enfants BEM, « seul » non vérifié) : la décision déplace le signal, le test doit se déplacer avec lui. | |
