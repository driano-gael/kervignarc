...
appelants, zéro changement de comportement. Marqueurs `DETTE-019` sur les **quatre** sites : `routage.py`, `pilotage_tour.py`, `frontend/src/features/routage/presentation.ts` (`partitionner` — la garde rangeait sinon tous les archers encore en lice parmi les sortis dès le tour 2) et `frontend/src/features/feu-vert/etat.ts` (`actionDuel`). **E05US010 doit balayer les quatre**, sans quoi le registre affichera « résorbée » à tort.

⚠️ **Un 3ᵉ site depuis E16US008 (28/08/2026), et il n'est pas dans le backend.** Le feu vert donne
...
n'est tenue par rien. À reprendre **en même temps** que les deux sites backend.
