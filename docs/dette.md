...
| [DETTE-004](#dette-004--messageerreur-dupliqué-dans-chaque-feature-front) | conception | `frontend/src/features/*/`, `frontend/src/shared/ui/MessageErreur.tsx` | **E00US013** (21/07/2026) : `MessageErreur` extrait dans `shared/ui/`, **19 copies retirées** (18 définitions `function MessageErreur` + le rendu inline verbatim de `postes/Postes.tsx`) — recompte terrain du grep, la baseline « 16/18 » sous-numérotait. Rendu **inchangé** (mêmes classes, même `role="alert"`). Les 6 autres `role="alert"` du front ont été **examinés et laissés** à dessein : blocs de **confirmation** à action (archers ×2 édition, NouvelArcher inscription — ton neutre, pas `--erreur`) et rendu **ambre** ad hoc du refus 409 de placement (`placement__alerte`, helper `messageErreur` conservé). Marqueurs `DETTE-004` retirés du code. |
...
> d'erreur et n'ont pas leur place dans `MessageErreur`. Le narratif ci-dessous est conservé comme
