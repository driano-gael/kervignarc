...
| [✅ DETTE-078](#-dette-078--quatre-réglages-détape-sont-refusés-après-avoir-été-persistés-résorbée-le-20092026) | conception | ✅ **résorbée le 20/09/2026 (E05US022)** | `backend/domain/deroule_etape.py` (`DEPART_A_BLANC`) ↔ `backend/application/phases.py` (`ServicePhases.ajouter` / `.modifier`) ↔ `backend/domain/format_tournoi.py` (`verifier_applicable`) | Les réglages `profondeur`, `poules`, `big_shoot_off` et `suisse` posés sur un **type de phase qui ne les lira jamais** n'étaient refusés que par `Phase.__post_init__`, c'est-à-dire à `instancier()` — après l'écriture de l'étape | **Trois sites, de gravité croissante** : `ajouter` laissait une étape **orpheline** brûlant un rang ; `ServiceFormats.appliquer` **détruisait le déroulé en place** avant de lever ; `modifier` répondait **200**, persistait, et faisait tomber **chaque lecture** ultérieure en 422 — tournoi illisible sur toutes les surfaces | **E05US027** pour l'inscription ; les deux sites graves trouvés en **3ᵉ et 4ᵉ passes de revue d'E05US022** (axes C2, C1, B) | **Résorbée par une instanciation à blanc avant chaque écriture** (`etape.instancier(DEPART_A_BLANC)`), aux trois sites. ⚠️ La résorption ferme le **préjudice**, pas l'asymétrie : les quatre gardes vivent toujours sur `Phase` et non sur `EtapeDeroule`, d'où trois poses à blanc au lieu d'un invariant. Les hisser reste souhaitable et rendrait ces trois lignes inutiles — US `refactor/`, sans urgence. ⚠️ Une base qui porte **déjà** un réglage fantôme persisté avant ce correctif reste illisible : aucune reprise n'est prévue, le cas n'ayant jamais été observé. |
...
`backend/domain/phase.py` (les gardes `ReglageDePoulesInvalide`,
`ConfigurationBigShootOffInvalide`, `ConfigurationSuisseInvalide`, `ProfondeurInvalide`) ↔
`backend/application/phases.py` (`ServicePhases.ajouter` et `ServicePhases.modifier`).
...
touchant quatre invariants de composition et leurs tests —, chaque site qui **écrit** instancie à
blanc avant d'écrire, `etape.instancier(DEPART_A_BLANC)`. Trois lignes, une machinerie déjà
introduite par la même US pour `FormatTournoi.verifier_applicable`. Test d'API
`test_un_reglage_pose_sur_le_mauvais_type_est_refuse_sans_rien_persister` : `POST` **et** `PUT`,
vu rouge au sabotage des deux poses.

⚠️ **Ce qui reste, et pourquoi ce n'est plus une dette** : les quatre gardes vivent toujours sur
`Phase` et non sur `EtapeDeroule`, d'où **trois** poses à blanc là où un invariant d'étape
suffirait. C'est une redondance, plus un préjudice : aucun état invalide n'atteint la base. Les
hisser reste souhaitable — ce jour-là, les trois lignes disparaissent —, mais rien ne l'exige.
