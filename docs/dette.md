...
|---|---|---|---|---|---|---|---|
| [DETTE-100](#dette-100--la-préséance-de-rôle-nest-déclenchable-par-aucun-écran) | conception | mineur | `frontend/src/features/saisie/Saisie.tsx` (`MessageErreurSaisie`, marqueur `DETTE-100`), `frontend/src/features/saisie/hooks.ts` (`onError`, marqueur `DETTE-100`), `frontend/src/features/saisie/api.ts` (`saisirVolee`, portée `'poste'` **en dur**) | Le refus de préséance **vers le bas** n'est déclenchable depuis aucun écran : l'unique appelant front de `POST /api/v1/saisie/volees` est la tablette, en portée `'poste'` (marqueur dans `api.ts`), et deux tablettes ont le **même** rang (ADR-0107 §3). ⚠️ **Ne pas lire « aucun écran ne fait se croiser deux rangs »** — c'était la rédaction de la 1ʳᵉ passe, et elle était **fausse** : le scoreur écrit au rang 2 par `POST /saisie/corrections` depuis l'écran d'`E16US019`. Ce croisement-là **est** arbitré depuis la revue ; c'est le sens descendant (admin → poste) qui n'a pas de surface | Le rendu ambre de `MessageErreurSaisie` et l'invalidation d'`onError` sont **corrects et testés, mais injoignables en production**. Le piège est pour le **prochain contributeur** : rien à l'écran ne dit que ce chemin n'a jamais tourné pour de vrai, et un test qui *simule* l'erreur ne le dit pas non plus. ⚠️ **Le revers de ne rien inscrire serait pire** : livrer le refus derrière `MessageErreur` générique afficherait une phrase de conflit sans recours le jour où un admin écrit par l'API | **E16US020** (12/09/2026) — écart **constaté au cadrage de la recette fonctionnelle**, pas en revue. Ce n'est pas un raccourci d'implémentation : la surface admin de saisie n'existe pas, et l'inventer sortait du périmètre arbitré | Se referme le jour où un écran d'administration écrit une volée de qualification. ⚠️ **Ce n'est PAS « sans rien à défaire »** : ouvrir ce chemin **arme** le refus sur la file hors-ligne, où il est retiré et journalisé **sans rien dire au marqueur** (`draineLaFile`, perte assumée par ADR-0037). **Critère de fin** : un appelant de `saisirVolee` en portée `'admin'`, le refus rendu **visible** sur le chemin de rejeu, et le scénario de recette de l'US d'introduction déroulable sans `curl` (cf. section Détail). Voisine de `DETTE-090` par la forme (capacité serveur sans écran), sans en partager la cause ni le critère. ⚠️ **À ne PAS résorber en ouvrant la route au scoreur** : `_role_de_saisie` lit `contexte is None` comme « admin » (cf. `test_un_scoreur_n_est_pas_une_identite_de_saisie_de_qualification`) |
...
volée. Deux croisements de rangs sont possibles :

- **scoreur → poste** (le scoreur corrige, la tablette ressaisit) : **arbitré**, et joignable depuis
  les écrans livrés. C'est la correction apportée en revue — `corriger_volee` pose `Role.SCOREUR`,
  sans quoi la tablette écrasait la correction en silence ;
- **admin → poste** (l'organisateur saisit, la tablette ressaisit) : **injoignable**. L'unique
  appelant front de `POST /saisie/volees` est `features/saisie/api.ts`, qui passe la portée
  `'poste'` **en dur**. Aucun écran d'administration n'écrit de volée de qualification, et deux
  tablettes sont à rang égal (ADR-0107 décision 3).

**La conséquence.** Le rendu ambre de `MessageErreurSaisie` et l'`onError` qui relit la vérité
serveur ne se déclenchent, pour ce refus, qu'en réponse à un appel d'API direct. Ils sont corrects
et testés, mais aucun marqueur ne les a vus en salle.
