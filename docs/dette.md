...
| [DETTE-015](#dette-015--modèle-de-source-de-phase-minimal-et-provisoire) | conception | mineur | `backend/domain/phase.py` (`SourcePhase`, `SequencePhases`) ; **élargie par E01US023** : `domain/format_tournoi.py` (`ModelePhase.source`), la `config` JSON de `format_tournoi`, `api/v1/formats.py` et `features/patrimoine/` | E05US001 amorce le **peuplement d'une phase par une autre** avec un modèle **volontairement minimal** : **une** source par phase, « rangs `[début..fin]` de la phase d'ordre *k* » (`SourcePhase(ordre_source, rang_debut, rang_fin)` + `effectif` facultatif). Le modèle **cible** (ADR-0004, E05US010) veut des **sources multiples** (gagnants **et** perdants d'un tour), le **routing en cascade** et la **division récursive** des plages | **Nul tant qu'E05US010 n'est pas prise** : la source unique « par rangs » suffit à composer qualif → élim directe / placement et à décider les trois contrôles de cohérence du CA (source vide / rangs inexistants / effectif incompatible). **À E05US010** : le modèle devra être **élargi** (sources multiples, routing) — les phases déjà saisies avec une source unique restent un sous-cas valide, mais l'API/DTO et le front de peuplement évolueront | E05US001 (amorce assumée, [ADR-0045](adr/0045-sequence-de-phases-cycle-de-vie-typage-source.md) §3) ; **arbitrée avec le commanditaire le 26/07/2026** (amorcer plutôt que reporter tout contrôle de source) | **E05US010** (placement intégral 1→N) — **élargie et re-cadrée le 31/07/2026** : elle porte désormais explicitement le CA « **sources multiples** » (une phase se peuple de plusieurs sources hétérogènes) et le CA « **plages relatives** » (fin ouverte, « le reste »), demandés par le commanditaire pour qu'un format composé pour 120 archers tienne à 82. Élargir `SourcePhase`/`SequencePhases` au peuplement complet, **sur les deux tables**. Marqueur `# DETTE-015` posé sur `SourcePhase`. ⚠️ **Surface de remédiation agrandie par E01US023** : le type du domaine est **réutilisé** (pas dupliqué — bon point), mais il est désormais **sérialisé dans une seconde table** ; E05US010 devra donc migrer aussi les `config` des formats déjà enregistrés, pas seulement celles des phases |
