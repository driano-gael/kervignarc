...
|---|---|---|---|---|---|---|---|
| [DETTE-110](#dette-110--les-libellés-et-lordre-de-la-navigation-des-maquettes-restent-transcrits-à-la-main) | technique | mineur | `maquettes/assets/appareils.js` (en-tête « SOURCE DE LA NAVIGATION », marqueur `DETTE-110`) · `frontend/src/features/admin/CoquilleAdmin.tsx` (tableau local `destinations`) | `E17US010` met les **identifiants** de destination sous garde mécanique, pas leurs **libellés** ni leur **ordre** : ceux-ci vivent dans un tableau local au composant `CoquilleAdmin`, donc hors de portée d'un import — seuls les identifiants sont exposés par `axes.ts` | Deux libellés avaient réellement dérivé et sont corrigés ici : « Phases (format) » et « Composer un déroulé », renommés par `E16US002` **un mois plus tôt** sans que rien ne le signale. Le prochain renommage repartira de zéro | **`E17US010`** (23/09/2026), périmètre assumé : remonter les libellés dans `axes.ts` serait la **3ᵉ** occurrence du remède « sortir un tableau local pour le rendre testable » (après `AXE_PAR_DESTINATION` puis `BESOIN_TOURNOI`), donc un **remède structurel** — US dédiée, jamais en douce dans l'US courante | Remonter un `LIBELLE_PAR_DESTINATION` dans `axes.ts` (`Record` exhaustif, comme ses deux voisins), `CoquilleAdmin` le consommant ; le garde-fou compare alors aussi les libellés. ⚠️ **L'ordre est un autre sujet** : il ne vit pas dans `axes.ts` et n'a pas à y entrer sans raison — la sidebar des maquettes peut légitimement s'en écarter |
...
**Où** : `maquettes/assets/appareils.js` — la table `DESTINATIONS` et l'en-tête
« SOURCE DE LA NAVIGATION » (marqueur `DETTE-110`) ; la source produit est le tableau local
`destinations` de `frontend/src/features/admin/CoquilleAdmin.tsx`.

`E17US010` rend la dérive **détectable** sur les identifiants de destination : le test front
`features/admin/maquettes-navigation.test.ts` importe `AXE_PAR_DESTINATION` et rougit sur toute
...
⚠️ **Inscrite plutôt que résolue, et c'est un choix de règle, pas de confort** : remonter les
libellés dans `axes.ts` serait la **3ᵉ** application du même remède, après `AXE_PAR_DESTINATION`
(E14US003) et `BESOIN_TOURNOI`. Le dépot traite un remède structurel **en US dédiée**, sur preuve
dans le code du jour — la preuve est faite, l'US reste à écrire. La faire ici aurait élargi une US
d'outillage documentaire à un refactor de ~30 entrées dans la coquille d'admin.
