...
| [DETTE-082](#dette-082--deux-endroits-pour-démarrer-le-même-tournoi) | conception | mineur | `frontend/src/features/tournois/Tournois.tsx` (monte `FriseCycleDeVie` **sans** `surJalon`) ↔ `frontend/src/features/jalons/PretADemarrer.tsx` et `frontend/src/features/completude/Completude.tsx` — marqueur `// DETTE-082` dans `frontend/src/features/accueil/FriseCycleDeVie.tsx` | ✅ **Resserrée par `E16US021` (12/09/2026), pas close.** Sur l'**accueil**, `demarrer` et `terminer` sont désormais un **renvoi** vers leur écran « prêt à… » : le doublon y a disparu. Il subsiste sur la destination « Tournoi », qui monte la même frise sans le renvoi, et **pour `archiver`**, dont le jalon n'est pas instruit (`404 jalon_non_instruit`) — l'y renvoyer mènerait à un écran vide | Restreint à un écran de moindre passage : l'organisateur qui démarre depuis la liste des tournois ne voit toujours pas ce qui manque et retombe sur le refus au clic. Aucun cas cassé, un chemin non amélioré — et le chemin le plus fréquenté (l'accueil, destination d'ouverture de l'axe pilotage) l'est désormais | **E16US012** (23/08/2026), qui l'assume à [ADR-0096](adr/0096-un-jalon-enumere-ses-gardes-au-lieu-de-les-lever.md) § Conséquences. ⚠️ Inscrite ici **en revue** : l'ADR la décrivait déjà, mais un angle mort qui ne vit qu'en section *Conséquences* n'apparaît à aucun tri de dette | **Quand `archiver` rejoindra la famille** : passer `surJalon` au montage de `Tournois.tsx` et ajouter `archiver` à `TRANSITIONS_EXPLIQUEES` ferme le reste — le mécanisme est là, il ne manque que le membre. ⚠️ Ne pas ajouter `archiver` à la table **avant** que son jalon réponde : le renvoi mènerait à un `404`. Un test verrouille le comportement « sans `surJalon`, boutons nus », pour que la fermeture soit un geste explicite |
...
`surJalon`) ↔ `frontend/src/features/jalons/PretADemarrer.tsx` et
`frontend/src/features/completude/Completude.tsx` — marqueur `// DETTE-082` dans
`frontend/src/features/accueil/FriseCycleDeVie.tsx`.
...
**✅ Ce qu'E16US021 a refermé (12/09/2026).** La frise accepte un rappel `surJalon` : fourni, les
transitions de `TRANSITIONS_EXPLIQUEES` deviennent un **renvoi** portant le nom de l'écran visé, au
lieu du bouton nu. L'**accueil** — destination d'ouverture de l'axe pilotage, donc le chemin le plus
fréquenté — le fournit. ⚠️ **Le rappel porte la transition, pas la destination** : typer `surJalon`
sur `DestinationAdminId` aurait fait importer `accueil → admin` quand `admin → accueil` existe déjà,
soit exactement le cycle de `DETTE-083`.

**Ce qui reste ouvert, et pourquoi c'est délibéré.** (1) `Tournois.tsx` monte la même frise **sans**
`surJalon` : les boutons y restent nus. (2) `archiver` n'est **pas** dans la table, parce que son
...
**Résorption.** Quand `archiver` rejoindra la famille : l'ajouter à `TRANSITIONS_EXPLIQUEES` et
passer `surJalon` au montage de `Tournois.tsx`. Le mécanisme existe ; il ne manque que le membre.
