...
> ⚠️ **La colonne *Résorption* est lue par l'atlas comme la liste des US résorbantes.** Il en
> extrait tout identifiant `ExxUSyyy` par expression régulière, sur la **cellule entière** : le
> contexte (« précédent à copier », « occurrence qui aggrave », « garde-fou posé par ») lui est
> invisible. **N'y nommer une US que si elle résorbe réellement la dette** — sinon, décrire en prose
> sans identifiant. Cinq artefacts de ce type ont été corrigés le 24/08/2026 ; le détail du piège et
> le remède d'outillage envisagé sont en [`DETTE-085`](#dette-085--les-deux-plans-de-cibles-sont-des-jumeaux-recopiés-composant-par-composant).

## Dette ouverte

| ID | Nature | Sévérité | Portée | Description | Impact | Introduite par | Résorption |
|---|---|---|---|---|---|---|---|
| [DETTE-115](#dette-115--la-classe-bouton--danger-ne-porte-plus-le-jeton-dont-elle-porte-le-nom) | conception | mineur | `frontend/src/app/App.css` (`.bouton--danger`, marqueur `DETTE-115`) · `frontend/src/shared/ui/DialogueConfirmation.tsx` (`TonConfirmation`, marqueur) — **38 occurrences** de `bouton--danger` dans **22 fichiers** de production, **14** de `'danger'` pour le `ton` | Depuis ADR-0114, l'action destructrice ne porte **aucune** couleur d'état ; la classe et la prop gardent pourtant le nom du jeton d'**alerte** `--danger` | Un nom qui dit le contraire de ce qu'il porte **invite** à y remettre l'ambre. Gardé mécaniquement par `charte.test.ts` sur les sélecteurs connus ; une **nouvelle** classe destructrice sous un autre nom échappe au test, qui connaît des sélecteurs et non l'intention | **E17US006** (26/09/2026) — renommage repoussé : il touchait les écrans d'`E17US012` (en vol, fusionnée le jour même) et touche encore `archers/Archers.tsx`, modifié par `E02US007`, en vol | Renommer en `bouton--destructeur` / `ton="destructeur"` **quand aucune branche en vol ne touche les sites** (aujourd'hui : `E02US007`), et étendre le motif `DESTRUCTEUR` de `charte.test.ts` à `--destructeur` (cherchable par motif, plus par liste). Sortie : plus aucune occurrence de `danger` hors jeton d'alerte |
...
| [DETTE-111](#dette-111--la-conversion--zone-de-blason--points--est-écrite-des-deux-côtés) | conception | mineur | ~~`backend/domain/serie.py` (`_points_zone`)~~ · ~~`frontend/src/features/saisie/volees.ts` (`pointsZone`)~~ — **résorbés par E17US011** : domicile unique `backend/domain/blason.py` (`points_zone`, public), servi par `BaremeQualification.points_par_zone`. **Site restant** : `frontend/src/features/saisie-duels/duel.ts` (`pointsZone`, marqueur `DETTE-111`), et son pendant de test `backend/tests/test_domain_blason.py` | « une zone vaut son chiffre, `M` vaut 0 » était écrit **cinq fois** — `serie.py`, `duel.py`, `application/generateur_scores.py` (`valeur_zone`), `volees.ts`, `duel.ts` ; il ne l'est plus qu'**une fois par langage**, et seulement pour les duels côté front. 2ᵉ cas de la famille de [DETTE-020](#dette-020--le-libellé-de-tour-a-deux-domiciles) | ⚠️ **Le remède de DETTE-020 (servir la valeur par le DTO) est inapplicable** : le front doit valoriser des volées que le serveur n'a **jamais reçues** — `serieOptimiste` conserve le `cumul` serveur, qui les ignore. Un total servi par le back serait figé pendant toute la coupure, donc pendant la garantie d'`E04US009`. Risque **rare** (`ZoneScore` est fermé par le règlement FFTA) mais **silencieux** : rien ne relie les deux sites | **`E17US008`** (24/09/2026) l'**aggrave** — `cumulSaisi` est un 3ᵉ consommateur, et le premier à porter un nombre lu comme un score. Non corrigée dans l'US : une US de fidélité aux maquettes n'a touché aucune ligne de Python, traverser les cinq couches y aurait élargi la revue pour le même résultat · **`E17US011`** (25/09/2026) résorbe la moitié « zone → points » **pour la qualification** et relève trois sites que la ligne ignorait (`duel.py`, `generateur_scores.py` — qui dupliquait « délibérément » un symbole privé —, `saisie-duels/duel.ts`) ; les deux premiers sont fusionnés | ✅ *Fait pour la qualification (E17US011)* : `points_par_zone` au DTO de barème, `points_zone` public et domicile unique, `pointsZone` lit la table — le hors-ligne est préservé. **Reste** : (a) `saisie-duels/duel.ts` — servir la même table au DTO des duels, puis y faire lire `pointsZone` ; (b) « quelles volées comptent » reste sans domicile au domaine, cf. le détail. **Ne pas cocher la dette comme résorbée** |
...
| [DETTE-103](#dette-103--trois-replis-de-casse-et-daccents-divergents-dans-le-front) | conception | mineur | `frontend/src/features/audit/presentation.ts` (`replier`), `frontend/src/features/suivi/suivi.ts`, `frontend/src/features/competition/barrage.ts` — marqueurs `DETTE-103` aux trois sites · **+ E17US007** : `frontend/src/features/archers/accueil.ts` (`normaliser`, marqueur `DETTE-103`) — 4ᵉ site | La même règle (replier casse et accents avant de comparer) est réécrite à **quatre** endroits, avec trois variantes : classe de diacritiques (`\p{Mn}` vs `[̀-ͯ]`), `toLocaleLowerCase('fr')` vs `toLowerCase()`, `trim()` dedans ou dehors | Deux champs de recherche répondent différemment à la **même** saisie. Aucun cas utilisateur cassé à ce jour — le français courant se replie pareil dans les trois — mais la divergence grandit à chaque US qui touche l'un des sites | **E16US016** — 3ᵉ occurrence réelle, donc seuil de la règle 16 atteint. ⚠️ L'US **aggrave** la divergence : elle migre `presentation.ts` vers une propriété Unicode et laisse les deux autres sur l'ancienne classe. Le remède est **volontairement hors de cette US** (règle 16 : ADR + US dédiée), mais il fallait un porteur — un constat qui ne vit que dans un corps de commit n'est pris par personne (`DETTE-074`, `DETTE-094`) · **aggravée par E17US007** (25/09/2026) : 4ᵉ copie, variante de `suivi.ts` (plage `[̀-ͯ]`, `toLowerCase()`, `trim()` dedans). ⚠️ Ouverte d'abord sous un numéro neuf, `DETTE-114`, qui la **doublait** en la disant « quatre copies identiques » — faux, c'est la divergence qui fonde cette ligne ; fusionnée ici en revue (axe C2) | Une fonction `replier` partagée dans `shared/` (~8 lignes, aucune abstraction neuve), les **quatre** sites migrés dessus (dont `archers/accueil.ts`, E17US007), un test d'ancrage. Patron : `DETTE-029` |
| [DETTE-100](#dette-100--la-préséance-de-rôle-nest-déclenchable-par-aucun-écran) | conception | mineur | `frontend/src/features/saisie/Saisie.tsx` (`MessageErreurSaisie`, marqueur `DETTE-100`), `frontend/src/features/saisie/hooks.ts` (`onError`, marqueur `DETTE-100`), `frontend/src/features/saisie/api.ts` (`saisirVolee`, portée `'poste'` **en dur**, marqueur), `frontend/src/features/validation-qualif/PanneauValidationQualif.tsx` (marqueur — l'écran scoreur **n'appelle pas** `/saisie/corrections`) | **AUCUN croisement de rangs n'est déclenchable depuis les écrans livrés**, et la cause est **double** : (a) l'unique appelant front de `POST /saisie/volees` est la tablette, en portée `'poste'` **en dur**, et deux tablettes ont le même rang (ADR-0107 §3) ; (b) `POST /saisie/corrections` — le seul endpoint qui fasse écrire le rang **scoreur** — n'a **aucun appelant front**, et `PanneauValidationQualif.tsx` le dit lui-même en tête (« la correction ne se fait pas ici »). ⚠️ **La 2ᵉ passe de revue avait remplacé cet énoncé par un faux** (« le scoreur écrit depuis l'écran d'`E16US019` ») : c'est la règle serveur qui a été complétée, pas une surface qui est apparue | Le rendu ambre de `MessageErreurSaisie` et l'invalidation d'`onError` sont **corrects et testés, mais injoignables en production**. Le piège est pour le **prochain contributeur** : rien à l'écran ne dit que ce chemin n'a jamais tourné pour de vrai, et un test qui *simule* l'erreur ne le dit pas non plus. ⚠️ **Le revers de ne rien inscrire serait pire** : livrer le refus derrière `MessageErreur` générique afficherait une phrase de conflit sans recours le jour où un admin écrit par l'API | **E16US020** (12/09/2026) — écart **constaté au cadrage de la recette fonctionnelle**, pas en revue. Ce n'est pas un raccourci d'implémentation : la surface admin de saisie n'existe pas, et l'inventer sortait du périmètre arbitré | Se referme le jour où **un** des deux chemins gagne un écran. ⚠️ **Ce n'est PAS « sans rien à défaire »** : ouvrir le chemin admin **arme** le refus sur la file hors-ligne, où il est retiré et journalisé **sans rien dire au marqueur** (`draineLaFile`, perte assumée par ADR-0037). **Critère de fin** : un appelant front de `/saisie/corrections` **ou** de `saisirVolee` en portée `'admin'` ; le refus rendu **visible** sur le chemin de rejeu ; et le scénario de recette déroulable sans `curl`. Voisine de `DETTE-090` par la forme (capacité serveur sans écran), sans en partager la cause ni le critère. ⚠️ **À ne PAS résorber en ouvrant la route au scoreur** : le rang se calcule à la frontière (`api/v1/saisie.py`, `Role.ADMIN if poste is None else …`) et élargir la garde sans y ajouter de branche donnerait au scoreur la préséance de l'organisateur (cf. `test_un_scoreur_n_est_pas_une_identite_de_saisie_de_qualification`) |
...
| [DETTE-006](#dette-006--cle_nom-nest-plus-chez-elle-dans-domainclubpy) | conception | mineur | `backend/domain/club.py` (`cle_nom`), `backend/domain/archer.py`, `backend/domain/doublons.py`, `backend/application/archers.py`, `backend/application/clubs.py` | `cle_nom` — le repli casse/accents des noms propres — vit dans `domain/club.py`, mais sert désormais **6** usages dont **4 hors du concept « club »** : `archer.cle_identite` (E02US002), le tri des archers (E02US003), la détection de doublons `domain/doublons.py` (E02US005) et la **recherche transverse** `domain/recherche.py` (E16US010). Sa propre docstring avait posé le seuil : « si un 2ᵉ usage hors club apparaît, extraire dans un `domain/texte.py` en US dédiée » | La fonction est **juste** ; seul son domicile est faux. Un lecteur d'`archer.py`, de `doublons.py` ou de `recherche.py` doit aller lire `club.py` pour comprendre comment se replient les noms d'archers, et le prochain usage hors club ira chercher la règle là où elle n'a plus de raison d'être | E02US002 (1ᵉʳ usage hors club) ; **seuil atteint** par E02US003 (2ᵉ) ; **3ᵉ usage hors club** par E02US005 (`domain/doublons.py`) ; **4ᵉ** par E16US010 (`domain/recherche.py`, recherche transverse) | US dédiée à créer (`refactor/…`) — déplacer dans `domain/texte.py`, 6 appelants, zéro changement de comportement |
...
| [DETTE-098](#dette-098--le-port-du-qr-sappelle-rattachement-alors-quil-sert-une-session) | conception | mineur | `backend/domain/ports.py` (`GenerateurDocumentsSalle.qr_rattachement`), `backend/infrastructure/pdf/documents_salle.py` | Le port rend **n'importe quel QR à l'écran** depuis E16US015, mais garde le nom `qr_rattachement`. Or `docs/glossaire.md` réserve « rattachement » au lien **appareil ↔ lieu** et qualifie le scoreur d'**itinérant** : le symbole affirme le contraire du glossaire | `grep -r rattachement` cesse de délimiter le monde des postes ; un 3ᵉ appelant (QR d'écran de salle) consacrerait le contresens au lieu de le révéler | E16US015 — réutilisation du port plutôt qu'un second port (bon choix, règle 2) ; le renommage touche port + adapter + 3 appelants + doublures de test, hors périmètre d'un commit de correction | **À planifier** — renommage mécanique en `qr_svg(url)`, zéro changement de comportement. Relevé par trois axes en 3ᵉ passe de revue |
| [DETTE-099](#dette-099--un-ajustement-du-plan-de-duels-peut-atterrir-sur-un-autre-tour) | conception | mineur | `backend/application/placement_duels.py` (`deplacer`, `regenerer`, `placer_les_restants`), `backend/api/v1/placement_duels.py`, `frontend/src/features/duels/Duels.tsx` | Les requêtes d'ajustement ne portent **pas** le tour : le serveur le **recalcule** à chaque écriture (`_charger` → `tour_a_poser`). Or, depuis E03US012, le tour posé peut basculer **sans aucun geste de l'organisateur** — il suffit qu'un scoreur valide ailleurs le dernier duel du tour. L'écran affiche alors le tour N, l'écriture s'applique au tour N+1 | Fenêtre étroite mais silencieuse : si l'archer déplacé dispute aussi le tour suivant, la pose part sur le **mauvais tour** sans aucun message ; sinon l'organisateur reçoit un `InscriptionIntrouvable` qui ne dit pas que le tour a changé. Avant E03US012 le plan était unique par phase : la course n'existait pas — c'est cette US qui l'ouvre | E03US012 (05/09/2026), relevée en revue (axe C2) | Faire porter le `tour` aux requêtes d'écriture (la **réponse** le porte déjà, `PlanDeDuelsReponse.tour`) et refuser en **409** si `tour != contexte.tour`, avec un message qui dit de rafraîchir. ~10 lignes serveur + le champ côté front, aucun pattern. Écarté de l'US courante parce qu'il change le **contrat de trois routes** — c'est une tranche à part entière, pas un correctif de revue. Marqueur `DETTE-099` posé sur `deplacer` |
| [DETTE-012](#dette-012--lurl-du-qr-de-cible-est-lorigine-de-la-requête-admin) | technique | mineur | `backend/application/documents_salle.py` (`_url_rattachement`, `_url_scoreur`) | L'URL encodée dans le QR de cible est **absolue**, bâtie sur l'**origine de la requête admin** (`request.base_url`, passée par l'API) : il n'existe pas de base URL publique configurée côté serveur. Générer les étiquettes depuis `localhost` (console du serveur) produit donc des QR pointant sur `http://localhost:8000/?poste=…`, inutilisables depuis une tablette | Un QR généré depuis `localhost` renvoie la tablette **sur elle-même** : le « filet » de re-rattachement (scanner le QR pour revenir sur sa cible) ne fonctionne pas. **3ᵉ site depuis E16US015** (`_url_scoreur`), où la conséquence diffère : le QR d'un scoreur reste **porteur de son code personnel** mais renvoie le téléphone sur lui-même — échec silencieux pour l'admin, incompréhensible pour le scoreur, qui garde toutefois le repli de retaper son code. **Sans effet dans le flux nominal** : le jour J, l'admin atteint le serveur par son **IP réseau** (les 30 tablettes aussi), donc `base_url` = l'IP LAN et le QR est correct | E09US008 (impression des QR) ; **choix tranché en réalisation** (règle 11/12 : pas de config réseau introduite en douce ici) | **À replanifier** — E11US001 (livrée le 26/07/2026) apporte l'*enabler* (nom public stable `kervignarc.local` annoncé en mDNS) mais **ne câble pas** la base URL : `_url_rattachement` encode toujours `request.base_url`. Reste à faire dans une US dédiée (base URL publique configurable, source unique des liens absolus). **Design** : encoder `kervignarc.local` suppose le mDNS résolu côté tablette (best-effort) → l'IP LAN reste le défaut sûr |
| [DETTE-016](#dette-016--montant-remboursé--tarif-courant-pas-somme-encaissée) | conception | mineur | `backend/domain/remboursement.py`, `backend/application/inscriptions.py`, `backend/application/departs.py`, `backend/application/archers.py` (`_remboursements_des_payees`, **3ᵉ site — E01US026**), `backend/infrastructure/db/repositories/referentiel.py` (`_encaisse`) | Le `montant_centimes` d'un remboursement fige le **tarif courant du départ au moment de l'effacement**, or le modèle ne stocke **jamais** la somme réellement versée (seul le booléen `paye` de l'inscription). Si le tarif d'un départ est **édité après** qu'une inscription y soit payée, le remboursement ouvert peut différer de l'encaissé | Sur un **mouvement d'argent**, un montant remboursé **faux** est possible — arguablement pire qu'absent. **Nul dans le flux nominal** (le tarif ne bouge pas après paiement) ; suppose une édition de tarif entre paiement et effacement | E08US005 ([ADR-0057](adr/0057-registre-de-remboursements.md), choix « tarif au moment de l'effacement ») ; **constaté en revue adversariale le 29/07/2026** | US dédiée — stocker `montant_paye_centimes` sur l'inscription à l'encaissement (ou **geler** le tarif d'un départ dès qu'une inscription y est payée). Marqueur `# DETTE-016` sur les **quatre** sites (trois services + le décompte de suppression) |
| [DETTE-017](#dette-017--auteur_admin-dupliqué-sur-5-sites) | conception | mineur | `backend/application/paiements.py`, `backend/application/placement.py`, `backend/application/remboursements.py`, `backend/application/pilotage_tour.py` (`_AUTEUR_ADMIN`) **+ E16US008** : `backend/application/forfaits.py` (`AUTEUR_ADMIN`, publique — l'API la lit) | La constante `"Administrateur"` (auteur des entrées d'audit d'un acte admin) est **dupliquée** sur **5** sites : paiements (E08US002), régénération de plan (E12US007), remboursements (E08US005), lancement de tour (E12US002), forfait de duel déclaré depuis l'administration (E16US008). Le seuil « factoriser au 3ᵉ cas » (CLAUDE.md § Dette) est dépassé de deux crans | Faible : littéral stable ; changer le libellé se fait en 5 endroits. ⚠️ **Le 5ᵉ site est le premier à être PUBLIC** (pas de `_` initial) parce que la frontière API en a besoin pour tracer l'auteur : la duplication cesse d'être interne à une couche, ce qui rend la résorption **plus** utile, pas moins | E08US002/E12US007 (2 sites) ; **3ᵉ site E08US005**, proposé en résorption dans [ADR-0057](adr/0057-registre-de-remboursements.md) ; ⚠️ **4ᵉ site `pilotage_tour.py` (E12US002) découvert le 28/08/2026 au cadrage d'E16US008** — il était là sans avoir jamais été inscrit, exactement l'aggravation silencieuse que le § Dette de `CLAUDE.md` proscrit ; **5ᵉ site E16US008**, inscrit dans son commit | US dédiée `refactor/` — extraire `AUTEUR_ADMIN` dans un module partagé (`application/`), 5 appelants, zéro changement de comportement. Marqueur `# DETTE-017` sur les sites |
| [DETTE-019](#dette-019--serviceroutage-jumeau-de-servicepilotagetour) | conception | mineur | `backend/application/routage.py`, `backend/application/pilotage_tour.py`, `frontend/src/features/routage/presentation.ts` (E04US018, **jamais inscrit avant E16US008**) **+ E16US008** : `frontend/src/features/feu-vert/etat.ts` | `ServiceRoutage` (E04US018) reprend de `ServicePilotageTour` (E12US002) trois éléments : `_sources_en_attente` (**corps identique**), la lecture « archer → pose du plan de duels » (`_plan_lu` / `_poses_du_tour_pose`, à la position près) et surtout la **garde tour-1** — « ne jamais annoncer la cible d'un match de tour ≥ 2, la pose persistée est celle du tour 1 » — écrite dans **deux formulations différentes** | Les deux premiers sont des dérivations sans enjeu. La **garde tour-1**, elle, est un invariant de sûreté physique : une cible périmée envoie un archer sur la mauvaise butte. Le jour où la pose persistée des cibles des tours ≥ 2 sera livrée, il faudra la lever **aux quatre endroits** ; en rater une ne casse rien de visible côté serveur — ça affiche seulement une mauvaise cible. ⚠️ La parité s'arrête là : le routage porte **en plus** l'alerte « duel non côte à côte » (`duels_separes`) que le feu vert, lui, **ne porte pas** — cf. DETTE-021, qui est le vrai défaut | E04US018 (2ᵉ occurrence ; la 1ʳᵉ est E12US002) ; ⚠️ **élargie par E16US008** — 4ᵉ site, dans le front | ⚠️ **Les deux déclencheurs annoncés sont passés sans 3ᵉ site** (constat du 02/08/2026, remarque de revue) : les deux canaux publics de l'épic 07 ont **réemployé** `ServiceRoutage` au lieu de recopier sa lecture — c'est le bon choix, et il vaut d'être acté plutôt que de laisser le registre prédire un passé révolu. Le déclencheur restant est **la pose des cibles au-delà du tour 1**, pas un canal de routage. ⚠️ **Aucune US ne porte cette résorption aujourd'hui.** Le registre a longtemps pointé l'US du placement intégral 1→N — **livrée le 31/07/2026** — alors que la garde tour-1 n'a pas bougé : il attendait une US déjà passée. Le travail restant est la **pose persistée** des cibles des tours ≥ 2. ⚠️ Ne PAS renommer d'US ici : un identifiant écrit dans cette cellule est lu par l'atlas comme « cette US a résorbé la dette », même précédé d'une négation. Extraction à la **3ᵉ occurrence** si elle survient. Remède pressenti : une lecture publique `ServicePlacementDuels.poses_par_archer` + un `cible_du_match(match, poses)` qui **porte** la garde tour-1, ~40 lignes déplacées, zéro changement de comportement. **Point d'entrée** de la levée : c'est là que la garde se lève. Marqueurs `DETTE-019` sur les **quatre** sites. ⚠️ **Élargie par l'US du feu vert — un site de plus, et il est dans le FRONT** : `frontend/src/features/feu-vert/etat.ts` (`actionDuel`, constante `SANS_RECOURS_HORS_TOUR_1`) teste `duel.tour > 1` pour **refuser** d'offrir le renvoi au plan de duels, puisque au-delà du tour 1 « cible non attribuée » ne se lève par aucun geste. Ce n'est pas une copie de la garde — c'est sa **conséquence rendue visible à l'organisateur** —, mais l'effet sur la résorption est le même : lever la garde côté serveur sans toucher ce fichier laisserait l'écran annoncer une limite qui n'existe plus, et **rien ne rougirait**. La couture serveur ↔ front n'est tenue par aucun test commun. ⚠️ **Et ce n'était pas le premier site front** : `frontend/src/features/routage/presentation.ts` (`partitionner`) portait déjà un marqueur `DETTE-019` **absent de cette table** — relevé en revue (axe C2) ; l'US d'origine est en colonne « Introduite par ». Quatre sites, donc, dont **deux dans le front** : c'est exactement le défaut que le même commit se félicitait d'avoir corrigé sur DETTE-017, reproduit sur la dette d'à côté. Le `grep DETTE-019` sauvait la résorption ; la **table**, elle, sous-comptait — et c'est elle qui sert à décider si la dette vaut une US. À reprendre **avec les deux sites backend ET les deux sites front**. ✅ **La garde tour-1 est LEVÉE par E03US012** (05/09/2026, [ADR-0106](adr/0106-la-pose-d-une-cible-appartient-a-un-tour.md)) : les **quatre** sites ont été balayés dans le même diff, et la comparaison est passée de `tour == 1` au **tour posé**. Le point de fond de cette dette — un invariant de sûreté écrit en quatre formulations — n'existe donc plus. ⚠️ **La dette reste ouverte pour son reliquat** : `_sources_en_attente` est toujours écrit deux fois, corps identique. C'est la part « dérivation sans enjeu » du constat, jamais l'invariant. Sévérité ramenée à **mineure** de fait. ⚠️ Et la prédiction du registre était **fausse** dans les deux sens : il attendait E05US010 (livrée, hors sujet — « 1→N » y désigne la profondeur de classement), alors qu'aucune US ne portait le sujet ; c'est le cadrage d'`E16US013` qui l'a fait remonter, en butant dessus. ⚠️ **Sites ramenés à DEUX, tous deux backend** (`routage.py`, `pilotage_tour.py` — `_sources_en_attente`, corps identique) : les deux jetons du **front** ont disparu avec la levée de la garde. Le geste de résorption étant un `grep`, un registre qui **sur-compte** est le symétrique exact du sous-comptage que cette même ligne raconte pour E16US008. |
...
| [DETTE-052](#dette-052--la-saisie-admin-devine-le-créneau-de-larcher) | conception | majeur | `backend/application/saisie.py` (`_depart_de_saisie`), routes `POST /api/v1/tournois/{id}/archers/{id}/volees` et suivantes **+ E16US019** : identité **scoreur** sur `GET /saisie/series/…`, `POST /saisie/validations`, `POST /saisie/annulations`, et la surface `frontend/src/features/validation-qualif/` | En saisie **admin** (`contexte=None`), le service ne reçoit aucun créneau : il le **devine** en prenant le plus petit identifiant de créneau où l'archer est inscrit. La saisie par **poste** ne souffre pas du défaut — la tablette porte son `depart_id` | Un archer engagé matin **et** après-midi verra l'admin écrire dans son créneau du matin, donc dans la qualification du matin. ⚠️ **Aggravé par E16US019** : le scoreur, lui, **voit** un sélecteur « Départ à valider » qui n'est pas transporté — il peut donc valider ou **annuler** la feuille du matin en ayant choisi l'après-midi, en `200` et sans un mot. Une annulation sur la mauvaise feuille rouvre des volées d'un créneau clos | Née de **E05US025** : la feuille pend désormais à une phase, donc il faut résoudre *laquelle* — la question ne se posait pas quand une série valait pour tout le tournoi. Le défaut est donc **révélé**, pas introduit : l'ancienne clé écrasait purement et simplement la seconde série (c'était DETTE-046) | Résorption : porter le `depart_id` sur les **cinq** routes concernées (saisie admin **+ les trois routes scoreur** : `GET /saisie/series/…`, `POST /saisie/validations`, `POST /saisie/annulations`) et le passer au service, comme le fait déjà le `ContexteSaisie` d'un poste. Marqueurs `DETTE-052` sur `_depart_de_saisie`, `_phase_qualification`, les deux routes d'écriture scoreur et le `ChoixCreneau` de `PanneauValidationQualif.tsx` |
...
| [DETTE-080](#dette-080--la-plomberie-détat-des-deux-formulaires-de-composition-est-écrite-deux-fois) | conception | mineur | `frontend/src/features/phases/Phases.tsx` (`FormulairePhase`) ↔ `frontend/src/features/deroule/Deroule.tsx` (le formulaire d'étape) — marqueurs `// DETTE-080` sur les deux | Les deux écrans qui composent des phases — « Phases du tournoi » et « Composer un format » — déclarent **la même plomberie d'état** pour chaque réglage : un `useState` initialisé par un `depuis…()`, une garde de type au moment de construire la charge utile (`estPoules ? vers…() : null`), et une remise à zéro après ajout. Dix réglages y passent aujourd'hui (type, effectif, profondeur, poules, Big Shoot Off, suisse, colline, découpage, arrêts, **titre**). Les **contrôles** eux-mêmes ne sont pas dupliqués : `shared/phases/Reglage*.tsx` est partagé, et c'est la moitié du problème déjà résolue | **Une des deux copies est déjà en retard** : `Deroule.tsx` réinitialise **7 champs sur 12** à l'ajout (`sources`, `effectif`, `profondeur`, `poules`, `big_shoot_off`, `suisse`, `titre`) — il manque **`colline`, `arrets`, `decoupage`, `type` et le barème**, qui se reportent donc en silence sur l'étape suivante. ⚠️ Une rédaction intermédiaire disait « ne réinitialise **aucun** champ », sur la foi d'un commentaire de `Phases.tsx` périmé depuis E06US006 — relevé en 2ᵉ passe de revue. Le registre était sourcé sur un commentaire au lieu du code, et le commentaire sur un état d'il y a deux mois : la vraie divergence en était **masquée**, et une US `refactor/` en aurait dérivé un périmètre faux. L'effet est borné au **brouillon d'atelier** — visible avant enregistrement et rattrapable —, d'où *mineur* et non *majeur*. ⚠️ Une première rédaction disait « aucun effet à l'exécution : les deux copies sont correctes » **trois lignes avant** de décrire cette divergence : les deux ne pouvaient pas être vraies (relevé en revue). Le coût principal reste l'**oubli d'un côté**, et il est **déjà arrivé trois fois** — la remise à zéro de la profondeur n'avait été appliquée qu'à un des deux formulaires (relevé en 2ᵉ passe d'E06US006, commentaire encore en place dans `Phases.tsx`), la colline manquait à la liste de reset, et `Deroule.tsx` ne réinitialise toujours **aucun** champ à l'ajout (« l'asymétrie est constatée, pas voulue », dit le code). C'est le pendant **front** de `DETTE-054` : là-bas des DTO jumeaux, ici la plomberie qui les remplit | Antérieure à cette US (les deux formulaires divergent depuis E01US024), **inscrite** par `E16US002` qui y ajoute le 10ᵉ réglage. ⚠️ **Le seuil du remède structurel est franchi depuis longtemps** — dix occurrences, pas trois — mais il se traite en US dédiée, pas en douce dans une US d'IHM (`CLAUDE.md` § Dette). ⚠️ **Ce que cette US a fermé à la place, parce que la preuve y était** : `configInchangee` dans `Phases.tsx`, qui supprime la recopie manuelle de config des widgets à champ unique — trois occurrences réelles et **deux bugs déjà payés** (le barrage effaçait le découpage ; le couple découpage/arrêts partait en 422). Portée locale, une fonction, donc pas d'ADR | US `refactor/` — extraire un `shared/phases/useCompositionDEtape.ts` détenant l'état des dix réglages, leurs gardes de type et leur reset, consommé par les deux formulaires. ⚠️ **Ne pas fusionner les deux écrans** : ils diffèrent réellement de portée (l'un travaille sur un tournoi, l'autre sur une brique de bibliothèque hors tournoi — même distinction que `DETTE-054` fait entre feuilles et racines). C'est la **plomberie** qui est identique, pas les écrans |
| [DETTE-096](#dette-096--le-modèle-de-données-affiche-une-date-du-jour-et-ignore-cinq-migrations) | technique | mineur | [`docs/modele-de-donnees.md`](modele-de-donnees.md) (bandeau de version + tables absentes) | Le document de référence du schéma ne reflète pas les migrations **0048 → 0051** : les tables créées par `0048_franchissement_arret`, `0049_arret_de_circonstance` et `0050_identite_visuelle_tournoi` n'y figurent pas, ni les colonnes `poste.noms_par_page` / `cadence_page_s` de `0051` | Une **source de vérité de schéma** incomplète, et — depuis E16US014 — **estampillée du jour** : le bandeau est passé à « v0.13, 31/08/2026 » alors qu'il manque trois tables. Une date fraîche sur un document faux est ce qui fait qu'on cesse de le vérifier ; avant, l'écart entre la date (07/08) et les migrations récentes donnait au moins un signal | **Constatée en revue d'E16US014** (axe C2), **pas introduite par elle** : la dérive date d'E05US033. Ce que le diff aggrave est la **datation**, pas le contenu — d'où l'inscription ici plutôt qu'une correction dans une US qui ne touche pas ces tables | Rattraper les quatre migrations en une passe documentaire : trois tables au format des voisines, deux colonnes au §POSTE, et un changelog qui reprend v0.13 → v0.17. **Critère de fin** : chaque migration de `migrations/versions/` postérieure à 0043 a sa trace au changelog. ⚠️ Ne pas le faire au fil de l'eau d'une US non concernée — c'est ce qui a produit l'écart |
| [DETTE-097](#dette-097--le-palmarès-projeté-ne-pagine-pas-alors-quil-peut-porter-des-dizaines-de-blocs) | conception | majeur | [`frontend/src/features/palmares/VuePalmares.tsx`](../frontend/src/features/palmares/VuePalmares.tsx) (monté sans pagination par `EcranSalle`) · **+ E06US009** : `frontend/src/features/palmares/VuePalmares.tsx` (`SectionCreneau`, `N` sections empilées) | E16US009 a donné une **pagination réglable** (`noms_par_page`, `cadence_page_s`) aux deux vues longues de l'écran projeté — classement et affectations. `VuePalmares` n'en a jamais reçu : elle rend `1 + n_catégories + n_clubs` sections — **+ E16US017** : une section de **plus**, le classement des clubs, à raison d'une ligne par club présent (donc l'ordre de grandeur de `n_clubs` une seconde fois), placée **entre** les podiums et le classement complet, donc repoussant ce dernier encore plus bas — **plus** le classement complet, d'un seul tenant | Tenable à huit blocs de quatre places, la portée **club** d'E16US014 peut en produire des dizaines (un tournoi de salle réunit 30 à 40 clubs), et la profondeur monte à 64. Le vidéoprojecteur montre le haut et **rien d'autre ne défilera jamais** — sans message, l'écran a simplement l'air d'être arrivé au bout ⚠️ **Aggravée par `E06US009` (19/09/2026), et son déclencheur change de nature** : la vue empile désormais **une section par créneau**, donc sa hauteur est multipliée par le nombre de départs. Avant, la dette ne mordait que si l'organisateur cochait explicitement la portée *club* — c'était un cas de bord, d'où la sévérité *mineur*. Elle mord maintenant sur **tout tournoi multi-créneaux**, c'est-à-dire le cas nominal : à 17 h, la salle voit « Départ n°1 » et rien d'autre, et personne ne fait défiler devant un vidéoprojecteur. ⚠️ C'est le symptôme que `E06US009` corrige côté serveur, **déplacé vers le pixel** (relevé par deux axes). Sévérité relevée à **majeur** en conséquence. | **E16US014** (31/08/2026) — relevée par l'axe adversarial. Le déclencheur est un **choix explicite** de l'organisateur (le défaut reste catégorie / 4 places), d'où *mineur* **à l'origine** (relevé en majeur le 19/09/2026) : rien ne casse tant que la portée club n'est pas cochée | Passer `pagination` à `VuePalmares` comme `VueClassement` le reçoit, en réutilisant le `ReglagePages` du poste — le mécanisme existe et se règle déjà par écran. **Critère de fin** : la vue Palmarès de l'écran de salle pagine et respecte `cadence_page_s`. ⚠️ À traiter avec E16US009 ou dans l'US qui rouvrira l'écran de salle, pas en catimini ⚠️ Marqueur `DETTE-097` posé sur `SectionCreneau` (`frontend/src/features/palmares/VuePalmares.tsx`) le 19/09/2026 : la ligne citait le fichier sans qu'aucun `grep` ne le ramène. |
| [DETTE-088](#dette-088--le-plafond-de-commentaires-ne-couvre-pas-tout-le-dépôt) | technique | mineur | `backend/tests/test_commentaires_bornes.py` (`_COUVERTS`, marqueur `# DETTE-088`) ↔ `frontend/src/commentaires.test.ts` (en-tête, marqueur `DETTE-088`) | Le plafond de huit lignes par bloc (règle 13, ADR-0099) couvre **tout le code de production** backend et **tout `frontend/src`**, mais laisse dehors `backend/tests/` (395 blocs), `backend/migrations/` (56), `atlas/statique/` (5), `frontend/eslint.config.js` (1 bloc de 14 lignes, **porteur de sécurité** : la frontière navigateur/Node) et `backend/kervignarc.spec` (1 bloc de 17 lignes — spec PyInstaller **exécutée** par `build_release.py`, que le `glob("*.py")` du détecteur ne voit pas). Deux limites du détecteur sont assumées de la même façon : une **ligne vide coupe un bloc** — le plafond borne le paragraphe, pas la zone de commentaire —, et un **gabarit multi-lignes** dont une ligne commence par `//` serait compté à tort (les littéraux simples, eux, sont neutralisés depuis la revue) | **Asymétrie visible entre les deux moitiés de la même règle** : un auteur de test front ne peut pas écrire un oracle de neuf lignes, son homologue backend le peut. Le risque n'est pas la divergence de code mais celle de la **règle telle qu'on la croit** — et une porte qui se croit plus large qu'elle n'est éteint la vigilance | **E00US027** (28/08/2026) — relevée par quatre axes de revue. Résorber `backend/tests/` demanderait 395 réécritures dans une US déjà à 452 fichiers : hors maille | **Une US d'outillage**. **Critère de fin**, vérifiable à l'exécution plutôt que chiffré : `_EXCLUS` ne contient plus `tests`, `test_aucun_fichier_ne_gagne_de_bloc_trop_long` reste vert, et les deux détecteurs couvrent le même périmètre — y compris à travers une ligne vide unique. ⚠️ Ne pas traiter `migrations/` — un fichier de migration est un artefact daté qu'on ne réécrit pas |
| [DETTE-089](#dette-089--le-câblage-du-routage-big-shoot-off-nest-ancré-par-aucun-test) | technique | mineur | `backend/bootstrap/composition.py` (marqueur `# DETTE-089`, paramètre `service_big_shoot_off`) | `ServiceRoutage` reçoit `service_big_shoot_off` en paramètre **optionnel** : sans lui, `_routage_big_shoot_off` rend `INDISPONIBLE` sur les quatre canaux de routage, et **aucun test ne monte `create_app`** pour l'éprouver. Le commentaire du fichier affirmait le contraire — il annonçait un `test_composition_routage_big_shoot_off` qui n'a jamais existé | **Une promesse de test est pire qu'une absence de test** : elle dispense le lecteur de vérifier. Le défaut réel est borné (un oubli de câblage se verrait à l'écran dès le premier Big Shoot Off), mais c'est exactement le mode de panne qu'ADR-0099 décrit | **E00US027** (28/08/2026) — le défaut préexistait sur `main` ; l'US a réécrit la ligne en gardant la promesse, et l'axe B l'a rattrapé | **Deux voies, la moins chère d'abord** — (a) rendre `big_shoot_off` **non optionnel** dans `ServiceRoutage.__init__` : un seul site de construction de production, qui le passe toujours, donc `mypy --strict` devient l'ancrage gratuitement ; (b) sinon écrire le test — `create_app()` puis interroger `app.state.service_routage` sur une phase de Big Shoot Off. **Critère de fin** : retirer `service_big_shoot_off` de la composition fait rougir la porte |
| [DETTE-091](#dette-091--la-liste-des-adr-structurants-dadr-0075-na-jamais-été-auditée) | conception | mineur | [`docs/adr/0075-le-depart-est-la-portee-sportive.md`](adr/0075-le-depart-est-la-portee-sportive.md) § « Portée de la règle » (les deux listes) | La règle « décision structurante ⇒ section *Porté dans le code par* » est **bornée** par une liste nominative tenue à la main. Cette liste a été oubliée **quatre fois** pour des ADR neufs, et le rétro-équipement du 08/08/2026 a par ailleurs **manqué des ADR de moteur** : `0050` (trouvé en revue d'E16US008), et au moins `0023`, `0027`, `0047`, `0048`, `0065` qui satisfont le critère sans porter de section | **La liste borne ce qu'une revue a le droit de relever** : un ADR qui n'y figure pas voit son absence de section explicitement exclue des défauts. Une liste incomplète ne rate donc pas seulement un ADR — elle **éteint la détection** sur celui-ci, durablement et sans symptôme. C'est le mode de panne d'ADR-0017, au niveau de la procédure au lieu du code. ⚠️ **Intervalle `0071`-`0079` audité le 20/09/2026** (4ᵉ passe de revue, tableau dans ADR-0075 § Portée) — jamais balayé jusque-là, les « seize retenus » s'arrêtant à `0070` et « Ajoutés depuis » commençant à `0080`. `0076` manquait aux deux listes et y est inscrit ; `0078` y figurait comme réouverture alors qu'il est neuf ; **`0071` entre au critère et ne porte pas sa section** — contrainte dure du moteur de placement. La dette **reste ouverte** : l'audit ne couvre qu'un intervalle sur les cent | **E16US008** (28/08/2026) — l'ADR demandait le balayage pour la deuxième fois sans le faire ; l'inscrire ici lui donne enfin un porteur | **Une US d'outillage documentaire**, ou le premier quart d'heure d'une US qui rouvre un ADR. **Critère de fin**, vérifiable : tout ADR *Accepté* non remplacé qui **satisfait le critère structurant** figure dans la liste des retenus et porte sa section ; la liste « hors critère » n'accueille que les exclusions dont la question s'est posée. ⚠️ Il ne s'agit PAS d'inscrire les ~100 fichiers de `docs/adr/` — `CLAUDE.md` ne fait vivre nominativement que les retenus. ⚠️ Le geste n'est PAS de recopier la liste : c'est de balayer `ls docs/adr/` et de rapprocher — une liste tenue à la main ne se répare pas au cas par cas |
| [DETTE-092](#dette-092--la-recherche-transverse-lit-trois-référentiels-entiers-à-chaque-frappe) | technique | mineur | [`backend/domain/ports.py`](../backend/domain/ports.py) (`ArcherRepository.tous`), [`backend/application/recherche.py`](../backend/application/recherche.py), [`backend/application/jalons.py`](../backend/application/jalons.py) (`ServiceJalons.apercus`), [`frontend/src/shared/hooks/useValeurRetardee.ts`](../frontend/src/shared/hooks/useValeurRetardee.ts), [`frontend/src/features/recherche/`](../frontend/src/features/recherche/) (`hooks.ts`, `RechercheTransverse.tsx`, `RechercheTransverse.test.tsx`) | La recherche transverse **charge tout puis filtre en mémoire** : `tous()` rend les archers de **toutes** les éditions, auxquels s'ajoutent `clubs.lister()` et `tournois.lister()` à chaque appel. L'aperçu de jalons fait de même en O(3·N) sur les tournois encore préparables. Le pari était **écrit, mais seulement en commentaire** | Aucun cas utilisateur ne casse aujourd'hui (mono-club, LAN, SQLite — règle 12), et l'anti-rebond front divise le coût par cinq. Mais le seuil qui « démentirait le pari » ne vivait nulle part où on le relira : c'est la **dette silencieuse** que `DETTE-064` et `DETTE-071` documentent déjà — « une dette qui ne vit qu'en commentaire n'est jamais prise » | E16US010 (29/08/2026), **inscrite en revue** : trois axes ont relevé le raccourci, aucun n'a trouvé de trace au registre | US `refactor/` — pousser le repli casse/accents **en base** (colonne normalisée + index, ou FTS) et remplacer `tous()` par un `chercher(fragment)` au port. **Critère de fin** : `tous()` disparaît de `ArcherRepository`. ⚠️ Ne pas se contenter d'un `LIKE` : il ne replie pas les accents, et c'est précisément la raison du filtrage en mémoire |
| [DETTE-093](#dette-093--un-verdict-de-porte-mécanique-peut-être-vert-sans-que-toutes-les-étapes-aient-tourné) | conception | majeur | [`.claude/commands/revue-us.md`](../.claude/commands/revue-us.md) (étape 0, point 5) ; **résorbée pour l'essentiel en `E00US031`** par `backend/porte.py` et `backend/tests/test_porte_couvre_la_ci.py` | L'agent de porte a rendu **PORTE VERTE** alors que `pip-audit` n'avait produit **aucun `EXIT`** et ne fait pas partie des deux omissions autorisées par sa propre définition — qu'il a de plus justifiée par des « permissions refusées » alors que l'outil est installé. La règle existe, écrite : « toute étape du périmètre qui n'a produit aucun `EXIT` interdit le verdict vert ». Rien ne la **vérifie** | **C'est un faux négatif de sécurité** : `pip-audit` est le contrôle de vulnérabilités de la règle 11 (c). Une porte verte sans lui fait passer un lot **sans audit**, et l'orchestrateur enchaîne sur la revue en croyant le diff prouvé. Un faux négatif de porte est invisible et durable — c'est le mode de panne de `DETTE-091`, au niveau de l'exécution au lieu de la liste | E16US010 (29/08/2026), **relevé en 2ᵉ passe de revue** ; consigné aux métriques, mais une ligne de mesure n'est pas un registre : personne ne la `grep` | US d'outillage. **Critère de fin, vérifiable** : le rapport de porte contient un `EXIT=` par étape `run:` de `ci.yml` hors les deux omissions énumérées, et l'orchestrateur **refuse** un verdict vert dont la table en compte moins. ⚠️ Ne pas se contenter de réécrire la définition d'agent : la règle y est déjà, c'est le **contrôle** qui manque |
| [DETTE-094](#dette-094--le-garde-fou-du-reset-bouton-repose-sur-une-liste-tenue-à-la-main) | technique | mineur | [`frontend/src/app/reset-bouton.test.ts`](../frontend/src/app/reset-bouton.test.ts) (`CLASSES_TEXTE_SUR_BOUTON`) | Le garde-fou qui empêche une classe-texte portée par un `<button>` d'hériter du chrome de marque énumère ses classes **à la main**. En ajouter une sans la déclarer ne fait rien tomber | Le fichier l'admet en commentaire, ce qui le rend honnête mais pas efficace : rien ne force un futur auteur à le lire, et `App.css` annonce à côté « Gardé par `reset-bouton.test.ts` », ce qui se lit comme une couverture complète. Même mode de panne que `DETTE-091` — une liste tenue à la main **éteint la détection** sur ce qu'elle omet | E16US010 (29/08/2026), 3ᵉ passe de revue : le garde-fou est né en 2ᵉ passe et trois axes ont relevé le trou | US d'outillage. **Critère de fin, vérifiable** : la liste est **dérivée** du JSX (balayage des `.tsx` de `frontend/src` pour les `className` littéraux portés par un `<button>`) au lieu d'être énumérée, et ajouter une classe non désarmée fait rougir **sans éditer le test**. ⚠️ `charte.test.ts` porte déjà un découpeur de règles CSS (ADR-0074) : le bon geste est de s'y greffer, pas d'entretenir deux lecteurs d'`App.css` |
| [DETTE-095](#dette-095--un-export-absent-de-la-table-de-lécran-disparaît-en-silence) | conception | mineur | [`frontend/src/features/exports/Exports.tsx`](../frontend/src/features/exports/Exports.tsx) (`documents`) · [`backend/bootstrap/composition.py`](../backend/bootstrap/composition.py) (`catalogue_exports`) | L'écran itère sur **sa** table de documents et cherche l'entrée correspondante au catalogue. Une entrée servie par le serveur dont l'`identifiant` n'y figure pas — ajoutée, ou simplement mal orthographiée d'un côté — n'est **pas rendue, et rien ne le dit** | L'organisateur ne voit jamais un document que le serveur lui propose, et aucun test ne rougit : les deux listes sont dans deux langages et personne ne les compare. C'est le mode de panne de `DETTE-091` et `DETTE-094` (une liste tenue à la main éteint la détection), transposé à une frontière HTTP. ⚠️ Il est **assumé** qu'ajouter un export touche l'écran (ADR-0101 §1) : la dette n'est pas là, elle est dans le **silence** | E16US007 (30/08/2026), inscrite à l'écriture — le catalogue et la table de l'écran sont nés dans le même diff · **élargie par E16US016** : le catalogue passe de **trois** documents à **cinq** (`palmares`, `audit`). Le mécanisme est inchangé, la surface d'exposition croît — et l'US a livré deux identifiants de plus **sans** que rien ne les rapproche de la table de l'écran. ⚠️ Un garde-fou **partiel** existe désormais côté backend (`test_l_openapi_declare_exactement_les_formats_du_catalogue` force tout document neuf à déclarer son chemin) : il attrape l'oubli de **route**, jamais l'oubli d'**entrée d'écran**, qui est l'objet de cette dette | **Critère de fin, vérifiable** : un `identifiant` servi au catalogue et inconnu de l'écran fait **rougir un test** (ou s'affiche en clair). Piste la moins chère : un test d'API qui épingle les identifiants du catalogue **et** un test front qui épingle ceux de la table, tous deux sur la même liste littérale versionnée — patron déjà employé entre `TYPES_DEROULES` et `TYPES_ARRETABLES` (E05US035), à ceci près qu'il faut ici franchir deux langages |
| [DETTE-090](#dette-090--aucun-écran-nannule-un-forfait-de-duel) | conception | majeur | `frontend/src/features/forfaits/api.ts` (`annulerForfaitDuel`, marqueur `DETTE-090`), `frontend/src/features/forfaits/hooks.ts` (`useAnnulerForfaitDuel`, marqueur `DETTE-090`, **sans appelant**), `frontend/src/features/feu-vert/FeuVert.tsx` (marqueur `DETTE-090` dans le dialogue), `frontend/src/features/saisie-duels/SaisieDuels.tsx` (`ForfaitDuel` — marqueur `DETTE-090` ; le bloc y annonçait la même réversibilité inexistante, sur l'écran du **scoreur** cette fois) · `frontend/src/features/forfaits/hooks.ts` (`useAnnulerForfaitQualif`, paramètre `portee` sans appelant admin) · `frontend/src/features/archers/FicheArcherPilotage.tsx` | `POST /forfaits/duel/annulation` est ouvert à l'admin **et** au scoreur (`D-15`, E16US008) et couvert par un test d'API — mais **aucun écran du produit ne l'appelle** : `useAnnulerForfaitDuel` n'a aucun appelant dans `frontend/src`, et ni le feu vert ni `SaisieDuels` n'exposent d'annulation de duel. Le seul moyen de défaire un forfait de duel est un POST manuel ⚠️ **Élargie le 30/08/2026 (E16US007, relevé en revue — axe D)** : le même défaut existe désormais en **qualification**. L'organisateur peut y déclarer un abandon depuis la fiche d'archer, mais **aucun écran ne le lui fait annuler** — seul le panneau de l'espace *scoreur* le fait. `D-15` (« qui peut déclarer doit pouvoir défaire ») n'est donc pas tenu pour lui, exactement comme sur les duels | **Un walkover est destructif et se pose sur l'écran du jour J** : il sort un archer du tableau. Le motif même de l'élargissement (`D-15` : « une faute de frappe resterait irréparable sans aller chercher un scoreur ») n'est donc pas tenu côté surface. ⚠️ Le piège est déjà passé une fois : le dialogue de confirmation **annonçait** la réversibilité « depuis l'espace scoreur ou ici même », deux chemins également inexistants — trois axes de revue l'ont relevé, dont deux en bloquant. ⚠️ Et la même promesse **survivait ailleurs** : `SaisieDuels.ForfaitDuel` annonçait « réversible depuis le panneau de qualification ou par re-régénération », deux chemins faux eux aussi (`annuler_en_qualification` résout la QUALIFICATION ; `regenerer` ne touche aucun forfait) — trouvé en 3ᵉ passe, sur un site que le `grep DETTE-090` ne voyait pas | **E16US008** (28/08/2026) — écart constaté en revue. Livrer l'écran d'annulation dans l'US demandait une surface neuve (où l'afficher ? le duel amont **disparaît** de la liste dès qu'il est tranché par le walkover), donc un choix de périmètre, pas une correction de revue | **Une US à part entière**, parce que la question est d'abord d'UX : où vit la liste des forfaits de duel déclarés, et depuis quel écran on les défait. **Critère de fin** : `useAnnulerForfaitDuel` a un appelant, `annulerForfaitDuel` retrouve son paramètre `portee`, et le `detail` du dialogue de `FeuVert` peut à nouveau promettre la réversibilité — sans mentir |
| [DETTE-087](#dette-087--laccent-secondaire-est-réglable-mais-ne-peint-presque-rien) | conception | mineur | `frontend/src/index.css` (`--brand-2-surface`, `--brand-2-border`, `--brand-2-text`, `--sur-brand-2`) ↔ `frontend/src/features/identite/jetons.ts` (marqueur `// DETTE-087`) **et** `frontend/src/features/identite/Identite.tsx` (marqueur `DETTE-087`, ligne d'aide sous « Couleur secondaire ») | L'identité d'un tournoi porte **deux** accents (`DV-06` : « logo + 2 accents »). Le second est saisi, validé, persisté, dérivé sur les deux thèmes et **émis en jetons** — mais **aucune feuille de style ne consomme ces quatre jetons** en dehors des vignettes d'aperçu de l'écran de préparation. L'organisateur règle donc une couleur dont l'effet réel est nul sur l'écran de salle et sur l'appli publique | **Un réglage sans effet est pire qu'un réglage absent** : il se règle, il s'affiche à l'aperçu, et rien ne bouge là où l'organisateur va regarder. Le coût est de **crédibilité**, pas de correction — rien n'est cassé, rien n'est faux, et le CA d'`E01US016` (« deux couleurs d'accent ») est tenu au sens strict. ⚠️ Le risque de dérive est réel : une US future peut consommer `--brand-2-*` **sans savoir** qu'aucune planche n'en décrit l'emploi, et poser une convention par accident | **E16US006** (25/08/2026) — arbitrage assumé au cadrage, inscrit à [ADR-0097](adr/0097-un-logo-de-tournoi-vit-en-base-avec-lui.md) § Conséquences. Décider **où** l'accent secondaire s'applique est un choix de mise en page qu'aucune des 36 planches ne porte : l'inventer aurait été du design fait en douce dans une US d'implémentation | **Une question au commanditaire, pas une US de code** : que doit peindre la couleur secondaire ? Trois pistes qui n'engagent à rien — le **bandeau** de l'écran de salle, les **pastilles de départ** (« Départ 2 »), ou le **fond** des en-têtes de tableau public. **Critère de fin** : au moins une règle de `salle.css` ou d'une feuille publique consomme `var(--brand-2-…)`, et le marqueur disparaît. ⚠️ **Ne rien décider est une réponse valide** — dans ce cas il faut *retirer* les quatre jetons plutôt que les laisser dormir |
| [DETTE-086](#dette-086--des-mises-en-page-nont-jamais-été-affichées) | technique | **majeur** | `frontend/src/app/App.css` — section « Plan de cibles et plan de duels » · `frontend/src/features/identite/identite.css` — marqueur `/* DETTE-086 */` en tête de chacun · **`frontend/src/features/competition/TableClassement.tsx`** (`LIGNES_PROJETEES_MAX`, marqueur `// DETTE-086`) depuis E16US009 · `.formulaire__actions` (E16US007) · **section « Blocs par départ, en tête de l'accueil »** (E16US021, marqueur `/* DETTE-086 */`) | **Élargie le 12/09/2026 (E16US021)** : 65 lignes de mise en page neuve — une grille `repeat(auto-fit, minmax(220px, 1fr))` et un chiffre à 20 px — posées en **tête de l'écran d'atterrissage** de l'axe pilotage, sans qu'aucune capture soit possible. Risque plus faible que les cas d'origine (`auto-fit` se replie seul, et le CA « tous ensemble » reste tenu même replié sur deux rangs) : le seul chiffre non mesuré est le `220px`. ⚠️ **Un défaut de contraste a néanmoins été trouvé par recalcul, pas à l'œil** — `opacity: .72` sur un bloc clos composait `--text-muted` à ≈ 2,95:1, sous AA, précisément sur le mot que `DV-03` charge de porter le sens ; corrigé dans l'US. C'est la 4ᵉ confirmation que cette ligne prédit juste. **Élargie le 27/08 par E16US009 — et cette fois la prédiction de la ligne s'est vérifiée en trois passes.** L'écran de salle projeté rejoint la liste : `LIGNES_PROJETEES_MAX = 9` est un **plafond de hauteur calculé depuis le CSS, jamais mesuré**, et les trois passes de revue ont produit **trois décomptes de chrome successifs** (144 px, 232 px, 273 px), chacun corrigeant le précédent, sur un écran que personne n'a branché. ⚠️ **Asymétrie assumée** : la vue jumelle `VueAffectations` n'a **aucun** plafond — elle déborde vers 55-66 noms réglés selon la résolution —, si bien que l'organisateur doit regarder **les deux** vues selon le régime (sous 27 noms le classement commande, au-delà il est figé et c'est la liste de noms). **Élargie le 25/08 par E16US006** (trois surfaces neuves, même cécité, risque moindre — un logo mal dimensionné pousse un bandeau, il n'efface aucun CA). **Toutes les largeurs de ces écrans sont calculées, aucune n'est mesurée.** L'US qui l'a refondu (E16US005) n'a jamais pu l'afficher : le contrôle visuel est impossible sur le poste de développement (extension navigateur non connectée) et un banc d'essai statique n'a pas pu être servi non plus. Les seuils (`78rem`, `62rem`), les pistes fixes (`7rem`, `13rem`) et la décision de tronquer la ligne du club reposent sur une arithmétique refaite à la main depuis le CSS, recoupée par quatre relecteurs — **jamais sur une capture** ⚠️ **Élargie le 30/08/2026 (E16US007)** : une section d'export porte désormais **un bouton par format**, aux libellés longs, dans un conteneur flex ; `flex-wrap: wrap` a été posé **sans avoir vu l'écran** — la largeur reste calculée, jamais mesurée | **Trois passes de revue, trois bloquants de mise en page**, chacun invisible aux 4 800 tests du dépôt et chacun trouvé par recalcul : un jumeau non aligné, un seuil mesuré sur le viewport au lieu de la colonne de contenu, une troncature effaçant la moitié du CA. Le 4ᵉ défaut du même type est probable, et aucun garde-fou du dépôt ne peut le voir : jsdom ne calcule aucune mise en page. Il subsiste par ailleurs une bande de largeurs — **[1249, 1377] px** — où l'écran offre moins de texte par couloir qu'avant l'US, `1366×768` étant en plein milieu | **E16US005** (24/08/2026) — inscrite par elle, sur le constat de son propre axe adversarial en 3ᵉ passe : *« ce livrable ne peut pas être validé par une revue supplémentaire — la prochaine passe trouvera un quatrième défaut du même type »* | **Un œil humain devant l'écran**, pas une US de code. **Critère de fin** : trois captures (1250, 1366, 1600 px) annexées à [`docs/fonctionnel/E16US005.md`](fonctionnel/E16US005.md), et l'arbitrage du seuil tranché sur ce qu'elles montrent. ⚠️ **Tant que ce n'est pas fait, toute US qui rouvre cet écran hérite du même angle mort** — et le remède structurel, lui, est un fait de poste : rétablir un moyen de rendre l'application (extension navigateur, ou un banc d'essai servi en HTTP) |
| [DETTE-085](#dette-085--les-deux-plans-de-cibles-sont-des-jumeaux-recopiés-composant-par-composant) | conception | **majeur** | `frontend/src/features/placement/Placement.tsx` et `frontend/src/features/duels/Duels.tsx` — un marqueur `// DETTE-085` en tête du bloc jumelé, dans chacun des deux fichiers | **Quatre composants recopiés d'un écran à l'autre** : `Cible`, `Case`, `JetonArcher`, `Reserve`, plus le type `Jeton`, la fabrique de jeton et le calcul `couloirs`. Le fond métier, lui, est mutualisé dans `placement/presentation.ts` (cinq symboles importés par `duels`) ; c'est le **rendu** qui reste double. ⚠️ Les copies ont **déjà divergé** — la lettre du couloir sur une case occupée, les libellés de réserve (« archers » / « duellistes ») — donc « à l'identique » est faux, et un rangement dimensionné là-dessus effacerait des écarts voulus | **Le coût n'est pas théorique : il a été encaissé.** La première rédaction d'E16US005 a appliqué son changement de mise en page au seul plan de qualification ; `Duels.tsx` calculait les repères, montait trois requêtes pour eux et ne les rendait jamais, tout en perdant la typographie de ses jetons (déplacée vers un `<span>` qu'il n'émettait pas). Ni `tsc`, ni le lint, ni la suite ne pouvaient le voir — une propriété *fournie* et jamais *consommée* n'est pas une erreur de type, et aucun test ne montait cet écran | **E16US005** (24/08/2026) — ⚠️ **elle ne crée pas la duplication** (E03US009 a cloné l'écran) mais elle est la première à la **payer**, et elle l'a payée **au prix fort** : la divergence décrite ici s'est produite dans le commit qui l'inscrit, et c'est la revue — trois axes — qui l'a vue, pas le compilateur. C'est ce qui fait passer la sévérité de *mineur* à **majeur** : le mécanisme n'est plus un risque, c'est un défaut constaté | Remonter les quatre composants dans `shared/plan-de-cibles/`, badges passés en `ReactNode` par l'appelant, en **US de rangement dédiée** (règle 16) — à traiter dans le même mouvement que `DETTE-083`, même geste et même destination. Le rangement devra **arbitrer** les divergences déjà installées plutôt que les aplatir. **Garde-fou posé par l'US qui l'inscrit** : un test de rendu par écran (`Placement.test.tsx`, `Duels.test.tsx`) couvrant les repères, la lettre de couloir, la typographie du nom, l'infobulle et la grille dérivée. Il rend rouge la divergence **sur ces points-là** — pas une divergence de style ou de badge, et le dire est le prix de la crédibilité de cette ligne. **Critère de fin** : `Duels.tsx` ne déclare plus aucun composant de grille **et** n'importe plus de `features/placement/` **que `presentation.ts`**. ⚠️ *La table est le chemin de lecture prescrit (§ Économie de contexte) : ce critère avait été corrigé dans la section Détail et laissé faux ici — le piège restait armé là où on le lit.* |
| [DETTE-084](#dette-084--lécran-prêt-à-terminer--redéduit-la-garde-de-statut-et-recopie-sa-phrase) | conception | mineur | `frontend/src/features/completude/Completude.tsx` (`const enCours`, qui pilote `questionPosee`, `pret`, `detail` et le bloc d'actions ; plus le littéral du refus — marqueurs `// DETTE-084`) ↔ `backend/domain/tournoi.py` (`MESSAGE_TERMINER_HORS_EN_COURS`, la phrase recopiée) et `backend/domain/jalon.py` (`evaluer_terminer`, la garde recopiée) | L'écran « Prêt à terminer ? » lit `/completude`, qui ne porte pas le statut du tournoi : il **redéduit** donc la garde de statut de `terminer` et **recopie** en TypeScript la phrase de refus `MESSAGE_TERMINER_HORS_EN_COURS`, que le front ne peut pas importer. C'est la **seule** garde encore redéduite côté front après E16US012 | Un reformulage de la phrase côté Python laisse l'écran afficher l'ancienne, avec toutes les suites vertes — les deux littéraux vivent dans deux runtimes qu'aucun test ne confronte. Idem si `terminer` acceptait un jour un autre statut. ✅ **Le volet fraîcheur est résorbé** (5ᵉ passe) : `useTournois()` est pollé à 5 s **sous la coquille admin**. Il ne l'était pas, et la direction grave n'avait pas été décrite — non pas « la tablette propose une action refusée » (auto-correcteur, le 409 détrompe), mais l'inverse : le PC **reprend** le tournoi, la tablette croit encore à la pause et **fait disparaître le bouton « Terminer »**, sans 409 ni message. L'appli empêchait alors sans avertir — l'inverse de `D-15`/`P-3`. Reste la **copie** de la garde et de sa phrase, qui est la dette proprement dite | **E16US012** (23/08/2026), arbitrage assumé à [ADR-0096](adr/0096-un-jalon-enumere-ses-gardes-au-lieu-de-les-lever.md) § Conséquences : brancher l'écran sur `/jalons/terminer` ajouterait un **second poll de 5 s par tablette** pour la même liste. ⚠️ Inscrite en **3ᵉ passe de revue** — l'arbitrage ne vivait qu'en section *Conséquences*, ce que `DETTE-082` désigne elle-même comme insuffisant | Migrer l'écran sur `/jalons/terminer`. ⚠️ **Une voie existe déjà, sans second poll** : `hors_sportif` n'est lu **dans cet écran** que par `messageConfirmationTerminer`, donc uniquement dans le dialogue de confirmation. L'écran peut poller le jalon (statut, lignes, `detail` — la copie et la péremption disparaissent ensemble) et ne lire `/completude` qu'à l'ouverture de la confirmation (`queryClient.fetchQuery`, hors poll). Coût honnête : `BoutonConfirme.detail` reçoit aujourd'hui une chaîne calculée au rendu, il faudrait qu'il accepte un contenu résolu à l'ouverture — une US courte. « Attendre que le coût du second poll baisse » n'était pas une résorption : ce coût ne baissera pas tout seul |
| [DETTE-082](#dette-082--deux-endroits-pour-démarrer-le-même-tournoi) | conception | mineur | `frontend/src/features/tournois/Tournois.tsx` (monte `FriseCycleDeVie` **sans** `jalons` — marqueur sur place) ↔ `frontend/src/features/jalons/PretADemarrer.tsx`, `frontend/src/features/completude/Completude.tsx` et `frontend/src/features/accueil/FriseCycleDeVie.tsx` — les quatre portent le marqueur `DETTE-082`, `grep` ramène donc le fichier à modifier | ✅ **Resserrée par `E16US021` (12/09/2026), pas close.** Sur l'**accueil**, `demarrer` et `terminer` sont désormais un **renvoi** vers leur écran « prêt à… » : le doublon y a disparu. Il subsiste sur la destination « Tournoi », qui monte la même frise sans le renvoi, et **pour `archiver`**, dont le jalon n'est pas instruit (`404 jalon_non_instruit`) — l'y renvoyer mènerait à un écran vide | Restreint à un écran de moindre passage : l'organisateur qui démarre depuis la liste des tournois ne voit toujours pas ce qui manque et retombe sur le refus au clic. Aucun cas cassé, un chemin non amélioré — et le chemin le plus fréquenté (l'accueil, destination d'ouverture de l'axe pilotage) l'est désormais | **E16US012** (23/08/2026), qui l'assume à [ADR-0096](adr/0096-un-jalon-enumere-ses-gardes-au-lieu-de-les-lever.md) § Conséquences. ⚠️ Inscrite ici **en revue** : l'ADR la décrivait déjà, mais un angle mort qui ne vit qu'en section *Conséquences* n'apparaît à aucun tri de dette | **Quand `archiver` rejoindra la famille** : ajouter son entrée à `jalonsDeLaFrise` (`CoquilleAdmin.tsx`) et passer `jalons` au montage de `Tournois.tsx`. ✅ **Le compilateur garde le geste** : une entrée neuve doit nommer sa destination, que `tsc` valide contre `DestinationAdminId` — la 1ʳᵉ rédaction disait « il ne manque que le membre », ce qui était faux et **dangereux** (la frise castait `t.nom`, la coquille repliait tout le reste sur `completude` : ajouter `archiver` aurait ouvert « Prêt à terminer ? » sans une erreur). ⚠️ Ne pas l'ajouter **avant** que son jalon réponde autre chose que `404`. Un test verrouille « sans `jalons`, boutons nus », pour que la fermeture du site restant soit un geste explicite |
| [DETTE-083](#dette-083--les-features-jalons-et-completude-simportent-mutuellement) | conception | mineur | `frontend/src/features/jalons/PretA.tsx` (importe `SectionCompletude` et le type `LigneCompletude`) ↔ `frontend/src/features/completude/Completude.tsx` (importe `PretA`) — marqueurs `# DETTE-083` | **Cycle d'imports entre deux features.** La coquille commune de la famille « prêt à… » dépend du membre qu'elle est censée généraliser ; `archiver` et `exporter` hériteront de cette dépendance vers `completude` sans aucun rapport métier. L'atlas le signale (règle 10 : aucune des deux ne peut plus être lue, testée ni retirée seule) | Aucun effet à l'exécution (les cycles de composants React se résolvent au rendu). Le coût est de lecture et de découpage : le jour où l'on voudra retirer ou déplacer `completude`, la famille entière suivra | **E16US012** (23/08/2026) — ⚠️ **elle le crée, elle ne le rejoint pas.** `main` compte **trois** clusters (`admin+tournois`, `archers`+23, `bareme+grain-validation`) ; cette US en ajoute un **quatrième** — `accueil, completude, jalons, paiements` — en y entraînant `accueil` et `paiements`, qui n'étaient enchevêtrés avec rien. *(La 1ʳᵉ rédaction disait « rejoint un mode préexistant » : faux, et faux dans le sens qui minorait la dette. Corrigé en 2ᵉ passe de revue, axe C2, contre l'atlas régénéré par le commit lui-même.)* | Remonter `SectionCompletude`, le type `LigneCompletude` et les fonctions `afficheEtat` / `detailLigne` dans `shared/` — précédent exact : `etatRencontre`, remontée en `shared/` à l'occasion de la dernière tranche des formats *(l'US n'est pas nommée : l'atlas extrait tout identifiant de cette cellule comme US résorbante — cf. `DETTE-085`)*. **US de rangement dédiée** (règle 16). ⚠️ **Le remède doit casser la composante, et le premier remède écrit ici visait la mauvaise arête** (corrigé en 3ᵉ passe, axe C2). Les arêtes préexistantes `accueil → completude`, `accueil → paiements` et `paiements → completude` forment un **DAG** : elles ne bouclent pas. Ce qui ferme la composante, ce sont les deux arêtes de **retour** créées par l'US : `completude → jalons` (`Completude.tsx` importe `PretA`) et `jalons → accueil` (`PretADemarrer.tsx` importe `useTransitions`). Le seul **cut unique** qui la dissout est donc de remonter la coquille `PretA` elle-même dans `shared/` — avec ce qu'elle traîne (`SectionCompletude`, le type `LigneCompletude`, `afficheEtat`/`detailLigne`). Critère de fin : plus aucun signal `features-enchevetrees` **citant** `accueil`, `completude`, `jalons` ou `paiements` — et non « porté par le sujet `accueil` », l'atlas ne nommant qu'un sujet par composante : une composante réduite à `{completude, jalons}` changerait de sujet et ferait croire à la résorption |
| [DETTE-081](#dette-081--editer-un-format-efface-le-seuil-de-barrage-quil-portait) | conception | **majeur** | `backend/api/v1/formats.py` (`EtapeDTO`, marqueur `# DETTE-081`) ↔ `frontend/src/features/patrimoine/api.ts` (`Etape`) | `ModelePhase` porte `barrage_jusqu_au` et le repository le **persiste** fidèlement, mais l'`EtapeDTO` du routeur des formats **ne déclare pas ce champ** — ni en entrée ni en sortie. Or `PUT /formats/{id}` est une édition **totale** : le client renvoie la liste complète des étapes, sans un champ qu'il n'a jamais reçu | Promouvoir un tournoi dont la qualification porte un barrage à 8 range bien le seuil dans le format ; **la première édition de ce format l'efface**, en 200 et sans un mot, et le format rejoué l'année suivante ne l'a plus. C'est le défaut que ce fichier même raconte **trois fois** en commentaire (`barrage_jusqu_au`, E01US024) — pendant que le champ qui lui a donné son nom est resté hors du DTO | **Antérieure à E16US002** ; découverte par l'axe **adversarial** de sa 2ᵉ passe de revue, qui l'a reproduite par exécution (`de_modele` → `vers_modele` : `8 → None`). Non introduite ni aggravée par cette US, qui ne touche pas ce champ — inscrite par elle parce qu'une dette découverte ne se repose pas. ⚠️ **Le réglage lui-même est un arbitrage antérieur assumé** (`docs/fonctionnel/E06US003.md` : « le seuil se règle sur la phase du tournoi, pas dans le modèle de format ») ; ce qui n'a **jamais** été décidé, c'est l'**effacement d'une valeur déjà promue** | US dédiée : `barrage_jusqu_au` sur `EtapeDTO` **et** sur `Etape` (front), plus un test d'aller-retour HTTP calqué sur `test_patrimoine_api.py::test_le_titre_dune_etape_survit_a_l_aller_retour_de_bibliotheque`, écrit pour exactement ce risque par l'US qui a introduit le titre de phase *(US non nommée ici : l'atlas extrait tout identifiant de cette cellule comme celui de l'US **résorbante** — cf. `DETTE-085`)*. ⚠️ **Vérifier au passage les autres champs de `ModelePhase` absents du DTO** : le mode de panne est structurel, pas propre au barrage |
...
| [DETTE-068](#dette-068--la-grille-de-revue-et-son-orchestration-vivent-dans-sept-fichiers-sans-garde-fou-de-cohérence) | conception | mineur | `.claude/commands/revue-us.md` et `.claude/agents/revue-axe-{a,b,c1,c2,d}.md` + `porte-mecanique.md` — marqueur `DETTE-068` dans la commande, § « Concordance des numéros » | ADR-0013 décision 8 sort les grilles de la commande vers sept fichiers versionnés. La commande garde l'orchestration, le préambule et la table de décharge ; chaque agent porte sa grille. **Rien ne vérifie mécaniquement que les deux restent d'accord** — la numérotation des règles, la liste de suspension, les renvois croisés sont tenus à la main | **Réel, et déjà constaté trois fois au commit d'introduction** : `docs/adr/0075:220` et `docs/dette.md:7` pointaient encore vers l'ancien emplacement de `12-ADR` et des règles 14-15 ; et `revue-axe-a.md` omettait `porte-mecanique.md` de sa liste de suspension — le fichier même qui suspendait la décharge sur ce lot. Un renvoi mort se lit comme une preuve : c'est le défaut « ADR creux » que la revue existe pour trouver, appliqué à sa propre procédure | `chore/agents-dedies-revue` (16/08/2026), ADR-0013 décision 8 — assumé au titre de la règle 12 : un test qui compare deux textes en prose coûterait plus que le mal, puisque chaque agent reformule légitimement | **La prochaine divergence constatée ouvre la résorption** — pas « la troisième » : le compte de trois est atteint à la rédaction, et un déclencheur déjà franchi ne se déclenche jamais *plus tard* (leçon de DETTE-067, corrigée ici à l'inscription). Piste la moins chère, sur preuve du code d'aujourd'hui : élargir `_RACINES_DE_CODE` de `backend/atlas/sources/adr.py` à `.claude/` — le contrôle `portage-inexistant` vérifierait alors l'**existence** des fichiers nommés par les tables « Porté dans le code par », ce qu'il ne fait pas aujourd'hui (4 des 7 lignes d'ADR-0013 lui sont invisibles). Ne couvre pas le contenu ⚠️ **Élargie le 21/09/2026** (5ᵉ passe de revue de l'US d'ancrage par identité, axe C2) : le contrôle `portage-symbole-absent` cherche ses symboles par **sous-chaîne dans le texte entier** du module (`backend/atlas/sources/adr.py`, `_symboles_absents`), **commentaires compris**. Un symbole **supprimé** reste donc publié « présent » dès que la prose du module le mentionne — constaté sur `FormatTournoi.appliquer`, que la 4ᵉ passe a dû sortir *à la main* de deux entrées d'ADR pour que le contrôle cesse de mentir. Le garde-fou est aveuglé par le fichier qu'il contrôle, et la parade retenue est une **convention de rédaction non mécanisée**. Critère de fin : un symbole cité uniquement en commentaire fait rougir ; le dépôt sait déjà lire de l'AST (`test_domain_isolation.py`). ⚠️ Le coût réel n'est pas les ~30 lignes du contrôle, c'est la passe sur les signaux que le durcissement lèvera sur la trentaine d'ADR portés. |
| [DETTE-069](#dette-069--un-sous-agent-peut-écrire-dans-larbre-et-pousser) | technique | **majeur** | `.claude/settings.json` (`deny`), `.claude/agents/*.md` (champ `tools`) — marqueur `DETTE-069` dans `revue-us.md` § « Où vivent les grilles » et dans `porte-mecanique.md`. ⚠️ **Élargie en `E00US031`** : les vérifications de la porte sont désormais des **sous-processus** de `backend/porte.py`, donc hors de portée du `deny` — seul `test_porte_couvre_la_ci.py` les confronte encore à la liste des refus | Les six agents du dispositif de revue n'ont ni `Edit` ni `Write`, mais gardent **`Bash` non scopé**. Le `deny` versionné ne refuse ni `git commit`, ni `git push`, ni `sed -i`, ni une redirection. « Tu ne modifies aucun fichier » est donc une **consigne de prompt**, pas une contrainte. **Élargie le 17/08/2026** : `.claude/settings.local.json`, non versionné donc jamais relu en revue, ouvre son `allow` sur **`Bash(*)`** et `PowerShell(*)` — les 28 entrées restreintes de `settings.json` sont **court-circuitées**, seul le `deny` versionné survit (il l'emporte sur l'`allow`) | **Constaté, pas théorique.** Le 17/08/2026, l'agent `porte-mecanique` (un `haiku`), appelé pour exécuter la porte, a corrigé un ADR, régénéré l'atlas, lancé `ruff format`, puis `git add` / `commit` / `push` — **22 fichiers**, dont l'intégralité des correctifs de revue en cours, sous un message qui n'en décrivait que deux et dont le justificatif était inventé (`e8d3258`). Ses corrections étaient justes ; le geste ne l'était pas. Le dommage principal n'est pas le fichier touché mais la **traçabilité détruite** : `git add -A` ramasse le travail d'autrui, et le message devient faux sans que personne n'ait menti | [ADR-0013](adr/0013-conduite-de-la-revue-d-us.md) décision 8 (16/08/2026) — l'axe adversarial de sa propre revue avait décrit le scénario mot pour mot quelques heures avant qu'il ne se produise | **Essai joué et conclu le 17/08/2026 : la piste du `Bash` scopé est morte.** Un agent déclaré `tools: Read, Bash(git log:*)` a exécuté `git status` (hors scope mais dans l'`allow` — seul le frontmatter aurait pu la refuser), `git count-objects` (hors `allow` et hors `deny`), et surtout un `touch` en **écriture**, fichier vérifié sur le disque. Le spécificateur est accepté sans erreur et **ignoré en silence**, en lecture comme en écriture. Scoper les six agents ne donnerait qu'un garde-fou de **façade** — pire que rien, puisqu'on cesserait de surveiller ce qui ne l'est pas. Il ne reste que le pansement (`git status` après chaque sous-agent dans `/revue-us`) : **la dette reste ouverte, sans piste de durcissement mécanique**. Deux passes, dont une non concluante, et l'enseignement de méthode qui en sort (sonder le geste que le garde-fou est censé empêcher ; énumérer *toutes* les couches qui peuvent dire oui) sont en section. Agent d'essai supprimé. Arbitrage laissé ouvert, **par poste donc hors dépôt** : remettre ou non `Bash(*)` dans `settings.local.json` |
| [DETTE-071](#dette-071--lavancement-dun-créneau-sert-les-réglages-complets-à-un-anonyme) | conception | mineur | `backend/api/v1/phases.py` (`lister_avancement`, `PhaseReponse` **et**, depuis E05US035, `lister_phases`, `EtapeReponse`), consommée par `frontend/src/features/en-cours/VueEnCours.tsx` — appli publique **et** écran de salle, via `frontend/src/features/phases/hooks.ts` (`useAvancementPhases`), qui rend le type `Phase` **entier** — et par l'écran de pilotage | `GET /api/v1/departs/{depart_id}/phases` est une **lecture ouverte** (E10US001) qui rend `PhaseReponse` **entière** : `sources`, `effectif`, `profondeur`, `poules`, `suisse`, `big_shoot_off`, `barrage_jusqu_au`, et depuis **E05US035** `decoupage`. Un anonyme sur le réseau lit donc les **réglages d'atelier** d'un créneau — la liste de sortants d'un Big Shoot Off, le nombre de rondes d'un suisse, la taille visée des poules — là où l'écran public n'a besoin que de `id`, `ordre`, `type`, `statut` | **Faible, et ce n'est pas un secret** : ces réglages sont annoncés en salle et lisibles sur le papier du greffe ; rien ici n'est une donnée personnelle ni un jeton. Le coût est celui d'une **surface d'API plus large que son usage** : chaque champ ajouté à `PhaseReponse` part au public sans décision, et c'est le mécanisme qui a produit trois défauts déjà corrigés ailleurs dans ce registre. À surveiller le jour où une phase portera un réglage qu'on ne veut pas annoncer. ⚠️ **E05US035 y ajoute `decoupage` sur les DEUX routes ouvertes** — `PhaseReponse` (avancement d'un créneau) et `EtapeReponse` (déroulé d'un tournoi, `GET /tournois/{id}/phases`, sans garde admin), cette dernière recevant aussi **`nb_volees`** (le nombre de volées du barème). Exposition **assumée et décidée** pour les trois champs — le nombre de tours d'une qualification est déjà annoncé publiquement par l'avancement (« tour 1 sur 2 »), donc le champ ne révèle rien de neuf. C'est la **décision** qui manquait, et que le marqueur réclame. ⚠️ **E05US027 y ajoute `colline`** (`nb_manches`, `portee_de_defi`) sur les **deux** mêmes routes ouvertes. Exposition **assumée** : `GET /api/v1/colline/etat/` est déjà une route ouverte par décision de cette US et sert ces deux champs, et `VueCollinePublique` annonce le nom du format au public — le champ ne révèle donc rien de neuf. La ligne est élargie plutôt que le champ fermé, mais elle est élargie **explicitement**, ce qui est le geste que cette dette réclame : le manquer, c'est reprendre le mécanisme même qu'elle décrit (« chaque champ ajouté part au public **sans décision** ») | **Préexistant** — la route est publique depuis sa création (E01US025, ADR-0075). Inscrit ici le 18/08/2026 à la demande de l'utilisateur, après que la revue d'**E05US031** l'a relevé deux fois (axes C2 et adversarial) : l'US en fait un consommateur **public** de plus, donc un motif de moins pour le laisser en l'état. L'écart était honnêtement signalé en [ADR-0089 § Conséquences] et dans l'en-tête de `phases-publiques/api.ts`, mais **absent du registre** — donc introuvable par `grep` à qui cherche la dette, ce que le § Dette de `CLAUDE.md` compte comme dette silencieuse | Dériver un **DTO public étroit** (`PhaseAvancementReponse` : `id`, `ordre`, `type`, `statut`) servi par cette route, et réserver `PhaseReponse` aux lectures admin — le patron est déjà celui d'`EtatPubliqueReponse` (ADR-0089 §5) *(US du patron non nommée : elle serait lue comme l'US résorbante et noierait la vraie, citée juste après — cf. `DETTE-085`)*. **US de résorption : `E10US009`.** Ne pas simplement ajouter une garde admin : l'appli publique et l'écran de salle en dépendent, les fermer casserait un usage réel |

## Dette résorbée

| ID | Nature | Portée | Soldée par |
|---|---|---|---|
| [DETTE-078](#-dette-078--quatre-réglages-détape-sont-refusés-après-avoir-été-persistés-résorbée-le-20092026) | conception | `backend/domain/deroule_etape.py` (`EtapeDeroule.verifier_instanciable`, `_DEPART_A_BLANC`), `backend/application/phases.py` (`ServicePhases.ajouter`, `ServicePhases.modifier`), `backend/domain/format_tournoi.py` (`FormatTournoi.verifier_applicable`) | **E05US022** — une garde d'instanciabilité avant chaque écriture d'étape (ADR-0078) |
| [DETTE-026](#-dette-026--une-source-de-phase-est-ancrée-par-ordre-pas-par-identité-résorbée-le-20092026) | conception | `backend/application/phases.py` (`_remapper`, `_realigner_avancements`), `backend/application/bareme_qualification.py` (`_decaler_dun_cran`), `backend/domain/phase.py` (`SourcePhase.ordre_source`), `backend/domain/ports.py` (les deux `reordonner`) | **E05US022** — la séquence s'ancre sur l'identité de l'étape (ADR-0078) |
| [DETTE-045](#-dette-045--le-palmarès-et-la-simulation-ne-voyaient-que-le-premier-départ-résorbée-le-19092026) | conception | `backend/application/palmares.py` (`_premier_depart`), `backend/application/simulation.py`, `backend/application/simulation_format.py`, **+ E16US017** : `backend/domain/classement_clubs.py` (`classer_clubs` — le décompte **agrège** sur un seul créneau, et son résultat est un **trophée remis**, pas un rang affiché) | **E06US009** — un palmarès par créneau, juxtaposés |
| [DETTE-001](#-dette-001--suppression-de-tournoi-non-cascadée-résorbée-le-19092026) | technique | `backend/infrastructure/db/models.py`, `backend/infrastructure/db/repositories/referentiel.py` (`TournoiRepositorySQL.supprimer` / `compter_descendance`), `backend/application/tournois.py`, `backend/migrations/versions/` | **E01US026** (19/09/2026, [ADR-0077](adr/0077-supprimer-un-tournoi-signaler-puis-confirmer.md)) — **la plus ancienne du registre**, ouverte à E01US002 et aggravée à chaque table ajoutée pendant treize mois. La suppression **signale** sa descendance (409 `tournoi_peuple`, décompte chiffré), l'admin **confirme**, et la purge s'exécute en **une transaction**. **Aucun `ON DELETE CASCADE` n'a été posé** — c'était la moitié de la décision : une cascade en base armerait une purge silencieuse sur tout autre chemin. Le `xfail` d'E02US010 (`test_supprimer_un_termine`) retombe vert. ⚠️ **Les ~45 marqueurs par colonne ont été retirés au profit d'UN énoncé en tête de `models.py`** : ils disaient tous « politique non tranchée », ce qui est désormais **faux**, et les recopier corrigés aurait fait 25 fois le même avertissement (règle 13). Ceux des migrations sont **repointés vers ADR-0077** plutôt que supprimés — une migration décrit son époque, mais une phrase fausse qui y survit se lit comme une preuve. ⚠️ **Deux tables manquaient à la purge du *départ* elle-même** (`arret_de_circonstance`, `franchissement_arret`) : FK *enforced*, donc supprimer un créneau portant un arrêt partait déjà en 500 — défaut **latent et indépendant** de la suppression de tournoi, trouvé en écrivant la cascade et corrigé ici. ⚠️ **La veille que cette ligne assurait est désormais mécanisée**, et c'était la condition pour la fermer : pendant treize mois, l'inventaire n'a tenu que parce qu'une revue relisait cette ligne à chaque table ajoutée — et elle l'a manqué deux fois (E05US033, E05US034). Deux tests la remplacent : `test_aucune_table_neuve_n_echappe_a_l_inventaire` dérive l'inventaire de `Base.metadata`, et `test_aucune_fk_de_la_descendance_ne_cascade_en_base` tient la liste blanche des `ON DELETE CASCADE`. Le premier a fait son travail **dès sa naissance**, en attrapant une table que le décor de test avait oubliée. S'y ajoutent le test d'exhaustivité (une ligne par table, cardinalités toutes distinctes) et celui du tournoi **voisin**, patrimoine du club compris — un `WHERE` trop large efface en silence, là où une table oubliée fait un 500 qu'on voit. ⚠️ **Le décompte est volontairement plus étroit que la purge** : il ne retient que ce qui ne se ressaisit pas, et la liste des exclus est nominative sur `DescendanceTournoi` |
| [DETTE-018](#-dette-018--la-suppression-darcher-perd-les-remboursements-résorbée-le-19092026) | conception | `backend/application/archers.py` (`_remboursements_des_payees`), `backend/infrastructure/db/repositories/referentiel.py` (`ArcherRepositorySQL.supprimer_avec_remboursements`), `backend/domain/remboursement.py` (`MotifRemboursement.ARCHER_SUPPRIME`) | **E01US026** (19/09/2026) — résorbée **dans l'US de DETTE-001**, sur décision du commanditaire : les deux dettes touchent le même code de cascade, et les traiter séparément aurait fait relire deux fois la descendance de l'archer. Le **3ᵉ et dernier chemin** d'effacement d'une inscription payée ouvre désormais un poste au registre (motif `archer_supprime`), comme la désinscription et la suppression de départ — 3ᵉ réalisation de la couture d'ADR-0057. `ServiceArchers` reçoit deux ports de plus (`DepartRepository` pour le tarif, `Horloge` pour la date). ⚠️ **Le signalement `ArcherEngage` sur-compte toujours** : il s'appuie sur `paye` seul quand l'ouverture exige en plus un créneau **tarifé**, donc un créneau gratuit marqué payé est annoncé sans rien ouvrir. Tolérable et **dit dans le code** — le message annonce plus que ce qui partira, jamais moins. ⚠️ **DETTE-016 reste ouverte et non aggravée** : le montant est le **tarif courant**, pas la somme réellement encaissée, sur ce chemin comme sur les deux autres. ⚠️ **Le tournoi, lui, EFFACE ses remboursements au lieu d'en ouvrir** (arbitrage du 19/09/2026) — ce n'est pas une incohérence : `remboursement.tournoi_id` est sa seule FK, donc supprimer le tournoi emporte le **registre lui-même** et un poste ouvert y serait détruit dans la même transaction. Le critère qui sépare les deux gestes est « **le registre survit-il ?** » ; à défaut de contrepartie, la somme encaissée est **chiffrée** à la confirmation |
| [DETTE-039](#-dette-039--la-cadence-et-la-taille-dune-page-projetée-sont-en-dur-résorbée-le-26082026) | technique | `frontend/src/shared/ui/pagination.ts` (`NOMS_PAR_PAGE`, `SECONDES_PAR_PAGE`), `backend/domain/ecran.py` (`ReglagePages`), migration `0051` | **E16US009** (26/08/2026, [ADR-0098](adr/0098-un-ecran-projete-pagine-au-lieu-de-defiler.md)) — le « (réglable) » du questionnaire `p06` est livré : les deux valeurs sont **réglées par écran**, persistées sur `poste`, et servies avec l'affichage que l'écran répète (état lu, ADR-0064). Les deux marqueurs `# DETTE-039` sont retirés. ⚠️ **Les constantes restent — elles ont changé de rôle** : ce ne sont plus le réglage mais le **défaut**, volontairement identique à `ReglagePages.par_defaut()` côté serveur, pour qu'aucun écran déjà installé ne change de comportement au déploiement. Un **test de chaque côté** épingle sa propre valeur en littéral : `pagination.test.ts` (« les défauts du module et ceux du serveur ») et `test_domain_ecran.py`, chacun citant l'autre en commentaire. ⚠️ **C'est un garde-fou de lecture, pas une contrainte de compilation** — changer le défaut serveur *et* son propre test laisserait le test front vert. Le dire est le prix de la crédibilité de cette ligne : trois artefacts affirmaient « interdit de diverger », ce que le mécanisme ne tient pas (relevé en revue, axe C2). ⚠️ **La seconde moitié de la dette ne se referme PAS par du code, et c'est dit plutôt que barré** : « `NOMS_PAR_PAGE = 40` n'a jamais été mesuré sur le vidéoprojecteur réel » reste vrai le 26/08/2026 — l'US rend la valeur **corrigeable sans recompiler**, elle ne la mesure pas. Le geste devient de l'exploitation, et l'écran d'admin le dit sous les deux champs (« à ajuster en regardant l'écran depuis le fond de la salle »). C'est une résorption **du raccourci technique**, pas de l'incertitude qu'il abritait |
...
### ✅ DETTE-078 — quatre réglages d'étape sont refusés **après** avoir été persistés *(résorbée le 20/09/2026)*

**Où** : `backend/domain/deroule_etape.py` (`EtapeDeroule.verifier_instanciable`,
`_DEPART_A_BLANC`) ↔ `backend/domain/phase.py` (les gardes `ProfondeurInvalide`,
`ReglageDePoulesInvalide`, `ConfigurationBigShootOffInvalide`, `ConfigurationSuisseInvalide`) ↔
`backend/application/phases.py` (`ServicePhases.ajouter`, `ServicePhases.modifier`) ↔
`backend/domain/format_tournoi.py` (`FormatTournoi.verifier_applicable`).

⚠️ **Plus aucun jeton `DETTE-078` dans le code** : le geste de résorption est un
`grep DETTE-nnn` (§ Dette de `CLAUDE.md`), et une dette fermée qui y répond encore rend ce geste
faux dans l'autre sens. L'historique vit ici, les sites renvoient à `E05US022`.

⚠️ **Les quatre réglages, vérifiés dans le code du jour le 20/09/2026** : `poules`,
`big_shoot_off`, `suisse`, **`profondeur`**. Ce **n'est pas** `decoupage`, que la rédaction
d'origine citait : il est gardé sur l'étape, `EtapeDeroule.__post_init__` appelant
`verifier_decoupage_applicable` — comme `colline`. Les deux fermés sont donc `decoupage` et
`colline`, les quatre ouverts ceux ci-dessus.
...
⚠️ **`ServicePhases.modifier` est le TROISIÈME site, et le plus grave, relevé le 20/09/2026**
(3ᵉ passe de revue d'E05US022, axe C2 ; qualifié de bloquant en 4ᵉ par C1 et B). Les trois, dans
l'ordre de gravité croissante : (1) `ServicePhases.ajouter`, (2) `ServiceFormats.appliquer`,
(3) `ServicePhases.modifier`. Il fait `replace(etape, …)` puis `self._deroules.enregistrer(...)` :
il n'instancie **jamais**, donc les quatre gardes ne se lèvent pas du tout. Un `PUT` qui retype
une phase en gardant son réglage répond **200**, persiste — et **chaque lecture** ultérieure
passant par `instancier` tombe en 422 : le suivi, le pilotage et l'affichage public. C'est le
mode de panne que `deroule_etape.py` documente déjà mot pour mot pour les `arrets`. La docstring
du DTO (`api/v1/phases.py`) affirme d'ailleurs que le réglage « lève 422 » — vrai au `POST`, faux
au `PUT`. C'est ce site qui fait passer la dette de **mineur** à **majeur** : il casse un cas
utilisateur réel. Il n'a pas été introduit par E05US022, seulement trouvé par elle.
...
**Pourquoi seule la colline était fermée.** Le champ neuf (`colline`) n'avait aucune raison de
naître avec ce défaut : sa garde est portée par `EtapeDeroule.__post_init__`, donc antérieure à
toute écriture, et testée aux deux étages (`test_domain_reglage_colline.py`, `test_colline_api.py`).
Les quatre autres étaient **hérités**, et l'argument pour ne pas les corriger — cela toucherait
quatre invariants de composition et leurs tests dans une US de format — **valait pour le hissage**,
pas pour la garde d'appel qui les a finalement refermés (voir ci-dessous).

**⚠️ Le DEUXIÈME site, `ServiceFormats.appliquer`, était bien pire que le premier — refermé lui aussi** (20/09/2026, trouvé par l'axe
adversarial de la revue). `ServiceFormats.appliquer` **supprime** le déroulé en place avant de
poser le neuf : une garde qui ne se lève qu'à `instancier()` laissait donc le tournoi sans aucune
phase **ni barème de qualification** — pas un trou de numérotation, une destruction. Le remède est
la **pose à blanc** de `FormatTournoi.verifier_applicable`, qui instancie désormais chaque étape
sur un créneau fictif (`_DEPART_A_BLANC`) pour rejouer les gardes avant toute écriture.

Conséquence pour la résorption : ces gardes ont maintenant **deux** lecteurs (`Phase.__post_init__`
et la pose à blanc). Les hisser vers `EtapeDeroule.__post_init__` rendra la seconde moitié de la
pose à blanc redondante — c'est le signe que la résorption a réussi, pas un oubli à conserver —
et refermera du même geste `ServicePhases.ajouter` **et** `ServicePhases.modifier`.

**✅ Résorbée le 20/09/2026 (E05US022, 4ᵉ passe de revue — axes C1 et B, qui ont relevé que la
ligne du registre qualifiait elle-même le site `modifier` de « casse un cas utilisateur réel »,
donc un **bloquant** au sens du § Dette de `CLAUDE.md`, pas une dette.)**

Le remède n'a pas été celui qui était prévu ici, et c'est pour cela qu'il a tenu dans l'US : au
lieu de **hisser** les quatre gardes vers `EtapeDeroule.__post_init__` — geste mécanique mais
touchant quatre invariants de composition et leurs tests —, chaque site qui **écrit** appelle
`EtapeDeroule.verifier_instanciable()` avant d'écrire. Trois lignes, une machinerie déjà introduite
par la même US pour `FormatTournoi.verifier_applicable`. ⚠️ **Une méthode nommée, pas un
`instancier(...)` dont on jette le résultat** (5ᵉ passe de revue) : un résultat perdu se lit comme
du code mort, et un nettoyage l'aurait supprimé sans rien faire rougir. Test d'API
`test_un_reglage_pose_sur_le_mauvais_type_est_refuse_sans_rien_persister`, **paramétré sur les
quatre réglages** et sur les deux verbes (`POST`, `PUT`), vu rouge au sabotage des deux appels.

⚠️ **Ce qui reste, et pourquoi ce n'est plus une dette** : les quatre gardes vivent toujours sur
`Phase` et non sur `EtapeDeroule`, d'où **trois** appels de garde là où un invariant d'étape
suffirait. C'est une redondance, plus un préjudice : aucun état invalide n'atteint la base. Les
hisser reste souhaitable — ce jour-là, le corps de `verifier_instanciable` se vide et les trois
appels deviennent inertes —, mais rien ne l'exige.

⚠️ **Aucune reprise de données** : une base portant déjà un réglage fantôme persisté par un `PUT`
d'avant ce correctif reste illisible. Le cas n'a jamais été observé, et une migration corrective
demanderait de deviner lequel des deux — le type ou le réglage — l'organisateur voulait.

### DETTE-088 — le plafond de commentaires ne couvre pas tout le dépôt

**Où** : `backend/tests/test_commentaires_bornes.py` (`_COUVERTS`, `_EXCLUS`) ↔
`frontend/src/commentaires.test.ts` (en-tête).

Le plafond de huit lignes par bloc contigu (règle 13, [ADR-0099](adr/0099-le-code-porte-des-pointeurs-pas-le-raisonnement.md))
est mécanisé des deux côtés depuis E00US027, mais les deux moitiés n'ont pas le même périmètre :

| | couvert | hors porte |
|---|---|---|
| backend | les cinq couches, `atlas/`, `release/`, les points d'entrée **`.py`** — 255 fichiers | `tests/` (395 blocs), `migrations/` (56), `kervignarc.spec` (17 l.) |
| front | tout `frontend/src` en `.ts` / `.tsx` / `.css`, **tests compris** — 399 fichiers | `frontend/eslint.config.js` (14 l., frontière navigateur/Node), `atlas/statique/` (5 blocs) |

⚠️ **L'asymétrie est dans le mauvais sens pour un lecteur** : `ADR-0099` écrit que « les docstrings de
test gardent leur statut à part », ce qui se lit comme une dispense — or seul le backend l'applique.
La formulation a été corrigée dans `stories/E00-socle.md` en revue : le statut est *sémantique*
(un oracle est une contrainte non déductible), pas une dispense de **forme**. La couverture cible
est celle du front ; le gap est côté backend.

⚠️ **Deux limites du détecteur, assumées et écrites plutôt que découvertes.**

**(a) Une ligne vide coupe un bloc**, des deux côtés : 8 lignes, une blanche, 8 lignes ne déclenche
rien. Le plafond borne donc le **paragraphe**, pas la zone de commentaire — et ce n'est pas
marginal : mesuré en revue, si une ligne vide unique ne coupait pas, **65 fichiers backend** et
**52 fichiers front** dépasseraient, jusqu'à 23 lignes (`domain/identite.py`) et 17
(`features/jalons/PretA.tsx`). Le chiffre est écrit ici pour qu'on ne croie pas la règle plus large
qu'elle n'est.

**(b) Le détecteur front n'a pas de tokenizer** : un **gabarit multi-lignes** dont une ligne commence
par `//` serait compté à tort. ⚠️ Les **littéraux simples** l'étaient aussi jusqu'à la revue de la
2ᵉ passe — `const ouvre = '/*'` basculait le scan en mode bloc et faisait compter du **code** comme
du commentaire. Le cas vivait dans `commentaires.test.ts` lui-même et ne rougissait pas parce que le
faux bloc tenait sous le plafond ; il est neutralisé et figé par un cas de test.

Les deux limites sont figées par un cas de test de chaque côté, pour qu'un futur lecteur ne les
prenne pas pour un bug.

### DETTE-089 — le câblage du routage Big Shoot Off n'est ancré par aucun test

**Où** : `backend/bootstrap/composition.py`, marqueur `# DETTE-089`.

`ServiceRoutage` reçoit `service_big_shoot_off` en paramètre **optionnel** (E05US028). S'il manque,
`_routage_big_shoot_off` rend `INDISPONIBLE` sur les quatre canaux de routage — un archer de Big
Shoot Off lit alors « destination indisponible » au lieu de sa manche. Aucun test ne monte
`create_app` pour vérifier ce branchement : `test_service_big_shoot_off.py` construit son propre
`ServiceRoutage`.

⚠️ Jusqu'au 28/08/2026, le commentaire du fichier **affirmait le contraire** : « Ancré désormais par
`test_composition_routage_big_shoot_off` ». Ce test n'a jamais existé. E00US027 a réécrit cette
ligne sans vérifier la promesse qu'elle portait, et l'axe B de la revue l'a rattrapé — c'est
l'illustration exacte de la thèse d'ADR-0099, commise dans l'US qui l'énonce.

### DETTE-091 — la liste des ADR structurants d'ADR-0075 n'a jamais été auditée

**Où** : [`docs/adr/0075-le-depart-est-la-portee-sportive.md`](adr/0075-le-depart-est-la-portee-sportive.md),
§ « Portée de la règle », les deux listes (« seize retenus » et « ajoutés depuis »).

`CLAUDE.md` § *Décision structurante ⇒ ADR* exige une section « Porté dans le code par », **mais**
délègue à ADR-0075 la liste nominative des ADR concernés — et pose que, hors de cette liste,
« l'absence de section **n'est pas un défaut à relever en revue** ». La liste ne documente donc pas
la règle : elle la **borne**.

Or elle n'est pas tenue, de deux façons distinctes qu'il faut séparer :

1. **Des ADR neufs n'y sont pas inscrits** — `0087`, `0089`, `0090`, `0091`, relevés un par un, quatre
   fois sur quatre *après coup*. Ceux-là **portaient** leur section : seule la liste manquait.
2. **Le rétro-équipement du 08/08/2026 a manqué des ADR de moteur** — mode de panne différent, et
   plus grave, parce que ces ADR ne portent **aucune** section. `0050` (forfait : il commande le
   classement de qualification *et* la reconstruction du tableau) a été trouvé en revue d'E16US008 ;
   un balayage rapide en donne au moins cinq autres qui satisfont le critère — `0023`, `0027`,
   `0047`, `0048`, `0065`.

⚠️ **Pourquoi c'est une dette et pas un oubli** : la liste éteint la détection sur ce qu'elle omet.
Un relecteur qui constate qu'un ADR n'y figure pas est *tenu* de ne rien relever. Le défaut est donc
silencieux par construction — c'est le mode de panne d'ADR-0017 (une décision non portée pendant
treize mois), transposé de la couche code à la couche procédure.

**Résorption** : un balayage unique. **Critère de fin** : tout fichier de `docs/adr/` au statut
*Accepté* non remplacé figure dans **l'une** des deux listes (retenu / hors critère), et tout ADR
retenu porte sa section. ⚠️ Le geste n'est pas de recopier la liste mais de balayer `ls docs/adr/`
et de rapprocher — ADR-0075 le dit déjà, deux fois, sans qu'il ait été fait.

### DETTE-092 — la recherche transverse lit trois référentiels entiers à chaque frappe

**Constat.** `ServiceRecherche._archers_correspondants` appelle `ArcherRepository.tous()` — un
`SELECT` sans filtre sur **toutes les éditions** —, puis `clubs.lister()` et `tournois.lister()`,
puis normalise chaque libellé avec `cle_nom` (NFKD + `casefold`). `ServiceJalons.apercus` suit le
même profil : une boucle sur `tournois.lister()` avec **trois lectures** par tournoi encore
préparable (`demarrer_sans_objet` coupe court sur l'historique, ce qui en évite l'essentiel).

**Pourquoi c'est délibéré.** Le repli casse/accents n'est pas exprimable en `LIKE` SQLite :
« leveque » n'y trouverait jamais « Lévêque », qui est la saisie réelle sur tablette. Filtrer en
mémoire est le seul moyen d'avoir **une seule** règle de correspondance, partagée avec la détection
de doublons (cf. `DETTE-006`). La règle 12 (« l'infra reste simple : mono-club, local ») couvre la
décision — **il n'y a donc pas d'ADR à ouvrir**, seulement une trace à poser.

**Conséquence.** Rien ne casse : quelques milliers de lignes en SQLite local restent sous le seuil
perceptible, et l'anti-rebond de 250 ms (`useValeurRetardee`) ramène « leveque » de sept requêtes à
une ou deux. Ce qui manquait est le **repère** : le commentaire du port disait « un référentiel qui
grossirait démentirait ce pari » sans que ce pari soit inscrit là où on cherche les dettes. Un
`grep DETTE-` ne le trouvait pas — exactement le défaut que `DETTE-064` et `DETTE-071` nomment.

**Pourquoi non corrigée dans l'US.** La corriger, c'est déplacer le repli en base (colonne
normalisée persistée + index, ou FTS), donc une **migration** et un second lieu de vérité pour la
normalisation — un chantier qui n'a rien à faire dans une US qui livre quatre CA d'écran.

**Résorption.** US `refactor/` dédiée. **Critère de fin, vérifiable** : `tous()` a disparu du port
`ArcherRepository`, remplacé par un `chercher(fragment, limite)` dont l'adapter SQL fait le travail.
⚠️ Le jour où on le fera, **ne pas retomber sur un `LIKE` nu** : il ne replie pas les accents, et
c'est la raison même de cette dette. Marqueurs `DETTE-092` sur **les sept sites** — trois au backend (le port et les deux services) et **quatre** au front (l'anti-rebond, le `staleTime`, le composant qui les consomme, et le test qui les tient). ⚠️ Le registre sous-déclarait ses propres marqueurs : le `grep` de résorption en rendait plus que la fiche n'en annonçait.

### DETTE-093 — un verdict de porte mécanique peut être vert sans que toutes les étapes aient tourné

**Constat.** À la 1ʳᵉ passe de revue d'E16US010, l'agent `porte-mecanique` a rendu **PORTE VERTE**
en listant `pip-audit` parmi ses « non exécuté », au motif de « permissions refusées sur
`pip install` ». Deux erreurs cumulées : `pip-audit` **est installé** dans le venv du poste (seul
`pip install` est refusé), et surtout sa propre définition ferme l'énumération des omissions
autorisées à **deux** — l'installation des dépendances Python et le script de synchro
`requirements.txt`↔`pyproject.toml` — et dit en toutes lettres : « toute étape du périmètre qui n'a
produit aucun `EXIT` interdit le verdict vert […] le verdict est alors **`PORTE INCOMPLÈTE`** ».

⚠️ **Correction du 29/08/2026, 3ᵉ passe de revue — la 1ʳᵉ rédaction de cette fiche était fausse sur
son point central.** Elle affirmait que « la règle y est déjà, complète et sans ambiguïté » et
interdisait de toucher la définition d'agent. **C'était l'inverse** : la règle 3 autorisait « les
**trois** omissions volontaires » et le gabarit de rapport disait « (1-3 ci-dessus) », alors que
l'énumération n'en contient que **deux**. Un exécutant disposait donc d'un troisième créneau **sans
identité**, où `pip-audit` — dont l'étape `ci.yml` commence par un `pip install` — se rangeait sans
enfreindre la lettre. **Le faux vert n'était pas une désobéissance : il était autorisé par le
texte.** Le trou textuel est fermé dans le même commit (« trois » → « deux », « 1-3 » → « 1-2 »,
et une note disant que le `pip install pip-audit` de l'étape d'audit ne relève pas de l'omission 1).
Ce qui reste dû est le **contrôle mécanisé** — voir *Résorption*.

**Conséquence.** C'est un **faux négatif de sécurité**. `pip-audit` porte le volet « vulnérabilité
connue » de la règle 11 (c) ; une porte qui se déclare verte sans l'avoir exécuté laisse passer un
lot **sans audit de chaîne d'approvisionnement**, et l'orchestrateur enchaîne sur une revue de
jugement en croyant le diff mécaniquement prouvé. Le défaut est **silencieux et durable** : rien
dans le rapport ne signale qu'une étape manque, puisque c'est le rapport lui-même qui l'omet. C'est
exactement le mode de panne de `DETTE-091` (une liste tenue à la main, quatre omissions
consécutives), transposé de la *liste* à l'*exécution*.

⚠️ **Le trou textuel est fermé ; le contrôle, lui, reste dû.** Un agent qui n'applique pas sa règle
rend un rapport **auto-cohérent**, exactement comme le hook d'atlas « compare du périmé à du périmé »
(`CLAUDE.md` § Cycle de branche). La 1ʳᵉ passe n'a été rattrapée que parce que l'orchestrateur a
recoupé `ci.yml` à la main, ce que la procédure impose — mais ce recoupement porte sur la **liste
des étapes**, pas sur le **décompte des `EXIT`**.

**Pourquoi non corrigée dans l'US.** Mécaniser le décompte touche `.claude/commands/revue-us.md` et
`.claude/agents/porte-mecanique.md` — c'est-à-dire **la porte elle-même**, dont toute modification
suspend la décharge mécanique et impose de rejouer l'axe A. Ce chantier n'a rien à faire dans une US
qui livre une recherche et des pastilles ; il doit être conduit et relu pour lui-même.

**Résorption.** US d'outillage. **Critère de fin, vérifiable et EXÉCUTABLE** : un contrôle
(`backend/tests/test_porte_rapport.py`, ou `python -m outils.porte --verifier <rapport>`) parse les
`run:` de `.github/workflows/ci.yml`, compte les `EXIT=` du rapport de porte, et **sort non nul**
s'il en manque hors des deux omissions énumérées. **Preuve de fin** : retirer une ligne `EXIT=`
d'un rapport verbatim fait rougir ce contrôle.
⚠️ **Une phrase de plus dans `revue-us.md` ne coche PAS cette case** — c'est le geste qui a échoué :
la règle était écrite et n'a pas suffi. La 1ʳᵉ rédaction de ce critère se satisfaisait pourtant
d'« et l'orchestrateur refuse », c'est-à-dire de cette phrase même : corrigé en 3ᵉ passe, un critère
de fin qui ne distingue pas le remède du symptôme rend la dette non résorbable.
**Résorbée pour l'essentiel par `E00US031`** (ADR-0110). Le critère de fin — « un contrôle parse
les `run:` de `ci.yml`, compte les étapes réellement jouées, et sort non nul s'il en manque » — est
atteint, et par construction plutôt que par consigne : `backend/porte.py` ne sort `0` qu'après
avoir joué toutes les vérifications de `GROUPES`, son tableau affiche `n/m` **rapporté au total**
(jamais à la sélection) et **nomme** les manquantes, et `backend/tests/test_porte_couvre_la_ci.py`
confronte sa liste à `ci.yml` dans les deux sens.

⚠️ **Ce qui reste, et pourquoi la ligne n'est pas fermée** : l'agent de porte **transcrit** ce
tableau, et rien ne vérifie sa transcription — c'est le dernier maillon humain, non mécanisable.
D'où les trois contrôles à l'œil inscrits à l'étape 0 de `revue-us.md` (mode, compte attendu,
présence de `pytest` et `vitest`). Marqueur `DETTE-093` dans `revue-us.md` — il a disparu de
`porte-mecanique.md` avec la réécriture d'`E00US031`, ce fichier ne portant plus d'énumération
d'étapes où l'accrocher.

### DETTE-094 — le garde-fou du reset `button` repose sur une liste tenue à la main

**Constat.** `App.css` porte un reset global `button { background; color; font-weight;
letter-spacing; … }`. Toute classe qui rend du **texte** mais est portée par un `<button>` doit le
désarmer, sinon elle hérite de l'aplat de marque — c'est le bloquant de la 2ᵉ passe d'E16US010
(`.badge-preparation` repeinte en rouge club, 1,5:1 en thème clair). Le garde-fou créé pour
l'empêcher, `frontend/src/app/reset-bouton.test.ts`, vérifie les **quatre** propriétés du reset,
mais seulement sur les **quatre classes qu'il énumère à la main**.

**Conséquence.** Ce que le test ne couvre pas est invisible : ni `tsc`, ni le lint, ni jsdom ne
voient une couleur. Une classe-texte neuve sur `<button>` passe sans bruit, et `App.css` annonce à
côté « Gardé par `reset-bouton.test.ts` » — une phrase qui se lit comme une couverture complète.
C'est le mode de panne de `DETTE-091` : **une liste tenue à la main borne ce que la détection peut
voir**. Preuve que ce n'est pas théorique : la 3ᵉ passe de revue a trouvé, en durcissant les
assertions, **six** manquements réels sur les quatre classes déjà listées — dont trois `color`
absentes, soit du blanc sur blanc en thème clair (`--sur-brand` = `--surface-0` = `#ffffff`).

**Pourquoi non corrigée dans l'US.** Dériver la liste demande de lire le JSX (extraction des
`className` littéraux portés par un `<button>`), donc un second parseur — alors que
`frontend/src/shared/charte.test.ts` (ADR-0074, E17US001) en porte déjà un pour les règles CSS. Le
bon geste est de s'y **greffer**, ce qui touche un garde-fou de charte : à conduire et relire pour
lui-même, pas au milieu d'une US qui livre une recherche.

**Résorption.** US d'outillage. **Critère de fin, vérifiable** : la liste des classes est dérivée du
code et non écrite ; **preuve de fin** — ajouter à un `.tsx` un `<button className="x">` dont le
bloc `.x` d'`App.css` ne désarme pas le reset fait rougir la suite **sans éditer aucun test**.
Marqueur `DETTE-094` sur la constante.


### DETTE-095 — un export absent de la table de l'écran disparaît en silence

**Nature** : conception · **Sévérité** : mineur · **Introduite par** : E16US007 (30/08/2026)

E16US007 sert un **catalogue d'exports** (`GET /api/v1/exports`) : quels documents, et dans quels
formats. [ADR-0101 §1](adr/0101-le-catalogue-d-exports-porte-les-formats-pas-les-url.md) décide,
volontairement, que le catalogue **ne porte pas les URL** — un export garde sa route et ses
commandes d'IHM (tri, départ), que l'écran connaît. Conséquence assumée et écrite : **ajouter un
format ne touche pas l'écran, ajouter un export si.**

Ce qui n'est pas assumé, c'est la **manière dont ça se passe mal**. `Exports.tsx` itère sur sa
propre table `documents` et va chercher, dans le catalogue reçu, l'entrée de même `identifiant`.
Une entrée servie mais absente de la table — un export neuf, ou un identifiant mal orthographié
d'un côté — n'est simplement **pas rendue** : pas d'erreur, pas de trace, rien à l'écran.

⚠️ Le sens inverse est bénin (l'écran n'affiche pas une section dont le serveur ne veut plus) ;
c'est celui-ci qui coûte, parce qu'il fait **disparaître une capacité livrée**.

Aucun mécanisme ne rapproche les deux listes : l'une est en Python au composition root, l'autre en
TypeScript dans l'écran. `tsc` ne voit rien — l'`identifiant` est une `string` des deux côtés — et
les tests de rendu servent un catalogue de doublure, donc ils sont verts par construction.

C'est le mode de panne déjà inscrit en `DETTE-091` et `DETTE-094` — *une liste tenue à la main
éteint la détection sur ce qu'elle omet* — transposé cette fois à une frontière HTTP, ce qui le
rend plus difficile à mécaniser que ses deux prédécesseurs.

**Critère de fin, vérifiable** : servir un `identifiant` inconnu de l'écran doit **rougir un test**,
ou à défaut apparaître en clair à l'écran plutôt que d'être avalé. La piste la moins chère est le
patron `TYPES_DEROULES` / `TYPES_ARRETABLES` (E05US035) : une liste littérale versionnée, épinglée
...
### DETTE-096 — le modèle de données affiche une date du jour et ignore cinq migrations

**Où** : [`docs/modele-de-donnees.md`](modele-de-donnees.md).

Le document ne reflète pas les migrations **0048 → 0051** : `franchissement_arret`,
`arret_de_circonstance` et `identite_tournoi` n'ont pas de section, et `POSTE` ignore
`noms_par_page` / `cadence_page_s`.

**Ce que E16US014 y ajoute, et pourquoi la ligne est ouverte maintenant** : l'US a mis le bandeau à
« **v0.13 — 2026-08-31** » pour y inscrire ses deux colonnes. La dérive lui est antérieure, mais
l'**estampille** est d'elle : un lecteur qui voyait « v0.12 — 07/08 » pouvait rapprocher la date des
migrations récentes et se méfier ; avec une date du jour, il ne vérifie plus. C'est le geste
imputable au diff, et c'est celui qui rend le silence durable.

⚠️ **Ne pas rattraper au fil de l'eau** dans une US qui ne touche pas ces tables — c'est exactement
ce qui a produit l'écart. Une passe documentaire dédiée, ou le premier quart d'heure d'une US qui
rouvre le schéma.

### DETTE-097 — le palmarès projeté ne pagine pas, alors qu'il peut porter des dizaines de blocs

⚠️ **Aggravation du 19/09/2026 — `E06US009`, et changement de déclencheur.** La vue empile
maintenant **une section par créneau** : sa hauteur est multipliée par le nombre de départs.
Jusqu'ici la dette supposait un choix explicite de l'organisateur (cocher la portée *club*), ce qui
en faisait un cas de bord — sévérité *mineur*. Elle vaut désormais pour **tout tournoi
multi-créneaux**, soit le cas nominal du projet : sur l'écran de salle, seuls les podiums du premier
créneau sont lisibles. ⚠️ **C'est le défaut que l'US corrige côté serveur, réapparu côté affichage** :
`EcranSalle.tsx` passe `pagination` à `VueClassement` et à `VueAffectations`, mais pas à
`VuePalmares`. Sévérité relevée à **majeur**. La résorption reste celle décrite plus bas (faire
entrer les sections dans la rotation), et `docs/fonctionnel/E06US009.md` le dit en « Limite connue ».

**Où** : [`frontend/src/features/palmares/VuePalmares.tsx`](../frontend/src/features/palmares/VuePalmares.tsx),
monté par `EcranSalle` sans réglage de pages.

E16US009 a livré la pagination des vues projetées (`ReglagePages`, réglable **par écran**) et l'a
branchée sur le classement et les affectations. Le palmarès ne l'a jamais reçue : il n'en avait pas
besoin à huit blocs de quatre places.

E16US014 change l'ordre de grandeur : portée **club** cochée, la vue rend un bloc par club présent —
30 à 40 dans un tournoi de salle — et la profondeur peut monter à 64. Le contenu dépasse alors très
largement l'écran, sans défilement ni découpage.

**E16US017 ajoute une section de plus**, et celle-là n'est pas conditionnée à un réglage exotique :
le classement des clubs rend **une ligne par club présent** — le même ordre de grandeur, une seconde
fois — dès qu'une portée *toutes catégories* ou *catégorie* est réglée, donc **au défaut**. Elle
s'insère **entre** les podiums et le classement complet, qu'elle repousse d'autant plus bas.

⚠️ **Jusqu'au 19/09/2026 — plus vrai depuis** : le déclencheur **était** un choix explicite de l'organisateur, et le défaut
(catégorie, 4 places) reste dans les clous. Mais la limite est **neuve**, et elle n'était tracée
nulle part — ni au registre, ni à l'ADR, ni à la fiche de recette qui fait justement passer le
testeur par l'écran de salle avec un réglage non standard.

### DETTE-090 — aucun écran n'annule un forfait de duel

*(Voisine de [`DETTE-100`](#dette-100--la-préséance-de-rôle-nest-déclenchable-par-aucun-écran), même forme — une capacité serveur sans écran —, cause et critère de fin distincts.)*

**Où** : `frontend/src/features/forfaits/api.ts` (`annulerForfaitDuel`),
`frontend/src/features/forfaits/hooks.ts` (`useAnnulerForfaitDuel`, sans appelant),
...
`useAnnulerForfaitDuel` n'est appelé nulle part, `SaisieDuels` n'importe que la déclaration, et le
feu vert n'offre pas d'annulation. Défaire un walkover posé par erreur demande aujourd'hui un POST
manuel.

⚠️ **Le mensonge a précédé la dette.** Le dialogue de confirmation du forfait annonçait « Réversible :
le forfait s'annule depuis l'espace scoreur ou ici même » — les **deux** chemins étaient faux, sur le
geste le plus destructif de l'écran le plus chaud du jour J. Trois axes de revue l'ont relevé (deux
en bloquant). Le texte dit désormais la vérité. Ce qui empêche la promesse de repousser n'est PAS un renvoi
visible — le jeton `DETTE-090` a été retiré du dialogue, une référence interne n'ayant rien à
faire dans un texte lu par l'organisateur — mais l'assertion de `FeuVert.test.tsx` sur « Aucun
écran ne défait un forfait de duel », doublée du marqueur en commentaire au-dessus du dialogue.

Pourquoi ce n'est pas corrigé dans l'US : il n'existe **aucun endroit** où poser le bouton. Le duel
amont quitte la liste des duels à venir dès qu'il est tranché par le walkover — un bouton posé sur sa
ligne serait hors d'atteinte l'instant d'après. Il faut donc une surface neuve (une liste des
forfaits de duel du tour, ou un bandeau persistant après succès), c'est-à-dire un choix d'UX et de
périmètre, pas une correction de revue.

**Résorption** : une US dédiée. **Critère de fin** — `useAnnulerForfaitDuel` a un appelant,
`annulerForfaitDuel` retrouve son paramètre `portee` (retiré par la revue d'E16US008 pour ne pas
laisser de code mort), et le `detail` du dialogue peut à nouveau promettre la réversibilité sans
mentir.

### DETTE-087 — l'accent secondaire est réglable mais ne peint presque rien

**Où** : `frontend/src/index.css` (les quatre jetons `--brand-2-*` / `--sur-brand-2`, dans les trois
déclinaisons de thème) ↔ `frontend/src/features/identite/jetons.ts` (marqueur `// DETTE-087`)
**et** `frontend/src/features/identite/Identite.tsx` (marqueur `DETTE-087` : la ligne d'aide affichée
sous « Couleur secondaire »).

**Le raccourci.** `DV-06` décrit l'identité d'un tournoi comme « **logo + 2 accents** », et le
tableau du CDC design §3.6 les nomme pour le club : « Rouge club `#B71918` **+ anthracite** ».
E16US006 livre les deux de bout en bout — saisie, validation, persistance, dérivation sur les deux
thèmes, émission en jetons CSS. Ce qu'elle **ne** livre pas, c'est un endroit où le second accent
serve : aucune règle de `salle.css` ni d'une feuille publique ne lit `var(--brand-2-…)`. Les seules
consommatrices sont les **vignettes d'aperçu** de l'écran de préparation, qui montrent donc une
couleur dont l'effet réel est nul.

**Pourquoi ce raccourci a été pris.** Décider ce que peint une couleur secondaire est un choix de
**mise en page**, pas un problème d'implémentation — et **aucune des 36 planches de `maquettes/` ne
le dit**. Trois options se valaient (le bandeau de l'écran de salle, les pastilles de départ, le fond
des en-têtes de tableau public) sans qu'aucune ne s'impose. Les départager en écrivant du CSS aurait
été du design fait en douce à l'intérieur d'une US d'implémentation, exactement ce que le projet
refuse ailleurs. L'alternative — ne pas livrer le second accent du tout — aurait contredit le CA.

**Ce que ça coûte, et ce que ça ne coûte pas.** Rien n'est cassé et rien n'est faux : le CA
« deux couleurs d'accent » est tenu au sens strict, les quatre jetons sont dérivés correctement et
passent les seuils WCAG comme les autres. Le coût est de **crédibilité** — un organisateur règle une
couleur, la voit dans l'aperçu, et ne la retrouve nulle part. Le risque de **dérive**, lui, est réel :
une US future peut se mettre à consommer `--brand-2-*` sans savoir qu'aucune décision ne dit où, et
installer une convention par accident — le mode de panne que `DETTE-080` décrit sous un autre nom.

**Ce que l'écran en dit, depuis la revue.** Une ligne d'aide prévient l'organisateur sous le champ :
« *Enregistrée, mais elle n'habille encore aucune surface : son emploi reste à décider.* » Ce n'est
pas une résorption — la couleur ne peint toujours rien — mais une **atténuation du coût de
crédibilité** décrit ci-dessus : la dette ne se présente plus comme une fonctionnalité. Elle vaut à
la dette un **second marqueur**, dans `Identite.tsx`.

**La résorption est une question, pas un chantier.** Elle tient en une réponse du commanditaire, puis
en quelques lignes de CSS. **Critère de fin** : au moins une règle d'une feuille de salle ou publique
consomme `var(--brand-2-…)`, **les deux marqueurs disparaissent, et la ligne d'aide de `Identite.tsx`
avec eux** — sans quoi l'écran continuerait d'annoncer « n'habille encore aucune surface » sous les
yeux de l'organisateur, une fois la question tranchée. ⚠️ **« On n'en veut pas » est une réponse
valide** — auquel cas il faut *retirer* les quatre jetons et le champ, pas les laisser dormir : un
réglage mort est plus coûteux qu'un réglage absent.

### DETTE-086 — des mises en page n'ont jamais été affichées

**Où** : `frontend/src/app/App.css`, section « Plan de cibles et plan de duels » (marqueur
`/* DETTE-086 */` en tête de section) **et**, depuis E16US006,
`frontend/src/features/identite/identite.css` (même marqueur en tête) **et**, depuis E16US009,
`frontend/src/features/competition/TableClassement.tsx` (`LIGNES_PROJETEES_MAX`, marqueur
`// DETTE-086`) **et**, depuis E16US021, `frontend/src/app/App.css`, section « Blocs par départ, en
tête de l'accueil » (même marqueur en tête).

> ⚠️ **Ligne élargie le 27/08/2026 par `E16US009` — et cette fois, la prédiction ci-dessous s'est
> vérifiée sous nos yeux.** Cette entrée annonçait, dès son inscription, qu'« un 4ᵉ défaut du même
> type est probable » et qu'« aucun garde-fou du dépôt ne peut le voir ». C'est exactement ce qui
> s'est produit, sur une **quatrième** surface : le classement projeté.
>
> Le déroulé, parce qu'il est le fait le plus instructif de cette dette :
>
> | passe | ce qu'elle a trouvé | par quel moyen |
> |---|---|---|
> | 1ʳᵉ | **bloquant** — `noms_par_page` calibré pour une liste **à 3 colonnes**, appliqué tel quel à un tableau **mono-colonne** : une page 3× trop haute, au réglage livré | recalcul depuis le CSS, par **deux axes indépendants** |
...
> rend des archers **jamais visibles**. C'est la seule garantie qu'un calcul puisse donner ici.
>
> ⚠️ **Deux résidus explicites, à ne pas découvrir le jour J** :
> 1. **`VueAffectations` n'a aucun plafond.** La liste de noms projetée déborde, elle, vers 55-66
>    noms réglés selon la résolution — et son débordement est tout aussi invisible. L'asymétrie est
>    **dite** dans l'aide de l'écran d'admin et dans la fiche de recette plutôt que gommée : sous
>    27 noms réglés le classement commande le réglage, au-delà il est figé et c'est la liste de
>    noms qu'il faut regarder. Poser le même plafond des deux côtés demande la mesure qui manque.
> 2. **Le plafond crée une plage inerte.** Au-delà de 27 noms réglés, monter la valeur ne change
>    plus rien au classement. Un cadran bloqué ressemble à une panne : c'est écrit à l'organisateur.
>
> *(Le lien avec `DETTE-085` vaut d'être noté : c'est la même famille de cécité — ce que jsdom ne
> calcule pas, aucun test ne le voit. La différence est que 085 porte sur du **rendu dupliqué** et
> 086 sur des **grandeurs devinées**.)*

> ⚠️ **Ligne élargie le 25/08/2026 par `E16US006`, et non doublée.** Cette US livre trois
> surfaces neuves — l'écran de préparation avec ses deux vignettes d'aperçu, les logos dans le
> bandeau de l'écran de salle, les logos au titre du tournoi côté public — **sans qu'aucune ait
> été affichée**, pour la raison **identique** : le contrôle visuel reste impossible sur ce
> poste. Les bornes de taille des logos (`3rem` / `12rem` en admin, `2.4em` / `10em` en salle)
> sont **choisies, pas mesurées**. La cause est un fait de poste, pas un défaut d'écran : lui
> inventer une seconde ligne aurait dispersé le remède, qui est unique — *rétablir un moyen de
> rendre l'application*. ⚠️ **Le risque propre à cette US est cependant différent et moindre** :
> un logo trop grand pousse un bandeau, il ne rend aucune information illisible ni ne fait
> disparaître un CA — au contraire du plan de cibles, où trois bloquants de suite portaient sur
> du texte tronqué.

**Le raccourci.** E16US005 a refondu la mise en page de deux écrans — une cible par ligne, repères
sur deux lignes, réserve collante, deux points de bascule responsive — **sans jamais l'afficher**.
Le contrôle visuel n'était pas possible sur le poste : l'extension navigateur n'était pas connectée,
et un banc d'essai statique reproduisant le DOM réel avec les vraies feuilles n'a pas pu être servi
non plus. Chaque valeur du CSS a donc été posée par une arithmétique refaite à la main —
`viewport − 368` pour la colonne de contenu, puis la chaîne des pistes, gouttières, paddings et
bordures jusqu'au texte utile — recoupée par quatre relecteurs qui ont refait le même calcul.

**Ce que ça coûte, et ce que ça a déjà coûté.** Trois passes de revue, trois bloquants de mise en
page, tous invisibles aux ~4 800 tests du dépôt :

1. le jumeau des duels ne rendait pas ce que le plan de cibles rendait — un *non-changement*, donc
   invisible à la lecture d'un diff ;
2. le seuil responsive mesurait le **viewport** alors que la contrainte est la **colonne de
   contenu**, 368 px plus étroite sous la coquille admin ;
3. la troncature qui rendait la largeur tenable effaçait la catégorie et le blason, soit la moitié
   du CA — et le correctif de ce défaut a lui-même laissé le blason illisible un cran plus loin.

Aucun n'était atteignable autrement que par le calcul, et **jsdom ne calcule aucune mise en page** :
`text-overflow`, `position: sticky`, les grilles et les *media queries* n'y existent pas. Un test
peut prouver que `--couloirs` vaut 4 ; aucun ne peut prouver qu'un nom est lisible.

⚠️ **Résidu connu et non résorbé** : il subsiste une bande de largeurs, **[1249, 1377] px**, où
l'écran offre moins de texte par couloir qu'avant l'US (104 px contre 121 à 1366 px). `1366×768` est
la résolution la plus répandue sur un portable d'organisation et se trouve en plein milieu. Le
correctif dérivé de l'arithmétique serait de porter le seuil de bascule à `90rem` — mais il **coûte
la réserve collante en dessous de 1440 px**, c'est-à-dire un CA né du cadrage. Cet arbitrage demande
de voir les deux rendus ; il est donc suspendu à la levée de cette dette.

**Pourquoi une dette et pas un bloquant.** Rien ici ne casse un cas utilisateur : l'écran fonctionne,
tous les gestes sont possibles, aucune règle de placement n'est touchée. Ce qui manque est une
**vérification**, et son absence est durable — elle survivra à cette US et pèsera sur toutes celles
qui rouvriront ces écrans. C'est exactement ce que le registre est fait pour porter, plutôt qu'une
phrase de prose dans un tracker, qui ne survit ni à un `/compact`, ni à un merge, ni à un `grep`.

**Résorption.** Un œil humain devant l'écran — ce n'est pas une US de code. **Critère de fin** :
trois captures (1250, 1366, 1600 px) annexées à
[`docs/fonctionnel/E16US005.md`](fonctionnel/E16US005.md), et l'arbitrage du seuil tranché sur ce
qu'elles montrent. Le remède **structurel**, lui, est un fait de poste et non de dépôt : rétablir un
moyen de rendre l'application au cours d'une US front — extension navigateur connectée, ou banc
d'essai statique servi en HTTP.

### DETTE-085 — les deux plans de cibles sont des jumeaux recopiés composant par composant

**Où** : `frontend/src/features/placement/Placement.tsx` et `frontend/src/features/duels/Duels.tsx`
(un marqueur `// DETTE-085` en tête du bloc jumelé, dans chacun des deux fichiers).

**Le raccourci.** L'écran des duels a été cloné depuis celui du placement à E03US009, et son
en-tête le dit en toutes lettres (« jumeau de `placement/Placement.tsx`, à deux différences près »).
Sont recopiés : les composants `Cible`, `Case`, `JetonArcher` et `Reserve`, le type `Jeton`, la
fabrique de jeton, le calcul `couloirs` et le `messageErreur` local (ce dernier déjà compté par
`DETTE-050`).

Le fond **métier** commun, lui, est bien mutualisé depuis E03US007 : `LIBELLE_RAISON`,
`RAISON_ANOMALIE`, `resumeCloisonnementNonRespecte` et, depuis E16US005, `reperesArcher` et
`ReferentielsDuPlan` vivent tous dans `placement/presentation.ts`, importé par les duels. C'est le
**rendu** qui reste en double.

⚠️ **« À l'identique » serait faux, et le dire aurait un coût.** Les deux copies ont déjà divergé,
indépendamment de tout bug : le placement rend la lettre du couloir sur une case **occupée**
(E03US011), pas les duels — écart refermé par E16US005 — et les libellés de réserve diffèrent
volontairement (« archers » contre « duellistes »). Une US de rangement dimensionnée sur l'hypothèse
« ce sont les mêmes composants » aplatirait des écarts voulus sans s'en apercevoir.

**Ce que ça coûte — et ce que ça a coûté.** ⚠️ **Cette dette n'est pas un risque projeté : elle a
mordu dans le commit même qui l'inscrit.** La première rédaction d'E16US005 devait appliquer le même
changement de mise en page aux deux écrans. Elle l'a appliqué à un seul : `Duels.tsx` construisait
`reperes`, montait trois requêtes de référentiel pour les alimenter, déclarait le champ dans son
type — et son `JetonArcher` rendait toujours `{jeton.nom}` nu. S'y ajoutait une **régression
visuelle** : la feuille partagée avait déplacé `font-size`/`font-weight` de `.jeton` vers
`.jeton__nom`, un `<span>` que l'écran des duels n'émettait pas.

Rien de mécanique ne pouvait l'attraper. `tsc` ne voit pas une propriété *fournie* et jamais
*consommée* ; le lint non plus ; et la suite ne montait pas cet écran. Ce sont trois axes de revue
qui l'ont vu — et les quatre livrables de suivi de l'US (story, tracker, journal, fiche de recette)
affirmaient tous, en détail, que l'alignement était fait.

**D'où la sévérité *majeur*** (et non *mineur*, comme la première rédaction de cette entrée le
disait) : le registre classe déjà `DETTE-079` en majeur pour le même mécanisme **sans** défaut
réalisé. Ici il l'est.

**Garde-fou posé en attendant le rangement.** E16US005 ajoute un **test de rendu par écran** —
`placement/Placement.test.tsx` et `duels/Duels.test.tsx` — qui vérifie que le jeton porte ses
repères et que la réserve distingue une mise à l'écart d'une anomalie. C'est le remède
**proportionné** : il ne fige aucune frontière que l'US de rangement devra de toute façon arbitrer,
et il transforme la prochaine divergence en test rouge. Vérifié en le réintroduisant : sans le
correctif, `Duels.test.tsx` échoue.

**Résorption.** Remonter les quatre composants dans `shared/plan-de-cibles/`, les badges passés en
`ReactNode` par l'appelant (c'est leur seule vraie différence de structure). Précédents de ce geste :
`etatRencontre`, remontée en `shared/` à l'occasion d'une US de format, et le remède prévu pour
`DETTE-083`. **US de rangement dédiée** (règle 16), à traiter **dans le même mouvement que
`DETTE-083`** : même geste, même destination, et un seul rangement front vaut mieux que deux.

⚠️ **Aucun identifiant d'US n'est nommé dans la cellule *Résorption* du tableau**, et c'est
délibéré : l'atlas en extrait `resorption_us` par expression régulière sur la cellule **entière** —
le contexte (« précédent », « garde-fou ») lui est invisible. Toute US qui y figure est donc publiée
comme l'US **résorbante**.

⚠️ **Ce piège a été tendu deux fois de suite dans cette même US, et la seconde est la pire.** La
1ʳᵉ rédaction citait `E05US027` en précédent — une dette du 24/08 déclarée résorbée par une US du
22/08, absurdité au moins visible. La 2ᵉ a retiré cet identifiant… et écrit « garde-fou posé en
attendant (E16US005) » dans la même cellule : l'atlas publiait alors `dettes_introduites: ["085"]`
**et** `dettes_resorbees: ["085"]` pour la même US, donnée parfaitement plausible qui se serait lue
« dette traitée ». Le tout dans le commit qui gravait la leçon inverse au registre de métriques
(constat 35). Rien de mécanique ne l'attrape : `controles.py` ne signale qu'une US *absente de
`stories/`*, et `--verifier` compare le généré à une source fausse.

**Remède structurel proposé, hors de cette US** : un contrôle dans `backend/atlas/controles.py`.
⚠️ **Mais pas le contrôle évident.** Un prédicat naïf `resorption_us ∩ introduite_par ≠ ∅` a été
essayé sur le registre : il rend **sept** entrées supplémentaires (`001`, `028`, `031`, `050`,
`064`, `071`, `074`). Instruites **une par une** en 3ᵉ passe de revue, elles se répartissent ainsi :

- **légitimes** — `028` (« E01US024 n'en avait résorbé que la moitié »), `001` (résolution partielle
  réelle), `050` (« constatée et partiellement résorbée par E16US003 ») : une dette peut être
  *élargie* et *partiellement résorbée* par la même US, et le prédicat crierait à tort ;
- **hors sujet** — `031` : sa cellule ne contient **pas une seule occurrence de « résorb »**, les US
  y figurent comme sites de marqueurs ou « élargie par » ;
- **artefacts, corrigés ici** — `064` (l'US y était nommée comme l'occurrence qui **aggrave**) et
  `071` (une US citée en *patron à copier*, la vraie résorbante étant nommée juste à côté et noyée) ;
- **`074`** : mention **barrée** dans la cellule, invisible à l'expression régulière. Laissée en
  l'état : le barré porte pour l'humain une attribution rétractée, information qu'un retrait effacerait.

⚠️ **La première rédaction de ce paragraphe était fausse, et dans le sens qui dispensait du travail.**
Elle affirmait que « plusieurs sont légitimes » en citant `DETTE-031` comme preuve — or `031` n'est
pas un cas de résorption partielle, et deux des sept étaient bien des artefacts du type qu'on venait
de corriger. La généralisation avait été faite depuis deux cellules lues et étendue aux cinq autres
sans les ouvrir (relevé par deux axes).

**Cinq artefacts corrigés au total par E16US005** — `085` (le sien), puis `083`, `081`, `064` et
`071` — le fichier étant déjà ouvert et l'atlas déjà régénéré, le coût marginal était nul.

Le contrôle utile doit donc porter sur autre chose qu'un prédicat d'intersection : n'extraire
`resorption_us` que d'un champ **balisé** au lieu de toute la cellule rendrait les mentions en prose
inoffensives par construction. ⚠️ Deux réserves pour l'US d'outillage qui le fera : la bascule est
**globale ou rien** (tant que les lignes ouvertes ne sont pas toutes balisées, `resorption_us`
s'effondre à vide partout, ce qui **perd** de l'information vraie et éteint en silence le contrôle
`resorption-hors-stories`) ; et le balisage ne peut pas être `Résorption :`, qui apparaît déjà **en
prose** au début de la cellule de `DETTE-064`.

**Critère de fin** : `Duels.tsx` ne déclare plus aucun composant de grille, et n'importe plus de
`features/placement/` **que `presentation.ts`**.

⚠️ La nuance n'est pas cosmétique : la première rédaction exigeait « n'importe plus **rien** de
`features/placement/` », ce que le remède décrit ne peut pas produire — il déplace les composants,
pas les cinq symboles de vocabulaire métier (`LIBELLE_RAISON`, `RAISON_ANOMALIE`,
`resumeCloisonnementNonRespecte`, `reperesArcher`, `ReferentielsDuPlan`) que le paragraphe
ci-dessus présente justement comme le **bon** état. Une US de rangement lisant ce critère aurait dû
démonter la mutualisation, ou ne jamais pouvoir clore la dette.

### DETTE-084 — l'écran « Prêt à terminer ? » redéduit la garde de statut et recopie sa phrase

**Où** : `frontend/src/features/completude/Completude.tsx` (marqueurs `// DETTE-084`) ↔
`backend/domain/jalon.py` (`evaluer_terminer`).

**Le raccourci.** E16US012 a supprimé toute déduction de statut côté front — sauf ici. Cet écran
lit `/completude`, qui ne porte pas le statut du tournoi ; pour ne pas mentir sur la garde de
`terminer` (qui n'accepte qu'un tournoi *en cours*), il la **redéduit** de la prop `statut` qu'il
reçoit déjà, et **recopie** la phrase de refus en TypeScript, faute de pouvoir importer la constante
Python.

**Pourquoi ne pas simplement lire le jalon.** `/jalons/terminer` rend exactement la même liste, mais
la confirmation de cet écran a besoin en plus du volet **administratif** (`hors_sportif`) pour
chiffrer les impayés. Le brancher sur le jalon signifierait donc **deux** lectures pollées à 5 s par
tablette au lieu d'une — ce qu'ADR-0096 a écarté explicitement.

**Ce que ça coûte.** Deux encodages de la même garde, et deux littéraux de la même phrase dans deux
runtimes qu'aucun test ne confronte : reformuler côté Python laisse l'écran afficher l'ancienne
phrase, tout au vert. Le garde-fou compensatoire est `Completude.test.tsx`, qui épingle la phrase et
le comportement hors *en cours* — il rougit si le front bouge seul, pas si le domaine bouge seul.

✅ **Le volet fraîcheur a été résorbé en 5ᵉ passe de revue** — `useTournois({ live: true })` sous la coquille admin. Ce
qui suit décrit ce qu'il en était, parce que la leçon vaut pour tout écran d'administration qui
dérive son comportement d'un statut : **il faut savoir d'où vient la fraîcheur de ce statut.** Le `statut` dont
dépendent le verdict, la raison et le bouton vient de `useTournois()`
(`frontend/src/features/competition/hooks.ts`), qui n'a **aucun** `refetchInterval` — `staleTime`
global de 30 s, `refetchOnWindowFocus: false`, et aucune transition de cycle de vie ne diffuse
d'événement WebSocket. Scénario du jour J, deux surfaces d'administration : le PC met le tournoi en
pause depuis la frise ; sur la tablette restée sur « Prêt à terminer ? », rien n'invalide
`['tournois']` — pas de mutation locale, pas d'événement, pas de refocus. La tablette affiche donc
pendant toute la pause le verdict et le bouton d'un tournoi *en cours*, jusqu'au 409 au clic.

⚠️ **Et la direction décrite d'abord était la bénigne.** « La tablette propose une action refusée »
s'auto-corrige : le 409 détrompe l'organisateur. L'inverse ne s'auto-corrige pas — le PC **reprend**
le tournoi, la tablette croit encore à la pause, **le bouton « Terminer » disparaît**, et rien ne
viendra la détromper. L'appli **empêche sans avertir**, exactement l'inverse de `D-15`/`P-3`. C'est
cette seconde direction qui a fait corriger plutôt qu'inscrire (5ᵉ passe, axe D).

Le défaut **préexistait** à E16US012 — `main` conditionnait déjà le bouton « Terminer » au statut —
mais l'US l'aggravait : le verdict, la raison et l'intro en dépendent aussi. **Ce volet est
désormais fermé** ; ce qui reste sous cette ligne est la **copie** de la garde et de sa phrase, qui,
elle, ne casse aucun cas utilisateur — d'où la sévérité *mineur* maintenue.

**Résorption.** Migrer l'écran sur `/jalons/terminer` — ce qui referme la copie de la garde et celle
de la phrase d'un seul geste. ⚠️ **C'est désormais possible sans rien perdre** : le jalon *terminer*
garde ses lignes à tout statut (asymétrie assumée avec *démarrer*, cf. ADR-0096 § Conséquences).
Tant que ce n'était pas le cas, cette résorption **aurait fait disparaître la liste sportive pendant
la pause** — elle recommandait le défaut qu'une passe venait de corriger (relevé en 5ᵉ passe, quatre
axes). Reste un point à traiter dans l'US de migration : le badge « complet » se lit aujourd'hui de
`sportif_complet`, que le DTO du jalon n'expose pas. ⚠️ **Une voie existe déjà, sans second poll**, contrairement à ce que la première
rédaction de cette section supposait : `hors_sportif` n'est lu que par `messageConfirmationTerminer`,
donc **uniquement dans le dialogue de confirmation**. L'écran peut donc poller le jalon et ne lire
`/completude` qu'à l'ouverture de la confirmation (`queryClient.fetchQuery`, hors poll). Coût
honnête : `BoutonConfirme.detail` reçoit aujourd'hui une chaîne calculée au rendu ; il faudrait qu'il
accepte un contenu résolu à l'ouverture — une US courte, pas un chantier. « Attendre que le coût du
second poll devienne acceptable » n'était pas une résorption : ce coût ne baisse pas tout seul.

### DETTE-082 — deux endroits pour démarrer le même tournoi

**Où** : `frontend/src/features/tournois/Tournois.tsx` (montage de `FriseCycleDeVie` sans
`jalons` — c'est **le** site restant) ↔ `frontend/src/features/jalons/PretADemarrer.tsx`,
`frontend/src/features/completude/Completude.tsx` et
`frontend/src/features/accueil/FriseCycleDeVie.tsx`. Les quatre portent le marqueur `DETTE-082`.

**Le raccourci.** E16US012 livre l'écran « Prêt à démarrer ? », qui énumère ce qui manque **avant** le
clic puis porte l'action. La frise du cycle de vie (E14US001) porte déjà, elle, les boutons
« Démarrer » / « Terminer ». Les fondre supposait la refonte de navigation que la fiche E16 annonce
sans l'avoir instruite : l'US a livré le nouveau chemin sans retirer l'ancien.

**Ce que ça coûte, exactement.** Rien n'est incohérent : les deux lisent la même topologie serveur
(`useTransitions`) et appellent la même transition, qui garde. Mais l'organisateur qui part de la
frise ne voit pas ce qui manque et retombe sur le refus au clic — le défaut que l'US supprime, par
l'autre porte. Ce n'est **pas** un bloquant : aucun cas utilisateur n'est cassé.

**✅ Ce qu'E16US021 a refermé (12/09/2026).** La frise accepte une table `jalons` — par nom de
transition, un couple `{ libelle, aller }`. Fournie, ces transitions deviennent un **renvoi**
portant le nom de l'écran visé, au lieu du bouton nu. L'**accueil** — destination d'ouverture de
l'axe pilotage, donc le chemin le plus fréquenté — la fournit. ⚠️ **C'est la coquille qui détient
libellé ET destination**, et ce n'est pas un détail de style : la 1ʳᵉ version ne faisait traverser
que la transition, ce qui obligeait la frise à caster `t.nom` et à **recopier** les deux libellés de
la barre latérale. Faire porter le couple par la coquille supprime les trois défauts d'un coup — le
cast, la copie, et le ternaire de repli qui envoyait tout membre inconnu sur `completude` — sans
créer l'import `accueil → admin` (le cycle de `DETTE-083`) : seules des **valeurs** traversent.

**Ce qui reste ouvert, et pourquoi c'est délibéré.** (1) `Tournois.tsx` monte la même frise **sans**
`jalons` : les boutons y restent nus. (2) `archiver` n'est **pas** dans la table, parce que son
jalon répond `404 jalon_non_instruit` — l'y renvoyer mènerait à un écran vide, ce qui serait pire
que le bouton nu. Un test (`FriseCycleDeVie.test.tsx`, « garde les boutons nus là où aucun renvoi
n'est fourni ») verrouille le cas 1, pour que sa fermeture soit un geste voulu et non un effet de
bord.

**Résorption.** Quand `archiver` rejoindra la famille : ajouter son entrée à `jalonsDeLaFrise`
(`CoquilleAdmin.tsx`) et passer `jalons` au montage de `Tournois.tsx`. Le mécanisme existe, et
**le compilateur garde le geste** — une entrée neuve doit nommer sa destination, validée contre
`DestinationAdminId`.

### DETTE-083 — les features `jalons` et `completude` s'importent mutuellement

**Où** : `frontend/src/features/jalons/PretA.tsx` ↔
`frontend/src/features/completude/Completude.tsx` (marqueurs `// DETTE-083`).

**Le raccourci.** La coquille commune `PretA` réutilise `SectionCompletude` et le type
`LigneCompletude` de la feature `completude` — c'est le bon geste (ne pas dupliquer un rendu qui
diverge au premier ajustement de pastille, motif de `DETTE-065`) — pendant que `Completude.tsx`
importe `PretA` pour migrer sur la famille. D'où un **cycle d'imports entre deux features**, que la
règle 10 (organisation par features autonomes) cherche à éviter et que l'atlas signale.

**Ce que ça coûte.** Rien à l'exécution. Le coût est de découpage : la coquille de la famille dépend
d'un de ses membres, donc `archiver` et `exporter` traîneront `completude` sans rapport métier.

⚠️ **Cette US crée le cluster, elle ne le rejoint pas** — la première rédaction de cette section
disait l'inverse, et c'est l'atlas régénéré par le commit lui-même qui l'a démentie (2ᵉ passe de
revue, axe C2). `main` compte **trois** clusters de features en cycle ; la branche en compte
**quatre**, le nouveau étant `accueil, completude, jalons, paiements`. `accueil` et `paiements`
n'étaient enchevêtrés avec rien avant cette US : elle les y entraîne par transitivité.

La nuance n'est pas rhétorique. Elle change le **périmètre du remède** : casser `jalons → completude`
laisse la composante debout. C'est la disparition du signal sur les **quatre** sujets qui dit que la
dette est résorbée.

**Résorption.** ⚠️ **Le premier remède écrit ici était faux**, et il l'était d'une façon qui aurait
coûté une US pour rien : il proposait de remonter `SectionCompletude` et le type `LigneCompletude`
dans `shared/`, ce qui coupe `jalons → completude`… et laisse debout
`completude → jalons → accueil → completude`. Le signal serait resté (3ᵉ passe de revue, axe C2).

Les arêtes préexistantes (`accueil → completude`, `accueil → paiements`, `paiements → completude`)
sont un **DAG** — c'est bien pourquoi `main` n'a aucun signal sur ces features. Les deux arêtes de
**retour** sont créées par cette US :

- `completude → jalons` (`Completude.tsx` importe `PretA`) ;
- `jalons → accueil` (`PretADemarrer.tsx` importe `useTransitions` / `useTransitionnerTournoi`).

Le **cut unique** qui dissout la composante est le premier : remonter la coquille `PretA` elle-même
dans `shared/ui/`, avec ce qu'elle traîne (`SectionCompletude`, le type `LigneCompletude`,
`afficheEtat` / `detailLigne`). Précédent : `etatRencontre`, remontée en `shared/` à l'occasion de la dernière tranche des formats *(US volontairement non nommée ici : l'atlas lirait l'identifiant comme celui de l'US résorbante — cf. `DETTE-085`)*.
Couper la seconde arête à la place laisserait un 2-cycle `completude ↔ jalons`.

C'est un **rangement transverse**, donc une US dédiée (règle 16) — qui gagnerait à traiter les trois
autres clusters dans le même mouvement. **Critère de fin** : plus aucun signal `features-enchevetrees` **citant** `accueil`, `completude`,
`jalons` ou `paiements`. ⚠️ Pas « porté par le sujet `accueil` » : l'atlas ne nomme qu'**un** sujet
par composante, donc une composante réduite à `{completude, jalons}` changerait simplement de sujet
— le signal d'`accueil` disparaîtrait alors que le cycle demeure.

### DETTE-081 — éditer un format efface le seuil de barrage qu'il portait

**Où** : `backend/api/v1/formats.py` (`EtapeDTO`, marqueur `# DETTE-081`) ↔
`frontend/src/features/patrimoine/api.ts` (`Etape`).

**Le mécanisme.** `ModelePhase.barrage_jusqu_au` existe depuis E01US024, et `_config_format` le
sérialise fidèlement — c'est même le correctif que ce fichier raconte trois fois en commentaire.
Mais l'`EtapeDTO` du routeur des formats ne déclare **pas** ce champ. Le client ne le reçoit donc
jamais, ne peut pas le renvoyer, et `PUT /formats/{id}` étant une édition **totale**, la valeur
persistée est écrasée par l'absence.

**Ce que ça coûte.** Un organisateur promeut en format un tournoi dont la qualification porte un
barrage jusqu'au rang 8. Le format le stocke. Il rouvre « Composer un format », change **n'importe
quoi** — un libellé, un effectif —, enregistre : le seuil a disparu. Aucune erreur, aucun message,
un `200`. L'année suivante, le format rejoué ne départage plus au tir.

**Pourquoi elle est inscrite ici.** Elle est **antérieure** à E16US002 et n'a pas été aggravée par
elle. Elle a été découverte par l'axe **adversarial** de sa 2ᵉ passe, qui l'a reproduite par
exécution plutôt que déduite (`EtapeDTO.de_modele(modele).vers_modele()` : `barrage = 8 → None`).
Une dette découverte ne se repose pas — mais elle ne se corrige pas non plus dans une US qui ne
touche pas ce champ.

⚠️ **Distinguer deux choses.** Que le barrage ne se **règle** pas à l'atelier est un arbitrage
antérieur, assumé et documenté (`docs/fonctionnel/E06US003.md` : « le seuil se règle pour l'instant
sur la phase du tournoi, pas dans le modèle de format »). Que l'atelier **efface** une valeur déjà
promue n'a jamais été décidé par personne : c'est la dette.

⚠️ **Le mode de panne est structurel, pas propre au barrage.** Tout champ de `ModelePhase` absent
de l'`EtapeDTO` subit le même sort. La résorption doit donc commencer par un inventaire des deux
listes, pas par l'ajout d'un champ. *(Le côté `deroule_etape` est, lui, verrouillé depuis E16US002 :
les paramètres de `_politiques_json` sont keyword-only sans défaut, donc mypy refuse un appelant
incomplet. La même idée appliquée au DTO fermerait cette dette-ci.)*

**Résorption** : US dédiée — `barrage_jusqu_au` sur `EtapeDTO` et sur `Etape` (front), plus un test
d'aller-retour HTTP calqué sur celui qu'E16US002 vient d'écrire pour le titre.
...
- `Deroule.tsx` réinitialise **7 champs sur 12** ; **`colline`, `arrets`, `decoupage`, `type` et le
  barème** n'y sont pas. C'est donc une divergence **ouverte**, pas seulement un risque — et
  ...
  collines d'affilée y reporte donc encore « Ladder portée 4 sur 8 manches ».

**Ce qu'E16US002 y ajoute** : le `titre`, 10ᵉ réglage, avec ses trois éléments de plomberie des deux
côtés.

⚠️ **Le contrôle lui-même a failli manquer au partage, et la revue l'a rattrapé.** La première
livraison écrivait le champ de titre **trois fois** (`FormulairePhase`, `ReglageTitre`,
`FormulaireEtape`), et les trois copies **divergeaient déjà** dans le commit qui les créait :
libellé, `className`, `placeholder` — et surtout `maxLength={80}`, miroir manuel du
`Field(max_length=80)` des deux DTO, recopié trois fois. La phrase ci-dessus (« les contrôles sont
partagés ») était donc **fausse pour le champ que l'US ajoutait**, dans le même commit qui l'écrivait :
une dette silencieuse cachée par une ligne de registre. Corrigé en revue —
`shared/phases/ChampTitre.tsx`, 8ᵉ exemplaire d'un pattern établi, avec `LONGUEUR_MAX_TITRE` comme
miroir **unique** de la borne serveur. Ce qui reste au registre est la **plomberie d'état**, pas le
contrôle. Le câbler à l'atelier n'était pas facultatif — le serveur porte `EtapeDTO.titre`, et un front
...
**Aggravation d'un autre ordre, inscrite ici faute de ligne à élargir (2ᵉ passe de revue).** La
fiche dépliable réunit désormais **trois** écrivains « édition totale » côte à côte — `ReglageTitre`,
`ReglageBarrage`, `ReglageDecoupageDePhase` —, contre deux avant l'US. Chacun reconstruit la config
complète depuis la **prop** `phase` via `configInchangee`, et `useModifierPhase` se contente
d'invalider : entre un clic sur *Enregistrer* et l'arrivée du refetch, la prop est **périmée**.
Renommer puis régler le barrage dans cette fenêtre **réécrit l'ancien titre**, avec un `PUT` qui
réussit et aucun message — et le champ de titre continue d'afficher la valeur que le serveur vient
d'annuler, si bien que la perte est invisible sur l'écran qui l'a provoquée.

**Assumé, pas corrigé**, et la raison est honnête : la fenêtre est d'un aller-retour LAN, la bascule
de fiche répare l'affichage, et le motif est **antérieur** (deux écrivains coexistaient déjà). Ce
qui est neuf est le **troisième**, et leur réunion dans un même panneau. Remède connu et borné, si
une mesure le réclame : faire rendre à `useModifierPhase` la phase fraîche du `PUT`
(`queryClient.setQueryData`, le routeur rend déjà `EtapeReponse`), ce qui referme la fenêtre à
l'instant du succès. Marqueur `// DETTE-080` sur `configInchangee`.
...
⚠️ **`E16US002` (22/08/2026) n'élargit PAS cette ligne, et c'est délibéré.** Son champ `titre` est
ajouté aux deux `EtapeDTO` **racines**, que la « Résorption » ci-dessous exclut nommément du remède
(« ce sont les *feuilles* qui sont identiques, pas les racines ») : ni paire neuve, ni champ ajouté
à une paire existante — rien à compter. Inscrit ici parce que la question s'est posée en revue et se
reposerait sinon ; le code portait d'abord un marqueur annonçant « une paire de plus », corrigé dans
le même commit. *(Écrit dans cette section et non dans la colonne « Introduite par » : le générateur
d'atlas y moissonne tout identifiant d'US, si bien qu'une **dénégation** s'y retournait en
attribution — le livrable de suivi annonçait « E16US002 introduit DETTE-054 ».)*
...
⚠️ **Élargie par E16US019 (11/09/2026), et l'aggravation est d'une autre nature.** Le raccourci
touchait jusqu'ici l'admin, qui n'a **aucune** affordance de créneau : rien ne lui promettait le
contraire. La surface scoreur neuve, elle, porte un sélecteur « **Départ à valider** » — qui ne
sert qu'à filtrer la **liste d'archers** : les trois appels qu'il déclenche (`GET /saisie/series/…`,
`POST /saisie/validations`, `POST /saisie/annulations`) ne transportent aucun `depart_id`, donc le
service **devine** comme pour l'admin. Sur un archer engagé matin **et** après-midi, le scoreur
choisit l'après-midi et lit, valide ou annule la feuille **du matin**, en `200` et sans un mot.

La dette cesse donc d'être « une portée implicite » pour devenir **une portée affichée puis
ignorée** — ce qui est pire à l'usage, parce que l'écran donne une raison de croire au cadrage.
Marqueur `DETTE-052` posé sur le sélecteur du panneau. Le remède reste le même et n'a pas changé
de nature : **porter le `depart_id` dans les corps de requête**, ce qui vaut désormais pour cinq
routes au lieu de trois. *(Relevé en revue d'E16US019 par deux axes indépendants.)*
...
### ✅ DETTE-001 — suppression de tournoi non cascadée (résorbée le 19/09/2026)
...
0. **d'abord se demander si la FK doit être nue — et le vérifier.** Une table dont les lignes sont
   ...
   **bloquant** (`CLAUDE.md` § Dette). ⚠️ Ce geste a été ajouté après E16US006, où les trois suivants
   ...
   ce cas apprend » plus bas. Un marqueur bien posé **répond d'avance** à la question « pourquoi ce
   raccourci ? » : il endort le relecteur au lieu de l'alerter ;
...
E16US006 ajoute `identite_tournoi` (clé primaire **et** FK → `tournoi.id`) en **`ON DELETE
CASCADE`** : **hors** de cette dette, comme `placement` et `volee`. C'est un **composant strict** de
l'agrégat `Tournoi` — une ligne, sans descendance, purement cosmétique, sans aucun sens hors de son
tournoi — et sa disparition est assumée par
[ADR-0097](adr/0097-un-logo-de-tournoi-vit-en-base-avec-lui.md). Le futur résolveur de DETTE-001 n'a
**rien à faire** dessus.

⚠️ **Ce que ce cas apprend sur la dette elle-même**, et qui vaut au-delà de l'US : la première
rédaction avait posé la FK **nue** avec le marqueur `# DETTE-001`, par application mécanique de la
procédure d'aggravation ci-dessus. Or la ligne d'identité **naît au premier réglage et n'est jamais
retirée** : le tournoi devenait *définitivement* indéracinable en 500, alors qu'un brouillon vide
restait supprimable jusque-là. Le raccourci n'était donc pas de la dette mais un **cas utilisateur
cassé** (§ Dette de `CLAUDE.md`), et il rendait faux l'argument par lequel ADR-0097 justifiait de
stocker les logos en base (« supprimer un tournoi supprime sa descendance »). Trouvé par l'axe
adversarial de `/revue-us`, **à l'exécution** — les quatre axes à grille avaient lu la FK et le
marqueur, et conclu à une aggravation régulière. Leçon : la procédure d'aggravation dit quoi faire
d'une FK que l'on **choisit** de laisser nue ; elle ne dispense pas de se demander si on doit la
laisser nue. Une table dont la seule ligne est un composant strict de l'agrégat parent n'entre pas
dans cette dette — elle cascade.
...
> (classe `bouton--danger`, **neutre** depuis ADR-0114 — cf. `DETTE-115`),
...
- `domain.recherche.correspond` / `classer` (E16US010) — replier le **fragment cherché** et les
  libellés (archers, clubs, tournois) de la **recherche transverse**.
...
⚠️ **4ᵉ usage hors club, le 29/08/2026 (E16US010).** La recherche transverse replie le fragment
saisi avec `cle_nom`, et c'est **le bon geste** : deux normalisations divergentes auraient donné
une recherche qui trouve « Lévêque » là où la détection de doublons ne le rapproche pas — deux
vérités sur les mêmes fiches. L'US **aggrave** donc la dette au lieu de la contourner, comme
`CLAUDE.md` § Dette le prescrit. Elle ne l'a pas résorbée pour la raison inchangée ci-dessous, et
parce qu'elle portait déjà quatre CA : le déplacement aurait noyé son diff. Le domicile devient
d'autant plus trompeur qu'un module nommé `recherche.py` importe sa règle de repli depuis
`club.py` alors qu'il ne cherche pas que des clubs.
...
à jour les **6** appelants (dont `domain/doublons.py`, E02US005, et `domain/recherche.py`,
E16US010). **Zéro changement de comportement**
...
placement adapté, **et les trois sites front ci-dessus**.

**Mise à jour du 24/08/2026 (E16US005) — deux des trois sites front sont refermés.** Les écrans de
placement et de duels ne déclarent plus chacun leur `POSITIONS` : la constante est **unique**, dans
`placement/presentation.ts`, avec son marqueur `// DETTE-010`. C'est une résorption **partielle** et
non un contournement : là où la revue cherchait l'oracle qui détecterait une divergence entre les
deux copies, le geste juste était de n'en avoir qu'une — le plafond se délestera en **un** endroit
côté plan. Restent à traiter : les **deux écrans de gabarit** (`PLAFONDS = [1,2,3,4]`) et le marqueur
manquant sur `gabarit_salle.py`, qui est la source du plafond.

Deux items **s'ajoutent** en revanche au périmètre de résorption :

- les deux `Math.min(cible.capacite, POSITIONS.length)` qui bornent la grille de couloirs — à
  retirer quand le plafond sera délesté ;
- le test `Placement.test.tsx` « un désaccord front/serveur sur le plafond n'ouvre pas de colonne
  fantôme » : il **doit tomber** à la résorption, et il faudra le **retourner** plutôt que le
  supprimer. *(Il n'a volontairement pas de miroir côté duels : depuis que la constante est unique,
  une seconde copie du même test ne détecterait rien de plus — la 1ʳᵉ rédaction de cette mise à jour
  en annonçait un, qui n'existait pas.)*

⚠️ Le périmètre ne se **réduit** pas pour autant : la grille CSS (`repeat(var(--couloirs, 4), …)`)
n'a jamais porté le plafond — le `4` qu'elle contient n'est qu'un repli si la propriété manque.
...
### DETTE-099 — un ajustement du plan de duels peut atterrir sur un autre tour

**Constat.** Les trois écritures du plan de duels (`deplacer`, `regenerer`, `placer_les_restants`)
ne reçoivent **pas** le tour visé : `_charger` le recalcule à chaque appel par `tour_a_poser`. Tant
que le plan était unique par phase (E03US009), il n'y avait rien à confondre. Depuis E03US012, le
tour posé **change tout seul** : le dernier duel du tour validé par un scoreur suffit à faire
basculer le plan sur le tour suivant, sans que l'organisateur ait touché à quoi que ce soit.

**Pourquoi ça compte.** L'organisateur ajuste le plan du tour N, quelqu'un valide ailleurs, il lâche
son jeton — et l'écriture s'applique au tour N+1. Deux issues, toutes deux mauvaises : si l'archer
dispute aussi le tour suivant, la pose part **silencieusement** sur le mauvais tour ; sinon il reçoit
un `InscriptionIntrouvable` dont le message ne dit **pas** que le tour a changé sous lui. La fenêtre
est étroite — quelques secondes autour de la fin d'un tour —, mais c'est justement le moment où
l'organisateur regarde le plan.

**Remède.** Faire porter le `tour` aux requêtes d'écriture et refuser en **409** s'il ne correspond
plus, avec un message qui dit de rafraîchir. La **réponse** le porte déjà (`PlanDeDuelsReponse.tour`,
E03US012), donc le front l'a sous la main : ~10 lignes serveur, un champ dans trois requêtes, aucun
pattern. C'est le patron « recalculé dans la file, jamais cru sur parole » d'ADR-0040/0056, appliqué
à l'ajustement.

**Pourquoi pas dans l'US courante.** Il change le **contrat de trois routes** et la forme des
requêtes côté front : c'est une tranche, pas un correctif de revue. Marqueur `DETTE-099` posé sur
`deplacer`.

### DETTE-098 — le port du QR s'appelle « rattachement » alors qu'il sert une session

**Le raccourci.** `GenerateurDocumentsSalle.qr_rattachement(url)` (`backend/domain/ports.py`) a été
**réutilisé** par E16US015 pour le QR de session d'un scoreur, au lieu qu'un second port soit ajouté.
Le choix est bon et reste défendu : le port reçoit une URL **déjà composée**, un second n'aurait
dupliqué qu'un nom (règle 2, [ADR-0105](adr/0105-le-qr-d-un-scoreur-porte-son-code-revele-un-a-la-fois.md)
§ Conséquences). Ce qui n'a pas suivi, c'est le **nom**.

⚠️ **Pourquoi c'est une dette et pas une coquille.** `docs/glossaire.md` réserve « rattachement » au
lien **appareil ↔ lieu** (`poste.code`) et dit le scoreur **itinérant, rattaché à aucune cible**. Le
symbole affirme donc le contraire du glossaire, et il a déjà coûté : E16US015 a dû **généraliser
trois docstrings** pour qu'elles restent vraies. Même espèce que `DETTE-011` (`Score` pour un agrégat
mono-flèche), `DETTE-042` (`position` pour un couloir de tir) et `DETTE-053`.

**Ce que ça rendra coûteux.** Le geste de délimitation du monde des postes est `grep -r rattachement` :
il rend désormais un faux positif. Et le port étant générique, un **3ᵉ appelant** (QR d'écran de
salle) consacrerait le contresens au lieu de le révéler.

**Résorption.** Renommage mécanique `qr_rattachement` → `qr_svg(url)` : 1 port, 1 adapter, 3
appelants, les doublures de test. **Zéro changement de comportement**, aucune migration. À prendre
dans une US de nettoyage, ou en passant sur la prochaine US qui touche `documents_salle`.

*(Inscrite le 05/09/2026. L'écart avait été identifié, différé et documenté en ADR et au glossaire
dès la 2ᵉ passe de revue — mais **pas** au registre : choisir « rien » sur un écart de nommage, c'est
accepter une dette, et une dette acceptée s'inscrit. Relevé par C2 et D en 3ᵉ passe.)*
...
**E16US015 (04/09/2026) — 3ᵉ consommateur, et le premier à encoder un secret PERSONNEL.** Le QR de
session d'un scoreur (`_url_scoreur`) bâtit `{origine}/scoreur#code=<code>` sur la même `base_url`.
⚠️ **La conséquence n'est plus la même que pour une cible** : là, une origine fausse ne coûte qu'un
QR mort (le filet de re-rattachement tombe) ; ici, le QR **contient quand même le code personnel**
du scoreur et renvoie son téléphone sur lui-même — l'admin ne voit rien, le scoreur ne comprend pas.
Le repli reste doux (retaper le code, qui figure sur sa carte), d'où une sévérité inchangée. La
parade est identique et **la recette la rappelle explicitement** (`docs/fonctionnel/E16US015.md`,
« Ce qu'il faut avant de commencer »). La résorption unique — base URL publique configurable —
bénéficiera désormais aux **trois** sites d'un coup.
...
`# DETTE-016` posé sur les **quatre** sites (les trois services qui construisent un poste, plus `_encaisse` du décompte de suppression — élargi le 19/09/2026 par E01US026) (`ServiceInscriptions.desinscrire`,
`ServiceDeparts._remboursements_des_payees`).

### DETTE-017 — `_AUTEUR_ADMIN` dupliqué sur 5 sites

**Constat.** L'auteur des entrées d'audit d'un acte **administrateur** est la constante locale
`_AUTEUR_ADMIN = "Administrateur"`, désormais **dupliquée** sur **cinq** sites applicatifs :
`application/paiements.py` (marquage de paiement, E08US002), `application/placement.py` (régénération
massive du plan, E12US007), `application/remboursements.py` (traitement d'un remboursement,
E08US005), `application/pilotage_tour.py` (lancement d'un tour, E12US002) et
`application/forfaits.py` (forfait de duel déclaré depuis l'administration, E16US008). La règle du
projet (« dupliquer une 2ᵉ fois et attendre le 3ᵉ cas ») place le **seuil de factorisation au 3ᵉ
cas** — dépassé de deux crans.

**Conséquence.** Faible : le littéral est stable et trivial. Mais le pronostic de cette section
s'est **vérifié deux fois** — « un 4ᵉ producteur d'audit admin re-dupliquera par mimétisme » —, et
un changement de libellé se ferait aujourd'hui en cinq endroits cohérents. C'est une dette de
**conception** (un invariant — « l'identité de l'admin dans l'audit » — qui n'a pas de domicile
unique), jumelle de DETTE-006.

⚠️ **Le 4ᵉ site n'a jamais été inscrit** : `pilotage_tour.py` (E12US002) portait `_AUTEUR_ADMIN`
sans que cette section ni la table ne le sachent — découvert le 28/08/2026 au cadrage d'E16US008,
en cherchant s'il existait une convention à réemployer. C'est l'aggravation silencieuse que le
§ Dette de `CLAUDE.md` proscrit, et elle a une conséquence concrète : qui aurait résorbé cette
ligne aurait extrait trois sites et laissé le quatrième derrière lui, avec un registre affichant
« résorbée ».

⚠️ **Le 5ᵉ site change la nature de la dette** : `AUTEUR_ADMIN` y est **publique** (sans `_`),
parce que la frontière API la lit pour tracer un forfait déclaré par l'organisateur. La duplication
n'est donc plus un détail interne à la couche application — un consommateur extérieur en dépend
désormais, ce qui rend l'extraction **plus** utile, pas moins.
...
(`application/`), **5** appelants, zéro changement de comportement. Marqueur `# DETTE-017` sur les
sites. ⚠️ Le grep de résorption doit chercher le **littéral** `"Administrateur"` et pas seulement le
nom de la constante : c'est ce qui aurait attrapé `pilotage_tour.py` dès E12US002.

### ✅ DETTE-018 — la suppression d'archer perd les remboursements (résorbée le 19/09/2026)
...
la pose persistée des cibles des tours ≥ 2 sera livrée, la garde devra être levée **aux quatre endroits**, sous
...
l'autre bout, et rien ne reliait l'US de placement à la garde qu'elle devra lever.

**Résorption attendue.** Les deux déclencheurs annoncés — les canaux publics du routage (`D-09`) —
sont **passés sans 3ᵉ site** : ils ont réemployé `ServiceRoutage` au lieu de recopier sa lecture.
Le périmètre réel est aujourd'hui de **quatre** sites, et aucune US ne le porte.
...
appelants, zéro changement de comportement. Marqueurs `DETTE-019` sur les **quatre** sites : `routage.py`, `pilotage_tour.py`, `frontend/src/features/routage/presentation.ts` (`partitionner`) et `frontend/src/features/feu-vert/etat.ts` (`actionDuel`). **L'US qui lèvera la garde doit balayer les quatre**, sans quoi le registre affichera « résorbée » à tort.

⚠️ **Fait le 05/09/2026 par E03US012** ([ADR-0106](adr/0106-la-pose-d-une-cible-appartient-a-un-tour.md)) : les quatre sites ont été balayés dans le même diff. Nuance relevée en revue, et elle compte pour lire cette ligne : **trois** portaient réellement la garde (`_duel_a_venir`, `_pose_a_annoncer`, `actionDuel`) ; le quatrième, `partitionner`, n'en portait que la **justification écrite** — son corps partitionne sur l'issue et n'a pas changé. Reste ouvert le seul reliquat : `_sources_en_attente`, écrit deux fois à l'identique, **deux sites backend**. Les marqueurs `DETTE-019` du front ont été retirés avec la garde.

⚠️ **Un quatrième site depuis E16US008 (28/08/2026), et il n'est pas dans le backend.** Le feu vert donne
désormais, sur chaque ligne bloquée, l'action qui lève le manquement — sauf pour « cible non
attribuée » **au-delà du tour 1**, où aucune action n'existe puisque la garde décrite ici interdit
d'attribuer une cible. `frontend/src/features/feu-vert/etat.ts` teste donc `duel.tour > 1` et
affiche la limite (`SANS_RECOURS_HORS_TOUR_1`) au lieu d'un lien qui n'aboutirait pas.

Ce n'est **pas** une troisième copie de la garde — c'est sa conséquence, rendue lisible à
l'organisateur. Mais pour la résorption l'effet est identique, et il est même pire : le jour où le
la pose persistée des cibles des tours ≥ 2 lèvera la garde côté serveur, un front laissé en l'état
annoncera une limite abolie,
sur l'écran le plus chaud du jour J, **sans qu'aucun test ne rougisse** — la couture serveur ↔ front
n'est tenue par rien. À reprendre **en même temps** que les trois autres sites (deux backend, un front).
...
⚠️ **Élargissement du 05/09/2026 (E03US012, ADR-0106 §6).** Tant que seul le tour 1 pouvait être
« prêt », ce défaut ne pouvait se manifester qu'au premier tour — celui où l'organisateur est
devant son écran et où les cibles viennent d'être générées. Depuis que **chaque** tour reçoit ses
cibles, il vaut jusqu'à la finale, c'est-à-dire là où deux archers séparés se remarquent le moins
(il ne reste que deux buttes occupées) et où l'erreur coûte le plus. Rien n'a été ajouté au
défaut ; c'est son domaine d'application qui a changé, et c'est ce qui justifie de relire cette
ligne avant la prochaine fenêtre de résorption plutôt qu'après.
...
| Phases du tournoi | `/tournois/{id}/phases` | ❌ scopé tournoi |
...
### ✅ DETTE-026 — une source de phase est ancrée par `ordre`, pas par identité *(résorbée le 20/09/2026)*

**Soldée par `E05US022`** ([ADR-0078](adr/0078-la-sequence-s-ancre-sur-l-identite-de-l-etape.md)).
Les quatre écrivains ont disparu : `_remapper`, `_realigner_avancements`, `_decaler_dun_cran`
et `PhaseRepository.reordonner` n'existent plus ; `DerouleRepository.reordonner` est devenu
`enregistrer_plusieurs`, une écriture d'ensemble qui ne contourne plus rien — l'unicité
`(tournoi, ordre)` ayant été **levée** (arbitrage du 20/09/2026). Il n'y a plus de remappage à
faire, donc plus de remappage à **oublier** : c'est la charge de vigilance qui est retirée.

⚠️ **Ce que la résorption n'apporte pas** : l'asymétrie format ↔ édition (ADR-0078 §3) n'est
tenue par **aucun test mécanique** — seulement par le typage et par `_source_json`, qui choisit
sa clé sur la classe. Réunir les deux formes « puisque c'est la même notion » reste possible.

*Ce qui suit décrit la dette telle qu'elle était.*
...
conséquence y est plus grave que partout ailleurs.** … `reordonner` fait deux écritures,
`supprimer` en fait trois … le rang **est** la clé de jointure, donc une phase restée sur son
ancien ordre pointe **l'étape voisine** et le créneau exécute un autre barème, sans la moindre
erreur.~~

**⚠️ Rectifié le 20/09/2026 par E05US022 ([ADR-0078](adr/0078-la-sequence-s-ancre-sur-l-identite-de-l-etape.md)),
et la ligne du tableau dit désormais la même chose que ce paragraphe.** Le rang n'est plus la clé
de jointure : une phase désigne son étape par `etape_id`. Deux conséquences.

- **`ServicePhases.reordonner` sort de la dette** : `_realigner_avancements` n'existe plus, et le
  geste tient en **une** écriture atomique (`DerouleRepository.enregistrer_plusieurs`).
- **`ServicePhases.supprimer` y reste, avec trois écritures** — retrait des avancements (désormais
  par identité), retrait de l'étape, recompactage des rangs —, mais ce qu'une panne laisse entre
  elles est une **numérotation trouée à l'écran**, plus un barème joué de travers. La dette cesse
  de pouvoir **fausser des résultats** ; elle interrompt une configuration, comme partout ailleurs.
...
concret**, patron `consigner_dans`. Marqueurs `# DETTE-025` posés sur la boucle de suppression
**et** sur la recopie de l'exigence dans `ServiceFormats.appliquer`, et sur les **trois écritures**
de `ServicePhases.supprimer` — soit les cinq écritures que le remède devra réunir. *(Le marqueur de
`ServicePhases.reordonner` a été retiré le 20/09/2026 : une écriture, atomique, plus un site.)*
...
### DETTE-029 — l'attribution des rangs ex æquo est **dupliquée de site en site**

La règle « deux entrées de même clé partagent le rang, on repart à `index + 1` dès que la clé change »
(sauts 1-2-2-4) existe désormais en **cinq** exemplaires — ⚠️ **le titre de cette section ne porte plus le compte** : il l'a porté jusqu'à E16US017, si bien que chaque site ajouté changeait l'ancre du lien qui pointe ici, et le lien du tableau était déjà mort depuis E06US004 — : `classement._ranger` (E01US012),
`poule.classement_de_poule` et `suisse.classement_suisse` (E05US015), `palmares._numeroter`
(E06US004) et `classement_clubs._ranger` (E16US017). La propagation du drapeau
...
**Ce que cela coûte.** Les sites **divergent déjà**, et sur plusieurs axes : `classement._ranger`
ne porte aucun drapeau `ex_aequo` (les deux de E05US015 si), `palmares._numeroter` exprime le rang
en couple `(min, max)` plutôt qu'en rang + drapeau, et `classement_clubs._ranger` (E16US017) range
sur un **triplet de médailles** au lieu d'un score — sa clé de tri porte en outre un libellé qui
**n'est pas un départage**, seulement un ordre d'affichage déterministe. Corriger la règle demande
désormais **cinq** modifications coordonnées, et un oubli produit un classement **cohérent et
faux** — le défaut qui ne se voit pas.

⚠️ **Chaque US qui ajoute un site élargit cette ligne**, elle n'invente pas de contournement local
(§ Dette de `CLAUDE.md`). C'est ce qu'ont fait E06US004 puis E16US017.
...
**Résorption attendue.** US `refactor/` dédiée. Marqueur `DETTE-029` aux **cinq** sites — le jeton se cherche au `grep DETTE-029`, où qu'il vive dans le commentaire.
...
### DETTE-100 — la préséance de rôle n'est déclenchable par aucun écran

⚠️ **Section réécrite en revue le 12/09/2026.** La 1ʳᵉ rédaction reposait sur un fait **faux** —
« aucun écran ne fait se croiser deux rangs » — et quatre axes l'ont réfutée. Ce qui suit est ce
qui reste vrai après correction du défaut qu'elle masquait.

**Le constat.** ADR-0107 refuse une écriture d'un rôle **inférieur** à celui qui a déjà écrit la
volée. Deux croisements de rangs sont possibles, et **ni l'un ni l'autre n'est joignable depuis un
écran** :

- **admin → poste** : l'unique appelant front de `POST /saisie/volees` est `features/saisie/api.ts`,
  qui passe la portée `'poste'` **en dur**. Aucun écran d'administration n'écrit de volée, et deux
  tablettes sont à rang égal (décision 3).
- **scoreur → poste** : le seul endpoint qui fasse écrire le rang du milieu est
  `POST /saisie/corrections`, et **aucun fichier de `frontend/src` ne le cite**. L'écran scoreur
  livré par `E16US019` (`PanneauValidationQualif.tsx`) ne fait que valider, annuler et refermer — il
  le dit lui-même en tête : *« la correction ne se fait pas ici »*.

⚠️ **La 2ᵉ passe de revue avait remplacé ce constat par un faux** — « le scoreur écrit au rang 2
depuis l'écran d'`E16US019` » — et six artefacts l'ont porté. Ce que la revue avait réellement
corrigé, c'est la **règle serveur** (`corriger_volee` ne posait aucune préséance, donc un rang 1
écrasait un rang 2) ; aucune surface n'est apparue. La leçon est celle que `stories/E16` portait
déjà : *j'avais corrigé un énoncé qui était juste*. Le réflexe manquant tient en un mot — chercher
l'**appelant**, pas raisonner sur l'endpoint.

**La conséquence.** Le rendu ambre de `MessageErreurSaisie` et l'`onError` qui relit la vérité
serveur sont corrects et testés, mais aucun marqueur ne les a vus en salle.

**Pourquoi ce n'est pas corrigé dans l'US.** Ouvrir une surface d'administration qui saisit des
volées de qualification est une **US à part entière** — où elle vit, ce qu'elle montre, comment elle
cohabite avec la tablette. Le périmètre d'`E16US020` a été arbitré le 12/09/2026 : la règle, pas la
surface.

**Résorption — et ce qu'elle ARME au passage.** Il n'y a rien à défaire, mais ce n'est pas pour
autant gratuit : le jour où un écran d'administration écrit, le refus atteint aussi le **rejeu
hors-ligne**, où `draineLaFile` retire la volée et journalise **sans rien dire au marqueur** (perte
assumée par [ADR-0037](adr/0037-file-de-saisie-hors-ligne-et-rejeu.md)). Une dette dont la résorption ouvre un
chemin muet doit le dire avant, pas après.

**Critère de fin** : un appelant de `saisirVolee` en portée `'admin'` ; le refus rendu **visible**
sur le chemin de rejeu ; et le scénario de recette de l'US d'introduction déroulable **sans `curl`**
(cf. [`fonctionnel/E16US020.md`](fonctionnel/E16US020.md)).

⚠️ **Ce qui ne résorbe PAS cette ligne** : ouvrir la route de saisie au **scoreur**. Le rang se
calcule dans `api/v1/saisie.py` par `Role.ADMIN if poste is None else Role.POSTE_DE_CIBLE` —
élargir la garde **sans ajouter de branche** y donnerait au scoreur la préséance de l'organisateur.
`test_un_scoreur_n_est_pas_une_identite_de_saisie_de_qualification` rougira ce jour-là.
*(Formulation reprise en 3ᵉ passe de revue : la déduction vivait au service, elle est remontée à la
frontière — le piège est le même, l'endroit a changé.)*

⚠️ **Voisine de [`DETTE-090`](#dette-090--aucun-écran-nannule-un-forfait-de-duel)** par la forme —
une capacité servie par le serveur qu'aucun écran n'expose — mais ni la portée, ni la cause, ni le
critère de fin ne se recouvrent : les fondre ferait de `DETTE-090` un fourre-tout et lui ferait
perdre son critère. À relire ensemble le jour où la famille atteint **trois** membres ; c'est alors
qu'une question structurelle se posera, pas avant.
...
⚠️ **Aggravation du 19/09/2026 — `E06US009`.** Le palmarès rend une section **par créneau** : sa
route, publique et pollée toutes les 30 s par chaque tablette, paie désormais `N` fois le classement
et la reconstruction des arbres. Le facteur est le nombre de départs. ⚠️ **La nuance compte** — ce
n'est pas du travail jeté : avant l'US, la route ne calculait qu'un créneau **et** n'affichait qu'un
podium sur quatre. On paie ce qu'on montre, ce qui n'était pas le cas du symptôme décrit plus bas.
Ce qui a bien été retiré, lui, c'est la lecture du référentiel des clubs, qui serait partie `N` fois
pour un résultat identique : elle est hissée dans `rendu`, hors de la boucle.

⚠️ **Rectification du 19/09/2026, en revue de l'US qui vient de l'écrire.** L'affirmation « ce n'est
pas du travail jeté » ne vaut **que pour le classement et les arbres**. `ServiceClassement.pour_phase`
relit `archers.par_tournoi`, `categories.par_tournoi`, les forfaits et `tournois.par_id` : quatre
lectures **à la maille tournoi**, rigoureusement identiques d'un créneau à l'autre, désormais faites
`N` fois par requête sur une route publique pollée. C'est exactement le mode de panne que l'US venait
de corriger pour `_libelles_club` — et il est resté invisible parce que le seul compteur du dépôt,
`_ClubsComptes`, ne compte que les clubs. **La leçon dépasse la ligne** : un correctif calibré sur ce
que le détecteur sait voir ferme le symptôme, pas la classe.
...
**Élargissement du 31/08/2026 — E16US014.** `ServicePalmares._libelles_club` lit **tout le
référentiel des clubs** pour nommer les podiums de club — le PDF doit les titrer et n'a pas d'écran
pour résoudre les identifiants à sa place. ⚠️ **Bornage levé le 04/09/2026 — E16US017.** Il disait : « la lecture n'a lieu que si la
portée `club` est réglée, or le défaut est *catégorie* seule — le cas courant ne paie donc
rien ». Le motif est tombé : le **classement des clubs** (ADR-0104) doit **nommer** ses clubs
dès qu'une portée **inter-club** est réglée, donc dès le défaut. Ce qui reste vrai, et c'est
tout : un tournoi qui ne récompense **rien** (`portees` vide) n'entraîne aucune lecture.
Inscrit ici plutôt qu'assumé en commentaire, ce qui est exactement le défaut fondateur de cette
ligne. Au passage, `GET /palmares` **économise** une lecture : `ServicePalmares.rendu` rend le
palmarès, sa restriction et le réglage en un seul `par_id`, là où la route en faisait deux.
...
⚠️ **Réexaminée le 22/08/2026 par `E16US002`** — l'US qui a levé le refus de la planche A07, dont
l'exigence `P-4` est l'origine de cette dette — et **maintenue hors périmètre par arbitrage du
commanditaire** : le remède touche la politique `Depth` côté domaine, soit un chantier moteur dans
une US déjà large côté IHM. À savoir en lisant `EPIC-16`, qui annonce désormais « aucun écran
refusé » : le refus portait sur la fiche, pas sur `P-4`, qui reste dû. *(Même remarque de placement
que pour DETTE-054 : hors de la colonne moissonnée par l'atlas.)*
...
### ✅ DETTE-039 — la cadence et la taille d'une page projetée sont **en dur** *(résorbée le 26/08/2026)*
...
✅ **Résorbée par `E16US009`** (26/08/2026, [ADR-0098](adr/0098-un-ecran-projete-pagine-au-lieu-de-defiler.md)).
Les marqueurs sont retirés ; la section est **conservée** parce que son raisonnement se reposera : la question
« ce réglage appartient-il à la vue ou à l'écran ? » s'est tranchée en faveur de l'**écran** (diagonale, distance,
longueur des noms sont des propriétés du **lieu**), et le prochain réglage d'affichage projeté devra se poser la
même question.

⚠️ **Ce que la résorption ne couvre pas.** La ligne portait deux choses : un **raccourci** (valeur en dur, non
réglable) et une **incertitude** (valeur jamais mesurée en salle). Seule la première est levée. La seconde est
désormais un geste d'exploitation — l'écran d'admin l'énonce sous les deux champs — et **une relecture humaine sur
vidéoprojecteur reste requise** pour savoir si 40 noms et 3 lignes de tête tiennent réellement à dix mètres.
...
⚠️ **2ᵉ livraison d'un bug par cette classe, le 20/09/2026 (E05US022) — et la première à en
livrer quatre d'un coup.** *(La 1ʳᵉ est celle de la veille, inscrite à la ligne du tableau.)*
En passant l'ancrage d'une source du **rang** à l'**identité** d'étape, l'US a laissé **trois**
appelants sur l'ancienne clé — `palmares._resultat_classant`, le 1..N final du palmarès, et
`completude._places_de_la_phase` — plus la clé d'amorçage du cache de `saisie_duels`.
`EtapeDerouleId` étant un alias d'`int`, **mypy strict n'a rien vu** ; et les décors faisaient
coïncider identité et rang, si bien que **4376 tests verts** couvraient du code cassé (poules et
suisse disparaissant du palmarès dès un réordonnancement). Les quatre sites sont corrigés et les
décors décorrélés **aux deux étages** : en mémoire, `poser_phase_factice` impose
`identite_d_etape(ordre, tournoi_id)` en 3 100+, disjoint des `PhaseId` en 101+ et de la séquence
de `FauxDerouleRepository` (9 000+) ; en base, `poser_phase_sql` brûle un identifiant sur une
étape éphémère du même tournoi, supprimée après la vraie pose, ce qui décale `id` de `ordre`.

⚠️ **La décorrélation SQL a une valeur mesurée** : elle a fait rougir **26 tests d'un coup**,
dont **25 dans `test_barrages_api.py`**, tous sur la même confusion de décor — le scénario
passait un `PhaseId` à une route qui adresse une `EtapeDerouleId`, vert tant que SQLite allouait
1 des deux côtés. Aucun n'était un défaut de production ; tous auraient masqué le prochain.
*(Mesure refaisable : retirer l'appel à `decaler_les_identites_sql` dans `poser_phase_sql` et
rejouer `pytest tests/`.)*

⚠️ **Deux faits de plus, trouvés en 4ᵉ passe** : (a) le **contrat de conformité de ports**
n'était décorrélé qu'en apparence — trois identités brûlées pour cinq phases, donc
`PhaseId 4 == EtapeDerouleId 4` ; porté à dix, plus une assertion de **disjonction** des deux
espaces, qui est le vrai invariant ; (b) `_IDENTITE_BRULEE` est passée de 5 000 à **20 000**,
5 000 tombant à l'intérieur de la bande conventionnelle `[3 100 ; 8 999]`.

⚠️ **Portée exacte de « décorrélé », parce qu'une promesse trop large en vaut moins qu'une
promesse bornée** (3ᵉ passe de revue) : le décalage vaut pour les décors qui passent par
`poser_phase_factice`, `poser_phase_sql`, `test_phase_repository._poser` et le contrat de
conformité de ports — c'est-à-dire ceux qu'on a ouverts. **Un décor qui écrit `deroule_etape`
lui-même sans brûler d'identifiant recoïncide**, et rien ne le signale hors du helper :
`poser_phase_sql` porte désormais un `assert etape.id != etape.ordre`, mais il ne voit que ce qui
passe par lui.

⚠️ **`Phase.etape_id` reste `| None` côté domaine alors que la colonne est `NOT NULL` depuis la
migration `0056`** : neuf replis défensifs muets en production (`domain/phase.py`,
`application/{completude,palmares ×3,saisie,saisie_duels}.py`, les deux adapters). Aucun n'est un
bug aujourd'hui ; les retirer touche `Phase.creer` et tous les décors qui fabriquent une phase
sans étape, donc cela appartient à l'US `NewType`, pas à celle-ci.

La classe **reste ouverte** pour autant : rien n'empêche le décor suivant de refaire la
confusion, et aucun outil ne la voit.
Remède inchangé et désormais **justifié par un cas réel** : `NewType` sur les identifiants, à
traiter en US dédiée — mypy attraperait alors la confusion mécaniquement.
...
### ✅ DETTE-045 — le palmarès et la simulation ne voyaient que le premier départ *(résorbée le 19/09/2026)*

**Soldée par `E06US009`.** Le palmarès rend **N sections, une par créneau** ; `_premier_depart` n'existe plus ; le rejeu de simulation et la **session de simulation pilotée** rejouent chaque créneau. ⚠️ **Un troisième site a été trouvé au cadrage** et n'était tracé nulle part : `application/pilotage_simulation.py` (`_etat`), le **seul des trois atteignable en production** — le site tracé (`simulation.py`) n'a, lui, aucun appelant de production. Le registre citait par ailleurs `simulation_format.py`, qui ne portait aucun marqueur et n'était pas concerné : il ne fabrique qu'un créneau, et son unicité est désormais **gardée** (`EtatSession.creneau_unique` lève au lieu d'indexer). Le classement des clubs est juxtaposé comme les podiums — arbitrage du commanditaire du 19/09/2026, qui **amende ADR-0104**.

⚠️ **Résorbée affichage ET moteur** — mais il a fallu trois passes de revue pour y arriver. Les deux
premières ne corrigeaient que la **portée d'affichage** : la session rendait `N` créneaux, mais son
harnais écrivait toutes ses volées dans **une** qualification (`qualification_du_tournoi`), si bien
que les autres sortaient avec leurs inscrits rangés à zéro et un arbre joué sur ces zéros. La 3ᵉ
passe a relevé que le CA avait été **rétréci** pour épouser ce code ; le commanditaire a tranché le
20/09/2026 pour la correction du moteur. `SessionSimulation` porte désormais une résolution
`archer → phase`, et les deux créneaux tirent réellement (mesuré avant/après).

Ce qui suit décrit la dette **avant** sa résorption.

**Où** : `backend/application/palmares.py` (`_premier_depart`), `backend/application/simulation.py`,
`backend/application/simulation_format.py`, **+ E16US017** : `backend/domain/classement_clubs.py` (`classer_clubs`).

⚠️ **Aggravation du 31/08/2026 — E16US014.** Jusqu'ici le symptôme était « le podium du matin »
pour une catégorie : le raccourci était réel mais **implicite**. Les deux portées livrées par
E16US014 le rendent **explicite et faux** — un bloc titré « **Toutes catégories** » annonce le
podium *du tournoi* en n'ayant vu qu'un créneau, et un podium *de club* est structurellement amputé,
un club répartissant typiquement ses archers entre créneaux (jeunes le matin, adultes
l'après-midi). Le mensonge cesse d'être une omission : il est écrit dans le titre du bloc. Marqueur
`DETTE-045` posé au point d'appel de `_premier_depart` dans `ServicePalmares._calculer`. La
résorption reste **E06US009**, dont cette aggravation renforce l'urgence.

⚠️ **Aggravation du 04/09/2026 — E16US017** ([ADR-0104](adr/0104-le-classement-des-clubs-se-compte-en-medailles-inter-clubs.md)).
Un cran de plus, et d'une autre nature : le classement des clubs **agrège** là où un podium ne
faisait que restreindre, et il ne rend pas un rang affiché mais **un lauréat unique** — le club
à qui l'on remet physiquement un trophée. Un club qui répartit ses archers entre créneaux
(jeunes le matin, adultes l'après-midi — le cas que cette ligne décrit déjà comme typique)
n'apporte au décompte que ses médailles du matin, et le classement s'annonce **définitif**
(`provisoire=False`) dès le premier créneau terminé. C'est la seule surface du produit qui
nomme un vainqueur global. Marqueur `DETTE-045` posé sur `classer_clubs`, au point où
l'unicité du lauréat naît. ⚠️ **Relevé en revue** (axes C1 et C2) : l'US avait d'abord jugé que
la ligne existante couvrait le cas sans l'élargir — elle le couvrait sur les **sites**, pas sur
le **symptôme**, et personne n'aurait su qu'il fallait rouvrir `classement_clubs.py` à la
résorption.
...
⚠️ **Ce remède a été ÉCARTÉ, et c'est une décision, pas un oubli.** `E06US009` rend `N` sections en **une** réponse et n'ouvre aucune route par départ — [ADR-0111](adr/0111-le-palmares-rend-toutes-ses-sections-en-une-reponse.md), écrit en revue précisément parce que la décision contredisait ce que le registre avait inscrit ici. Laisser la phrase ci-dessus sans ce renvoi faisait du registre sa propre contradiction — le cas que la section voisine qualifie de « pire des cas : il se lit sans effort, et il est faux ».
...
**Le tri des appelants de `qualification_du_tournoi`, déplacé ici en E00US027.** Il vivait dans un
commentaire de `portee.py` ; le retirer sans le reverser aurait détruit la seule trace du décompte,
et `portee.py` n'en garde plus qu'un renvoi d'une ligne. Cinq appelants, deux catégories :

- **portés au créneau** ([ADR-0082](adr/0082-le-palmares-est-une-lecture-de-departs.md)) —
  `forfaits`, `feuille_de_marque._bareme_du_creneau`, `saisie.avancement_cible`, et les **deux**
  comptages de `completude` ;
- **replis assumés, justes tels quels** — `saisie._phase_qualification_ou_none`, qui ne s'en sert
  que lorsqu'aucun créneau n'est résoluble, et `pilotage_simulation`, qui simule un déroulé
  mono-qualification.

⚠️ **Ce tri a été faux à sa première rédaction, et le compte tombait juste par compensation** : il
annonçait six appelants en comptant `forfaits` deux fois et en oubliant `feuille_de_marque` — d'où
une grille de 20 volées imprimée pour un tour de 15. **Recompter par `grep` avant d'ajouter ou de
retirer une ligne ici** : une énumération fausse est pire qu'absente, la prochaine US s'y fiera.
...
résolvent par `par_depart_et_type(depart_id, QUALIFICATION)`.

⚠️ **Cette dette ne porte PAS le harnais de simulation.** `E06US009` lui avait d'abord renvoyé le fait que la session pilotée n'écrive ses volées que dans une seule qualification — à tort : ici il s'agit des **forfaits**. Ce volet a été corrigé dans `E06US009` même (`SessionSimulation.qualif_par_archer`), sur arbitrage du commanditaire.

**Même famille que DETTE-045 et
...
### DETTE-101 — Le journal d'audit se charge en entier pour être filtré à l'écran

**Ce qui a été fait.** L'écran « Journal d'audit » livré par `E16US016` appelle
`GET /api/v1/tournois/{id}/audit`, qui rend **toutes** les entrées du tournoi, puis fait tout le
reste dans le navigateur : filtre par type d'acte, recherche libre, et pagination par tranches de
50. Le serveur n'a gagné **aucun** paramètre.

**Pourquoi.** Le périmètre a été arbitré avec le commanditaire le 18/09/2026, avant d'écrire une
ligne. Trois niveaux d'écran étaient sur la table ; la « version riche » — filtres cumulés auteur /
type / période, recherche incluse — a été écartée explicitement parce qu'elle demandait *« une route
de filtrage côté serveur, pas seulement du front »* pour un besoin non constaté. Le niveau retenu
(« liste + filtres simples + détail ») est tenable sans toucher au backend. Ce n'est donc pas un
raccourci pris en cours de route, mais le revers d'un arbitrage.

**Ce que ça coûte.** La planche A18 chiffre un journal réel à **1 284 entrées** pour une matinée. À
~250 octets de JSON par entrée, l'écran télécharge environ **300 Ko** à chaque ouverture — sur le
réseau local du jour J, depuis le PC de l'organisateur, c'est imperceptible. D'où la sévérité
**mineure**. ⚠️ Deux réserves honnêtes : (a) **le seuil de gêne n'est pas mesuré**, aucun journal
réel n'ayant été produit ; (b) le volume croît avec la **durée** du tournoi et le nombre de
validations, pas avec l'effectif — un tournoi sur deux jours à quatre départs n'est pas dans le même
ordre de grandeur que la planche.

**Ce qui n'est PAS en cause.** L'export, lui, ne passe pas par l'écran : `ServiceExportAudit` relit
le journal côté serveur et rend le fichier. Alourdir l'écran ne dégrade donc pas le document qui
sert réellement au litige.

**Résorption.** Porter le filtre et la pagination au port `AuditRepository`
(`par_tournoi(tournoi_id, action=None, limite=None, curseur=None)`), les remonter par
`ServiceAudit.lister` et la route, puis retirer le filtrage client de `Audit.tsx`.
⚠️ **La recherche libre ne suit pas mécaniquement** : `presentation.ts` replie casse **et
accents**, **pas les espaces** — « le guén » trouve « LE GUEN », « leguen » non. *(La 1ʳᵉ rédaction
disait l'inverse, contredisant le test livré à côté : corrigé en revue, axe D.)* SQLite ne le fait pas sans colonne normalisée — c'est
exactement l'obstacle de `DETTE-006` (`cle_nom`), déjà ouverte sur six sites. Les deux se traitent
ensemble ou pas du tout : une recherche serveur *sans* repli serait une **régression** visible pour
l'organisateur, sur la seule fonction que cet écran existe pour rendre.

### DETTE-102 — Deux libs de rendu échappent au typage strict

**Ce qui a été fait.** `backend/pyproject.toml` porte deux `[[tool.mypy.overrides]]` avec
`ignore_missing_imports = true` : `reportlab.*` (depuis E09US001) et `openpyxl.*` (depuis
`E16US016`). Sans eux, `mypy --strict` refuse l'import d'une lib sans `py.typed`.

**Ce que ça coûte, exactement.** La portée est bornée à l'infrastructure de rendu — la logique
maison (typage des cellules, neutralisation, montants, dimensions de grille) reste vérifiée. Mais à
l'intérieur de cette frontière, les annotations deviennent **décoratives** :
`_ecrire_ligne_xlsx(feuille: Worksheet, …)` annote contre un `Any`, `Workbook.active` est traité
comme non-`Optional` alors que les stubs le donnent nullable, et une faute de frappe sur le
garde-fou anti-formule (`data_type = "str"`) passerait le typage. C'est `test_tableur_grille.py`
qui rattrape au runtime — d'où la sévérité **mineure**, et non majeure.

**Pourquoi c'est une dette et pas un simple fait technique.** Le commentaire d'origine disait « même
cas, même remède que ReportLab ». C'est vrai du remède, faux du cas : **`types-openpyxl` existe et
est officiel** (typeshed), là où `types-reportlab` ne l'est pas. Pour openpyxl, l'écart est donc
**choisi** — une dépendance de dev de plus (règle 11) contre la vérification d'une quinzaine
d'appels — et non subi. Un arbitrage réversible qui n'est écrit nulle part est exactement ce que ce
registre existe pour porter. *(Relevé par deux axes de revue ; le précédent ReportLab n'avait jamais
été inscrit — cette entrée couvre les deux.)*

**Résorption.** Déclarer `types-openpyxl` au groupe `dev` de `pyproject.toml`, retirer le bloc
`openpyxl.*`, vérifier que `mypy --strict` reste vert. ⚠️ Le volet **ReportLab ne se ferme pas** au
même geste : il attend un paquet de stubs officiel, qui n'existe pas à ce jour.

### DETTE-103 — Trois replis de casse et d'accents divergents dans le front

> **Aggravée par `E17US007` (25/09/2026) : un 4ᵉ site**, `features/archers/accueil.ts`
> (`normaliser`, la recherche des inscriptions A09), copié sur la variante de `suivi.ts`. Le titre
> dit encore « trois » : c'est l'état au moment de l'inscription.

**Ce qui a été fait.** Trois features replient indépendamment casse et accents avant de comparer du
texte saisi : `features/suivi/suivi.ts` (recherche d'archer), `features/competition/barrage.ts`, et
`features/audit/presentation.ts` (recherche du journal, livrée par `E16US016`). Les trois font
`normalize('NFD')` puis retirent les marques, mais **aucune des trois ne le fait pareil**.

**Pourquoi c'est inscrit maintenant.** La 3ᵉ occurrence est le seuil que `CLAUDE.md` § Dette fixe
pour *proposer* un remède structurel — et la règle 16 dit que ce remède se traite en **ADR + US
dédiée, jamais en douce dans l'US courante**. L'arbitrage de ne pas le faire ici est donc le bon.
Ce qui manquait est le **porteur** : la constatation ne vivait que dans le corps de commit
d'`E16US016`, qui n'est ni greppable ni relu. Le registre a déjà écrit deux fois que c'est
insuffisant (`DETTE-074` : « une dette déléguée en commentaire de code n'est prise par personne » ;
`DETTE-094` : « honnête, mais pas efficace »). *(Relevé en 2ᵉ passe de revue, axe C2.)*

⚠️ **`E16US016` aggrave la divergence** : elle fait passer `presentation.ts` de `[̀-ͯ]` à
`\p{Mn}` — un changement justifié en soi, mesuré, et plus lisible — mais laisse `suivi.ts` sur
l'ancienne classe. Les deux sont équivalentes sur le français courant ; elles cessent de l'être sur
les marques rares.

**Ce que ça coûte.** Rien de cassé aujourd'hui : les trois replis donnent le même résultat sur un
nom d'archer français. Le coût est un **écart qui grandit** — chaque US qui touche un site sans
toucher les deux autres l'élargit, et c'est exactement ce qui vient de se produire.
...
**Où** : `backend/application/pilotage_simulation.py` — `_etat`, le calcul de `duels_total`
(marqueur `DETTE-109`) ; la barre est rendue par `frontend/src/features/simulation/Simulation.tsx`.

`duels_total` se dérive de l'effectif — `somme(max(0, effectif - 1))` par tableau —, ce qui compte
les duels d'un arbre à élimination **sans** son match pour la 3ᵉ place. `duels_faits`, lui, compte
tous les duels effectivement joués. Sur un tableau de quatre, cela fait 4 joués pour 3 annoncés.

**Mesuré en revue** (décor à deux créneaux de quatre archers) : `duels_faits=8`,
`duels_total=6` — soit une barre à 133 %.

⚠️ **Antérieure à `E06US009`**, dont le diff n'a pas changé la formule : il a seulement supprimé le
second parcours d'arbres qui la nourrissait. C'est en relisant cette ligne réécrite qu'un axe
adversarial a mesuré l'écart. Inscrite plutôt que corrigée dans l'US : redéfinir un compteur de
progression n'est pas du ressort d'une US de palmarès, et les deux compteurs doivent bouger
ensemble sous peine de déplacer l'écart.

### DETTE-108 — les DTO du front sont des miroirs manuels que rien ne valide

**Où** : `frontend/src/shared/api/client.ts` (`fetchJson`, marqueur `DETTE-108`), chaque
`frontend/src/features/*/api.ts`, et la coquille d'application (aucun `ErrorBoundary`).

`fetchJson<T>` **transtype** la réponse JSON en `T` ; il ne la valide pas. Les interfaces front sont
donc des copies manuelles des modèles Pydantic, sans lien mécanique avec eux. Quand le serveur
change un champ, **rien** ne rougit : ni `tsc` (le miroir ment de façon cohérente), ni `vitest` (si
la feature n'a pas de test de rendu), ni la porte.

**Effet, constaté** : `E06US009` a changé `EtatSessionReponse` (`classement`/`tableaux` →
`creneaux`) sans migrer `features/simulation/`. Résultat : `TypeError` au premier rendu, et comme
`frontend/src` n'a **aucun** `ErrorBoundary`, c'est **toute** l'application admin qui tombait en
page blanche — pendant que la porte complète rendait **14/14**. Les cinq axes de revue l'ont vu ;
aucun outil du dépôt ne le pouvait.

**Résorption, en deux gestes indépendants** :
- **(a)** un `ErrorBoundary` racine (~30 lignes, aucune dépendance) : il ne ferme pas la dérive,
  mais il **borne le rayon** d'un écran au lieu de l'appli entière. C'est le geste bon marché ;
- **(b)** valider réellement les réponses — types générés depuis `/openapi.json`, ou un validateur
  de schéma. C'est un **ajout de dépendance** (règle 11) : arbitrage du commanditaire, ADR et US
  dédiée.

⚠️ **L'option « rien » est défendable — pour (b) seulement**, et la distinction décidera : un seul
incident constaté, déploiement LAN mono-club, règle 12 (« la rigueur va au moteur métier, pas à
l'outillage »). Pour **(a)**, aucun de ces trois arguments ne s'applique : il ne coûte ni dépendance
ni ADR, et son coût-de-ne-rien-faire est **constaté**, pas pronostiqué. La question n'y est donc pas
« faut-il ? » mais « dans quelle US » — la première qui rouvre la coquille d'application. Ce qui
manquait n'était pas le remède mais l'**inscription** — la classe ne vivait que dans deux commentaires, dont
le renvoi (`DETTE-081`) désignait une tout autre dette.

### DETTE-106 — cinq orthographes du libellé de créneau

**Où** : `backend/domain/depart.py` (`libelle_creneau`) ; `frontend/src/features/departs/libelle.ts`
(`libelleCreneau`) ; `frontend/src/features/placement/PlanCiblesPublic.tsx` et
`frontend/src/features/inscriptions/InscriptionsArcher.tsx` (deux `libelleDepart` locaux) ;
`frontend/src/features/admin/BandeauContexte.tsx` (composé en ligne).

Le même créneau se nomme « Départ **n°**2 — 14:00 » côté serveur et « Départ 2 — 14:00 » côté front,
plus trois variantes locales. Les deux principaux **se déclarent chacun domicile unique** dans leur
propre docstring — « le libellé a un domicile unique depuis E01US026 » d'un côté, « **un seul**,
partout » de l'autre. Deux affirmations vraies séparément, fausses ensemble : ni l'un ni l'autre
n'avait regardé en face de lui.

**Effet.** Jusqu'à `E06US009` la divergence restait entre écrans. Depuis, elle est **sur le même
écran** : le palmarès titre ses sections avec la forme serveur (il le doit — le PDF est généré côté
serveur et n'a pas de front), tandis qu'un sélecteur de créneau voisin affiche la forme front.

⚠️ **La forme serveur ne peut pas simplement changer.** Elle est persistée **verbatim** dans
`remboursement.creneau` ([ADR-0057](adr/0057-registre-de-remboursements.md)) pour
survivre à la suppression du départ : la modifier ferait cohabiter deux orthographes dans la même
colonne d'un registre d'argent — ce que le commentaire de `libelle_creneau` avertit déjà d'éviter.
C'est donc le **front** qui doit s'aligner, pas l'inverse.

**Pourquoi ce n'est pas traité dans `E06US009`.** Unifier cinq sites est un remède structurel : il se
traite en ADR + US dédiée, jamais en douce dans l'US courante. `E06US009` a fait le minimum juste —
utiliser l'existant (`Depart.libelle_creneau`) plutôt que d'inventer une sixième orthographe.

**Résorption.** Aligner les quatre sites front sur la forme persistée ; `PlanCiblesPublic` est une
duplication **pure** de `libelleCreneau` et disparaît par simple import. Les deux composites
(`BandeauContexte` : état ; `InscriptionsArcher` : tarif) gardent leur suffixe mais partent du même
préfixe. ⚠️ **Partout où le front ne dispose pas du créneau, le libellé doit descendre dans la
réponse** — c'est ce qu'`E06US009` a fait pour le seul palmarès.
...
### DETTE-104 — des copies locales de `FauxTournoiRepository`

`backend/tests/conftest.py` porte une doublure **partagée** de `TournoiRepository` depuis E16US012,
et son en-tête dit déjà que « ~25 autres modules en gardent une copie locale ». Ce que E01US026
apporte, ce n'est pas la découverte, c'est le **chiffre** : ajouter **une** méthode au port
(`compter_descendance`) a demandé **25 éditions mécaniques** dans des modules qui ne testent pas la
suppression, plus 25 imports. Le même geste se rejouera à chaque élargissement du port.

**Ce que ce n'est pas.** Il n'y a **aucun risque de faux vert** : `mypy --strict` vérifie la
conformité structurelle au site d'appel, donc une copie non mise à jour fait échouer la porte
mécanique — ce qui est exactement ce qui s'est produit, et ce qui a rendu le coût visible.

**Résorption.** Rapatrier les copies sur `conftest.FauxTournoiRepository`, **dans l'US qui touchera
ces décors pour une autre raison**. Pas d'US dédiée : le geste est mécanique, sans valeur propre, et
une branche entière de renommage de doublures coûterait plus qu'elle ne rapporte (règle 12).
⚠️ **Aucun marqueur `DETTE-104` n'est posé en code** — un marqueur par copie serait précisément
la duplication qu'on déplore. La mesure est le `grep` de la colonne « Portée » (il compte aussi
`conftest.py`, d'où un fichier de plus que de copies).
**Résorbée en partie par `E17US012` (26/09/2026)** : trois décors touchés pour une autre raison
(`test_service_paiements`, `test_service_listes_impression`, `test_service_completude`) importent
désormais la doublure partagée — 25 copies → 22. *(Relevé en revue, axe C2 : le déclencheur
« l'US qui touchera ces décors » était atteint, et le corps de commit invoquait la dette sans la
résorber.)*

### DETTE-111 — la conversion « zone de blason → points » est écrite des deux côtés

> **Résorbée en partie par `E17US011` (25/09/2026).** La règle vit une seule fois au domaine
> (`blason.points_zone`), et le poste de qualification la **lit** dans `points_par_zone` au lieu de
> la réécrire. Deux choses restent ouvertes : le jumeau **`saisie-duels/duel.ts`**, que cette ligne
> ne connaissait pas (le DTO des duels ne sert pas encore de table), et la règle d'**agrégation**
> décrite ci-dessous. Le paragraphe « Où » qui suit décrit l'état **d'avant** l'US.
...
### DETTE-115 — la classe `bouton--danger` ne porte plus le jeton dont elle porte le nom

**Où** : `frontend/src/app/App.css` (`.bouton--danger`) et `frontend/src/shared/ui/DialogueConfirmation.tsx`
(`TonConfirmation`), marqueur `DETTE-115` aux deux sites ; **38 occurrences** de `bouton--danger` dans
**22 fichiers** de production, **14** de `'danger'` pour le `ton` (compte du 26/09/2026).

**Le raccourci.** [ADR-0114](adr/0114-l-action-destructrice-se-signale-par-la-forme-pas-par-la-couleur.md)
retire toute couleur d'état à l'action destructrice : le bouton passe en contour épais **neutre**. Le
nom, lui, est resté — `bouton--danger`, `ton="danger"` — alors que `--danger` désigne désormais
**uniquement** l'ambre de l'alerte. Le renommage a été repoussé parce que des sessions parallèles
réécrivaient les mêmes fichiers : les écrans d'`E17US012` (fusionnée le jour même), puis
`archers/Archers.tsx` pour `E02US007`. Deux branches réécrivant les mêmes `className` produisent un
conflit par fichier.

**Ce qui garde le piège, et ce qui ne le garde pas.** `charte.test.ts` refuse tout jeton d'état (forme
à repli et encres `--sur-*` comprises) dans les règles destructrices connues, et toute retouche du
contour du bouton ou du dialogue. ⚠️ Il connaît des **sélecteurs**, pas l'intention : une nouvelle
classe destructrice sous un autre nom lui échappe. Le renommage réduit ce trou — un préfixe
`--destructeur` se cherche par motif, là où `danger` est partagé avec l'alerte.

**Remède** : renommer en `bouton--destructeur` / `ton="destructeur"` quand plus aucune branche en vol
ne touche les sites,
et passer le motif `DESTRUCTEUR` du test d'une liste à un préfixe. Condition de sortie : `grep danger`
ne renvoie plus que le jeton d'alerte et ses usages.
