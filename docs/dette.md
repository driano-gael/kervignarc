...
dupliqué qu'un nom (règle 2, [ADR-0106](adr/0105-le-qr-d-un-scoreur-porte-son-code-revele-un-a-la-fois.md)
...
appelants, zéro changement de comportement. Marqueurs `DETTE-019` sur les **quatre** sites : `routage.py`, `pilotage_tour.py`, `frontend/src/features/routage/presentation.ts` (`partitionner` — la garde rangeait sinon tous les archers encore en lice parmi les sortis dès le tour 2) et `frontend/src/features/feu-vert/etat.ts` (`actionDuel`). **L'US qui lèvera la garde doit balayer les quatre**, sans quoi le registre affichera « résorbée » à tort.
