...
assumée par [ADR-0037](adr/0037-file-hors-ligne-et-rejeu.md)). Une dette dont la résorption ouvre un
...
⚠️ **Ce qui ne résorbe PAS cette ligne** : ouvrir la route de saisie au **scoreur**.
`_role_de_saisie` lit `contexte is None` comme « admin » ; élargir la garde sans faire porter le
rôle par `ContexteSaisie` donnerait au scoreur la préséance de l'organisateur.
