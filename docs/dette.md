...
|---|---|---|---|
| [DETTE-043](#-dette-043--la-charte-impose-inter-lapplication-ne-lembarque-pas-résorbée-le-26092026) | conception | `frontend/src/index.css` (`@font-face`, `--sans`) | **E17US005** — Inter 4.1 **variable** embarquée (`frontend/src/assets/fonts/`, OFL 1.1), servie en `font/woff2` par `api/spa.py` |
...
### ✅ DETTE-043 — la charte impose Inter, l'application ne l'embarque pas *(résorbée le 26/09/2026)*
...
**Résorption (E17US005, 26/09/2026).** Arbitrage du commanditaire : **embarquer**, fichier
**officiel** (`InterVariable.woff2` de la release `rsms/inter` v4.1, tel quel). Deux écarts au plan
ci-dessus, chacun pour une raison vérifiée dans le code du jour : **une police variable**, pas des
graisses statiques — le front emploie 400, 500, 600, 700 et 800, et la fiche n'en comptait que
deux ; et **`src/assets/`**, pas `public/` — Vite nomme le fichier par son contenu, donc un cache de
tablette ne sert jamais une version périmée. Piège trouvé en route : `mimetypes` ignore `.woff2`
sous Windows, d'où un `add_type` dans `api/spa.py`. Garde-fou : `frontend/src/shared/police.test.ts`.
