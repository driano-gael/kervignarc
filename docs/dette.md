...
⚠️ **Le coût principal est ailleurs, et il casse un cas réel : la fraîcheur.** Le `statut` dont
...
Le défaut **préexiste** à E16US012 — `main` conditionnait déjà le bouton « Terminer » à
`statut === 'en_cours'` — mais l'US l'**aggrave** : le verdict et la raison en dépendent désormais
aussi. Le membre voisin `PretADemarrer` n'a pas ce problème : il tient tout du serveur, rafraîchi
en 5 s. *(Relevé en 4ᵉ passe de revue, axe D.)*

**Résorption.** Migrer l'écran sur `/jalons/terminer` — ce qui referme la copie **et** la péremption
d'un seul geste. ⚠️ **Une voie existe déjà, sans second poll**, contrairement à ce que la première
