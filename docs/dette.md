...
|---|---|---|---|---|---|---|---|
| [DETTE-070](#dette-070--la-vue--tableaux--sappelle-désormais--rencontres--et-la-clé-na-pas-suivi) | conception | mineur | `backend/domain/ecran.py` (`VueEcran.TABLEAUX`), `frontend/src/features/ecrans/api.ts` (`VueEcran`, `LIBELLE_VUE`, `TOUTES_LES_VUES`), `frontend/src/features/public/AccueilPublic.tsx` (`type Vue`), `frontend/src/features/salle/EcranSalle.tsx` (le dispatch), la table `poste` en base (les déroulés de vues persistés), et — **ajouté en revue** — les fiches de recette `docs/fonctionnel/E07US005.md` et `docs/fonctionnel/E16US004.md`, qui demandent de cliquer sur un onglet « Tableaux » désormais libellé « Rencontres » : elles servent d'oracle de non-régression, une note de renvoi datée y a été posée | La vue publique et projetée nommée `tableaux` ne montre plus seulement un **arbre de duels** : depuis E05US031 elle rend la phase qui se joue, quelle qu'en soit la forme — poules, système suisse, Big Shoot Off. Son **libellé** est devenu « Rencontres » ; sa **clé** est restée `tableaux`, dans le domaine, dans l'API, dans le front et **en base** | **Aucun effet utilisateur** : le libellé est juste partout où quelqu'un lit. Le coût est de lecture pour qui reprend le code — un `grep Rencontres` ne trouve pas la vue, un `grep tableaux` en trouve deux sens différents (l'arbre de duels *et* la vue générique), et `features/tableaux/` reste une feature d'arbre alors que la vue homonyme n'en est plus une. C'est le genre d'écart qui se redécouvre en cherchant longtemps, pas un défaut de comportement | **E05US031** (17/08/2026), volontairement : la valeur est **persistée** sur chaque poste-écran (ADR-0064 §3 — le déroulé de vues est un réglage de préparation, en base), donc la renommer imposait une migration de données pour changer un mot, **et** cassait tout déroulé déjà composé. L'arbitrage est écrit dans [ADR-0089 §3](adr/0089-les-vues-publiques-rendent-les-formats-sans-arbre.md) | Renommage `tableaux` → `rencontres` **avec migration** de la colonne `poste.sequence_vues` (et des consignes en mémoire, qui ne survivent pas au redémarrage — donc rien à migrer de ce côté). À faire **d'un bloc**, jamais au fil de l'eau : une clé renommée à moitié programme des pages vides sur des écrans que personne ne regarde de près. Candidat naturel à une US de dette groupée avec `DETTE-042` (`position` → `couloir`), qui est exactement le même geste — un renommage retardé par une migration |
...
| [DETTE-057](#dette-057--une-rencontre-de-poule-désynchronisée-na-aucune-sortie) | conception | majeur | `backend/application/poules.py` (`_rencontre_affichee`, `_duel_courant`), `frontend/src/features/poules/SaisiePoules.tsx` | Quand la population d'une phase de poules change après des tirs, les rencontres déjà jouées portent des duellistes qui ne sont plus ceux que la composition désigne. Le tir est **masqué** (ADR-0049 §4) et l'écriture **refusée** (409) — les deux sont justes. Mais **aucun geste ne rétablit** la rencontre : il n'existe pas de purge des tirs orphelins d'une phase | Le `match_numero` d'une poule est un **compteur** qui court sur toute la phase, pas une position d'arbre : un seul archer inscrit ou retiré peut désynchroniser **toutes** les rencontres tirées d'un coup. La seule sortie est de rétablir la population à l'identique. Atténué par la revue : l'écran **dit** l'état (« tir désynchronisé — à rétablir ») au lieu d'afficher « à tirer », et la file hors-ligne ne se fige plus — mais la donnée reste inatteignable | Revue d'E05US023 (2ᵉ passe, axe adversarial). **Non corrigé dans l'US** : la seule résorption connue est un geste admin de **suppression de tirs**, donc une donnée de score effacée — cela ne se décide pas dans un correctif de revue | Deux pistes, à trancher : (a) un geste admin explicite « purger les rencontres orphelines de cette phase », confirmé et journalisé ; (b) rendre le `match_numero` **stable** sous recomposition en le dérivant de `(poule, tour, position)` plutôt que d'un compteur global — cela réduit le rayon d'explosion sans le supprimer, la composition changeant de toute façon les paires |
...
| [DETTE-062](#dette-062--changer-le-format-du-tir-repartitionne-des-volées-déjà-validées) | conception | **majeur** | `backend/application/phases.py` (`ServicePhases.modifier`, marqueur `# DETTE-062`), `backend/application/big_shoot_off.py` (`_scores_par_manche`, `_scores_de_la_manche`, `_prochaine_volee`) | Le découpage manche → volées est **dérivé du réglage à chaque lecture** (`(m-1)·V+1 … m·V`) et n'est stocké nulle part. `modifier` réécrit `volees` / `fleches_par_volee` sans aucune garde sur l'existence de séries | Passer `volees` de 1 à 2 **en cours de phase** re-partitionne des volées déjà validées dans d'autres manches : les scores de manche changent, les éliminations se rejouent autrement, **sans le moindre message**. Modifier `fleches_par_volee` compare des volées à 3 flèches avec des volées à 6. C'est une classe de risque **neuve** — la qualification n'a aucun regroupement dérivé | Revue d'E05US028 (axe C2). Non corrigé dans l'US : la garde propre demande de lire les séries de la phase depuis `ServicePhases`, donc une dépendance neuve au `SerieRepository` dans le service de composition — à peser plutôt qu'à câbler en fin d'US | Résorption : refuser la modification de `volees` / `fleches_par_volee` d'un Big Shoot Off dès qu'une `Serie` existe sur une de ses phases ; les autres champs (`eliminations`, options) restent libres, ils ne déplacent aucune borne. Pendant de DETTE-057 côté **format du tir** au lieu de **population** |
| [DETTE-064](#dette-064--létape-de-déroulé-se-recopie-champ-par-champ-dans-les-décors-de-test) | technique | majeur | `backend/tests/conftest.py` — `poser_phase_sql` **et** `poser_phase_factice`, marqueurs `# DETTE-064` | Les deux helpers construisent une `EtapeDeroule` en **recopiant les champs de la `Phase` un à un**. Rien ne rougit quand on en oublie un : le décor pose alors une phase **non réglée** là où le test croit avoir posé une phase réglée | Aucun effet en production — c'est du décor de test. Le coût est un **faux négatif coûteux** : le test échoue en `phase_pas_reglee` sur une phase parfaitement réglée, et l'on cherche le défaut dans le service. ⚠️ **Le piège s'est déclenché trois fois de suite**, à chaque réglage neuf | **E05US023** (poules, 1ʳᵉ), **E05US028** (Big Shoot Off, 2ᵉ), **E05US026** (système suisse, 3ᵉ). Le commentaire du helper avait **prédit** la 3ᵉ mot pour mot (« il le sera à chaque réglage neuf ») sans qu'elle soit tracée ici : une dette qui ne vit qu'en commentaire n'est jamais prise. Le seuil « 3ᵉ occurrence réelle » de `CLAUDE.md` est atteint **sur preuve** | Résorption : une fabrique unique **du domaine** — `EtapeDeroule.de_phase(phase)` — que les deux helpers appellent, et qui vit à côté d'`EtapeDeroule.instancier` (son inverse). La colline (`E05US027`) en fera une 4ᵉ occurrence si rien n'est fait. US `refactor/` dédiée |
| [DETTE-065](#dette-065--le-garde-dautorisation-des-routeurs-décriture-est-recopié-six-fois) | conception | **majeur** | `backend/api/v1/{saisie,saisie_duels,forfaits,poules,big_shoot_off,suisse}.py` — `_exiger_meme_tournoi` (marqueur `# DETTE-065`) et `_cle_idempotence` | Les deux fonctions sont écrites **à l'identique** dans six routeurs d'écriture. `_exiger_meme_tournoi` **est** le contrôle qui empêche un scoreur d'écrire hors de son tournoi (403 `scoreur_hors_tournoi`) | Aucun effet aujourd'hui : les six copies sont correctes. Le coût est qu'un **7ᵉ routeur d'écriture qui l'oublierait ne ferait rougir personne** — même angle mort que `DETTE-054` pour les DTO jumeaux, mais appliqué à une **autorisation** et non à un champ de composition. C'est ce qui le rend majeur et non mineur | Les cinq premières copies datent d'E04US002 → E05US028 ; la **6ᵉ** est ajoutée par **E05US026** et relevée en revue. Le contraste y est net : la même branche consacre un ADR entier à un remède structurel sur sa 3ᵉ occurrence pendant qu'elle ajoute la 6ᵉ d'un autre, en silence | Résorption : `exiger_meme_tournoi` rejoint `api/dependances.py`, où vit déjà `exiger_scoreur` ; la fabrique de clé d'idempotence rejoint un `api/v1/_idempotence.py`. US `refactor/` dédiée — **pas** dans une US de moteur |
| [DETTE-066](#dette-066--le-bot-de-simulation-se-retire-des-formats-à-la-main) | conception | mineur | `backend/application/simulation_format.py` (`_TYPES_DEROULABLES`, marqueur `# DETTE-066`), `backend/bootstrap/composition.py` (`fabriquer_harnais_simulation`) | `_TYPES_DEROULABLES` part de la table **dérivée** `TYPES_JOUES` et lui **retranche à la main** les formats que le bot ne sait pas jouer. Les deux tables répondent à des questions différentes — « un service de production déroule-t-il ce type ? » contre « le **bot** sait-il le jouer ? » — et rien ne le garantit | Un format rendu jouable sans être retiré de cette liste fait annoncer `joue=True, 0 tour, 0 duel` à l'atelier : des zéros lus comme un **constat**, et le bandeau « le moteur ne sait pas encore dérouler ce type » disparaît. C'est arrivé | **3ᵉ occurrence** : poules (E05US023), Big Shoot Off (E05US028), système suisse (**E05US026**, oubli relevé en revue). Le seuil du remède structurel de `CLAUDE.md` est atteint **sur preuve** | Résorption : une capacité `simulable_par_le_bot` au registre de contrat (`domain/contrat_phase.py`), dont `_TYPES_DEROULABLES` dérive — ou, si le bot doit vraiment apprendre les formats, une US qui construit les services manquants dans le harnais. À trancher en US dédiée ; la colline en fera un 4ᵉ retrait sinon |
...
### DETTE-065 — le garde d'autorisation des routeurs d'écriture est recopié six fois

**Ce que fait le code.** `_exiger_meme_tournoi(scoreur, tournoi_id)` et `_cle_idempotence(...)` sont
écrits **à l'identique** dans six routeurs : `saisie.py`, `saisie_duels.py`, `forfaits.py`,
`poules.py`, `big_shoot_off.py` et `suisse.py`.

**Pourquoi c'est majeur alors que les six copies sont justes.** Parce que la fonction dupliquée
**est** le contrôle d'accès : elle refuse (403 `scoreur_hors_tournoi`) un scoreur qui écrirait hors
de son tournoi. Rien — ni mypy, ni ruff, ni un test générique — ne rougit si un **7ᵉ** routeur
...
touche six routeurs d'écriture et leurs tests, il n'a rien à faire dans une US de moteur.
...
### DETTE-070 — la vue « tableaux » s'appelle désormais « Rencontres », et la clé n'a pas suivi

**Ce que fait le code.** Le catalogue de vues de l'écran de salle (ADR-0064) porte une entrée
`VueEcran.TABLEAUX = "tableaux"`. Jusqu'à E05US031 elle ne savait afficher qu'un **arbre de duels** ;
elle rend désormais la phase qui se joue, quelle qu'en soit la forme — arbre, poules, système suisse,
Big Shoot Off ([ADR-0089](adr/0089-les-vues-publiques-rendent-les-formats-sans-arbre.md)). Le libellé
affiché est passé à « Rencontres », côté console de pilotage comme côté onglet public. **La clé, non**
— ni dans `domain/ecran.py`, ni dans l'API, ni dans le front, ni dans les lignes `poste` déjà en base.

**Pourquoi c'est assumé et non un oubli.** Le déroulé de vues d'un écran est un **réglage de
préparation persisté** (ADR-0064 §3) : il vit en base, sur le poste, et survit au redémarrage du
serveur le matin du jour J. Renommer la clé demande donc une **migration de données** — et sans
elle, tout déroulé déjà composé programme une vue inconnue, c'est-à-dire une page vide sur un écran
que personne ne regarde de près. Payer une migration pour changer un mot, dans une US dont ce n'est
pas le sujet, n'était pas le bon arbitrage.

**Ce que ça coûte.** Rien à l'utilisateur : partout où quelqu'un lit, le mot est juste. Le coût est
pour qui reprend le code, et il est double :

- `grep Rencontres` ne trouve pas la vue ; `grep tableaux` en trouve **deux sens** — la feature
  `features/tableaux/` (l'arbre de duels, toujours exacte) et la clé de vue (devenue générique) ;
- `features/tableaux/VueTableaux` n'est plus le point d'entrée de l'onglet, c'est
  `features/phases-publiques/VuePhases` — le nom de fichier ne le dit pas.

**Résorption.** Renommage `tableaux` → `rencontres` **avec migration** de `poste.sequence_vues`. Les
consignes de prise de contrôle, elles, vivent en mémoire (ADR-0064 §3) et ne survivent pas au
redémarrage : rien à migrer de ce côté. À faire **d'un bloc** — une clé renommée à moitié est pire
que pas renommée du tout, pour la raison ci-dessus. Candidat naturel à un groupement avec
**DETTE-042** (`position` → `couloir`, E16US001) : c'est exactement le même geste, un renommage de
vocabulaire retardé par une migration, et les grouper amortit la seule chose qui coûte.
...
**US dédiée**, pas en douce. La colline (`E05US027`) en fera une 4ᵉ occurrence si rien n'est fait.
...
### DETTE-054 — cinq paires de DTO jumeaux entre les deux routeurs de composition
...
de `ReglagePoulesDTO`, `ReglageBigShootOffDTO` et `ReglageSuisseDTO` des deux fichiers. ⚠️ La consigne disait « trois paires » et ne nommait que `ReglagePoulesDTO` : qui aurait résorbé en lisant ce détail aurait extrait trois paires et laissé les deux dernières — exactement le défaut que la ligne du tableau reproche à E05US028 (relevé en revue d'E05US026).
...
#### Élargissement du 17/08/2026 — E05US031 : les trois formats sans arbre passent au public

`ServicePoules.etat`, `ServiceSuisse.etat` et `ServiceBigShootOff.etat` portent le marqueur
`# DETTE-031` depuis leurs US respectives : chacun **recompose la phase entière, chaîne amont
comprise**, à chaque lecture. Jusqu'ici la borne n'était pas technique mais **sociale** — ces
lectures ne servaient que l'atelier de l'organisateur et les pavés de saisie des scoreurs, soit
quelques dizaines de tablettes tenues par des gens qui ont autre chose à faire que rafraîchir.

Cette US les branche sur l'**appli publique** et l'**écran de salle**. La charge n'est plus bornée
par le nombre de postes de travail mais par le nombre de **spectateurs présents** — exactement ce
que le précédent E07US005 décrivait déjà (« *en autant d'exemplaires qu'il y a de spectateurs* »),
et le quatrième endpoint public au même régime après `/suivi-deroule`, `/routage/{id}/affectations`
et `/tableaux/{id}`. S'y ajoute `GET /departs/{id}/phases`, que l'index public interroge à chaque
ouverture d'onglet.

⚠️ **Le multiplicateur qui n'est pas dans le compte des surfaces** : `shared/realtime/useRealtime.ts`
appelle `invalidateQueries()` **sans clé**. Chaque validation de score, n'importe où dans le
tournoi, invalide donc *tout* le cache de *chaque* client connecté — y compris les clés neuves de
cette US. Le coût d'une journée ne se lit pas « une lecture par spectateur » mais « une
reconstruction complète par spectateur **et par score validé** ». C'est bénin sur un réseau local
mono-club et ça reste le bon compromis pour aujourd'hui (l'invalidation fine est sa propre source
de bugs), mais c'est le facteur qui rendra la mesure nécessaire en premier.

*Relevé en revue par l'axe adversarial : le commit de l'US n'élargissait pas cette ligne. C'est la
**4ᵉ récidive** du même oubli sur cette dette précise — et cette fois-ci l'US ne l'assumait même pas
en commentaire. Le motif est constant : on ajoute un consommateur à un service déjà marqué `#
DETTE-031`, et le marqueur, étant déjà là, ne rappelle rien à personne. La leçon pour la suite :
**brancher une surface publique sur un service marqué est en soi un élargissement**, qu'on ait ou
non touché la ligne du service.*
