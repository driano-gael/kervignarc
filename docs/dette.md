...
| [DETTE-084](#dette-084--lécran-prêt-à-terminer--redéduit-la-garde-de-statut-et-recopie-sa-phrase) | conception | mineur | `frontend/src/features/completude/Completude.tsx` (`bloquant={!enCours}` et le littéral du refus, marqueurs `// DETTE-084`) ↔ `backend/domain/jalon.py` (`evaluer_terminer`) | L'écran « Prêt à terminer ? » lit `/completude`, qui ne porte pas le statut du tournoi : il **redéduit** donc la garde de statut de `terminer` et **recopie** en TypeScript la phrase de refus `MESSAGE_TERMINER_HORS_EN_COURS`, que le front ne peut pas importer. C'est la **seule** garde encore redéduite côté front après E16US012 | Un reformulage de la phrase côté Python laisse l'écran afficher l'ancienne, avec toutes les suites vertes — les deux littéraux vivent dans deux runtimes qu'aucun test ne confronte. Idem si `terminer` acceptait un jour un autre statut | **E16US012** (23/08/2026), arbitrage assumé à [ADR-0096](adr/0096-un-jalon-enumere-ses-gardes-au-lieu-de-les-lever.md) § Conséquences : brancher l'écran sur `/jalons/terminer` ajouterait un **second poll de 5 s par tablette** pour la même liste. ⚠️ Inscrite en **3ᵉ passe de revue** — l'arbitrage ne vivait qu'en section *Conséquences*, ce que `DETTE-082` désigne elle-même comme insuffisant | Migrer l'écran sur `/jalons/terminer` le jour où le coût du second poll sera acceptable (ou où la complétude et le jalon seront servis par une seule lecture). Le miroir disparaît alors des deux côtés |
...
**Résorption.** Migrer l'écran sur `/jalons/terminer` quand le coût du second poll sera acceptable,
ou quand complétude et jalon seront servis par une seule lecture. Le miroir disparaît alors des deux
côtés.
...
autres clusters dans le même mouvement. **Critère de fin** : plus aucun signal `features-enchevetrees`
porté par le sujet `accueil`.
