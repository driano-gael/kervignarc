...
| [DETTE-102](#dette-102--trois-libs-sans-pytyped-échappent-au-typage-strict) | technique | mineur | `backend/pyproject.toml` (les trois blocs `[[tool.mypy.overrides]]`, marqueurs `DETTE-102`) ; portée réelle : `backend/infrastructure/pdf/*.py`, `backend/infrastructure/tableur/grille.py` et `backend/infrastructure/import_inscrits/resultarc.py` | `reportlab`, `openpyxl` et `xlrd` (élargie par **E02US007**) ne publient pas de marqueur `py.typed` : leurs symboles valent `Any` sous `mypy --strict`, donc les appels à ces libs ne sont pas type-vérifiés | Concrètement : `_ecrire_ligne_xlsx(feuille: Worksheet, …)` est annoté contre un type qui vaut `Any` — l'annotation est **inerte** —, `Workbook.active` est donné non-`Optional` alors qu'il l'est, et une faute sur le garde-fou CWE-1236 (`data_type = "str"` au lieu de `"s"`) passerait `mypy`. Rattrapé au runtime par `test_tableur_grille.py` (et, pour xlrd, `test_lecteur_import_inscrits.py` sur le vrai fichier), d'où *mineur* | **E16US016** — relevée par les axes B et C2. ⚠️ Les deux cas ne sont **pas** identiques : `types-reportlab` n'est pas officiel, tandis que **`types-openpyxl` existe et vient de typeshed**. L'écart n'est donc pas subi mais **choisi** (une dépendance de plus, règle 11) — c'est la définition d'une dette assumée, et le précédent ReportLab n'avait jamais été inscrit | Déclarer `types-openpyxl` au groupe `dev`, retirer l'override `openpyxl.*`, vérifier que `mypy --strict` reste vert. Le volet ReportLab ne se ferme que si un paquet de stubs officiel apparaît. Volet xlrd (E02US007) : **`types-xlrd` existe** (typeshed, `2.0.0.20260518`, vérifié le 26/09/2026) — même sortie qu'openpyxl : le déclarer au groupe `dev` et retirer l'override `xlrd.*` |
...
### DETTE-102 — Trois libs sans `py.typed` échappent au typage strict
...
**Élargie par E02US007** : un troisième bloc, `xlrd.*`, pour la lecture du classeur Résult'Arc
(trois appels dans `infrastructure/import_inscrits/resultarc.py`, rattrapés au runtime par
`test_lecteur_import_inscrits.py` sur le vrai fichier).
