...
c'est la raison même de cette dette. Marqueurs `DETTE-092` sur **les six sites** — trois au backend (le port et les deux services) et trois au front (l'anti-rebond, le `staleTime`, et le test qui les tient). ⚠️ Le registre sous-déclarait ses propres marqueurs : le `grep` de résorption en rendait plus que la fiche n'en annonçait.
...
⚠️ **Ce n'est pas la définition d'agent qu'il faut réécrire** : la règle y est déjà, complète et
sans ambiguïté. Ce qui manque est un **contrôle** — un agent qui n'applique pas sa règle rend un
rapport auto-cohérent, exactement comme le hook d'atlas « compare du périmé à du périmé »
...
**Résorption.** US d'outillage. **Critère de fin, vérifiable** : le rapport de porte contient un
`EXIT=` par étape `run:` de `ci.yml` hors les deux omissions énumérées, **et** l'orchestrateur
refuse un verdict vert dont la table en compte moins. Marqueur `DETTE-093` dans les deux fichiers de
`.claude/`. ⚠️ Le geste **n'est pas** d'ajouter une phrase de plus à la définition d'agent — la
phrase existe et n'a pas suffi.
