...
| [DETTE-082](#dette-082--deux-endroits-pour-démarrer-le-même-tournoi) | conception | mineur | `frontend/src/features/jalons/PretADemarrer.tsx` et `frontend/src/features/accueil/FriseCycleDeVie.tsx` (marqueurs `// DETTE-082`) | Depuis E16US012, l'organisateur dispose de **deux** endroits pour démarrer un tournoi : la frise du cycle de vie (action **nue**, sur l'accueil admin) et l'écran « Prêt à démarrer ? » (action **expliquée**, sur le pilotage). Les deux appellent la même transition serveur, qui reste juge — rien n'est incohérent, mais l'un ne dit pas ce que l'autre annonce | Un organisateur qui clique depuis la frise ne voit **pas** ce qui manque et retombe sur le refus au clic, c'est-à-dire exactement le défaut que l'US existe pour supprimer — simplement par l'autre porte. Aucun cas cassé, un chemin non amélioré | **E16US012** (23/08/2026), qui l'assume à [ADR-0096](adr/0096-un-jalon-enumere-ses-gardes-au-lieu-de-les-lever.md) § Conséquences. ⚠️ Inscrite ici **en revue** : l'ADR la décrivait déjà, mais un angle mort qui ne vit qu'en section *Conséquences* n'apparaît à aucun tri de dette | À instruire quand `ARCHIVER` rejoindra la famille (la frise porte aussi ce bouton) : soit la frise renvoie vers le jalon, soit elle porte l'explication. La refonte de navigation complète relève de la fiche E16, pas d'un correctif. **Candidat désigné : `E16US021`** (l'accueil parle par départ), qui ajoute une 3e surface au même écran et doit donc dire ce qu'elle enlève |
...
**Où** : `frontend/src/features/jalons/PretADemarrer.tsx` et
`frontend/src/features/accueil/FriseCycleDeVie.tsx` (marqueurs `// DETTE-082`).
...
« Démarrer » / « Terminer » sur l'accueil admin. Les fondre supposait la refonte de navigation que la
fiche E16 annonce sans l'avoir instruite : l'US a livré le nouveau chemin sans retirer l'ancien.
...
**Résorption.** Quand `ARCHIVER` rejoindra la famille : la frise porte aussi ce bouton, et la
question se posera pour de bon sur trois transitions au lieu d'une. Deux options — la frise renvoie
vers le jalon, ou elle porte l'explication.
