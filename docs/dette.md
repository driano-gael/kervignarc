...
| [DETTE-056](#dette-056--lespace-scoreur-porte-deux-sélecteurs-de-créneau-indépendants) | conception | mineur | `frontend/src/features/scoreur-session/EspaceScoreur.tsx`, `frontend/src/features/poules/SaisiePoules.tsx` (marqueur `# DETTE-056`), `frontend/src/features/saisie-duels/SaisieDuels.tsx`, `frontend/src/features/big-shoot-off/SaisieBigShootOff.tsx` (E05US028), `frontend/src/features/departs/hooks.ts` (`useCreneauDesDuels`) | L'espace scoreur monte `SaisieDuels`, **puis** `SaisiePoules`, **puis `SaisieBigShootOff`** (E05US028) ; chacun appelle `useCreneauDesDuels`, dont le choix vit en `useState` **local**, et chacun rend son propre `ChoixCreneau`. **Trois** sélecteurs de créneau côte à côte, indépendants, qui divergent au premier changement | Sur tablette, en salle : le scoreur change de créneau dans un panneau, saisit dans l'autre, et score les rencontres du mauvais départ — avec des identifiants valides, donc **sans la moindre erreur**. Le risque est réel mais borné : les trois panneaux partent du même créneau par défaut, et il faut une bascule manuelle pour les désaccorder. ⚠️ **Aggravé par E05US028** : à trois panneaux, le nombre de couples désaccordables triple, et le troisième est l'écran d'une **finale jouée devant le public** | Revue d'E05US023 (2ᵉ passe, axes A, C1 et adversarial). Une première tentative masquait le panneau des poules quand le créneau n'en portait aucune : elle **créait un cul-de-sac** (le sélecteur vit dans le JSX retiré, l'état est local, donc plus aucun retour possible) et rendait la branche d'erreur morte. Retirée, d'où cette ligne | Remonter le choix de créneau dans `EspaceScoreur` et le passer en prop aux deux panneaux — ou le porter par un store de session partagé, comme `useSessionScoreurStore`. Une US front dédiée : c'est le geste qui règle réellement la désynchronisation, pas un masquage |
...
**Ce que fait le code.** `atlas/statique/` porte ~1 470 lignes de JavaScript navigateur (coquille,
...
**Résorption.** Deux déclencheurs, écrits ici pour ne pas être négociés au cas par cas : si
`atlas/statique/` dépasse **2 000 lignes**, ou si un défaut de rendu échappe **deux fois**.
⚠️ **Le premier déclencheur est à 20 lignes** (1 980 au 16/08/2026) : la tranche suivante de
l'atlas le franchira. Il a été écrit pour ne pas être renégocié — la prochaine US qui touche
`atlas/statique/` ouvre donc la résorption, elle ne relève pas le seuil.
