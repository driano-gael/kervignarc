...
fichiers : `app/routeur.ts` (**pur** — analyse et construction de chemins, correspondance monde ↔
rôle) et `app/useChemin.ts` (l'abonnement à `history`, via `useSyncExternalStore`).
...
l'abonnement — seuls ces deux fichiers et **trois** appels à `naviguer` (`App`, `ChangerDeRole`,
`EspacePoste`) sont concernés. Le marqueur vit en tête de `routeur.ts`, avec l'historique complet de
l'arbitrage.
