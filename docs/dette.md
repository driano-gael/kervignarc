...
délibéré : l'atlas en extrait `resorption_us` par expression régulière, si bien qu'y citer une US en
**précédent** la ferait passer pour l'US **résorbante** — une dette du 24/08/2026 déclarée résorbée
par une US mergée le 22/08. `DETTE-083` porte déjà cet artefact sur `main` (défaut préexistant,
signalé sans être corrigé ici, cf. corps de la PR).

**Critère de fin** : `Duels.tsx` ne déclare plus aucun composant de grille **et** n'importe plus
rien de `features/placement/`.
...
`afficheEtat` / `detailLigne`). Précédent : `etatRencontre`, remontée en `shared/` à E05US027.
