...
| | couvert | hors porte |
|---|---|---|
| backend | les cinq couches, `atlas/`, `release/`, les points d'entrée racine | `tests/` (397 blocs), `migrations/` (56) |
| front | tout `frontend/src` en `.ts` / `.tsx` / `.css`, **tests compris** | `frontend/*.config.*`, `atlas/statique/` (5 blocs) |
...
⚠️ **Deux limites du détecteur, assumées et écrites plutôt que découvertes** : une **ligne vide
coupe un bloc** des deux côtés — 8 lignes, une blanche, 8 lignes ne déclenche rien —, si bien que le
plafond borne le **paragraphe** et non la zone de commentaire ; et le détecteur front, faute de
tokenizer, compterait un gabarit contenant des lignes `//` comme du commentaire. Aucun cas
aujourd'hui, et les deux sont figées par un cas de test pour qu'un futur lecteur ne les prenne pas
pour un bug.
