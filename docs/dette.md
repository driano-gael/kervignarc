...
**Résorption.** Remonter `SectionCompletude`, le type `LigneCompletude` et les fonctions
`afficheEtat` / `detailLigne` dans `shared/` (précédent : `etatRencontre`, E05US027). C'est un
**rangement transverse**, donc une US dédiée (règle 16) — et qui gagnerait à traiter les autres
clusters dans le même mouvement plutôt que ce seul cycle.
