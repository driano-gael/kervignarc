...
| [DETTE-034](#dette-034--une-phase-de-consolation-serait-mal-classée-au-palmarès) | conception | mineur | `backend/domain/palmares.py` (`calculer_palmares`, règle du bloc) | Le palmarès situe chaque archer par la phase la plus **tardive** qui l'a classé. Sur une phase de **consolation** (les perdants du tour *n* repris par une phase avale), la règle ferait passer le vainqueur du repêchage **devant** le finaliste du tableau principal — il n'a pourtant disputé que les places basses | **Nulle aujourd'hui** : aucun `RoutingRepechage` n'est câblé en production (**DETTE-028**) et la sémantique de `SourcePhase.par_issue_de_tour` n'est pas tranchée (**DETTE-033**), donc aucun tournoi réel ne compose une telle séquence. Devient réelle le jour où un moteur consommera les prélèvements | E06US004 ([ADR-0067](adr/0067-palmares-agregation-des-rangs-de-phases.md) §6) — signalée **par l'implémenteur**, pas trouvée en revue : la règle du bloc est correcte pour la cascade qualification → tableau, et fausse dès qu'une phase avale ne dispute pas les mêmes rangs que la précédente | **Ne pas deviner** — même raison qu'à DETTE-033 : décider ici, dans un canal d'affichage, ce que la séquence n'a pas tranché referait l'erreur qu'ADR-0065 §3 a refusé de commettre. La règle appartient à l'US qui exécutera le prélèvement, qui saura dire *quels rangs* une phase avale dispute. Figée par `test_la_phase_la_plus_tardive_l_emporte`, qui **documente** la règle actuelle et devra être révisé ce jour-là. Marqueur `# DETTE-034` sur `calculer_palmares` |
