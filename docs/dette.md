...
| [DETTE-004](#dette-004--messageerreur-dupliqué-dans-chaque-feature-front) | conception | mineur | `frontend/src/features/*/` (18 occurrences) | Le composant `MessageErreur` est copié **à l'identique** dans chaque feature — même signature, même corps, mêmes classes — au lieu de vivre dans `shared/` | Tout changement du rendu d'erreur (ex. le token d'alerte **ambre** du CDC design, `DV-03`) se fait en 17 endroits, avec le risque d'en oublier un : les erreurs sont précisément ce que l'utilisateur voit quand ça va mal | E00US011 puis chaque feature (`admin`, `bareme`, `blasons`, `categories`, `competition`, `gabarits` ×2) ; **aggravée** par E01US015 (8ᵉ copie), E02US001 (9ᵉ copie), E02US003 (10ᵉ copie, feature `archers`), E03US004 (11ᵉ copie, feature `placement`), E10US003 (12ᵉ et 13ᵉ copies, features `scoreurs` et `scoreur-session`) puis E04US001 (14ᵉ copie, feature `poste`) ; E00US015 **relocalise** sans aggraver (copie `competition`→`tournois`, bloc d'homonyme→`archers/NouvelArcher`, `archers` réutilise sa copie exportée — décompte inchangé) ; E04US002 (15ᵉ copie, feature `saisie`) ; E08US002 ajoute la copie de la feature `paiements` — grep exhaustif des définitions : **18 copies dans 17 features** (le narratif ci-contre sous-numérote depuis longtemps, et la liste détail avait aussi perdu `saisie` ; le compte faisant foi est celui du grep). C'est cette accumulation qui a fait **remonter E00US013 de J3 en tête de file** (décidé le 21/07/2026) | E00US013 (factoriser les briques d'UI partagées) |
