...
|---|---|---|---|---|---|---|---|
| [DETTE-114](#dette-114--la-normalisation-dun-texte-de-recherche-est-recopiée-quatre-fois) | technique | mineur | `frontend/src/features/archers/accueil.ts` (`normaliser`, marqueur `DETTE-114`) — site neuf · jumeaux : `features/audit/presentation.ts`, `features/competition/barrage.ts`, `features/suivi/suivi.ts` | « retirer les accents, passer en minuscules, rogner » est écrit **quatre fois** dans le front, chaque recherche ayant recopié la précédente | Faible aujourd'hui (quatre copies identiques), mais **silencieux** : une recherche qui trouve « Hélias » en tapant « helias » et une autre qui ne le trouve pas se contrediraient sans qu'aucun test ne compare les deux | **E17US007** (25/09/2026) — la 4ᵉ copie. Le § *Dette* de `CLAUDE.md` autorise un remède structurel dès la 3ᵉ occurrence, mais **en US dédiée**, jamais en douce dans l'US courante : la copie est donc assumée et tracée ici | Extraire `normaliserPourRecherche` dans `shared/` et y faire pointer les quatre sites, avec **un** test. Petit `refactor/`, sans porteur désigné — ne pas le réattribuer par pronostic |
...
### DETTE-114 — la normalisation d'un texte de recherche est recopiée quatre fois

**Où** : `frontend/src/features/archers/accueil.ts` (`normaliser`, marqueur `DETTE-114`) — la copie
neuve d'`E17US007`, pour la recherche de l'écran des inscriptions (A09). Ses trois aînées :
`features/audit/presentation.ts`, `features/competition/barrage.ts`, `features/suivi/suivi.ts`.

**Pourquoi assumée** : c'est la **4ᵉ** occurrence, donc le remède structurel est fondé sur preuve
(§ *Dette* de `CLAUDE.md`) — mais il se traite « en ADR + US dédiée, jamais en douce dans l'US
courante ». `E17US007` est une US de fidélité aux maquettes : y déplacer trois fonctions d'autres
features aurait élargi sa revue à trois écrans qu'elle ne touche pas.

**Remède** : `shared/` gagne une `normaliserPourRecherche`, les quatre sites l'importent, un seul test
la garde. Aucun ADR n'est nécessaire (pas de pattern, une fonction), une US `refactor/` suffit.
