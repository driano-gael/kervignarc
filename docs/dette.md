...
| [DETTE-064](#dette-064--létape-de-déroulé-se-recopie-champ-par-champ-dans-les-décors-de-test) | technique | majeur | `backend/tests/conftest.py` — `poser_phase_sql` **et** `poser_phase_factice`, marqueurs `# DETTE-064` | Les deux helpers construisent une `EtapeDeroule` en **recopiant les champs de la `Phase` un à un**. Rien ne rougit quand on en oublie un : le décor pose alors une phase **non réglée** là où le test croit avoir posé une phase réglée | Aucun effet en production — c'est du décor de test. Le coût est un **faux négatif coûteux** : le test échoue en `phase_pas_reglee` sur une phase parfaitement réglée, et l'on cherche le défaut dans le service. ⚠️ **Le piège s'est déclenché trois fois de suite**, à chaque réglage neuf | **E05US023** (poules, 1ʳᵉ), **E05US028** (Big Shoot Off, 2ᵉ), **E05US026** (système suisse, 3ᵉ), **E05US033** (découpage en tours, 4ᵉ — les **arrêts** font une 5ᵉ occurrence d'un genre voisin : ils n'ont pas de miroir sur `Phase`, donc les helpers ne peuvent même pas les recopier, et un décor d'arrêts doit écrire l'étape lui-même). Le commentaire du helper avait **prédit** la 3ᵉ mot pour mot (« il le sera à chaque réglage neuf ») sans qu'elle soit tracée ici : une dette qui ne vit qu'en commentaire n'est jamais prise. Le seuil « 3ᵉ occurrence réelle » de `CLAUDE.md` est atteint **sur preuve** | Résorption : une fabrique unique **du domaine** — `EtapeDeroule.de_phase(phase)` — que les deux helpers appellent, et qui vit à côté d'`EtapeDeroule.instancier` (son inverse). E05US033 en a fait une 4ᵉ, câblée dans son propre commit ; la colline (`E05US027`) en fera une 5ᵉ. ⚠️ **Et le remède ne couvrira pas tout** : `EtapeDeroule.de_phase(phase)` ne peut pas recopier un champ que `Phase` ne porte **pas**, ce qui est le cas des `arrets` par conception (ADR-0091 §2, cycle d'import). La fabrique règle la classe de défaut « champ oublié », pas la classe « champ absent du miroir » — à savoir avant de la croire complète. US `refactor/` dédiée |
...
- **(a)** une garde `TournoiEnPause` (409) sur les mêmes points d'écriture que `PhaseEnPause`, en
  ...
  déduire ;
- **(b)** à défaut, **corriger la docstring** et retirer la promesse. Coût nul, et cela ramène la
  dette de « l'application mente » à « la capacité n'existe pas », qui est honnête.

⚠️ **Ne pas laisser en l'état.** Des deux volets constatés le 19/08/2026, c'est le seul qui subsiste,
et c'est celui qui **ment à l'organisateur** — le volet phase, lui, ne faisait que ne rien faire.
