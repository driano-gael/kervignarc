...
l'onglet « Rencontres » et l'écran de salle appellent cette route à chaque ouverture. Elle n'était
jusque-là lue que par l'écran de **pilotage** (admin). Le front n'y prend que quatre champs — le type
`PhasePublique` de `features/phases-publiques/api.ts` ne déclare que ce qu'il consomme, et son
en-tête signale l'écart — mais un client qui ne demande pas un champ n'empêche pas le serveur de le
servir.
...
une US, pas un correctif de bord. L'écart était signalé en [ADR-0089](adr/0089-les-vues-publiques-rendent-les-formats-sans-arbre.md)
§ Conséquences et dans l'en-tête de `phases-publiques/api.ts`, mais **pas ici** : un écart consigné
dans un ADR n'est pas trouvable par quelqu'un qui cherche la dette, ce que le § Dette de `CLAUDE.md`
compte comme dette silencieuse.* **US de résorption : `E10US009`.**
