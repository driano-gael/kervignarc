...
| [DETTE-085](#dette-085--les-deux-plans-de-cibles-sont-des-jumeaux-recopies-composant-par-composant) | conception | mineur | `frontend/src/features/placement/Placement.tsx` et `frontend/src/features/duels/Duels.tsx` (marqueurs `// DETTE-085` sur les quatre composants jumeaux) | **Quatre composants recopies a l’identique dans deux fichiers** : `Cible`, `Case`, `JetonArcher`, `Reserve`, plus le type `Jeton`, la fabrique de jeton et le calcul `couloirs`. Les deux ecrans ne different que par leurs badges (mixite / adjacence) et par la confirmation d’impact. Le fond commun — vocabulaire de reserve, banniere de cloisonnement — est deja mutualise dans `placement/presentation.ts` ; c’est le **rendu** qui ne l’est pas | Toute evolution de la grille se paie deux fois, et un oubli d’un cote passe `tsc` sans bruit — exactement le mecanisme du defaut d’E03US007, ou la copie `RaisonConflit` des duels etait restee a trois valeurs quand le serveur en emettait quatre, affichant une reserve **sans motif**. La copie a ete detectee ici parce que l’US devait modifier les deux ; elle ne le sera pas toujours | **E16US005** (24/08/2026) — ⚠️ **elle ne cree pas la duplication** (elle date d’E03US009, qui a clone l’ecran de placement pour les duels) : elle en **paie le cout pour la premiere fois de facon visible**, en appliquant deux fois le meme changement de mise en page. C’est la preuve qui manquait au registre | Remonter les quatre composants dans un module partage — `shared/plan-de-cibles/`, les badges passes en `ReactNode` — sur le modele d’`etatRencontre` (E05US027) et de `PretA` (`DETTE-083`). **US de rangement dediee** (regle 16) : introduire ce pattern dans l’US courante serait un remede structurel en douce, et la 3ᵉ occurrence n’existe pas encore — deux plans, pas trois. ⚠️ **A traiter avec `DETTE-083`** : meme geste, meme destination (`shared/`), et une seule US de rangement front vaut mieux que deux |
...
Ce qui est recopié : les composants `Cible`, `Case`, `JetonArcher` et `Reserve`, le type `Jeton`, la
fabrique de jeton, le calcul `couloirs` et le `messageErreur` local (ce dernier déjà compté par
`DETTE-050`). Ce qui **ne** l'est pas : les badges (mixité côté qualification, adjacence côté duels)
et la confirmation d'impact d'E12US007.

Le fond **métier** commun, lui, est bien mutualisé depuis E03US007 : `LIBELLE_RAISON`,
`RAISON_ANOMALIE`, `resumeCloisonnementNonRespecte` et, depuis cette US, `reperesArcher` vivent tous
dans `placement/presentation.ts`, importé par les duels. C'est le **rendu** qui reste en double.

**Ce que ça coûte.** Rien à l'exécution. Le coût est un **oubli silencieux** : un changement
appliqué à un seul des deux écrans compile, passe le lint et passe les tests. Le précédent est
documenté dans le code lui-même (`placement/api.ts`) : à E03US007, la copie de `RaisonConflit` côté
duels est restée à trois valeurs quand le serveur en a émis une quatrième, et la réserve des duels
s'est affichée **sans motif** — sans que `tsc` voie quoi que ce soit, un miroir manuel ne
contraignant pas la donnée reçue.

⚠️ **Cette US ne crée pas la duplication : elle la paie.** C'est le premier changement qui doit
toucher les deux écrans à l'identique (une cible par ligne, repères sous le nom, réserve collante),
donc la première preuve **dans le code du jour** que la copie coûte. Elle est inscrite ici, et non
corrigée, parce que la corriger serait introduire un pattern en douce dans l'US courante — ce que
le § Dette de `CLAUDE.md` proscrit — et que la 3ᵉ occurrence n'existe pas : il y a **deux** plans
de cibles dans le produit, pas trois.

**Résorption.** Remonter les quatre composants dans `shared/plan-de-cibles/`, les badges passés en
`ReactNode` par l'appelant (c'est leur seule vraie différence). Précédents de ce geste :
`etatRencontre` remontée en `shared/` à E05US027, et le remède prévu pour `DETTE-083`.
**US de rangement dédiée** (règle 16), à traiter **dans le même mouvement que `DETTE-083`** : même
geste, même destination, et un seul rangement front vaut mieux que deux. **Critère de fin** :
`Duels.tsx` ne déclare plus aucun composant de grille, et un changement de mise en page du plan se
