...
### DETTE-054 — sept paires de DTO jumeaux entre les deux routeurs de composition
...
de `ReglagePoulesDTO`, `ReglageBigShootOffDTO`, `ReglageSuisseDTO`, `DecoupageDTO` et `ArretProgrammeDTO`
des deux fichiers — **sept** paires, pas cinq. ⚠️ **Ce paragraphe a été faux deux fois de suite, et c'est l'information la plus utile qu'il porte.** La consigne disait d'abord « trois paires » en ne nommant que `ReglagePoulesDTO` (relevé en revue d'E05US026), puis « cinq » alors qu'E05US033 en ajoutait deux (relevé en revue d'E05US033). Chaque fois, qui aurait résorbé en lisant ce détail aurait laissé les dernières derrière lui — exactement le défaut que la ligne du tableau reproche à E05US028. Le titre lui-même portait un compte périmé. **Corollaire pour la prochaine US qui ajoute un réglage d'étape** : le compte est à trois endroits (titre, ligne de tableau, ce paragraphe), et les trois doivent bouger ensemble.
