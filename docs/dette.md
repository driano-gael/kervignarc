...
|---|---|---|---|---|---|---|---|
| [DETTE-100](#dette-100--la-préséance-de-rôle-nest-déclenchable-par-aucun-écran) | conception | mineur | `frontend/src/features/saisie/Saisie.tsx` (`MessageErreurSaisie`, marqueur `DETTE-100`), `frontend/src/features/saisie/hooks.ts` (`onError`, marqueur `DETTE-100`), `frontend/src/features/saisie/api.ts` (`saisirVolee`, portée `'poste'` **en dur**) | La règle de préséance d'ADR-0107 est tenue par le **serveur** et couverte de bout en bout, mais **aucun écran du produit ne fait écrire un admin sur une volée de qualification** : l'unique appelant front de `POST /api/v1/saisie/volees` est la tablette, en portée `'poste'`. Deux tablettes ayant le **même** rang (ADR-0107 §3), le `409` `ecriture_de_role_inferieur` **ne peut pas se produire depuis l'IHM** | Le rendu ambre de `MessageErreurSaisie` et l'invalidation d'`onError` sont **corrects et testés, mais injoignables en production**. Le piège est pour le **prochain contributeur** : rien à l'écran ne dit que ce chemin n'a jamais tourné pour de vrai, et un test qui *simule* l'erreur ne le dit pas non plus. ⚠️ **Le revers de ne rien inscrire serait pire** : livrer le refus derrière `MessageErreur` générique afficherait une phrase de conflit sans recours le jour où un admin écrit par l'API | **E16US020** (12/09/2026) — écart **constaté au cadrage de la recette fonctionnelle**, pas en revue. Ce n'est pas un raccourci d'implémentation : la surface admin de saisie n'existe pas, et l'inventer sortait du périmètre arbitré | Se referme **toute seule** le jour où un écran d'administration écrit une volée de qualification — il n'y a rien à défaire, seulement un chemin à ouvrir. **Critère de fin** : un appelant de `saisirVolee` en portée `'admin'`, et le scénario de `docs/fonctionnel/E16US020.md` déroulable sans `curl`. ⚠️ **À ne PAS résorber en ouvrant la route au scoreur** : `_role_de_saisie` lit `contexte is None` comme « admin » (cf. `test_un_scoreur_n_est_pas_une_identite_de_saisie_de_qualification`) |
...
**Le constat.** `E16US020` livre la règle d'ADR-0107 : une écriture d'un rôle **inférieur** à celui
qui a déjà écrit la volée est refusée en `409`. Elle est tenue par le serveur, et prouvée du domaine
à l'API. Mais l'**unique** appelant front de `POST /api/v1/saisie/volees` est
`features/saisie/api.ts`, qui passe la portée `'poste'` **en dur** — la tablette. Aucun écran
d'administration n'écrit de volée de qualification.

Or ADR-0107 §3 dit que **deux rôles égaux ne s'arbitrent pas**. Toutes les écritures venues de l'IHM
portant le même rang, le refus **ne peut pas se produire depuis les écrans livrés**. Le seul moyen
de l'observer aujourd'hui est un appel HTTP direct avec une session d'administrateur.

**La conséquence.** `MessageErreurSaisie` (le rendu ambre du refus) et l'`onError` qui relit la
vérité serveur sont **corrects, testés, et injoignables en production**. C'est exactement la forme de
défaut que `DETTE-085` décrit — du code que rien n'atteint — à ceci près qu'il est **volontaire**
ici, et que le test qui le couvre *simule* l'erreur au lieu de la provoquer.

⚠️ **Le piège n'est pas le code mort, c'est la lecture qu'on en fera.** Un contributeur qui lit
`Saisie.tsx` en conclura que ce refus a été vu à l'écran. Il ne l'a pas été.

**Pourquoi ce n'est pas corrigé dans l'US.** Ouvrir une surface d'administration qui saisit des
volées de qualification est une **US à part entière** — il faut décider où elle vit, ce qu'elle
montre, et comment elle cohabite avec la tablette. C'est un choix de périmètre, et le périmètre de
`E16US020` a été arbitré le 12/09/2026 : *la volée de qualification, côté règle, pas côté surface*.

**Pourquoi le front a quand même été livré.** Le chemin admin **existe** côté serveur
(`autoriser_saisie` rend `None` pour une session d'administrateur) : le `409` est atteignable par
l'API dès aujourd'hui, et le laisser retomber sur `MessageErreur` générique afficherait une phrase de
conflit **sans dire le recours**. On livre donc l'écran prêt, et on inscrit qu'il n'a pas servi.

**Résorption.** Elle ne demande rien à défaire — seulement un chemin à ouvrir. **Critère de fin** :
un appelant de `saisirVolee` en portée `'admin'`, et le scénario de
[`../docs/fonctionnel/E16US020.md`](fonctionnel/E16US020.md) déroulable **sans `curl`**.

⚠️ **Ce qui ne résorbe PAS cette ligne** : ouvrir la route au **scoreur**. `_role_de_saisie` lit
`contexte is None` comme « admin » ; élargir la garde sans faire porter le rôle par `ContexteSaisie`
donnerait au scoreur la préséance de l'organisateur. `test_un_scoreur_n_est_pas_une_identite_de_saisie_de_qualification`
rougira ce jour-là, et c'est son unique office.
