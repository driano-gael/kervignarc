...
|---|---|---|---|---|---|---|---|
| [DETTE-107](#dette-107--le-harnais-de-simulation-pilotée-ne-joue-quune-qualification) | conception | majeur | `backend/application/pilotage_simulation.py` (`ouvrir_sur_harnais`, `_creneaux`, marqueur `DETTE-107`) · `frontend/src/features/simulation/Simulation.tsx` | Le harnais résout **une** phase de qualification pour toute la session (`qualification_du_tournoi`) et y écrit toutes ses volées. Depuis `E06US009`, la session rend un bloc **par créneau** : les `N-1` autres affichent donc leurs inscrits **rangés à zéro**, et leur arbre est tout de même monté et joué sur ces zéros | Sur un tournoi multi-créneaux, le cockpit montre `N-1` classements à 0 point surmontés d'un podium de duels arbitrés sur des zéros — *un demi-résultat présenté comme un résultat*, la formule même dont `DETTE-045` se servait pour décrire le défaut que l'US venait de retirer. ⚠️ Avant l'US ces créneaux n'étaient **pas affichés du tout** : elle les rend visibles sans les rendre justes. Surface de démo/QA, derrière `exiger_admin` | **Ouverte en 2ᵉ passe de revue d'`E06US009`** (19/09/2026, mesurée par l'axe D : 4 lignes à 0 et un arbre joué sur le 2ᵉ créneau). Le volet était d'abord renvoyé à `DETTE-022`/`DETTE-047` — **à tort** : ces deux-là traitent des **forfaits**, pas de la phase d'écriture du harnais | Résoudre la qualification **par créneau** dans `ouvrir_sur_harnais`, comme `ServiceSaisie` le fait déjà en production (`la_plus_courante(par_depart(...))`, ADR-0082). ⚠️ **Tant qu'elle est ouverte**, ne pas piloter de simulation sur un tournoi multi-créneaux ; la limite est ancrée par `test_chaque_creneau_de_la_session_porte_son_classement_et_ses_arbres`, qui assère les totaux à zéro et **tombera** le jour où le moteur deviendra multi-qualification |
...
⚠️ **Résorbée côté PORTÉE D'AFFICHAGE, pas côté moteur** (précisé en revue, axe C1). La session de
simulation pilotée rend bien `N` créneaux, mais son harnais écrit toutes ses volées dans **une**
phase de qualification (`qualification_du_tournoi`, cf. `DETTE-022` et `DETTE-047`) : à quatre
créneaux, trois s'affichent **vides**. Ce volet appartient aux deux dettes citées, pas à celle-ci —
mais il fallait le dire, sinon « les trois sites résorbés » se lisait comme une promesse de moteur.
...
⚠️ **Cette dette ne porte PAS le harnais de simulation.** `E06US009` lui avait d'abord renvoyé le fait que la session pilotée n'écrive ses volées que dans une seule qualification — à tort : ici il s'agit des **forfaits**. Ce volet a son propre identifiant, `DETTE-107` (relevé en 2ᵉ passe de revue).
...
### DETTE-107 — le harnais de simulation pilotée ne joue qu'une qualification

**Où** : `backend/application/pilotage_simulation.py` — `ouvrir_sur_harnais` (résolution de
`session.phase_qualif_id` par `qualification_du_tournoi`) et `_creneaux` (marqueur `DETTE-107`).

Le harnais écrit **toutes** ses volées dans une seule phase de qualification, celle que
`application/portee.py` rend pour le tournoi — c'est-à-dire celle du **premier** créneau. Avant
`E06US009`, le cockpit n'affichait qu'un créneau : le raccourci était invisible.

**Effet, mesuré en revue** (décor à 2 créneaux × 4 archers) : le second créneau rend **4 lignes de
classement à 0 point** — ses inscrits y figurent, `ServiceClassement.pour_phase` les garde
explicitement — et **son arbre est monté et ses duels joués** sur ces zéros. Le cockpit affiche donc
un podium arbitré sur un classement de zéros.

⚠️ **Ce n'est pas « un créneau vide »**, et la nuance a coûté une passe de revue : la première
rédaction du commentaire, de la story et du registre écrivait « les autres sortent d'ici avec un
classement vide ». Trois artefacts, la même phrase, fausse dans le sens rassurant.

⚠️ **Mal attribuée au départ.** Le volet était renvoyé à `DETTE-022` et `DETTE-047`, qui traitent
l'une et l'autre des **forfaits** de qualification. Les résorber ne corrigerait pas le cockpit, et
le jeton `DETTE-047` posé ici polluait le `grep` de résorption de cette dette-là.

**Résorption** : résoudre la qualification **par créneau** dans `ouvrir_sur_harnais`, comme
`ServiceSaisie` le fait déjà en production (`la_plus_courante(par_depart(...))`, ADR-0082). La
limite est **ancrée par un test** (`test_chaque_creneau_de_la_session_porte_son_classement_et_ses_arbres`
assère `total == 0` sur le second créneau) : il tombera le jour où le moteur changera, ce qui est le
signal voulu.
...
⚠️ **L'option « rien » est défendable** et doit être dite : un seul incident constaté, déploiement
LAN mono-club, règle 12 (« la rigueur va au moteur métier, pas à l'outillage »). Ce qui manquait
n'était pas le remède mais l'**inscription** — la classe ne vivait que dans deux commentaires, dont
