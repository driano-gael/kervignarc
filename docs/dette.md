...
| [DETTE-078](#dette-078--quatre-réglages-détape-sont-refusés-après-avoir-été-persistés) | conception | **majeur** | `backend/domain/deroule_etape.py` (`__post_init__`, marqueur `# DETTE-078`) ↔ `backend/domain/phase.py` (les gardes `Configuration*Invalide`) | Les réglages `poules`, `big_shoot_off`, `suisse` et `profondeur` ne sont refusés — quand ils sont posés sur un **type de phase qui ne les lira jamais** — que par `Phase.__post_init__`, c'est-à-dire à `instancier()`. Or l'étape rejoint le déroulé **avant** cette instanciation : une requête invalide laisse donc derrière elle une étape **orpheline**, sans instance de phase, occupant un `ordre` que l'ajout suivant ne réutilise pas | Le déroulé d'un tournoi se troue (rangs 1, 2, 4) **du fait d'une requête que le serveur a rejetée en 422**. L'organisateur voit un rang manquant sans avoir rien composé de tel, et n'a aucune piste : l'erreur qu'il a lue disait que rien n'avait été fait. Faible fréquence — il faut retyper une phase déjà réglée — mais l'effet est durable et silencieux | **E05US027**, relevée par l'axe adversarial de la revue et **reproduite par exécution** (`POST /tournois/1/phases` → 422, puis étape orpheline en base). Le défaut est **hérité** : il existe pour les quatre réglages depuis leurs US respectives. L'US du jour ne l'aggrave pas — elle ferme le cas **neuf** (`colline`, garde portée par `EtapeDeroule.__post_init__` et testée aux deux étages) et inscrit les quatre autres ici plutôt que de les corriger en douce | ⚠️ **Revue le 20/09/2026 (E05US022), et le périmètre était doublement faux.** (a) Un 2ᵉ site, `ServiceFormats.appliquer`, **détruisait le déroulé en place** avant de lever — refermé dans l'US par la pose à blanc de `verifier_applicable`, qui instancie désormais chaque étape sur un créneau fictif. (b) Un **3ᵉ site subsiste, et c'est le pire** : `ServicePhases.modifier` fait `replace(etape, …)` puis `enregistrer`, **sans instancier** — un `PUT` qui retype une phase en gardant son réglage répond **200 et persiste**, puis **chaque lecture** tombe en 422. Il ne troue pas une numérotation, il rend un tournoi existant illisible. (c) Le 4ᵉ réglage listé était **faux** : `decoupage` est déjà gardé par `EtapeDeroule.__post_init__` (`verifier_decoupage_applicable`), comme `colline` ; celui qui ne vit que sur `Phase` est **`profondeur`**. Une US de résorption dérivant son périmètre de l'ancienne ligne aurait hissé un garde déjà hissé et oublié l'autre — le défaut même de `DETTE-080`. Hisser les quatre gardes restantes de `Phase` vers `EtapeDeroule.__post_init__`, sur le modèle de celle de `colline`, avec un test d'API « le refus ne persiste rien » par réglage. US `refactor/` dédiée : le geste est mécanique mais touche quatre invariants de composition et leurs tests, et il n'a rien à faire dans une US de format |
...
⚠️ **`ServicePhases.modifier` est un second site, plus grave, relevé le 20/09/2026** (3ᵉ passe de
revue d'E05US022, axe C2). Il fait `replace(etape, …)` puis `self._deroules.enregistrer(...)` :
...
**⚠️ Un TROISIÈME site existait, et E05US022 l'a refermé** (20/09/2026, trouvé par l'axe
...
**Résorption** : hisser les quatre gardes (`poules`, `big_shoot_off`, `suisse`, `profondeur`)
vers `EtapeDeroule.__post_init__` sur le modèle de `colline`, avec un test d'API « le refus ne
persiste rien » par réglage **et par verbe** (`POST` comme `PUT`). US `refactor/` dédiée.
