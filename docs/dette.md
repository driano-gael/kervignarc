...
`064`, `071`, `074`), dont plusieurs sont **légitimes** — une dette peut être *élargie* et
*partiellement résorbée* par la même US, et `DETTE-031` l'est par quatre. Le contrôle utile doit
donc porter sur autre chose : par exemple n'extraire `resorption_us` que d'un champ **balisé**
(`Résorption : ExxUSyyy`) au lieu de toute la cellule, ce qui rendrait les mentions en prose
inoffensives par construction. C'est une US d'outillage.

**Trois artefacts corrigés au passage par E16US005** (le fichier était déjà ouvert, l'atlas déjà
régénéré, le coût marginal nul) : `085` (le sien), `083` et `081`, où l'US citée l'était clairement
en *précédent à copier*. Les sept autres demandent un arbitrage cas par cas et ne sont pas touchés.
...
**Mise à jour du 24/08/2026 (E16US005).** Les marqueurs `// DETTE-010` sont **posés** sur les deux
constantes `POSITIONS` du front (`placement/Placement.tsx`, `duels/Duels.tsx`) ; il reste à en poser
un sur `gabarit_salle.py`, qui est la source du plafond. Deux items **s'ajoutent** au périmètre de
résorption, tous deux introduits par E16US005 :
...
- le test `Placement.test.tsx` « un désaccord front/serveur sur le plafond n'ouvre pas de colonne
  fantôme » et son miroir côté duels : ils **doivent tomber** à la résorption, et il faudra les
  retourner plutôt que les supprimer.
