...
**La dette reste donc à deux domiciles** (le troisième a été refermé avant merge), et son remède est
inchangé — `features/saisie-duels/duel.ts` doit à son tour consommer le libellé servi. E07US005
...
Marqueurs `# DETTE-025` posés sur la boucle de suppression **et** sur la recopie de l'exigence,
dans `ServiceFormats.appliquer` — les deux écritures que le remède devra réunir.
