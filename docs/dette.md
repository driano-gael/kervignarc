...
| [DETTE-063](#dette-063--le-système-suisse-noccupe-aucun-couloir) | conception | mineur | `backend/domain/contrat_phase.py` (ligne `SUISSE`, `plan_de_cibles=AUCUN`, marqueur `# DETTE-063`), `backend/application/suisse.py` | Une phase de système suisse est **jouable** (rondes appariées, rencontres saisies, classement lu) mais n'occupe aucun couloir : aucun service ne lui pose de plan de cibles. Le registre de contrat le déclare honnêtement `AUCUN` | Borné, et de même nature que `DETTE-059` (Big Shoot Off) : le jour J, les tireurs d'une ronde trouvent leur butte par l'écran de salle ou par le juge. Mais l'organisateur ne peut pas **préparer** la salle pour cette phase, là où il le peut pour une qualification, un tableau ou des poules | **E05US026.** Le CA demandait « le plan de cibles suit — mêmes règles d'empreinte et de contiguïté qu'en poules », et le mécanisme existe (`domain/placement_poules.py`, blocs de couloirs contigus). Ce qui manque est le service qui le pose **pour ce type**, plus une décision de vocabulaire : le port et la table s'appellent `placement_poule`, or une ronde de suisse n'est pas une poule. Non fait dans l'US, dont le périmètre avait déjà triplé (arbitrage du 15/08/2026) | Résorption : réutiliser `BlocDeCouloirs` pour **un bloc unique** couvrant `2 × (effectif ÷ 2)` couloirs, les couloirs de chaque rencontre étant dérivés à la lecture comme en poules. ⚠️ Aucune migration si l'on réutilise `placement_poule` — mais alors le nom ment, et c'est le vrai arbitrage à rendre : renommer le port et la table (migration), ou tracer le mensonge. À trancher **avant** de coder |
...
### DETTE-063 — le système suisse n'occupe aucun couloir
...
(`domain/placement_poules.py` : un groupe occupe un **bloc de couloirs contigus**, les couloirs de
chaque rencontre étant dérivés à la lecture). Deux choses manquent, et la seconde est un arbitrage,
pas du code :

1. le **service** qui pose ce bloc pour une phase de suisse ;
2. une décision de **vocabulaire**. Le port et la table s'appellent `placement_poule`, avec une
   colonne `poule_numero` — or une ronde de suisse n'est pas une poule. Y ranger un bloc de suisse
   ne coûte **aucune migration**, mais fait mentir le nom (règle 3 : le vocabulaire est cohérent
   entre code, API, UI et doc). Renommer coûte une migration.
