...
# Onglets du classeur : le tableau principal et les tableaux de placement.
FEUILLE_PRINCIPAL = "sheet3"  # « TABLEAU 1 OK »
FEUILLE_PLACEMENT = "sheet4"  # « TABLEAU 2 OK »
...
RE_SOURCE = re.compile(r"^(Perdant|Gagnant)\s*M\s*(\d+)$", re.IGNORECASE)
RE_ARCHER = re.compile(r"^Archer\s*(\d+)$")


def _cellules(archive: zipfile.ZipFile, feuille: str) -> dict[tuple[int, int], str]:
    """La feuille sous forme `{(ligne, colonne): texte}`, colonnes numérotées à partir de 1."""
    chaines = [
        "".join(t.text or "" for t in si.iter(f"{NS}t"))
        for si in ET.fromstring(archive.read("xl/sharedStrings.xml")).findall(f"{NS}si")
    ]
    grille: dict[tuple[int, int], str] = {}
    for ligne in ET.fromstring(archive.read(f"xl/worksheets/{feuille}.xml")).iter(f"{NS}row"):
        ...
        for (ligne, colonne), texte in grille.items():
            ...
            for decalage in (1, 2, 3):
                ...
                if re.fullmatch(r"\d+", voisin) and 1 <= int(voisin) <= 120:
                    table[voisin] = [source.group(1).lower(), int(source.group(2))]
                break
...
def main() -> None:
    ...
    with zipfile.ZipFile(CLASSEUR) as archive:
        principal = _cellules(archive, FEUILLE_PRINCIPAL)
        placement = _cellules(archive, FEUILLE_PLACEMENT)
