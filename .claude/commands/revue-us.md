...
> 9. **Tests** : unitaires priorité domaine (couverture élevée), intégration adapters/endpoints, déterministes (pas d'horloge/aléa non maîtrisé). L'**oracle 120** doit rester vert. Le diff ajoute/maintient les tests attendus.
...
5. **Ne lance pas** `git commit`/`push` toi-même sans l'aval (prompt `ask`) : propose-les. Le commit/push des correctifs suit le workflow habituel.
