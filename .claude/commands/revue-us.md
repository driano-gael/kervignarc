...
- S'il restait des **bloquants** non résolus, relance une passe de revue **cadrée sur les fichiers touchés par les correctifs** : grille complète, mais sur ce seul périmètre. Le reste du diff a déjà été relu et n'a pas bougé — le refaire relire coûte une passe entière (diff intégral + registre de dette) pour un résultat connu. Une passe intégrale (retour Étape 1) ne se justifie que si les correctifs ont **débordé** des fichiers déjà relus.
