...
**Dernière mise à jour : 20/08/2026, 10 h 45** · **118 US livrées** · dernière : `E05US034`
*(**la pause se voit, et se pose en cours de journée** — l'angle mort assumé d'`E05US033` est fermé,
et la capacité devient déployable. Le public et l'**écran de salle** annoncent la pause par un
bandeau (« le tir est suspendu par l'organisation ») au lieu d'un discret « en pause » ; le **tableau
de bord** rappelle qu'on attend (« 2 phases attendent votre relance depuis 14 min ») et se tait dès
que tout est relancé ; le **pilotage** dit où en est chaque phase (« Ronde 3 — tour 3 sur 5 ») et
laisse **poser une pause le jour J** (« bloquer dans 2 tours ») ; l'écran de **saisie du suisse**
nomme ce qui manque, en séparant *pas encore saisie* de *saisie non validée*.
⚠️ **Le geste du jour J a produit un ADR** : poser une pause pendant que la salle tire n'est **pas**
éditer le déroulé. L'ajouter à l'`EtapeDeroule` l'aurait fait rejouer par le créneau du soir, qui n'a
aucune raison de s'arrêter pour une panne réparée à midi. ADR-0076 avait déjà tranché — composer se
fait au tournoi (§4), **faire vivre se fait au départ** (§5) — sans qu'on ait eu à y revenir : d'où
`ArretDeCirconstance`, porté par le créneau et rejoué par personne
([ADR-0092](../docs/adr/0092-un-arret-pose-le-jour-j-appartient-au-creneau.md), migration `0049`).
⚠️ **Périmètre coupé au cadrage** : la fiche mêlait cinq CA d'IHM et un **chantier moteur** — rendre
la qualification divisible en tours —, profil exact qui avait déjà fait couper `E05US033`. La
lisibilité passe d'abord ; le chantier devient `E05US035`, 3ᵉ report du même CA.
⚠️ **Deux CA laissés ouverts par les tranches précédentes sont tranchés ici** : l'« état de tour
lisible » se rend par un **message circonstancié, sans clôture persistée** (ADR-0090 §5 dérive
l'avancement à la lecture ; en persister un second aurait fait deux sources pour une même vérité), et
la mention publique reste **sobre** — pas d'heure de reprise, que rien ne ferait tenir.
⚠️ **Défaut de registre relevé au passage** : `0091` manquait à la liste nominative d'ADR-0075 —
**4ᵉ** omission consécutive, la première constatée **hors revue**. `0091` et `0092` ajoutés, avec le
constat que rien dans le dépôt ne réclame cette inscription : la rendre vérifiable est une US.)*
...
> | **🎯 1** | `E05US035` | **La qualification se découpe en tours.** Sortie du périmètre d'`E05US034` au cadrage du 20/08/2026 — **3ᵉ report du même CA** (`E05US033` → `E05US034` → ici), et le motif n'a jamais changé : dériver le tour d'une qualification n'est pas un reste de plomberie. Il faut résoudre sa **population réelle** (deux qualifications peuvent coexister dans un créneau, ADR-0082, donc « les archers de cette phase » ≠ « les inscrits du créneau »), tenir compte du **plan de cibles** et **soustraire les forfaits**. ⚠️ **Deux tables doivent bouger ensemble** — le registre d'avancement du suivi et la table de refus par type (`TYPES_DEROULES` / `TYPES_ARRETABLES`) ; `backend/tests/test_arrets_api.py` porte les deux oracles en vis-à-vis pour qu'ils tombent ensemble. ⚠️ **Piège déjà identifié** : le tour d'une qualification peut **reculer** (un retardataire fait baisser le minimum du plateau) — `phases_a_arreter` le sait, tout calcul neuf doit l'être aussi. |
...
| 76quaterdecies | E05US035 | **La qualification se découpe en tours** | 🎯 *(sortie d'`E05US034` au cadrage du 20/08/2026 : « 20 volées en 2 tours de 10 », pour qu'une pause puisse s'y poser. Suppose de résoudre la **population réelle** d'une qualification (ADR-0082), le plan de cibles et les forfaits — un chantier moteur, pas un ajout à une table de types)* |
...
- **Règle de comptage d'un jalon** : `n/N` compte les **lignes portant un identifiant d'US**
  ...
  **dans leur section d'origine**, pas dans le jalon — sans quoi la même US serait comptée deux
  fois. C'est cette règle qui donne J0 12/12, J1 46/46, J2 14/14, J3 21/24 et J4 0/7.
