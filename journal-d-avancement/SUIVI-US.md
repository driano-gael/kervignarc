...
**aussi vers le bas** (l'organisateur qui saisit par erreur bloque la cible ; recours = annuler).
⚠️ **Périmètre arbitré le 12/09/2026 : la qualification seule** — `DETTE-065` n'est donc ni élargie
ni résorbée. ⚠️ **Défaut trouvé en rédigeant la recette, inscrit `DETTE-100`** : **aucun écran ne
déclenche le refus** — la tablette est le seul appelant de `POST /saisie/volees`, et deux tablettes
sont à rang égal. La règle est tenue par le serveur et prouvée de bout en bout ; le rendu ambre livré
est **prêt mais injoignable**. ⚠️ **Trois liens morts vers ADR-0107 corrigés au passage** (`…-par-le-rang-…`
...
> | ~~🎯 1~~ ✅ | ~~`E16US020`~~ | **Livrée le 12/09/2026** — la hiérarchie `poste de cible < scoreur < admin` est le **premier ordre entre rangs du dépôt** (ADR-0107, dont la section « Porté dans le code par » passe de « rien à ce jour » à une table décision → module → test). Migration `0055` : l'**état persisté** que quatre axes avaient déclaré requis. ⚠️ **Le piège central a tenu** : `saisie_par` reste déclaratif, le rôle vient de la garde — deux tests l'épinglent, dont un où la tablette signe « Administrateur ». ⚠️ **Deux renoncements assumés** (rang égal non arbitré — **test négatif** ; refus vers le bas) et **un périmètre arbitré le 12/09** : la qualification seule, donc `DETTE-065` n'est **ni élargie ni résorbée**. ⚠️ **`DETTE-100` ouverte, trouvée en rédigeant la recette** : **aucun écran ne déclenche le refus** (la tablette est le seul appelant, et deux tablettes sont à rang égal) — la règle est prouvée serveur, le rendu ambre est prêt mais injoignable. Ce n'est pas un raccourci : la surface admin de saisie n'existe pas, et l'inventer sortait du périmètre. |
