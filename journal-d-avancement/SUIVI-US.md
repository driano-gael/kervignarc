...
**Dernière mise à jour : 12/09/2026, 13 h 04** · **139 US livrées** · dernière : `E16US020`
*(**deux écritures, une même volée : le rôle tranche** — la réponse S09 (« hiérarchie → archer <
scoreur < admin ») devient une règle de serveur, sur les **identités réelles** du dépôt :
`poste de cible < scoreur < admin`. Une volée retient le **rôle de la garde** (jamais `saisie_par`,
déclaratif — c'était le piège central, trouvé au découpage d'`E16US011`), une écriture de rang
inférieur est refusée en `409`, et **annuler une validation remet la préséance à zéro** — sans quoi
l'US contredirait le CA d'`E16US019`. Migration `0055` : l'état persisté que quatre axes de revue
avaient déclaré **requis** contre une 1ʳᵉ rédaction qui le croyait dérivable de la garde.
⚠️ **Deux renoncements assumés**, dits avant décision : à **rang égal** rien n'est arbitré (deux
tablettes — **le cas le plus fréquent en salle**), épinglé par un **test négatif** ; et le refus joue
**aussi vers le bas** (l'organisateur qui saisit par erreur bloque la cible ; recours = **il
ressaisit lui-même** — « annuler la validation » était faux, il n'y a rien à annuler sur une volée
non validée).
⚠️ **Périmètre arbitré le 12/09/2026 : la qualification seule** — `DETTE-065` n'est donc ni élargie
ni résorbée. ⚠️ **`DETTE-100`** : le refus **vers le bas** reste
injoignable — aucun écran d'administration ne saisit de volée. ⚠️ **Cinq majeurs de revue traités
en 2ᵉ passe**, dont trois que la grille seule n'aurait pas vus : `corriger_volee` n'apposait
**aucune** préséance (le scoreur corrigeait, la tablette écrasait en silence — la règle se
contournait par le **choix de l'endpoint**) ; le
`409` était réputé **transitoire** par la file hors-ligne, donc une volée refusée **gelait le
drainage de la tablette** et les suivantes ne partaient jamais (ADR-0037 contredit) ; et ma section
« Porté dans le code par » fabriquait **7 faux signaux d'atlas** sur son propre ADR, noyant le seul
contrôle capable d'attraper un ADR-0017 bis. ⚠️ **Trois liens morts vers ADR-0107 corrigés au passage** (`…-par-le-rang-…`
au lieu de `…-par-le-role-…`, dans ce fichier, `stories/E16` et `stories/README.md`).
Avant elle, `E16US019` — **annuler une validation pour corriger** — le scoreur reçoit enfin l'**écran de validation de
...
> | **🎯 1** | reste d'`E16` | `E16US021` (accueil par départ — **périmètre à borner avant de coder**), `E16US016` (reliquat d'exports — **arbitrage `xlsx`, règle 11**). **L'ordre entre les deux reste libre.** Deux membres de la famille « prêt à… » restent à instruire (*prêt à archiver*, *prêt à exporter*) — le second se rattache à `E16US016`. |
...
> **quatorze livrées** (plus `E16US011`, close par découpage sans code) ; le tableau complet est en
...
| E16US021 | L'accueil du tournoi parle par départ | — | ⬜ *(A02, arbitrage du 10/09/2026 : l'encart va sur l'**Accueil** (destination par défaut de l'axe pilotage), pas sur « Suivi du déroulé » — qui fait l'inverse du CA, un créneau **à la fois**. L'accueil ne porte aujourd'hui que des **totaux tournoi**. ⚠️ Le risque n'est pas le rendu mais le **doublon d'affordance** : la frise, la pastille de relance et « Prêt à démarrer ? » vivent déjà là — l'US doit dire ce qu'elle **enlève**. ⚠️ À vérifier au cadrage : une lecture **agrégée par départ** existe-t-elle côté serveur, ou l'accueil monterait-il une requête par créneau à chaque poll (`DETTE-031`) ? ⚠️ « toutes les informations utiles au déroulé » est la formule la plus large du questionnaire — à borner avec le commanditaire **avant** de coder)* |
