...
`archiver`, dont le jalon répond `404`. ⚠️ **Zéro requête de plus** : `lister_avec_etat` calculait déjà
un avancement complet par créneau et **jetait tout sauf l'état**)*.
...
> | ~~🎯 1~~ ✅ | ~~`E16US021`~~ | **Livrée le 12/09/2026** — l'accueil s'ouvre sur **un bloc par départ** (horaire, quota, état, effectif du créneau, pause) au lieu de totaux tournoi. ⚠️ **Le cadrage était la moitié de l'US** : « toutes les informations utiles au déroulé » a été bornée avec le commanditaire **avant** la première ligne, et **deux de ses réponses se contredisaient** — elles auraient supprimé une capacité d'`E05US034` et perdu deux chiffres non ventilables. ✅ **`DETTE-082` resserrée** (la frise renvoie vers « prêt à… » sur l'accueil), et **zéro requête de plus** : le service jetait déjà l'avancement qu'il calculait. |
