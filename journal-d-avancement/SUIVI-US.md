...
| E16US011 | Ce que trois questionnaires « validés » demandaient quand même | — | ✅ *(**close le 10/09/2026 — par DÉCOUPAGE, sans livrer une ligne de code.** Devenue une **carte** : ses sept règles ont été vérifiées une par une dans le code du jour **avant** tout arbitrage, et **trois des sept constats de sa rédaction d'origine étaient faux**. Résultat : **deux CA tombent sans coûter une ligne** — le retardataire est **déjà tenu** (ADR-0024 + « Placer les restants ») et le reliquat de vocabulaire « position » est **faux pour la 5ᵉ fois d'affilée**, avertissement désormais éteint partout ; **un CA est déplacé** (`A09`/licence → `E02US007`, ce qu'ADR-0014 et ADR-0015 avaient nommé mot pour mot comme point de réouverture) ; **un CA est écarté** (`P05`/horaires prévisionnels : rien à borner, le refus est déjà tranché depuis le 04/08/2026 et tracé dans `VueTableaux.tsx` — le CA demande une **création**) ; le reste part en **quatre US filles**. ⚠️ **Sa « contradiction » S08 n'en était pas une** : « plus de modification une fois validé » et « une validation peut être annulée » décrivent **une procédure en deux temps**, pas deux règles qui s'opposent — et le constat était à moitié faux, les **duels** tenant déjà la règle. ⚠️ **Trouvé au découpage, absent de toutes les rédactions** : `Volee.saisie_par` est **déclaratif** — un nom libre venu du corps de la requête —, donc inutilisable comme source d'autorité ; sans ce constat, `E16US020` aurait livré une hiérarchie qu'un poste contourne en se déclarant admin)* |
| E16US018 | Le panneau de routage rend la tablette tout seul | — | 🎯 *(S06. **Front seul, aucune route, aucune migration** : la distinction « un tour suit » existe déjà côté serveur (`IssueRoutage`). Le minuteur de 3 min est **absent** sans ambiguïté ; la place finale du perdant, elle, est **déjà rendue — par accident**, par repli sur `None`, donc à épingler par un test plutôt qu'à corriger. ⚠️ Un point à trancher au cadrage : quelles issues comptent comme « un tour suit » (`en_attente` et `indisponible` sont les cas douteux))* |
