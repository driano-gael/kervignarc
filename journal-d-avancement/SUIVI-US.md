...
**Dernière mise à jour : 24/08/2026, 17 h 09** · **124 US livrées** · dernière : `E16US005`
*(**le plan de cibles tient sur la largeur d'un PC** — une **cible par ligne**, couloirs côte à côte
et alignés d'une bande à l'autre, au lieu d'une mosaïque de vignettes de 160 px où les noms étaient
coupés. La largeur gagnée porte, sous chaque nom, **club · catégorie · blason** : soit exactement ce
qui décide des deux badges que le serveur posait déjà au niveau **cible** — « mixité non garantie »,
« cloisonnement non respecté » — et qui désignaient jusqu'ici un problème sans dire **lequel** des
quatre occupants le causait. Un club non renseigné dit « **club inconnu** », jamais « aucun club » —
c'est le cas que le serveur traite comme *indécidable* pour la mixité, donc la cause même du badge ;
un référentiel non chargé n'affiche **rien** plutôt qu'un « Club #7 ». La **réserve** devient un panneau **collant** à droite —
le glisser-déposer natif ne fait pas défiler la page, et une cible par ligne allongeant le plan, la
laisser en pied d'écran aurait *aggravé* le geste que le questionnaire demandait de simplifier. Le
**plan de duels est aligné** dans le même diff (arbitrage du cadrage : même classe CSS, même
utilisateur, même PC). ⚠️ **Deux CA sur trois étaient déjà tenus** — le puits de réserve depuis
`E03US004`, la distinction « mis de côté » / « impossible à placer » depuis `E03US007`, la
préservation des placements manuels par « Placer les restants » : ils gagnent ici les tests de
non-régression qui leur manquaient. ⚠️ **Le reliquat de vocabulaire annoncé n'existait pas.** **Front seul, aucune
migration, aucun DTO touché** ; `DETTE-085` inscrite — les deux plans sont des jumeaux recopiés
composant par composant, et cette US est la première à payer la copie.)*
...
| E16US007 | Impressions, exports et podiums paramétrables | — | ⬜ *(**à redécouper avant de prendre** ; porte aussi le membre **prêt à exporter** de la famille, dont la forme est désormais posée — ADR-0096)* |
| E16US008 | Feu vert : agir depuis la ligne du duel qui bloque | — | ⬜ *(la forme du « puis-je passer à la suite » lui est fournie par ADR-0096 : plus rien à inventer de ce côté)* |
...
- **Règle de comptage d'un jalon** : `n/N` compte les **lignes portant un identifiant d'US**
  ...
  **dans leur section d'origine**, pas dans le jalon — sans quoi la même US serait comptée deux
  fois. C'est cette règle qui donne J0 12/12, J1 46/46, J2 14/14, J3 24/26 et J4 0/7.
