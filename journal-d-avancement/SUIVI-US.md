...
**Dernière mise à jour : 20/08/2026, 16 h 10** · **119 US livrées** · dernière : `E05US035`
*(**la qualification se découpe en tours, et peut enfin s'arrêter**. Trois fois reportée, la voici :
l'organisateur règle « 20 volées en **2 tours** de 10 » à l'atelier, l'écran lui dit ce que ça donne
avant qu'il ne soumette, et une **pause programmée** peut désormais se poser sur le format que tout
le monde tire. L'avancement se lit au rythme du **dernier archer** — population réelle de la phase
(deux qualifications peuvent coexister dans un créneau), archers réellement **placés**, forfaits
**soustraits** : c'est ce trio qui avait fait reporter le CA trois fois, et compter faux ne produit
pas une erreur mais une salle qu'on coupe trop tôt, ou qui n'avance jamais.
⚠️ **Un obstacle absent de la fiche a été découvert en implémentant** : la table qui refusait l'arrêt
(`TYPES_DEROULES`) décide **aussi** du plancher d'inscrits (E05US021). Y verser la qualification
aurait fait **refuser le démarrage** d'un tournoi à qualification prélevée, en échange d'un réglage
d'affichage. « Arrêtable » devient donc une **capacité à part entière** du registre de phase, à côté
de « déroulé » — exactement le raisonnement qui avait déjà séparé « sait lire le classement »
([ADR-0093](../docs/adr/0093-une-qualification-se-decoupe-en-tours-egaux.md)).
⚠️ **Le découpage est tenu hors du barème** — *avancer ≠ classer*, ADR-0090. Conséquence concrète :
modifier le découpage en cours de phase ne **re-partitionne aucun score**, là où le même geste sur un
Big Shoot Off rejoue des éliminations (`DETTE-062`). Ce n'était pas une élégance, c'était la
différence entre les deux.
⚠️ **Le test écrit pour tomber est tombé.** `E05US033` avait laissé un test affirmant « aucun lecteur
d'avancement n'est ajouté par cette tranche », en annonçant : *« le jour où E05US035 branchera la
qualification, les deux devront bouger ensemble »*. Il a tenu, et il est retourné en comparaison des
deux registres.
⚠️ **`0093` est inscrit du premier coup** à la liste nominative d'ADR-0075, après **quatre**
omissions consécutives — le rappel écrit par l'US précédente a fonctionné, sur l'US qui la suivait
immédiatement. Le constat de fond ne change pas : rien dans le dépôt ne réclame cette inscription.
**Aucune migration.** `DETTE-054` élargie d'une 7ᵉ paire de DTO jumeaux.)*
...
- **Règle de comptage d'un jalon** : `n/N` compte les **lignes portant un identifiant d'US**
  ...
  **dans leur section d'origine**, pas dans le jalon — sans quoi la même US serait comptée deux
  fois. C'est cette règle qui donne J0 12/12, J1 46/46, J2 14/14, J3 22/24 et J4 0/7.
