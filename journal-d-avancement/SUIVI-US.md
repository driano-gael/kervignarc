...
demande, un scoreur à la fois**, l'appel réseau lui-même désarmé. ⚠️ **La forme d'URL n'est PAS celle
...
| E16US015 | Un QR par scoreur | 04/09/2026 | ✅ *(le scoreur ouvre sa session **en scannant**. ⚠️ **La question de sécurité a été tranchée au cadrage** : le QR porte le code **en clair** — il n'expose rien de plus que la liste admin, le PDF des cartes et la base, qui le stockent déjà en clair —, mais le risque est **contextuel** (un QR se photographie d'un mètre) donc traité par l'affichage : **à la demande, un scoreur à la fois**, l'appel réseau lui-même désarmé par `enabled`. ⚠️ **La forme d'URL n'est PAS celle du jumeau** : `…/scoreur?code=` et non `…/?poste=` — le chemin nomme le monde, le routeur d'adresses (ADR-0059) aiguille seul, **zéro ligne** ajoutée à `resoudreRole`, le cœur risqué de l'entrée (ADR-0042). ⚠️ **Fuite trouvée en rédigeant la recette** : un scoreur **déjà connecté** qui rescanne ne monte pas le formulaire — l'effacement de l'adresse a dû remonter dans `EspaceScoreur`. Rotation d'un code **écartée du périmètre** par le commanditaire)* |
