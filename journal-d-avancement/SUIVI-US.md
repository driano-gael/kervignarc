...
**aucune** préséance (le scoreur corrigeait, la tablette écrasait en silence — le défaut même de
l'US, déplacé d'un endpoint à l'autre, et **le seul croisement joignable depuis les écrans**) ; le
...
> | ~~🎯 1~~ ✅ | ~~`E16US020`~~ | **Livrée le 12/09/2026** — la hiérarchie `poste de cible < scoreur < admin` est le **premier ordre entre rangs du dépôt** (ADR-0107, dont la section « Porté dans le code par » passe de « rien à ce jour » à une table décision → module → test). Migration `0055` : l'**état persisté** que quatre axes avaient déclaré requis. ⚠️ **Le piège central a tenu** : `saisie_par` reste déclaratif, le rôle vient de la garde — deux tests l'épinglent, dont un où la tablette signe « Administrateur ». ⚠️ **Deux renoncements assumés** (rang égal non arbitré — **test négatif** ; refus vers le bas) et **un périmètre arbitré le 12/09** : la qualification seule, donc `DETTE-065` n'est **ni élargie ni résorbée**. ⚠️ **`DETTE-100`** : le refus **vers le bas** reste injoignable (aucun écran d'administration ne saisit). ⚠️ **Revue : 5 majeurs, 0 bloquant, traités en 2ᵉ passe.** Le plus coûteux — `corriger_volee` n'apposait aucune préséance, donc le scoreur corrigeait et la tablette écrasait **en silence** : le défaut de l'US déplacé d'un endpoint à l'autre, et **le seul croisement de rangs joignable depuis les écrans**. Quatre axes l'ont trouvé indépendamment. Aussi : le `409` gelait la **file hors-ligne** (volées suivantes jamais envoyées, ADR-0037 contredit), le « recours » annoncé dans **cinq artefacts** n'existait pas, ma table de portage fabriquait **7 faux signaux d'atlas**, et deux dettes se publiaient **résorbées par l'US qui les ouvre**. |
