...
**Dernière mise à jour : 10/09/2026, 20 h 56** · **137 US livrées** · dernière : `E16US018`
...
> | **🎯 1** | reste d'`E16` | `E16US019` (annuler une validation), `E16US020` (hiérarchie de rôles — **ADR-0107 écrit** ; **état persisté requis**), `E16US021` (accueil par départ — **périmètre à borner avant de coder**), `E16US016` (reliquat d'exports — **arbitrage `xlsx`, règle 11**). **L'ordre entre les quatre reste libre.** Deux membres de la famille « prêt à… » restent à instruire (*prêt à archiver*, *prêt à exporter*) — le second se rattache à `E16US016`. |
...
| E16US019 | Annuler une validation pour corriger | — | ⬜ *(S08, arbitrage du 10/09/2026 : **annuler → corriger → revalider**, et `corriger_volee` **disparaît** — un seul chemin d'écriture sur un score validé, donc un seul endroit où tenir l'audit. Les **duels** tiennent déjà la règle (`DuelVerrouille`) : rien à y faire pour ce CA. `Volee.verrouillee` étant **dérivé** de `validee_par`, annuler = reposer le champ à `None` — donc **probablement aucune migration**. ⚠️ Périmètre à trancher : l'annulation vaut-elle pour un **duel** ? Valider un duel fait avancer le tableau — l'annuler défait une progression déjà diffusée, soit une US de moteur. ⚠️ Retirer une route est un changement d'API, et l'audit doit **gagner** une trace au lieu d'en perdre une)* |
