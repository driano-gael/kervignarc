...
> ⚠️ **136 fiches closes, mais 134 livraisons.** Le compteur est **recalculé par l'atlas** et compte
> des **fiches**, pas des branches — deux fiches sont closes sans livraison propre : `E01US016`
> (absorbée par `E16US006`, 25/08/2026) et `E16US011` (**close par découpage** le 10/09/2026, elle se
> remplace par quatre US filles). On le **note** plutôt que de forcer le chiffre à la main : c'est le
> compteur mécanique qui fait foi, la nuance se lit ici.
...
| E16US020 | Deux postes, une même volée : le rôle tranche | — | ⬜ *(S09, **candidate à un ADR** — premier ordre entre rôles du dépôt, qui n'a aujourd'hui que des prédicats indépendants. Il n'existe **aucun** mécanisme de conflit : la file d'écriture **sérialise** sans **arbitrer**, donc c'est du *last-write-wins* silencieux. ⚠️ **Le revers a été exposé au commanditaire et assumé** : une hiérarchie tranche **sans prévenir** — un scoreur qui se trompe écrasera silencieusement un archer qui avait raison ; le refus explicite (`409` + rafraîchir) est **écarté, pas oublié**. ⚠️ **Migration attendue** : le rôle doit être **persisté** et tiré du **jeton**, `saisie_par` étant déclaratif)* |
