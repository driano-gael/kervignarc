...
le **05/08** en écrans pleins. **7 planches sur 9** ne proposent plus les variantes jugées, et sur
...
⚠️ **L'écart 🔴 restant n'est pas un oubli** : mettre les flèches dans la ligne d'archer heurte un
invariant établi en revue le 05/08 (une zone tapable de plus y détruisait le tampon de frappe du
pavé). Il part en **`E17US011`**, l'invariant nommé. ⚠️ **Défaut trouvé en chemin, qu'aucun relevé ne
...
> en silence** le contrôle `derniere-us-orpheline` — constaté et corrigé en revue le 10/09/2026.

**Précédente : `E16US015`**
*(**un QR par scoreur** — le scoreur ouvre sa session **en scannant**, au lieu de retaper son code.
⚠️ **La question de sécurité annoncée par la fiche a été posée au commanditaire et tranchée avant
d'implémenter** : le QR porte le code **en clair**, parce qu'il n'expose rien de plus que la liste
admin, le PDF des cartes et la base, qui le stockent déjà ainsi — le risque est **contextuel** (un QR
se photographie d'un mètre, un code demande de s'approcher), donc traité par l'affichage : **à la
demande, un scoreur à la fois**. ⚠️ **La forme d'URL n'est PAS celle
du jumeau cible** : `…/scoreur#code=` et non `…/?poste=` — le chemin nomme le monde, le routeur
d'adresses ([ADR-0059](../docs/adr/0059-routage-par-role-dans-l-url-routeur-maison.md)) aiguille
seul, **zéro ligne** ajoutée à `resoudreRole`, que son propre en-tête nomme le cœur risqué de
l'entrée ([ADR-0042](../docs/adr/0042-modele-d-entree-choix-de-role-explicite.md)). ⚠️ **Deux
constats non prévus** : `ScoreurRepository.par_id` ne filtre pas par tournoi — sans garde
d'appartenance, l'admin d'un tournoi lisait le code d'un scoreur d'un autre ; et un scoreur **déjà
connecté** qui rescanne ne monte pas le formulaire, si bien que l'effacement du code dans l'adresse a
dû remonter d'un cran. **Rotation d'un code écartée du périmètre** par le commanditaire)*
...
> | ~~🎯 1~~ ✅ | ~~`E16US015`~~ | **Livrée le 04/09/2026** — un QR par scoreur, révélé **un à la fois sur geste explicite**. La question de sécurité annoncée par la fiche a été posée au commanditaire et tranchée avant d'implémenter ; voir sa ligne pour les trois constats. |
> | ~~🎯 1~~ ✅ | ~~`E16US011`~~ | **Close le 10/09/2026 — par DÉCOUPAGE, sans une ligne de code**, comme ses propres Notes le prescrivaient. Les sept règles ont été vérifiées **dans le code du jour avant tout arbitrage**, et **sept des dix lignes de son tableau corrigent sa rédaction d'origine** : le CA « retardataire » est **déjà tenu**, le reliquat de vocabulaire « position » est **faux pour la 5ᵉ fois d'affilée** (avertissement éteint partout), et **ni l'une ni l'autre de ses deux « contradictions » n'en était une**. S08 (« plus de modification une fois validé » **vs** « une validation peut être annulée ») décrit **une procédure en deux temps**, pas deux règles qui s'opposent — et son constat était à moitié faux, les **duels** tenant déjà la règle. A09 (licence) n'était pas contredit par ADR-0014/0015 : ces deux ADR **nomment `E02US007` comme point de réouverture**, mot pour mot. **Quatre arbitrages rendus** : l'annulation de validation s'ajoute et `corriger_volee` est **conservé** (3 rédactions : supprimé, élargi, puis gardé — il porte la garde inter-tournois, la trace avant/après et un auteur authentifié, et le supprimer auto-signait `D-03`) au lieu d'annuler → corriger → revalider ; A09 part en `E02US007` ; la **hiérarchie** `poste de cible < scoreur < admin` est retenue (ADR-0107) (⚠️ revers assumé — elle tranche **sans prévenir**, le `409` explicite est écarté, pas oublié) ; l'encart par départ va sur l'**Accueil**. ⚠️ **Un piège absent de toutes les rédactions, trouvé au découpage** : `Volee.saisie_par` est **déclaratif** — un nom libre venu du corps de la requête — donc inutilisable comme source d'autorité ; sans ce constat `E16US020` livrait une hiérarchie contournable en se déclarant admin. ⚠️ **Défaut de tracker corrigé au passage** : `E16US013` figurait **deux fois** dans le tableau d'`EPIC-16`, gonflant le compte annoncé. |
> | ~~🎯 1~~ ✅ | ~~`E16US018`~~ | **Livrée le 10/09/2026** — le panneau de routage **rend la tablette tout seul** : fermeture au bout de **3 minutes**, signal **discret et non chiffré** (barre + « Retour automatique »), et la **poignée de réouverture** ajoutée côté duels. ⚠️ **Le cadrage a supprimé les deux questions au lieu de les arbitrer** : un minuteur **unique**, aveugle aux issues, clôt d'un coup l'**agrégation** et le **cas terminal**. La règle « il reste dès qu'une ligne est terminale » a été écartée sur mesure : il y a un battu à **chaque** duel, elle n'aurait **jamais** refermé l'écran de duels — US livrée sans effet, tests verts. ⚠️ **Le CA « place finale » n'était pas « tenu par accident »** : il est fondé dans `Tableau.classement()` (« places **acquises** ») — manquaient le **pointeur** au site du routage et le sens « pas encore décidé ⇒ rien annoncé », qu'aucun test ne couvrait. ✅ **Écart de planche fermé à la source** : `maquettes/s06-routage.html` porte le signal et sa question « combien de temps » passe à *tranchée*. ⚠️ **La revue a rendu 8 majeurs et zéro bloquant, dont DEUX tests placebo** — le test backend épinglait des valeurs par défaut de dataclass (sortie anticipée en `PROCHAIN_DUEL`), le test front une assertion sur un élément sans texte : les deux étaient verts sous la régression qu'ils nommaient. Refaits, et **vérifiés par mutation**. ⚠️ **Trois défauts de production trouvés par l'axe adversarial seul** : l'ancre du minuteur vivait dans le composant, donc un refetch en échec (`retry: false`) le remontait avec 3 min neuves — le panneau ne se refermait **jamais** sur un wifi de salle ; la poignée duels rouvrait un instantané d'une heure sous un libellé muet ; et les **avis permanents** (pause de 15-20 min) étaient emportés à la 3ᵉ minute — d'où `Routage.avis_permanent`, DTO **additif**, arbitré par le commanditaire. [ADR-0108](../docs/adr/0108-un-panneau-d-annonce-se-referme-sur-une-duree-jamais-sur-ses-issues.md) écrit. |
> | ~~🎯 1~~ ✅ | ~~`E16US019`~~ | **Livrée le 11/09/2026 — et le périmètre a doublé au cadrage, pour une raison qui n'était écrite nulle part.** Le CA « l'écran dit ce qu'il fait » supposait un écran de validation de qualification qui **n'existait pas** : le backend exposait `POST /saisie/validations` depuis E04US002, **aucun composant front ne l'appelait**, et le découpage d'`E04US002` le disait en toutes lettres — « la surface **scoreur** (§7.3) » n'était dans aucune de ses quatre tranches, et aucune US ne l'a reprise depuis. L'US livre donc **la surface scoreur de la qualification** (voir, valider) **plus** l'annulation. ⚠️ **Le commanditaire a écarté les DEUX branches que la fiche proposait** sur le sort du score pendant la correction, pour une troisième : la volée **reste comptée** — [ADR-0109](../docs/adr/0109-une-volee-en-correction-reste-comptee.md), migration `0054`. ⚠️ **Deux affirmations de la fiche étaient fausses et ont été corrigées** : « aucune migration » (le découplage verrou/compte exige un état persisté) et « le lot se recalcule par le grain » (`saisir_volee` n'impose aucun ordre : un lot réel peut être **non contigu**). |
> | ~~🎯 1~~ ✅ | ~~`E16US020`~~ | **Livrée le 12/09/2026** — la hiérarchie `poste de cible < scoreur < admin` est le **premier ordre entre rangs du dépôt** (ADR-0107, dont la section « Porté dans le code par » passe de « rien à ce jour » à une liste à puces (format ADR-0109 — la table initiale produisait 7 faux signaux d'atlas)). Migration `0055` : l'**état persisté** que quatre axes avaient déclaré requis. ⚠️ **Le piège central a tenu** : `saisie_par` reste déclaratif, le rôle vient de la garde — deux tests l'épinglent, dont un où la tablette signe « Administrateur ». ⚠️ **Deux renoncements assumés** (rang égal non arbitré — **test négatif** ; refus vers le bas) et **un périmètre arbitré le 12/09** : la qualification seule, donc `DETTE-065` n'est **ni élargie ni résorbée**. ⚠️ **`DETTE-100`** : le refus **vers le bas** reste injoignable (aucun écran d'administration ne saisit). ⚠️ **Revue : 5 majeurs, 0 bloquant, traités en 2ᵉ passe.** Le plus coûteux — `corriger_volee` n'apposait aucune préséance : deux chemins d'écriture sur le même agrégat, un seul qui revendique, donc la règle se contournait **par le choix de l'endpoint**. Quatre axes l'ont trouvé indépendamment. ⚠️ **Et la 2ᵉ passe a dû se corriger elle-même** : elle avait justifié ce correctif par « le scoreur écrit depuis l'écran d'`E16US019` » — faux, `POST /saisie/corrections` n'a **aucun appelant front**. Un énoncé juste remplacé par un faux dans six artefacts, relevé par trois axes en 2ᵉ passe. Réflexe à garder : chercher l'**appelant**, pas raisonner sur l'endpoint. Aussi : le `409` gelait la **file hors-ligne** (volées suivantes jamais envoyées, ADR-0037 contredit), le « recours » annoncé dans **cinq artefacts** n'existait pas, ma table de portage fabriquait **7 faux signaux d'atlas**, et deux dettes se publiaient **résorbées par l'US qui les ouvre**. |
> | ~~🎯 1~~ ✅ | ~~`E16US021`~~ | **Livrée le 12/09/2026** — l'accueil s'ouvre sur **un bloc par départ** (horaire, quota, état, effectif du créneau, pause) au lieu de totaux tournoi. ⚠️ **Le cadrage était la moitié de l'US** : « toutes les informations utiles au déroulé » a été bornée avec le commanditaire **avant** la première ligne, et **deux de ses réponses se contredisaient** — elles auraient supprimé une capacité d'`E05US034` et perdu deux chiffres non ventilables. ✅ **`DETTE-082` resserrée** (la frise renvoie vers « prêt à… » sur l'accueil ; le compilateur garde désormais l'ajout d'`archiver`). ⚠️ **Le vrai défaut a été trouvé en revue** : la liste des départs n'était **pollée par personne** — sa fraîcheur venait de `useRealtime`, qui n'invalide que sur événement et ignore la reconnexion, et les inscriptions n'émettent aucun `LiveEvent`. L'effectif du matin serait resté figé sur l'écran d'atterrissage. |
...
> | 🔒 **en attente de vous** | **tour 2 des questionnaires `S**`** | **Trois planches de saisie n'ont plus d'étalon** — `S04`, `S05`, `S08` —, et aucune US ne peut les résorber sans deviner. Les questionnaires du 04/08 ont été remplis sur les **vignettes** ; les planches ont été redessinées le 05/08. Les feuilles de saisie existent déjà (`maquettes/questionnaires/s0*.html`) et produisent le `.md` à déposer. ⚠️ C'est **votre** temps, pas celui de l'assistant : d'où sa place ici. |
>
> ---
>
...
> reste est spécifié dans [`stories/E16`](../stories/E16-retours-maquettes.md), **vingt-et-une US** dont
> **quinze livrées** (plus `E16US011`, close par découpage sans code, et `E16US013` **parquée** —
> l'épic est donc **soldé** au 18/09/2026) ; le tableau complet est en
> section « Retours du questionnaire de maquettes
> (EPIC-16) », qui fait foi. *(Ce compte disait « douze US dont trois livrées », figé au 04/08/2026 :
> corrigé le 29/08/2026 — `E16US013` est née depuis, et sept US ont été livrées entre-temps. Corrigé
> à nouveau le 10/09/2026 : le découpage d'`E16US011` ajoute quatre US filles, et le compte de la
> section détaillée était lui-même faux — `E16US013` y figurait en double.)*
>
...
> et ~~`E16US011`~~ **close le 10/09/2026 par découpage** — devenue une carte, remplacée par
> `E16US018` (routage : minuteur de 3 min) · `E16US019` (annuler une validation) · `E16US020`
> (hiérarchie de rôles sur écriture concurrente) · `E16US021` (accueil par départ), plus un CA
> déplacé (`E02US007`) et deux CA retirés
...
> vivant et A09 contre ADR-0014/0015~~ **aucune des deux n'en était une** : S08 décrit une procédure
> en deux temps, et A09 était déjà **prévu** par ADR-0014/0015, qui nomment `E02US007` comme point
> de réouverture).
>
...
## Retours du questionnaire de maquettes (EPIC-16) — 🔶 **en cours (20/21)**

> ⚠️ **Le compte annoncé ici était faux jusqu'au 10/09/2026** (« 14/18 ») : `E16US013` figurait **deux
> fois** dans le tableau ci-dessous, et le dénominateur comptait des **lignes**, pas des US. Les deux
> lignes sont fusionnées. Le compte du 10/09/2026 est **15 sur 21** : 14 US livrées, plus `E16US011`
> **close par découpage** — elle ne livre aucun code, elle se remplace par quatre US filles
> (`E16US018` → `E16US021`), un CA déplacé (`E02US007`) et deux CA retirés.
...
| E16US015 | Un QR par scoreur | 04/09/2026 | ✅ *(le scoreur ouvre sa session **en scannant**. ⚠️ **La question de sécurité a été tranchée au cadrage** : le QR porte le code **en clair** — il n'expose rien de plus que la liste admin, le PDF des cartes et la base, qui le stockent déjà en clair —, mais le risque est **contextuel** (un QR se photographie d'un mètre) donc traité par l'affichage : **à la demande, un scoreur à la fois**, le composant qui charge le QR n'étant **monté** que sur geste. ⚠️ **La forme d'URL n'est PAS celle du jumeau** : `…/scoreur#code=` et non `…/?poste=` — le chemin nomme le monde, le routeur d'adresses (ADR-0059) aiguille seul, **zéro ligne** ajoutée à `resoudreRole`, le cœur risqué de l'entrée (ADR-0042). ⚠️ **Deux fuites de la même famille, une trouvée en rédigeant la recette, l'autre en revue** : un scoreur **déjà connecté** qui rescanne ne monte pas le formulaire, et une **tablette déjà rattachée** ne monte pas l'espace scoreur du tout — l'effacement de l'adresse a fini **au shell** (`App.tsx`), seul niveau que tous les mondes traversent. ⚠️ **Un bloquant en revue** : `qrOuvert` était indexé sur l'`id`, que SQLite réattribue — supprimer un scoreur dont le QR est ouvert puis en créer un autre rouvrait le QR **sans clic** sur le nouveau venu, servi depuis le cache. ⚠️ **L'axe C2 a eu raison contre l'auteur sur l'ADR** : [ADR-0105](../docs/adr/0105-le-qr-d-un-scoreur-porte-son-code-revele-un-a-la-fois.md) amende ADR-0025, dont la Décision 2 (« secret distribué sur papier et **retapé** ») devenait fausse en silence. Rotation d'un code **écartée du périmètre** par le commanditaire)* |
| E16US013 | Le lancement d'un tour : automatique ou manuel | — | ⬜ *(**cadrée puis reposée le 05/09/2026**. Le cadrage a établi trois faits qui la rendaient sans valeur en l'état : le feu vert n'existe **que** pour les phases à tableau (`PhasePasUnTableau` — ni poules, ni suisse, ni colline, ni Big Shoot Off, ni qualification, or c'est pour eux que la fiche voulait l'automatique) ; « lancer » **n'autorise rien** (ADR-0056 §3 : trace d'audit + `LiveEvent`, aucun statut — un scoreur peut saisir un duel jamais lancé) ; et aucun duel n'était prêt au-delà du tour 1. L'automatique n'aurait donc émis **qu'une annonce par tableau**, au tour 1, où tout est prêt d'un coup. Arbitrage du commanditaire : lever le verrou d'abord (`E03US012`). ⚠️ **ADR-0091 §5 affirme déjà que le pilotage tour par tour est possible « sans second mode »**, par un arrêt programmé à chaque tour — mais les deux leviers ne font pas la même chose : l'arrêt **gèle la saisie**, le lancement ne gèle rien. À trancher si la fiche est reprise)* |
| E16US016 | Exports : les formats et documents qui restent dus | 18/09/2026 | ✅ *(les trois CA dus sont livrés : écran de consultation du **journal d'audit** — la route existait depuis `E10US005` et aucun écran ne l'appelait —, format **xlsx** (`openpyxl`, arbitrage du commanditaire, règle 11), et le **palmarès** entre au catalogue. La route publique `/palmares.pdf` est **renommée franchement**, sans alias. [ADR-0101 §6](../docs/adr/0101-le-catalogue-d-exports-porte-les-formats-pas-les-url.md) (ADR rouvert) : composition et rendu séparés — une classe compositrice, un rendu injecté. `DETTE-101` inscrite, `DETTE-095` élargie. Aucune migration)* |
...
| E16US011 | Ce que trois questionnaires « validés » demandaient quand même | — | ✅ *(**close le 10/09/2026 — par DÉCOUPAGE, sans livrer une ligne de code.** Devenue une **carte** : ses sept règles ont été vérifiées une par une dans le code du jour **avant** tout arbitrage, et **sept des dix lignes de son tableau corrigent sa rédaction d'origine**. Résultat : **deux CA tombent sans coûter une ligne** — le retardataire est **déjà tenu** parce que **rien ne place tout seul** (ADR-0024) — ne pas invoquer « Placer les restants » comme preuve, c'est la branche **automatique** que la question binaire d'A09 opposait au choix retenu et le reliquat de vocabulaire « position » est **faux pour la 5ᵉ fois d'affilée**, avertissement désormais éteint partout ; **un CA est déplacé** (`A09`/licence → `E02US007`, ce qu'ADR-0014 et ADR-0015 avaient nommé mot pour mot comme point de réouverture) ; **un CA reste DÛ** (`P05`/horaires prévisionnels : **deux lectures possibles**, question à reposer en priorité — l'autorité qui le disait « écarté » était **circulaire**, un commentaire de code et non un arbitrage) ; le reste part en **quatre US filles**. ⚠️ **Sa « contradiction » S08 n'en était pas une** : « plus de modification une fois validé » et « une validation peut être annulée » décrivent **une procédure en deux temps**, pas deux règles qui s'opposent — et le constat était à moitié faux, les **duels** tenant déjà la règle. ⚠️ **Trouvé au découpage, absent de toutes les rédactions** : `Volee.saisie_par` est **déclaratif** — un nom libre venu du corps de la requête —, donc inutilisable comme source d'autorité ; sans ce constat, `E16US020` aurait livré une hiérarchie qu'un poste contourne en se déclarant admin)* |
| E16US018 | Le panneau de routage rend la tablette tout seul | 10/09/2026 | ✅ *(S06. **Front seul côté production**, aucune route, aucune migration — le seul test backend épingle la place finale. Minuteur de **3 min**, signal **discret non chiffré**, **poignée de réouverture** ajoutée côté duels. ⚠️ **Les deux questions de cadrage ont été supprimées, pas arbitrées** : un minuteur **unique et aveugle aux issues** clôt l'agrégation **et** le cas terminal. La règle « il reste dès qu'une ligne est terminale » était un piège mesuré — un battu à **chaque** duel, donc écran de duels **jamais** refermé, US sans effet et suite verte. ⚠️ **La fiche disait la place finale « tenue par accident » : c'est inexact** — elle est fondée dans `Tableau.classement()` (places **acquises**, décernées par un match terminal). Manquaient un **pointeur** au site du routage et le sens « pas encore décidé ⇒ rien annoncé » — test ajouté (petite finale jouée, finale non tirée : aucun finaliste ne porte de rang). ⚠️ **`docs/fonctionnel/E04US018.md` était périmée** (« rang publié en fin de phase » là où E07US008 rend une fourchette) : **reprise**, pas étendue. ⚠️ **Écart ouvert** : le signal ne figure pas dans la planche S06 — tracé dans `docs/maquettes.md`, à reporter sous peine qu'`E17US008` le retire)* |
| E16US019 | Annuler une validation pour corriger | — | ✅ *(**livrée le 11/09/2026**. Le scoreur reçoit **l'écran de validation de qualification** que `E04US002` annonçait « à venir » et qui n'avait jamais été pris par aucune US — sans lui, le CA « l'écran dit ce qu'il fait » était insatisfaisable. Annuler rouvre **le lot** validé ensemble et rend la volée à la **tablette**, qui ressaisit ; le scoreur revalide. ⚠️ **Trois affirmations de cette fiche étaient fausses** : (1) « `corriger_volee` disparaît » — il est **conservé**, tranché dès la 2ᵉ passe de revue du 10/09 ; (2) « probablement aucune migration » — le CA arbitré le 11/09 sépare le **verrou d'écriture** du **compte**, ce qu'une property à un seul champ ne peut pas porter : migration `0054`, deux colonnes ; (3) « le lot se recalcule par le grain » — `saisir_volee` n'impose **aucun ordre** de saisie alors que `valider` prend « les N premières non validées », donc un lot réel peut être **non contigu** (volées 1 et 3) et le calcul rouvrirait des volées que personne n'a annulées ; l'identité du lot est **persistée**. ⚠️ **Le périmètre duel a été tranché : NON** — `DuelVerrouille` tient déjà S08, et le barrage de places rouvre **volontairement** sur saisie (y appliquer S08 restaurerait un bug corrigé). ⚠️ **Asymétrie laissée ouverte** : l'annulation est admin **ou** scoreur (S08), mais la validation reste scoreur seul — un admin qui annule ne revalide pas lui-même ; rien ne se bloque, le score restant compté. [ADR-0109](../docs/adr/0109-une-volee-en-correction-reste-comptee.md), recette [`E16US019`](../docs/fonctionnel/E16US019.md))* |
| E16US020 | Deux écritures, une même volée : le rôle tranche | — | ✅ *(**livrée le 12/09/2026**. S09, [**ADR-0107**](../docs/adr/0107-une-ecriture-concurrente-est-arbitree-par-le-role-de-qui-ecrit.md) — premier ordre entre rangs du dépôt, qui n'a aujourd'hui que des prédicats indépendants. Il n'existe **aucun** mécanisme de conflit : la file d'écriture **sérialise** sans **arbitrer**, donc c'est du *last-write-wins* silencieux. ⚠️ **RECTIFIÉ en revue le 10/09/2026 : la 1ʳᵉ rédaction disait `archer < scoreur < admin`, et ce rang du bas N'EXISTE PAS** — `D-13` / ADR-0030 ont aboli le rôle archer le 14/07/2026 au profit d'un **poste ouvert** identifié par le lieu. Le scénario sur lequel l'arbitrage avait été présenté (« un scoreur écrasera un archer ») ne pouvait pas se produire. Ordre retenu : **`poste de cible < scoreur < admin`**. ⚠️ **Le revers reste assumé** — une hiérarchie tranche **sans prévenir** — et **elle ne couvre pas le cas le plus fréquent** : deux tablettes de cibles ont le **même** rang, donc elle n'arbitre rien entre elles ; le `409` est **écarté, pas oublié**. ⚠️ **Migration probable, pas certaine** : le rang peut se **dériver de la garde** à la frontière API au lieu d'être persisté, `saisie_par` étant déclaratif et donc inutilisable comme autorité — **tranché à l'implémentation : la colonne**, migration `0055`, la voie « dériver de la garde » n'existant pas (la garde ne rend que le rôle ENTRANT, jamais celui qui a déjà écrit). ⚠️ **Périmètre arbitré le 12/09/2026 : la qualification seule** — les autres formats gardent le dernier-écrit-gagne, et `DETTE-065` n'est ni élargie ni résorbée. ⚠️ **Le refus vers le BAS est confirmé** (l'organisateur qui saisit par erreur bloque la cible ; **le recours est que l'organisateur ressaisisse lui-même la valeur juste**, à rang égal — annuler la validation ne remet la préséance à zéro que sur une volée **déjà validée**, ce que celle-ci n'est pas. Rédaction corrigée en 3ᵉ passe de revue : les cinq autres artefacts disaient déjà le vrai, celui-ci **fait autorité** comme point de reprise). ⚠️ **`DETTE-100` ouverte** : **aucun écran ne déclenche le refus** — la tablette est le seul appelant de `POST /saisie/volees`, deux tablettes sont à rang égal, donc le `409` n'est atteignable que par l'API. Trouvé en rédigeant la recette fonctionnelle, pas en revue. ADR-0107 § « Porté dans le code par » rempli **en relisant le code livré**)* |
| E16US021 | L'accueil du tournoi parle par départ | ✅ | ✅ *(livrée le 12/09/2026 — **un bloc par départ** en tête de l'accueil : horaire, quota, état de cycle, **effectif du créneau**, et la **pause là où elle a lieu**. ⚠️ **La formule la plus large du questionnaire a été bornée avec le commanditaire avant de coder**, comme la fiche l'exigeait : l'**avancement des phases est exclu** — il ne se lit que par `suivi-deroule`, scopée à **un** créneau (ADR-0075) et reconstruisant tous les tableaux (`DETTE-031`), soit N lectures chères à chaque tick de l'écran d'atterrissage. ⚠️ **Deux réponses du commanditaire se contredisaient** et ont été rouvertes avant d'implémenter : retirer la pastille de relance sans retenir les pauses aurait **supprimé de l'accueil** la capacité d'`E05US034` ; retirer les trois chiffres aurait perdu « Réglés » et « Postes en ligne », que rien ne ventile par départ. ✅ **`DETTE-082` resserrée** : sur l'accueil, `demarrer`/`terminer` **renvoient** vers leur écran « prêt à… » au lieu d'agir — elle reste ouverte sur la destination « Tournoi » et pour `archiver`, dont le jalon répond `404 jalon_non_instruit`. ⚠️ **Aucune route neuve, aucun aller-retour de plus** : `ServiceDeparts.lister_avec_etat` calculait déjà un avancement complet par créneau et **jetait tout sauf l'état** ; l'effectif passe par `CompteurEngages`, la seule définition existante — un `len()` local aurait fait diverger l'accueil du reste sur les doubles inscriptions)* |
...
| ~~E17US008~~ | Confronter les **9 planches de saisie** `S**` et résorber | J3 | ✅ *(livrée le 24/09/2026 — **relevé des 9 planches** dans `EPIC-17`, **5 écrans résorbés** (`S01`, `S02`, `S03`, `S04`, `S09`), sous le plafond de six. La tablette prend sa largeur (ligne d'archer 575 → 694 px), le pavé ne s'ouvre plus hors de l'écran (y=742 → y=275), le hors-ligne devient un **aplat pleine largeur** et le cumul cesse d'afficher 0 toute la série. ⚠️ **Le CA « questionnaire → variante retenue » a cédé sur cet axe** : questionnaires du 04/08 sur vignettes, planches redessinées le 05/08 — **7 planches sur 9** ne proposent plus ce qui a été jugé, lettres **inversées** sur `S01`. `S04`, `S05`, `S08` n'ont **plus d'étalon** et sont hors résorption. ⚠️ **`S07` n'a aucun écran ni endpoint** — US non livrée, pas écart de fidélité. L'écart 🔴 restant part en `E17US011`)* |
| E17US009 | Confronter les **7 planches publiques** `P**` et résorber | J3 | ⬜ |
| E17US011 | La **ligne d'archer porte la volée en cours** (S02) | J3 | ⬜ *(sortie du relevé d'`E17US008` — seul écart 🔴 laissé ouvert. ⚠️ Sa résorption heurte un invariant établi en revue le 05/08 : une zone tapable de plus dans la ligne **détruisait le tampon de frappe du pavé**. Le test se dérive de là. Débloque aussi les deux 🟠 restants — largeur de la grille et touches à 90 px), et **résorbe `DETTE-111`** — la conversion « zone → points » écrite en Python et en TypeScript : le barème servira un `points_par_zone`, ⚠️ la **règle**, pas le **résultat**, le hors-ligne interdisant de servir un total)* |
...
| E05US022 | Ancrer la séquence sur **l'identité** de l'étape | `DETTE-026` | ✅ *(livrée le 20/09/2026 — [ADR-0078](../docs/adr/0078-la-sequence-s-ancre-sur-l-identite-de-l-etape.md) amendé sur deux points, migration `0056` ; `DETTE-025` change de **nature**, pas de taille)* |
...
- **close par découpage** : la fiche devient une **carte**, sa capacité part en **US filles**. Elle
  compte **✅** — le travail de découpage *est* le livrable — et les filles comptent au dénominateur.
  À distinguer d'**⛔ absorbée** (la capacité est livrée par une autre US) et de **caduque** (la
  capacité a disparu). **Une à ce jour** : `E16US011` (10/09/2026). *(`E01US016` relève, elle, de
  l'absorption par `E16US006`.)*
