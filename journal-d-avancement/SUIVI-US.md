...
les quatre panneaux de l'espace scoreur partagent enfin **un seul** créneau — quatre sélecteurs
indépendants faisaient scorer le mauvais départ sans la moindre erreur visible. Issue de routage
...
> | ~~🎯 1~~ ✅ | ~~`E05US031`~~ | **Livrée le 17/08/2026** — les **trois** formats sans arbre atteignent l'application publique et l'écran de salle : poules et système suisse par une **forme commune** (« rencontres groupées + classement », décor `RONDES_APPARIEES` partagé), le Big Shoot Off par une vue propre (sa manche est un tir collectif sans adversaire). L'onglet « Tableaux » devient « **Rencontres** » et s'alimente désormais à la **liste des phases du créneau**, pas au routeur des tableaux — un arbre est un cas particulier de phase, pas le sommaire. [ADR-0089](../docs/adr/0089-les-vues-publiques-rendent-les-formats-sans-arbre.md) révise ADR-0064. ⚠️ **Le périmètre a été élargi au cadrage** (historique des tours, « mon chemin », classement d'une phase terminée) et **reversé dans `stories/`**. ⚠️ **Du backend, contre l'attente de la fiche** : la vérification demandée par son propre CA a montré que le Big Shoot Off était le seul des trois sans lecture publique — ses deux routes sont alignées sur celles de ses jumeaux (`/etat` publique, `/saisie` scoreur). Ouvre `DETTE-070` (mineure) : la clé de vue reste `tableaux` là où le libellé dit « Rencontres », parce qu'elle est persistée en base. |
...
| 76decies | E05US031 | **Le public voit les formats sans arbre** | ✅ *(livrée le 17/08/2026 : poules, système suisse et Big Shoot Off atteignent l'appli publique et l'écran de salle. Forme commune pour les deux formats appariés, vue propre au Big Shoot Off — [ADR-0089](../docs/adr/0089-les-vues-publiques-rendent-les-formats-sans-arbre.md), qui révise ADR-0064. L'onglet « Tableaux » devient « Rencontres ». Périmètre élargi au cadrage — historique, « mon chemin », classement d'une phase terminée — et reversé dans `stories/`. Porte du backend : le Big Shoot Off n'avait aucune lecture publique. Ouvre `DETTE-070`)* |
