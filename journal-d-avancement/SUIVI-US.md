...
**Dernière mise à jour : 10/09/2026, 18 h 48** · **136 US livrées** · dernière : `E03US012`
*(**poser les cibles des tours suivants** — passé le premier tour, les duellistes reçoivent leur
cible **sans clic**, dès que leur tour est déterminé. ⚠️ **Ce n'était pas un confort, c'était un
trou** : `place = match.tour == 1` rendait `pret_a_lancer` **toujours faux** au-delà du tour 1 —
le feu vert restait éteint et le routage muet pour toute la fin du tableau, quarts, demies et
finale comprises. ⚠️ **L'US est née d'un cadrage avorté** : `E16US013` (lancement automatique)
n'avait rien à lancer, et a été **remise à ⬜** après arbitrage du commanditaire. ⚠️ **La clé de
`placement_tableau` était le blocage** — `(phase_id, inscription_id)` ne peut pas représenter deux
poses du même archer : migration `0053`, changement de clé primaire, reprise en tour 1
([ADR-0106](../docs/adr/0106-la-pose-d-une-cible-appartient-a-un-tour.md)). ⚠️ **`DETTE-019` est
résorbée sur son fond** (la garde tour-1, écrite en quatre formulations dont **deux dans le
front**, balayées dans le même diff) et **`DETTE-021` est élargie** : le défaut ne valait qu'au
tour 1, il vaut désormais à tous. ⚠️ **Le registre prédisait faux depuis six semaines** : il
attendait `E05US010`, livrée le 31/07 et **hors sujet** (son « 1→N » désigne la profondeur de
...
> ⚠️ **136 fiches closes, mais 134 livraisons.** Le total ci-dessus est **écrit à la main** et
...
> | **🎯 1** | reste d'`E16` | `E16US018` (routage : minuteur de 3 min — **front seul, aucune migration**, la plus petite du lot), `E16US019` (annuler une validation), `E16US020` (hiérarchie de rôles — **ADR-0107 écrit** ; **état persisté requis**), `E16US021` (accueil par départ — **périmètre à borner avant de coder**), `E16US016` (reliquat d'exports — **arbitrage `xlsx`, règle 11**). **Le 🎯 est posé sur `E16US018`** (la plus petite, front seul, aucune migration) ; **l'ordre entre les quatre autres reste libre**. Deux membres de la famille « prêt à… » restent à instruire (*prêt à archiver*, *prêt à exporter*) — le second se rattache désormais à `E16US016`. ⚠️ **`E16US013` a quitté ce rang le 05/09/2026** : son cadrage a montré qu'elle n'avait **rien à lancer** (voir sa ligne), et le commanditaire a arbitré de lever le verrou d'abord — c'est `E03US012`, livrée. La reprendre suppose de trancher d'abord la tension avec ADR-0091 §5. |
...
| E16US018 | Le panneau de routage rend la tablette tout seul | — | 🎯 *(S06. **Front seul côté production, aucune route, aucune migration** — mais le test qui épingle la place finale est un test **backend**. La distinction « un tour suit » existe déjà côté serveur (`IssueRoutage`) ; le minuteur de 3 min est **absent** sans ambiguïté ; la place finale du perdant est **déjà rendue — par accident**, par repli sur `None`, donc à épingler plutôt qu'à corriger. ⚠️ **Trois des quatre CA sont conditionnels**, constat de 2ᵉ passe de revue : **(1) deux** points à trancher et non un — quelles issues comptent (`en_attente`, `indisponible`) **et** comment on agrège plusieurs lignes ; **(2) l'agrégation décide si l'US a une valeur** — `SaisieDuels` monte le panneau sur les **deux** duellistes, donc une ligne terminale à chaque duel : la règle « il reste dès qu'une ligne est terminale » rendrait le minuteur **mort-né** sur cet écran, tests verts ; **(3)** le compte à rebours est un **ajout** postérieur au questionnaire (variante C non retenue), à écrire dans la planche S06 sous peine qu'`E17US008` le retire ; **(4)** `docs/fonctionnel/E04US018.md` est **périmée** et à reprendre, pas seulement à étendre. « La plus petite du lot » reste vrai ; « sans surprise » non)* |
