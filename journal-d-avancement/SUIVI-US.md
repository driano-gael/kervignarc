...
**Dernière mise à jour : 22/08/2026, 20 h 48** · **122 US livrées** · dernière : `E16US002`
...
livrées par les six US de formats. **Aucune migration**, contre « migration » annoncée au tracker.
`DETTE-080` inscrite ; une duplication **fermée** sur preuve (`configInchangee`, deux bugs déjà
payés).)*
...
> | **🎯 1** | `E16US012` | **Famille « prêt à… »** — la seule contrainte d'ordre du reste d'`EPIC-16` : elle **recoupe** `E16US007` (exports) et `E16US008` (feu vert), et doit être instruite **avant** qu'elles ne figent chacune leur variante dans leur coin. ⚠️ **À instruire, pas à coder d'emblée** : c'est une refonte de navigation (quatre écrans « puis-je passer à l'étape suivante, et sinon que manque-t-il ? » — prêt à démarrer / terminer / archiver / exporter) qui recoupe aussi la frise du cycle de vie (E14US001, ADR-0026). **ADR probable.** `E16US003` n'en a livré qu'un seul écran, sur le périmètre qui était le sien. |
