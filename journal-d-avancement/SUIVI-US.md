...
**Dernière mise à jour : 21/08/2026, 13 h 14** · **120 US livrées** · dernière : `E05US029`
*(**des poules de niveau en une seule étape**. Un tournoi club **en cascade** — 36 archers, une
phase de 6 poules de 6 pour estimer les niveaux, puis 6 poules **par niveau** pour les affiner — se
monte désormais en **une** étape au lieu de six écrites à la main. L'organisateur choisit
« Par niveau », l'aperçu lui répond « 6 poules de niveau : rangs 1-6, 7-12, … », et le vainqueur du
groupe des 31ᵉ-36ᵉ ressort **31ᵉ**, jamais 1ᵉʳ. Le mode est un **réglage**, pas un `TypePhase` neuf
(règle 2) : mêmes rencontres, même barème, même départage — seul change *qui joue avec qui*.
⚠️ **La fiche annonçait le mauvais remède, et la vérification l'a corrigé.** Elle concluait qu'il
fallait porter l'espace de rangs **au groupe** (`rang_premier` par poule). Il suffit en fait que le
**classement de phase se lise groupe par groupe** : chaque poule occupe alors sa tranche, et le
`rang_premier` unique de la phase décale l'ensemble comme avant. Le remède annoncé aurait fait
**deux mécanismes** pour situer un même archer dans l'espace de rangs — la seconde vérité que
`DETTE-034` documente. D'où le titre de l'ADR : le mode commande **aussi la lecture**, et les deux
versants sont indissociables — n'en porter qu'un produit le classement « bien formé, plausible et
faux » d'ADR-0081 ([ADR-0094](../docs/adr/0094-le-mode-de-composition-d-une-poule-commande-aussi-la-lecture-de-son-classement.md)).
⚠️ **Trois arbitrages tranchés au cadrage**, tous reversés à la fiche (règle 9) : la **cascade à
resserrement** entre au périmètre et est éprouvée de bout en bout ; les **groupes du bas** absorbent
le surplus quand l'effectif ne tombe pas juste — question qui **ne se posait pas au serpent**, les
groupes y étant équilibrés par construction ; et le garde-fou « 2ᵉ phase de poules au serpent » est
un **refus** avec dérogation à cocher, pas un bandeau : le défaut monte un tournoi jouable mais
dépourvu de l'intérêt visé, ce qui ne se voit qu'en salle, une fois les groupes affichés.
⚠️ **Le prédicat du refus porte sur la source, pas sur le rang dans le déroulé** — une phase nourrie
par la qualification garde le serpent, même si des poules la précèdent.
**Aucune migration**, et **aucun tournoi déjà réglé ne change de composition**. `DETTE-054` élargie —
non d'une paire, mais de **deux champs** dans une paire existante, mode d'aggravation qu'un décompte
en « paires » ne voit pas.)*
...
> | **🎯 1** | `E05US027` | **La colline jouable** — 4ᵉ et dernière tranche du découpage d'`E05US023`, celle qui referme `DETTE-028` sur son volet « moteurs sans appelant ». **Indépendante d'`E05US026`** : plaçable avant, après ou entre, au gré de la fenêtre. Elle bénéficiera du port unifié posé par `E05US026` — quatrième occurrence, plus aucune duplication à écrire. |
...
- **Règle de comptage d'un jalon** : `n/N` compte les **lignes portant un identifiant d'US**
  ...
  **dans leur section d'origine**, pas dans le jalon — sans quoi la même US serait comptée deux
  fois. C'est cette règle qui donne J0 12/12, J1 46/46, J2 14/14, J3 23/25 et J4 0/7.
